...
cdb.admin.Header = cdb.core.View.extend({
  ...
  _showPrivacyDialog: function(e) {
    ...
    if (this.options.user.featureEnabled('new_modals')) {
      ...
      // TODO: remove one new modals are public
  ...
  _changeMetadata: function(ev) {
    ...
    if (this.options.user.featureEnabled('new_modals')) {
      dlg = new cdb.editor.EditVisMetadataView({
        ...
        onShowPrivacy: function() {
          self._showPrivacyDialog();
        },
        onDone: function(nameChanged) {
          // Check if attr saved is name to change url when
          // visualization is table type
          if (nameChanged && !this.vis.isVisualization()) {
      ...
      dlg = new cdb.admin.MetadataDialog({
        maxLength:  this._MAX_DESCRIPTION_LENGTH,
        vis:        this.model,
        user:       this.options.user,
        onResponse: function(data) {
          self._onSetAttributes(data);
        }
      });
    }

    dlg.appendToBody();
    dlg.open({ center: true }); // TODO: remove center param when old modal is deprecated
  },
    ...
    function setTitle(val) {
      if (val !== self.model.get('name') && val != '') {
        ...
        if (self.model.isVisualization()) {
          ...
          if (self.options.user.featureEnabled('new_modals')) {
            ...
            self.change_confirmation = new cdb.admin.BaseDialog({
              ...
              descriptionSafeHtml: self._TEXTS.rename['desc_' + (self.model.isVisualization() ? "vis" : "table")],
