...
describe('data/feature-definition-model', function () {
  ...
  describe('.fetch', function () {
    beforeEach(function () {
      this.fakeQueryRowModel.fetch.and.callFake(function (attrs, options) {
        options && options.success(attrs);
      });
    });
    ...
    it('should unbind change binding', function () {
      spyOn(this.feature, 'off');
      this.feature.fetch();
      expect(this.feature.off).toHaveBeenCalled();
    });

    it('should set properties after fetch', function () {
      spyOn(this.feature, 'set');
      this.feature.fetch();
      expect(this.feature.set).toHaveBeenCalled();
    });

    it('should listen changes after fetching', function () {
