...
cdb.admin.OverlayPropertiesBar =  cdb.core.View.extend({
  ...
  _destroyDeleteTipsy: function() {

    var $el = this.$el.find(".btn.btn-delete");
    var tipsy = $el.data('tipsy');

    if (tipsy) {
      $el
      .tipsy('hide')
      .unbind('mouseleave', this._destroyTipsy);
    }
  ...
  _getOverlaysZIndex: function() {
    ...
    var canvas_mode = this.options.vis.get("canvas_mode");

    var overlays = this.overlays.filter(function(o) { 
      return o.get("device") === canvas_mode && (o.get("type") === "text" || o.get("type") === "image");
    });
  ...
  _enableTipsy: function() {
    ...
    this.$el.find("a[data-tipsy]")
    .bind('mouseleave', this.destroyTipsy)
    .tipsy({
      fade:    true,
      html:    true,
      gravity: "s",
