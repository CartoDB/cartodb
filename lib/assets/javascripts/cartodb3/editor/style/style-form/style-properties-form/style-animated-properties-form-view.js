...
module.exports = CoreView.extend({
  ...
  render: function () {
    var type = this._styleModel.get('type');
    this.clearSubViews();

    if (type === TYPES.HEATMAP) {
      this._initAnimatedEnablerView();
    } else {
      this._genAnimatedFormView();
    }

    this.$el.toggleClass('u-tSpace-xl', type === TYPES.HEATMAP);
    return this;
  },
  ...
  _genAnimatedFormView: function () {
    var type = this._styleModel.get('type');
    var cssClass = type === TYPES.HEATMAP ? 'Editor-formInner--nested' : '';
