(function() {
  ...
  var SelectorView = cdb.core.View.extend({
    ...
    initialize: function() {
      ...
      this.template_base = _.template("<p class='<% if (bucket == undefined || bucket == '') { %>empty<% } %>'><% if (bucket == undefined) { %>null<% } else if (bucket == '') {%>empty<% } else { %><%= bucket %><% } %></p> <div class='value'><%= value %></div>");
    },
    ...
    render: function() {
      ...
      var pretty_bucket = this._cleanString(this.model.get("bucket"));
  ...
  cdb.admin.mod.SelectorFilter = cdb.core.View.extend({
    ...
    render: function() {
      ...
      var empty = this.model.items.models.length == 0 || (this.model.items.models.length == 1 && this.model.items.at(0).get("bucket") == null);
      var only  = !empty && this.model.items.models.length == 1;
    ...
    _toggleMode: function() {
      ...
      if (!list_view) {
        ...
        this.$textarea.fadeIn(250, function() {
          self.$textarea.find("textarea").val("");
          self.model.set({ free_text: "" });
        });
