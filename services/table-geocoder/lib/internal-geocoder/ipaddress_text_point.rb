  ...
  module InternalGeocoder
    ...
    class IpAddressTextPoint < AbstractQueryGenerator
      ...
      def search_terms_query(page)
        %Q{
          SELECT DISTINCT(quote_nullable(trim(#{@internal_geocoder.column_name}))) AS ipaddress
      ...
      def dataservices_query(search_terms)
        ...
        "WITH geo_function AS (SELECT (geocode_ip(Array[#{ipaddress}])).*) SELECT q, null, geom, success FROM geo_function"
      end
