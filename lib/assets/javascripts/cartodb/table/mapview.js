...
var SearchInput = Backbone.View.extend({
    ...
    _renderListItem: function(service, index, item, selected) {
        var query = this.query;
        var lower_query = query.toLowerCase();
        var queryLen = query.length;
        var name;
        var startPos = item.dataset.toLowerCase().indexOf(lower_query);
        if (startPos == -1) {
            name = item.dataset;
        } else {
            name = item.dataset.substr(0, startPos) + '<span class="found-sub">' + item.dataset.substr(startPos, queryLen) + '</span>' + item.dataset.substr(startPos + queryLen);
        }

        var data, match = null, matched_attribute = null;
        startPos = null;
        if (item.data != null) {
            data = item.data;
            for (var key in data) {
        ...
        if (matched_attribute != null) {
            item.matched = matched_attribute;
        }
                ...
                self.map.addCartodbLayerFromTable(item.dataset, self.user.get('username'), {
                    ...
                    success: function() {
                        ...
                        if (!item.is_dataset && typeof item.matched === 'string') {
                            var layer = self.map.layers.last();
                            layer.applySQLView("SELECT * FROM " + item.dataset + " WHERE " + item.matched + "='" + item.data[item.matched] + "'");
                        }
                    },
                    error: function() {}
                });
    ...
    _setItems: function(items) {
        ...
        for (var key in items) {
            ...
            for (var i in items[key]) {
                popupHtml += this._renderListItem(key, i, items[key][i], Object.keys(self._items).length === 1 && i == 0);
            }
