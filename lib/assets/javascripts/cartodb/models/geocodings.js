  ...
  cdb.admin.Geocoding = cdb.core.Model.extend({
    ...
    georeference_messages: {
      ...
      paid: "<% if (!success) { %><p>It seems that some of your rows didn't finish succesfully. Perhaps these rows contained empty values or perhaps we just didn't know what the values meant. Unsuccessful rows don't count against your quota, so we encourage you to take a look and try again.</p><% } %>" + 
          "<p><% if (remaining_quota > 0) { %>This job cost you <strong>$0</strong> because we love you! You have <strong><%= remaining_quota %></strong> geocoding tokens remaining in your free quota this month.<% } else { %>This job cost you $<%= price %> because <strong><%= remaining_quota + used_quota %></strong> of them were over your freely available quota. This will be added to next months bill.<% } %></p>",
      ...
      free: "<p>It seems that some of your rows didn't finish succesfully. Perhaps these rows contained empty values or perhaps we just didn't know what the values meant. We encourage you to take a look and try again.</p><p>This job cost you <strong>$0</strong> because we love you for being you!</p>",
  ...
  _onGeocodingComplete: function(msg) {
    ...
    if (msg.kind != "high-resolution") { // FREE
      ...
      if (success){
          ...
          title = (msg.real_rows == 1 ? "One" : msg.real_rows) + " out of " + (msg.processable_rows == 1 ? "one" : msg.processable_rows) + " row" + (msg.processable_rows == 1 ? " " : "s ") + (msg.processable_rows == 1 ? "was" : "were") + " succesfully turned into " + (msg.geometry_type || 'point') + "s!"
