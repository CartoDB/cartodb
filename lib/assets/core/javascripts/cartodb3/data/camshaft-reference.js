...
module.exports = {
  ...
  getDefaultCartoCSSForType: function () {
    return _.template([
      ...
      '  ::outline {',
      '    dot-fill: <%= point.stroke.color.fixed %>;',
      '    dot-width: <%= (point.stroke.size.fixed + point.fill.size.fixed) %>;',
      '    dot-opacity: <%= point.stroke.color.opacity %>;',
      '  }',
      '  dot-width: <%= point.fill.size.fixed %>;',
      '  dot-fill: <%= point.fill.color.fixed %>;',
      '  dot-opacity: <%= point.fill.color.opacity %>;',
      '}',
