require 'spec_helper'
require 'carto/tracking/services/pubsub'
      ...
      describe PubSubTracker do
        ...
        describe '#send_event' do
          ...
          it 'should do nothing when no user id sent' do
            ...
            result = PubSubTracker.instance.send_event(:metrics, '', 'wrong example')

            result.should == nil
          end

          it 'should do nothing when no event type is sent' do
            @topic.stubs(:publish_async)

            result = PubSubTracker.instance.send_event(nil, '', 'wrong example')

            result.should == nil
          end

          it 'should do nothing when unknown event type sent' do
            @topic.stubs(:publish_async)

            result = PubSubTracker.instance.send_event(:fake_Event, '', 'wrong example')
          ...
          it 'should raise error if publishing did not succeeded' do
            CartoDB::Logger.expects(:error).with(has_entry(message: 'PubSubTracker: error publishing to topic test-topic for event track user'))
