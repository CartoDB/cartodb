...
describe('organization/invite_users/invite_users_dialog_view', function() {
  ...
  fdescribe("form view", function() {
    ...
    it("should render enable signup message if org signup is disabled", function() {
      this.formView.model.set('enable_organization_sign_in', true);
      expect(this.$form.find('.js-signInMessage').length).toBe(1);
    });

    it("should set to true the model enable_organization_sign_in attribute", function() {
      var $button = this.$form.find('.js-enableSignInButton');
      $button.click();
      expect(this.formView.model.get('enable_organization_sign_in')).toBe(true);
    });
