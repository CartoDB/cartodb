cdb.admin.overlays.Annotation = cdb.geo.ui.Annotation.extend({
  ...
  _onKeyDown: function(e) {
    ...
    if (selected) {
      ...
      var editable = this.editModel.get("mode");
  ...
  _savePosition: function() {
    ...
    if (textAlign === "right") {
      x = x + this.$el.width() + lineWidth + this.$el.find(".ball").width();
    } else {
      x = x - lineWidth;
    }
  ...
  hide: function(callback) {
    ...
    this.unbind("mouseup", this._savePosition, this);
  ...
  _disableEditingMode: function(save) {
