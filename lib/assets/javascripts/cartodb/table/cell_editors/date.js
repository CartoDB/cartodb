...
cdb.admin.EditDateDialog = cdb.admin.SmallDialog.extend({
  ...
  render_content: function() {
    ...
    //   var $content = this.$content = $('<p>' +
    //     this.model.toJSON().day + '/' +
    //     this.model.toJSON().month + '/' +
    //     this.model.toJSON().year + ' </p>' +
    //     '<p>' +
    //     this.model.toJSON().time.split(':').slice(0,2).join(':') +
    //     '</p>');
    //   return $content;
    // } else {
  ...
  _initWidgets: function() {
    ...
    // this.$content.find("div.day").unbind();
    // Year spinner
    ...
    // this.$content.find("div.year").unbind();
    ...
    if(this.options.readOnly) {
      this.$content.find('input, select')
        ...
        .attr('readOnly', 'true');
      this.$content.find('select').select2('disable');
    }
  },
  ...
  showAt: function(x, y, width, fix) {
    ...
    var dialog_width = this.$el.width() + 5;
