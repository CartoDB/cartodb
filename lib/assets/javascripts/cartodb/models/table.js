...
(function() {
  ...
  cdb.admin.CartoDBTableData = cdb.ui.common.TableData.extend({
    ...
    discreteHistogram: function(nbuckets, column, callback, error) {
      ...
      var query = 'SELECT distinct(<%= column %>) as bucket, count(*) as value FROM <%= table %> where <%= column %> is not null GROUP BY <%= column %> ORDER BY value DESC LIMIT <%= nbuckets %> + 1';
      ...
      this._sqlQuery(sql, function(data) {
        ...
        callback({ rows: data.rows, count: count });
    ...
    histogram: function(nbuckets, column, callback, error) {
      ...
      'select  ' +
        'count(<%= column %>) as v,   ' +
        'round(((<%= nbuckets %> - 1)*(<%= column %> - bounds.lower)/bounds.span)) as bucket  ' +
         'from <%= table %>, bounds  ' +
