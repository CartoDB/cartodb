...
describe('data/widget-definitions-collection', function () {
  ...
  describe('when a model is created', function () {
    ...
    it('should set a new order when a widget is created', function () {
      ...
      expect(this.collection.at(0).get('order')).toBe(0);
      widget.set('order', 10);
      this.collection.create(this.req);
    ...
    describe('when confirmed created', function () {
      ...
      describe('when definition changes order', function () {
        it('should change widgetModel order', function () {
          this.widgetModel = this.dashboard.widgets.get(this.model.id);
          spyOn(this.widgetModel, 'set');
          spyOn(this.widgetModel, 'update');
          this.model.set('order', 10);
          expect(this.widgetModel.set).toHaveBeenCalledWith('order', 10);
          expect(this.widgetModel.update).not.toHaveBeenCalled();
        });
      });
