...
(function() {
  ...
  cdb.admin.CartoDBTableMetadata = cdb.ui.common.TableProperties.extend({
    ...
    aliasNames: function (sc) {
       sc = sc || 'aliases';
       var column_aliases = (this.get(sc)) ? (this.get(sc).columns) ?  this.get(sc).columns : {} : undefined;
       return column_aliases;
    },

    getColumnAlias: function (columnName) {
      var returnValue = null;
      for(var key in this.attributes.aliases.columns){
        if(key === columnName){
          returnValue = this.attributes.aliases.columns[key];
        }
      }
      return returnValue;
    },		      
    ...
    renameAlias: function (columnName, aliasOldName, newAliasName, opts) {
      ...
      var aliases_object = this.get('aliases');
      var aliasList = aliases_object.columns;
      ...
      aliases_object.columns = aliasList;
      this.set('aliases', aliases_object);
