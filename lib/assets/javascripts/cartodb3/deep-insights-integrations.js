...
F.prototype._onLayerDefinitionChanged = function (m) {
  if (!m.isNew()) {
    ...
    if (attrs.type) {
      ...
      if (!m.get('type') === 'torque' || attrs.cartocss) {
    ...
    if (attrs.style_properties) {
      var animatedState = {
        animated: m.get('type') === 'torque',
        column: m.styleModel.get('animated') && m.styleModel.get('animated').attribute
      };
      if (!_.isEqual(animatedState, this._lastTSChange)) {
...
F.prototype._manageTimeSeriesForTorque = function (m) {
  ...
  this._lastTSChange = {
    animated: newLayer.get('type') === 'torque',
    column: animatedChanged && animatedChanged.attribute
  };
};
