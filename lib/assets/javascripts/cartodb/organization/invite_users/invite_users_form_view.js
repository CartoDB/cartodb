...
module.exports = cdb.core.View.extend({
  ...
  render: function() {
    ...
    this._toggleWhitelistedDomainError(false, '');
    this._initViews();
    return this;
  },
  ...
  _initViews: function() {
    ...
    this.$(".js-tagsList").tagit({
      ...
      beforeTagAdded: function(ev, ui) {
        ...
        //The email domain is not whitelisted in the organization
        var email_domain = value.replace(/.*@/, "");
        if (self.options.organization.get("whitelisted_domains").includes(email_domain)) {
          self._toggleWhitelistedDomainError(false, '');
        } else {
          self._toggleWhitelistedDomainError(true, email_domain);
          return false;
        }
      },
  ...
  _toggleWhitelistedDomainError: function(visible, tagContent) {
    var errorMessageElement = this.$('.js-addDomainToWhitelist');

    errorMessageElement[ visible ? 'show' : 'hide' ]();
    errorMessageElement.html("<div class='u-inner'><p class='FlashMessage-info'>If you want to invite <b>\""+ tagContent +"\"</b> users you need to add this domain to your whitelist to proceed the invitation.</p><button class='FlashMessage--button'><span>Add to whitelist</span></button></div>");
  },
