...
cdb.admin.dashboard.TableItem = cdb.core.View.extend({
  ...
  render: function() {
    ...
    var name             = this._cleanString(this.model.get("name"), 30);
    var description      = this.model.get("description");
    var pretty_row_count = this._formatNumber(this.model.attributes.table.row_count);

    var extra_tags_count = attrs.tags.length - this._TAGS_PER_ITEM;

    var template_vars = { sync_status: "error", sync_message: "ERROR WITH SYNC", sync_error_message: 'The source is not available anymore. Please check it’s location', order: this.options.table_options.get("order"), row_count: pretty_row_count, name: name, description: description, tags: tags, extra_tags_count: extra_tags_count };
