  ...
  cdb.admin.LayersPanel = cdb.ui.common.TabPane.extend({
    ...
    _bindSort: function() {
      ...
      var isTorque = _.contains(self.map.layers.pluck('type'), 'torque');
      var ori_pos;

      this.$el.sortable({
        axis: "y",
        // It is only possible to sort CartoDB tiled layers
        items: "> .layer_panel",
        handle: ".layer_info a.info",
        contaiment: "parent",
        opacity: 0.5,
        start: function(event, ui) {
          ori_pos = $(ui.item).index();
        ...
        change: function(event, ui) {
          ...
          if (isTorque) {
            // Torque layer
            if ($(ui.item).attr('layer-type') == "torque") {
              ...
              if (ui.placeholder.index() != 1) {
                ...
                self.$('section.layer_panel:eq(' + ori_pos + ')').before(
                  $(ui.placeholder)
                    .outerHeight(active_height)
                    .css('display','block')
                )
