describe("Basemap adder dialog", function() {
  ...
  it("should show an error if the mapbox tile doesn't exist", function(done) {
    ...
    var val = basemap_dialog.mapboxPane.$el.find("input").val("http://a.tiles.mapbox.com/v3/tiles-tiles-tiles-tiles/{z}/{x}/{y}.png/error");
  ...
  it("should show an error if the xyz tile doesn't exist", function(done) {
    ...
    var val = basemap_dialog.zxyPane.$el.find("input").val("http://this_is_not_the_basemap_your_looking.for");
  ...
  it("should show a warning if the xyz tile has the right format but is not valid", function(done) {
    basemap_dialog.render();
    spyOn(basemap_dialog.zxyPane, '_warningChooser');

    var val = basemap_dialog.zxyPane.$el.find("input").val("http://ftp.sensefly.com/postflight-examples/01-small-village-merlishachen/map/display/{z}/{y}/{x}.png");
    ...
    basemap_dialog.zxyPane._checkTile(val.val());
