...
cdb.admin.models.Filter = cdb.core.Model.extend({
  ...
  _fetchDateHist: function() {
    ...
    this.table.data().date_histogram(this.DEFAULT_HIST_BUCKETS, this.get('column'), function(hist, bounds) {
      ...
      console.log('rabo_histogram', hist, bounds);

      if (hist) {
        self._changeDateHist(hist, bounds);
      } else {
        self.trigger('error', "date histogram couldn't be generated");
        console.error('error', "date histogram couldn't be generated");
      }
    });
  ...
  _changeDateHist: function(hist, bounds) {
    ...
    if (!this.has('lower')) {
      ...
      console.log(bounds.lower, bounds.upper);
      ...
      if (_.isNaN(parseInt(lower))) lower = bounds.lower;
      if (_.isNaN(parseInt(upper))) upper = bounds.upper;

      /*console.log(typeof lower, typeof upper, "lower: ", lower, "upper: ", upper);
      ...
      if (typeof lower == 'object') {
        upper = this.get("upper").getTime();
      } else if (typeof upper == 'string') {
        upper = parseInt(new Date(this.get("upper")).getTime(), 10);
      }*/
