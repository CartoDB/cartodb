...
var rampList = require('builder/components/input-color/input-quantitative-ramps/ramps');
  ...
  describe('image', function () {
    ...
    describe('with image enabled', function () {
      ...
      describe('with image', function () {
        ...
        it('should change fill attribute when color changes', function () {
          // Trigger _updateImageColor via model change
          this.view.model.set('fixed', '#0FF1CE'); // rgb(15, 241, 206)

          var image = this.view.$('.js-image');
          var cssFill = image.css('fill').toLowerCase();
          expect(cssFill).toBe('rgba(15, 241, 206, 0.5)'); // Default opacity 0.5
        });

        it('should change fill attribute when opacity changes', function () {
          // Trigger _updateImageColor via model change
          this.view.model.set('opacity', 0.1);

          var image = this.view.$('.js-image');
          var cssFill = image.css('fill').toLowerCase();
          expect(cssFill).toBe('rgba(192, 255, 238, 0.1)'); // Default color rgb(192, 255, 238)
        });
      });
    });
  });
