...
cdb.admin.MapOverlays = cdb.core.View.extend({
  ...
  setLegend: function (legend) {
    ...
    $(".cartodb-legend-stack").children().on('dragstart', function(e) {
    ...
    // disable map drag while dragging legend
    $('.cartodb-legend-stack').on('mousedown touchstart', function(event) {
      if (mapView.map_leaflet) {
          mapView.map_leaflet.dragging.disable();
        }
    });

    $('.cartodb-legend-stack').on('mouseup touchend', function(event) {
      if (mapView.map_leaflet) {
