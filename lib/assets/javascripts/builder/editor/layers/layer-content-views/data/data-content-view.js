...
module.exports = CoreView.extend({
  ...
  render: function () {
    ...
    if (this._isErrored()) {
      this._showError();
    } else if (this._isDataFiltered) {
      this._showFilteredData();
    } else if (!this._hasGeom) {
  ...
  _initBinds: function () {
    this.listenTo(this._layerContentModel, 'change:state', this._reRenderIfViewValuesHaveChanged);
  ...
  _reRenderIfViewValuesHaveChanged: function () {
    ...
    Promise.all([hasGeomPromise, isDataFilteredPromise])
      .then(function (values) {
        var hasGeom = values[0];
        var isDataFiltered = values[1];
        ...
        if (isDataFiltered !== self._isDataFiltered) {
          self._isDataFiltered = isDataFiltered;
          needsRender = true;
        }
