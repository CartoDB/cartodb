...
var DataServicesApiCheck = require('./data-services-api-check');
...
var MAP = {
  ...
  'trade-area': {
    ...
    checkIfValid: function (configModel) {
      if (!configModel) throw new Error('configModel is required');
      var dsReady = DataServicesApiCheck.get().isReady();
      return !!configModel.dataServiceEnabled('isolines') && dsReady;
    },
    onboardingTemplate: require('../components/onboardings/analysis/analyses/area-of-influence.tpl'),
    genericType: 'area-of-influence'
  },
...
var checkIfInternalGeocodingIsEnabled = function (configModel) {
  if (!configModel) throw new Error('configModel is required');
  var dsReady = DataServicesApiCheck.get().isReady();
  return !!configModel.dataServiceEnabled('geocoder_internal') && dsReady;
};

var checkIfRoutingIsEnabled = function (configModel) {
  if (!configModel) throw new Error('configModel is required');
  var dsReady = DataServicesApiCheck.get().isReady();
  return !!configModel.dataServiceEnabled('routing') && dsReady;
};

var checkIfExternalGeocodingIsEnabled = function (configModel) {
  if (!configModel) throw new Error('configModel is required');
  var dsReady = DataServicesApiCheck.get().isReady();
  return !!configModel.dataServiceEnabled('hires_geocoder') && dsReady;
};
...
var isAnalysisValidByType = function (type, configModel) {
  ...
  return analysis && analysis.checkIfValid ? analysis.checkIfValid(configModel) : true;
};
...
module.exports = {
  ...
  isAnalysisValidByType: function (type, configModel) {
    return isAnalysisValidByType(type, configModel);
  },
