describe("DashboardPaginator", function() {
  ...
  it("should generate a view_all link", function() {
    ...
    runs(function() {
      expect(paginator.per_page).toEqual(tables._PREVIEW_ITEMS_PER_PAGE);
  ...
  it("should render search links", function() {
    ...
    runs(function() {
      var $a = $(paginator.$('.page')[1]);
      expect($a.attr("href")).toEqual("/search/CartoDB/1");

      expect(paginator.$('.prev').length).toEqual(1);

      expect(paginator.$('.next').length).toEqual(1);
      expect(paginator.$('.next').attr("href")).toEqual("/search/CartoDB/2");
      expect(paginator.total_entries).toEqual(100);
  ...
  it("should render tag links", function() {
    ...
    runs(function() {
      var $a = $(paginator.$('.page')[1]);
      expect($a.attr("href")).toEqual("/tag/CartoDB/1");

      expect(paginator.$('.prev').length).toEqual(1);

      expect(paginator.$('.next').length).toEqual(1);
      expect(paginator.$('.next').attr("href")).toEqual("/tag/CartoDB/2");
      expect(paginator.total_entries).toEqual(100);
  ...
  it("should render pages on paginator when needed", function() {
    ...
    expect(paginator.$('.page').length).toEqual(7);
    expect(paginator.$('.prev').length).toEqual(1);
    expect(paginator.$('.next').length).toEqual(1);
  ...
  it("should trigger event on page click", function() {
    ...
    paginator.bind('goToPage', function() {
      clicked = true;
    });
