  ...
  describe('with a default ramp', function () {
    beforeEach(function () {
      this.model = new Backbone.Model({
        ...
        range: ['#1d4f60', '#266b6e', '#36877a', '#4da284', '#6dbc90', '#96d2a4', '#c4e6c3'],
        attribute: 'column1',
    ...
    it('should render properly', function () {
      ...
      expect(this.view.$('.is-selected').parent().data('val')).toContain('#1d4f60,#266b6e,#36877a,#4da284,#6dbc90,#96d2a4,#c4e6c3');
    });

    it('should select ramp on click', function () {
      this.view.$('.js-listItem:eq(2)').click();
      var ramp = _.toArray(rampList)[2][7];
      expect(this.model.get('range').join(',').toLowerCase()).toBe('#1d4f60,#266b6e,#36877a,#4da284,#6dbc90,#96d2a4,#c4e6c3');
