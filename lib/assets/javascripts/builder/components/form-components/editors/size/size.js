...
Backbone.Form.editors.Size = Backbone.Form.editors.Base.extend({
  ...
  _initViews: function () {
    ...
    this._tabPaneView.render();

    this.$el.append(this._tabPaneView.$el);
  },
  ...
  _getTabPanes: function () {
    ...
    if (attrs && attrs.hidePanes) {
      if (!_.contains(attrs.hidePanes, FIXED)) tabPaneTabs.push(this._buildFixedPane());
      if (!_.contains(attrs.hidePanes, BY_VALUE)) tabPaneTabs.push(this._buildValuePane());
    } else {
      tabPaneTabs = [this._buildFixedPane(), this._buildValuePane()];
    }
    return tabPaneTabs;
  },
