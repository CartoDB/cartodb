...
describe('data/query-schema-model', function () {
  ...
  describe('when a query is changed', function () {
    ...
    describe('when fetch', function () {
      ...
      it('should not fetch with a wrapped query if query alters data', function () {
        this.model.set('query', 'DELETE FROM foo WHERE username="paco"');
        Backbone.Model.prototype.fetch.calls.reset();
        this.model.fetch();
        expect(Backbone.Model.prototype.fetch.calls.argsFor(0)[0].method).toBe('POST');
        expect(Backbone.Model.prototype.fetch.calls.argsFor(0)[0].data.q).toBe('DELETE FROM foo WHERE username="paco"');
      });
