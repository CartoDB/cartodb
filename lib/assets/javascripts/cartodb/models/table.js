...
(function() {
  ...
  cdb.admin.CartoDBTableData = cdb.ui.common.TableData.extend({
    ...
    histogram: function(nbuckets, column, callback, error) {
      var tmpl = _.template('with bounds as ( ' + 
            'SELECT min(<%= column %>) as lower, (max(<%= column %>) - min(<%= column %>)) as span FROM  <%= table %> '  +
          ') ' +
          'select sum(<%= column %>) as value, ((<%= nbuckets %>*(<%= column %>- bounds.lower)/bounds.span))::integer as bucket ' +
          'from <%= table %>, bounds ' + 
      ...
      function(data) {
        ...
        var max = data.rows[0].value;
        var min = data.rows[0].value;
        for(var r in data.rows) {
          var row = data.rows[r];
          hist[row.bucket] = row.value;
