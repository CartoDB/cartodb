...
describe('helpers/reset-style-per-node', function () {
  beforeEach(function () {
    ...
    resetStylePerNode(this.nodeDefModel, this.layerDefModel, this._forceStyle, this._resetQuery);
  });
  ...
  describe('reset query', function () {
    it('should reset schema and geometry models if param is enabled', function () {
      ...
      this.nodeDefModel.queryGeometryModel.bind('change:ready', spyCallback);
      resetStylePerNode(this.nodeDefModel, this.layerDefModel, false, true);
      expect(spyCallback).toHaveBeenCalled();
      expect(this.nodeDefModel.queryGeometryModel.get('ready')).toBeFalsy();
      expect(this.nodeDefModel.querySchemaModel.get('ready')).toBeFalsy();
    });
  });
  ...
  describe('when query-geometry-model and query-schema-model are ready', function () {
    ...
    describe('when query-geometry-model and query-schema-model are fetched', function () {
      ...
      describe('and cartocss is custom', function () {
        beforeEach(function () {
          this.layerDefModel.set('cartocss_custom', true);
          this.nodeDefModel.queryGeometryModel.set({
            simple_geom: 'polygon',
            status: 'fetched'
          });
