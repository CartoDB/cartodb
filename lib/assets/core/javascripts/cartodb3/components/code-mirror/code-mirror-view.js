...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    this._autocompleteChars = opts.autocompleteChars || 3;
  ...
  _initViews: function () {
    var options = _.defaults(_.extend({}, this.model.toJSON()), {
      readonly: false,
      lineNumbers: true
    });
    ...
    var defaultExtraKeys = {
      ...
      'Ctrl-Space': this._completeAfter.bind(this)
    };

    var extraKeys = this._autocompletePrefix ? _.defaults(_.extend({}, {
      "'{'": this._completeIfAfterOpeningCurly.bind(this),
      'Ctrl-Space': this._completeIfAfterCtrlSpace.bind(this)
    }), defaultExtraKeys) : defaultExtraKeys;
  ...
  _completeIfAfterOpeningCurly: function (cm) {
    return this._completeAfter(cm, function () {
  ...
  _completeIfAfterCtrlSpace: function (cm) {
    ...
    var openingMustache = cm.getRange(CodeMirror.Pos(cur.line, cur.ch - 2), cur) !== '{{';
    if (openingMustache && self._autocompletePrefix) {
      opts = { autocompletePrefix: self._autocompletePrefix };
    }

    return self._completeAfter(cm, null, opts);
  },
  ...
  _onKeyUpEditor: function (cm, event) {
    ...
    var triggers = this._autocompleteTriggers;
    ...
    if (NOHINT.indexOf(code) === -1) {
      ...
      this._autocompleteTimeout = setTimeout(function () {
        ...
        return self._completeAfter(cm, function () {
          ...
          if (str.length > autocompleteChars) {
            var listTriggers = _.filter(triggers, autocompleteHandler);
            var listHints = _.filter(hints, autocompleteHandler);

            return listTriggers.length > 0 || listHints.length > 0;
          }
        }, opts);
      }, 150);
    }
  ...
  _completeAfter: function (cm, pred, opts) {
    var self = this;

    if (!pred || pred()) {
      if (!cm.state.completionActive) {
        self._showAutocomplete(cm, _.extend({}, opts));
      }
    }
