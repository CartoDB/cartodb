...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    // get legends for this layerDefModel and this._legendTypes.pick('value')
    // if empty array, create default model
    // else this.model = collection[0]
  ...
  _onSelectLegendType: function (mdl) {
    ...
    if (mdl.get('selected') === false) {
      // LegendManager.remove(this.model);
  ...
  _onChangeModel: function () {
    // this.model.set(this._formModelset.toJSON());
    // Legendmanager.add(this.model);
