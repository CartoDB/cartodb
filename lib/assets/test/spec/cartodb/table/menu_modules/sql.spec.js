describe("cdb.admin.mod.SQL", function() {
  ...
  it("should not save the editor when the sql hasn't changed", function() {
    var called = false;
    model.set({ query: sql='select * from rambo where id = 1' });
    model.set = function() {
      called = true;
  ...
  it("should save the editor when the sql has changed", function() {
    var called = false;
    model.set({ query: sql='select * from rambo where id = 1' });
    model.set = function() {
      called = true;
    ...
    view.render();
    view.codeEditor.setValue('pizza');
    view.applyQuery();
