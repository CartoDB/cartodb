...
module.exports = CoreView.extend({
  ...
  _initBinds: function () {
    ...
    this.listenTo(this._layerDefinitionsCollection, 'add', this._onAddLayerDefinition, this);
  ...
  _onAddLayerDefinition: function (model) {
    var analysisDefinitionNodeModel = model.getAnalysisDefinitionNodeModel();
    var queryRowsCollection = analysisDefinitionNodeModel.queryRowsCollection;

    var launchGeoreferenceOnboarding = function () {
      if (!model.hasAnalyses() &&
          !model.isCustomQueryApplied() &&
          !queryRowsCollection.isEmpty()) {
        GeoreferenceOnboardingLauncher.setLayerId(model.get('id'));
        GeoreferenceOnboardingLauncher.launch();
      }
    };

    this.listenTo(queryRowsCollection.statusModel, 'change:status', function () {
      if (queryRowsCollection.isFetched()) {
        launchGeoreferenceOnboarding();
      }
    }, this);
