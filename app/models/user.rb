  ...
  def load_cartodb_functions(files = [])
    in_database(as: :superuser) do |user_database|
      user_database.transaction do
        if files.empty?
          glob = Rails.root.join('lib/sql/sites-enabled/*.sql')
          sql_files = Dir.glob(glob).sort
        else
          sql_files = files.map {|sql| Rails.root.join('lib/sql/sites-enabled', sql).to_s}.sort
        end
        sql_files.each do |f|
          if File.exists?(f)
            ...
            CartoDB::Logger.info "SQL function #{File.basename(f)} doesn't exist in lib/sql/sites-enabled directory. Not loading it."
          end
        end
      end
    end
