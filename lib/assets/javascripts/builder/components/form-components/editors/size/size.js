...
Backbone.Form.editors.Size = Backbone.Form.editors.Base.extend({
  ...
  initialize: function (opts) {
    ...
    this.value = new Backbone.Model(this.model.get(opts.key));
  ...
  _initBinds: function () {
    ...
    this.value.bind('change', function () {
      self.model.set(self._keyAttr, self.value.toJSON());
    }, this);
  },
  ...
  _initViews: function () {
    ...
    if (this.value.get('range') && this._tabPaneTabs.length > 1) {
  ...
  _updateFixedValue: function () {
    var range = this.value.get('range');
    if (range) {
      // when coming from range calculate the average...
      var avg = 0.5 * (+range[0] + +range[1]);
      this.value.set('fixed', avg);
      this.value.unset('range');
    }
  },

  _updateRangeValue: function () {
    var fixed = this.value.get('fixed');
    if (fixed !== null && fixed !== undefined && this.model.get('attribute')) {
      var editorAttrs = this.options.editorAttrs;
      var range = (editorAttrs && editorAttrs.defaultRange) || [fixed, fixed];
      this.value.set('range', range);
      this.value.unset('fixed');
    }
  },
