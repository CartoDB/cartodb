...
cdb.config.setUrlVersion = function(modelName, v) {
  cdb.config.set(modelName + '_url_version', v || 'v1');
}

cdb.config.urlVersion = function(modelName) {
  var version = cdb.config.get(modelName + '_url_version')
  return version || 'v1';
}
...
(function() {
  ...
  Backbone.sync = function(method, model, options) {
    ...
    if (method !== 'read') {
      ...
      if (model.subrogateKeys) {
        Backbone.cachedSync.invalidateSubgorateKeys(getValue(model, 'subrogateKeys'));
      }
    }
  ...
  Backbone.cachedSync = function(namespace, sync) {
    ...
    Backbone.cachedSync.subrogateKeys[subrogateKey] = cached;

    return cached;
  }

  Backbone.cachedSync.subrogateKeys = {};

  Backbone.cachedSync.invalidateSubgorateKeys = function(keys) {
    _.each(keys, function(k) {
      var s = Backbone.cachedSync.subrogateKeys[k];
      if (s) {
        s.invalidate();
      } else {
        cdb.log.debug("subrogate key not found: " + k);
      }
    });
  }
