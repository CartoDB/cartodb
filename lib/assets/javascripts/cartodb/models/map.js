...
cdb.admin.CartoDBLayer = cdb.geo.CartoDBLayer.extend({
  ...
  moveHistoryPosition: function(property, n) {
    ...
    if(newPosition >= 0 && newPosition < this.get(property+'_history').length) {
      ...
      this.set(property, this.get(property+'_history')[newPosition], { silent: true });
      this.trigger('refresh:'+property);
    } else {
      if(newPosition < 0 && Math.abs(newPosition) <= this[property+'_storage'].get().length) {
        this[property+'_history_position'] = newPosition;
        this.set(property, this[property+'_storage'].get(this[property+'_storage'].get().length - Math.abs(newPosition)), { silent: true });
        this.trigger('refresh:'+property);
      }
    }
  },
  ...
  redoStyle: function() {
    this.moveHistoryPosition('tile_style', 1)
  },
  ...
  redoQuery: function() {
    this.moveHistoryPosition('query', 1)
  },
