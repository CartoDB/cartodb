...
module.exports = CoreView.extend({
  ...
  render: function () {
    ...
    if (this._geometryModel.get('status') !== 'fetched' && this._querySchemaModel.get('status') !== 'fetched') {
  ...
  _initBinds: function () {
    this._geometryModel.bind('change:status', this._onStatusChange, this);
    this._querySchemaModel.bind('change:status', this._onStatusChange, this);
  ...
  _onStatusChange: function () {
    ...
    var geometryStatus = this._geometryModel.get('status');

    if (querySchemaStatus !== 'fetched' && querySchemaStatus !== 'unfetched' &&
        geometryStatus !== 'fetched' && geometryStatus !== 'unfetched') {
      this.render();
    }
  },
  ...
  _renderBodyRow: function (mdl) {
    var view = new TableBodyRowView({
      ...
      simpleGeometry: this._geometryModel.get('simple_geom'),
      tableViewModel: this._tableViewModel
    });
    this.addView(view);
