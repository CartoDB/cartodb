describe("cdb.admin.LayerPanelView", function() {
  ...
  beforeEach(function() {
    ...
    this.createView = function() {
      if (view) view.clean(); // remove prev if there is any
  ...
  it('should show UI features that might be disabled by default', function() {
    expect(view.$('.sql_mod')[0]).toBeDefined();
    expect(view.$('.cartocss_mod')[0]).toBeDefined();
  });

  describe('when some UI features are disabled', function() {
    beforeEach(function() {
      user.set('feature_flags', ['disabled_ui_sql', 'disabled_ui_cartocss']);
      this.createView(); // since the modules are added when the view is created, not when rendered
    });

    it('should now show the SQL or CartoCSS modules', function() {
      expect(view.$('.sql_mod')[0]).not.toBeDefined();
      expect(view.$('.cartocss_mod')[0]).not.toBeDefined();
    });
  });
  ...
  describe("layer info", function() {
    ...
    it("should add alias if the layer is in a visualization", function() {
      ...
      view.model.set('table_name_alias', 'test_jamon');
      view.setLayerName(view.dataLayer);

      expect(view.$('.layer-info .info .name').text()).toBe('test jamon');
      expect(view.$('.layer-info .info .order').text()).toBe('1');
      expect(view.$('.layer-info .info .name_info').text()).toBe('view of test jamon');
    });
