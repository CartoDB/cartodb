...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    var addGeom = this.options.menu.addToolButton('add_feature', ['mapLite', 'map']);
  ...
  enableMap: function() {
    ...
    if (!this.map_enabled) {
      ...
      if (this.map.getBaseLayer()) {
        this.baseLayerChooser.setActiveBaselayer();
      }
      ...
      this.retrigger('editInfoWindow', this.infowindow);
