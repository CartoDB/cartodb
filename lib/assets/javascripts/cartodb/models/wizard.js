...
cdb.admin.WizardProperties = cdb.core.Model.extend({
  ...
  _getPreviousState: function(type, geom) {
    var k = type + "_" + geom;
  ...
  active: function(type, props, options) {
    ...
    var defaultWizardProperties = this.defaultStyleForType(type);
    var previousWizardProperties = {};
    var currentWizardProperties = {};

    if (options.restore) {
      previousWizardProperties = this._getPreviousState(type, currentGeom);
    }
