cdb.admin.overlays.MobileLayers = cdb.core.View.extend({
  ...
  _renderLayer: function(data) {
    ...
    if (data.wizard_properties) {
      ...
      data.wizard_properties.off("change");

      var self = this;

      data.wizard_properties.on("change", function(model) {
        if (model.get("type") === "torque") {
          //debugger;
        }
      }, this);
...
cdb.admin.overlays.MobileLayer = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    if (this.model.legend) {
      ...
      this.model.legend.off("change");

      this.model.legend.on("change", function() {
        this.render();
      }, this);
  ...
  _renderLegend: function() {
    ...
    //legend.model.on("change", function() {
      //debugger;
    //}, this);
...
cdb.admin.overlays.Mobile = cdb.core.View.extend({
  ...
  _removeSearch: function() {
    ...
    //if (!this.search && this.layers.getLayerCount() === 0) this.model.set("open", false);
