...
(function() {
  ...
  cdb.admin.Row = cdb.core.Model.extend({
    ...
    fetch: function() {
      ...
      var sql = null;
      if(table.sqlView) {
        sql = 'WITH q AS (' + table.data().getSQL() + ') SELECT *, ST_AsGeoJSON(q.the_geom,4) as cdb_geojson from q OFFSET 1 LIMIT 1';
      } else {
        sql = 'SELECT *, ST_AsGeoJSON(the_geom,4) as the_geom from ' + table.get('name') + ' WHERE cartodb_id = ' + this.get('cartodb_id');
      }
    ...
    hasGeometry: function() {
      try {
        var g = JSON.parse(this.get('the_geom'));
  ...
  cdb.admin.SQLViewData = cdb.admin.CartoDBTableData.extend({
    ...
    * For security reasons, we need them to be send as a POST request.
    * @method sync
    ...
    */
    ...
    sync: function(method, model, options) {
      return Backbone.sync.call(this, this.syncMethod, this, options)
    },
    ...
    refillTableList: function(size) {
      ...
      if(elementToAdd) {
        $.when(this.fetchButNotSet()).done(function(res) {
          ...
          for(var i = currentSize; i < limit; i++) {
            self.add(res.tables[i]);
