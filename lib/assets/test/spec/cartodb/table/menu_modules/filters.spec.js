...
describe("cdb.admin.Filters", function() {
  ...
  it("should apply queyr on changes", function() {
    filterView.filters.add(filter);
    filterView._filtersChanged();
    expect(filterView.options.dataLayer.sqlView.getSQL()).toEqual(
      "SELECT * FROM tst WHERE (test >= 1 AND test < 10)"
    )
  });
  ...
  describe('cdb.admin.mod.SelectorFilter', function() {
    ...
    it("should disable/enable items", function() {
      ...
      $(view.$('ul.items li')[0]).trigger('click');
