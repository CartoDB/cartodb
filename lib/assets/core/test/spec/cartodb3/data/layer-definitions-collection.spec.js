...
describe('data/layer-definitions-collection', function () {
  ...
  describe('.findNonGeoreferencedLayer', function () {
    beforeEach(function () {
      this.collection.reset([{
        id: 'l-0',
        order: 0,
      ...
      this.layerDefinitionModel = this.collection.at(1);
      this.queryRowsCollection = this.a1.queryRowsCollection;
      this.queryGeometryModel = this.a1.queryGeometryModel;
      this.querySchemaModel = this.a1.queryGeometryModel;

      this.queryRowsCollection.statusModel.attributes.status = 'fetched';
      this.queryRowsCollection.reset([{}]);
      this.querySchemaModel.attributes.status = 'fetched';
      this.queryGeometryModel.attributes.status = 'fetched';

      spyOn(this.a1, 'isCustomQueryApplied').and.returnValue(false);
      spyOn(this.layerDefinitionModel, 'hasAnalyses').and.returnValue(false);
    });

    it('should take first data layer if layerDefinitionModel is not passed as argument', function () {
      this.queryRowsCollection.statusModel.attributes.status = 'fetched';
      this.queryRowsCollection.reset([{}]);
      this.querySchemaModel.attributes.status = 'fetched';
      this.queryGeometryModel.attributes.status = 'fetched';
      this.queryGeometryModel.attributes.simple_geom = '';
      this.collection.findNonGeoreferencedLayer(this.callback);
      expect(this.callback).toHaveBeenCalledWith(this.layerDefinitionModel);
    });

    describe('there is a non georeferenced layer', function () {
      it('should not return georeferenced layer if it has a custom query applied', function () {
        this.a1.isCustomQueryApplied.and.returnValue(true);
        this.collection.findNonGeoreferencedLayer(this.callback, this.layerDefinitionModel);
        expect(this.callback).not.toHaveBeenCalled();
      });

      it('should not return georeferenced layer if it has analyses', function () {
        this.layerDefinitionModel.hasAnalyses.and.returnValue(true);
        this.collection.findNonGeoreferencedLayer(this.callback, this.layerDefinitionModel);
        expect(this.callback).not.toHaveBeenCalled();
      });

      it('should not return georeferenced layer if it doesn\'t have any data', function () {
        this.queryRowsCollection.statusModel.attributes.status = 'fetched';
        this.queryRowsCollection.reset([]);
        this.querySchemaModel.attributes.status = 'fetched';
        this.queryGeometryModel.attributes.status = 'fetched';
      ...
      it('should return non georeferenced layer if it doesn\'t contain geometry', function () {
        this.queryRowsCollection.statusModel.attributes.status = 'fetched';
        this.queryRowsCollection.reset([{}]);
        this.querySchemaModel.attributes.status = 'fetched';
        this.queryGeometryModel.attributes.status = 'fetched';
        ...
        spyOn(this.collection, 'findNonGeoreferencedLayer').and.callThrough();
        this.collection.findNonGeoreferencedLayer(this.callback, this.layerDefinitionModel);
        expect(this.callback).toHaveBeenCalledWith(this.layerDefinitionModel);
        expect(this.collection.findNonGeoreferencedLayer.calls.count()).toBe(1);
      });

      it('should return next layer when current is not valid', function () {
        this.collection.add({
          id: 'l-2',
          order: 1,
          kind: 'carto',
        ...
        // Make l-2 a layer with geom
        var layerDefinitionModel = this.collection.at(2);
        var queryRowsCollection = this.b1.queryRowsCollection;
        var queryGeometryModel = this.b1.queryGeometryModel;
        var querySchemaModel = this.b1.queryGeometryModel;

        queryRowsCollection.statusModel.attributes.status = 'fetched';
        queryRowsCollection.reset([{}]);
        querySchemaModel.attributes.status = 'fetched';
        queryGeometryModel.attributes.status = 'fetched';
        queryGeometryModel.attributes.simple_geom = 'point';
        spyOn(this.b1, 'isCustomQueryApplied').and.returnValue(false);
        spyOn(layerDefinitionModel, 'hasAnalyses').and.returnValue(false);

        // Make l-1 a layer without geom
        this.queryRowsCollection.statusModel.attributes.status = 'fetched';
        this.queryRowsCollection.reset([{}]);
        this.querySchemaModel.attributes.status = 'fetched';
        this.queryGeometryModel.attributes.status = 'fetched';
        ...
        spyOn(this.collection, 'findNonGeoreferencedLayer').and.callThrough();
        this.collection.findNonGeoreferencedLayer(this.callback, layerDefinitionModel);
        expect(this.callback).toHaveBeenCalledWith(this.collection.at(1));
        expect(this.collection.findNonGeoreferencedLayer.calls.count()).toBe(2);
      });
    });

    describe('there is NOT a non georeferenced layer', function () {
      it('should not return callback or loop again', function () {
        this.queryRowsCollection.statusModel.attributes.status = 'fetched';
        this.queryRowsCollection.reset([{}]);
        this.querySchemaModel.attributes.status = 'fetched';
        this.queryGeometryModel.attributes.status = 'fetched';
        this.queryGeometryModel.attributes.simple_geom = 'point';
        spyOn(this.collection, 'findNonGeoreferencedLayer').and.callThrough();
        this.collection.findNonGeoreferencedLayer(this.callback);
        expect(this.callback).not.toHaveBeenCalled();
        expect(this.collection.findNonGeoreferencedLayer.calls.count()).toBe(1);
