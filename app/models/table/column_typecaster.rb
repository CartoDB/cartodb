module CartoDB
  class ColumnTypecaster
    ...
    def run
      ...
      user_database.transaction { straight_cast(@new_type.convert_to_db_type) }
      ...
      user_database.transaction do
        ...
        send conversion_method_for(old_type, new_type)
    ...
    def column_type(column_name)
      user_database.schema(table_name, schema: @schema, reload: true).select do |c|
        c[0] == column_name.to_sym
      end.flatten.last.fetch(:db_type).to_s
    end # column_type
    ...
    alias number_to_datetime boolean_to_datetime
    ...
    def pg_replace_expression(column_name, replacement_map=[])
      ...
      expression = %{replace("#{column_name}", '#{k1}', '#{v1}')}

      # rest
      replacement_map.each do |k, v|
        expression = %{replace(#{expression}, '#{k}', '#{v}')}
      end
