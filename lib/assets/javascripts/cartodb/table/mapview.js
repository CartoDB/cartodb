...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    _.bindAll(this, "_renderOverlays", "_renderOverlay", "_addOverlays", "_bindMapOptions", "_bindOverlays");
  ...
  clearMap: function() {
    ...
    this.widgetsDropdown.clean();
    this.mapOptionsDropdown.clean();
  ...
  _getHeaderOverlay: function() {
    ...
    var overlays = this.overlays.filter(function(m) { return m.get("type") == "title" });
    return overlays[0];
  ...
  _onChangeCanvasMode: function() {
    ...
    if (mode == "desktop") {
      ...
      var el = $("div.map div.cartodb-map"),
          curWidth   = el.width(),
          curHeight  = el.height(),
          autoWidth  = el.css('width', 'auto').width(); //temporarily change to auto and get the width.
          autoHeight = el.css('height', 'auto').height(); //temporarily change to auto and get the width.

          el.width(curWidth).animate({ left: "15px", top: "70", marginLeft: "0", marginTop: "0"}, 200, function() { });

          el.height(curHeight).animate({ height: autoHeight }, 70, function() {
            el.css('height', 'auto'); 
          });

          el.width(curWidth).animate({ width: autoWidth }, 70, function() {
            el.css('width', 'auto'); 
          });
      ...
      $("div.map div.cartodb-map").animate({ width: 229, marginLeft: -229/2, left: "50%" }, 200);
      $("div.map div.cartodb-map").animate({ height: 327, marginTop: -327/2,   top: "50%" }, 200);
    }
  ...
  _onChangeHeaderContents: function() {
    ...
    headerOverlay.set({ title: this.vis.get("name"), description: this._getMarkdown(description) });
  ...
  _setupTitleOverlay: function(widget) {

    // Update the title and description in case they were changed somewhere
    widget.model.set({ title: this.vis.get("name"), description: this._getMarkdown(this.vis.get("description")) }, { silent: true });

    window.header = widget; // TODO: remove this

    widget.bind("hide_title", function() {
      table.mapTab.$el.removeClass("show_title");
    });

    widget.bind("hide_description", function() {
      table.mapTab.$el.removeClass("show_description");
    });

    widget.bind("show_title", function() {
      table.mapTab.$el.addClass("show_title");
    });

    widget.bind("show_description", function() {
      table.mapTab.$el.addClass("show_description");
    });

    widget.bind("hidden", function() {
      table.mapTab.$el.removeClass("show_description");
      table.mapTab.$el.removeClass("show_title");
    });
  ...
  _renderOverlay:  function(delayed_animation, data) {
    ...
    var widget = new cdb.admin.widgets[type]({ model: data });

    if (type == 'Title') { this._setupTitleOverlay(widget); }
    ...
    var model = new cdb.admin.models.Widget({
      type: "title",
        ...
        extra: {
          ...
          description:      this._getMarkdown(this.vis.get("description")),
          show_title:       false,
  ...
  _onResetOverlays: function(overlays) {
    ...
    if (this._hasTitleOverlay() == undefined) {
      this._addTitleOverlay();
    }
