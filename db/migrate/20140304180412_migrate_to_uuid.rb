Sequel.migration do
  up do
    ...
    TABLE_LIST.each do |table_name, columns|
      fields_count += columns.length
      table_rows = SequelRails.connection[table_name.to_sym].count
      tables_data = !(table_rows == 0)
      columns.each do |column|
        result = SequelRails.connection.fetch(%{
          SELECT data_type
          FROM information_schema.columns
          WHERE table_name='#{table_name}' AND column_name='#{column}';
        }).first
        next unless result[:data_type] != 'uuid'

        outdated_fields[table_name] = [] if outdated_fields[table_name].nil?
        outdated_fields[table_name] << column
      end
    end
    outdated_fields_count = 0
    outdated_fields.each do |_k, v|
      outdated_fields_count += v.length
    end
    ...
    if tables_data == false
      if outdated_fields_count > 0 # one or more ids are not uuid
        uuid_extension = SequelRails.connection.fetch(%{
