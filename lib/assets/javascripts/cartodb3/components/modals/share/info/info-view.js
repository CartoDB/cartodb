...
module.exports = CoreView.extend({
  ...
  render: function () {
    var published = this._mapcapsCollection.length > 0
                    ? _t('components.modals.share.last-version', { date: moment(this._mapcapsCollection.first().get('created_at')).format('Do MMMM YYYY, hh:mm') })
                    : _t('components.modals.share.last-version', { date: moment(this._visDefinitionModel.get('updated_at')).format('Do MMMM YYYY, hh:mm') });
