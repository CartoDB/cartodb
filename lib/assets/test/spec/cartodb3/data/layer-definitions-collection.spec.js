...
describe('data/layer-definitions-collection', function () {
  ...
  describe('.createLayerForTable', function () {
    ...
    it('should place the layer below the "Tiled" layer on top', function () {
      ...
      var newCartoDBLayer = this.collection.createLayerForTable('tableName');

      expect(this.collection.models).toEqual([ CartoDBLayer, newCartoDBLayer, TiledLayer ]);
      expect(this.collection.pluck('order')).toEqual([ 0, 1, 2 ]);
    ...
    it('should place the layer below the "Torque" layer on top', function () {
      ...
      expect(this.collection.models).toEqual([ CartoDBLayer, newCartoDBLayer, TorqueLayer ]);
      expect(this.collection.pluck('order')).toEqual([ 0, 1, 2 ]);
    ...
    it('should place the new layer above the "CartoDB" layer on top', function () {
      ...
      expect(this.collection.models).toEqual([ CartoDBLayer_0, CartoDBLayer_1, newLayer ]);
      expect(this.collection.pluck('order')).toEqual([ 0, 1, 2 ]);
  ...
  describe('.createLayerForAnalysisNode', function () {
    ...
    describe('when given node is a source node', function () {
      beforeEach(function () {
        ...
        spyOn(this.collection, 'create').and.callThrough();
        this.collection.createLayerForAnalysisNode('a0', 2);
      ...
      it('should created new layer with table name', function () {
        expect(this.newLayerDefModel.get('table_name')).toEqual('something');
      });

      it('should have re-ordered layers', function () {
        expect(this.collection.pluck('order')).toEqual([0, 1]);
      });

      it('should appear on the given position', function () {
        expect(this.newLayerDefModel.get('order')).toEqual(1);
      });
    });
    ...
    describe('when given node is not a head of any layer', function () {
      ...
      describe('should create new layer with', function () {
        ...
        it('should set table name of prev layer', function () {
  ...
  describe('when analysis node is removed', function () {
    ...
    it('should not have created source nodes for layers that have non-sources', function () {
      expect(this.analysisDefinitionNodesCollection.pluck('id')).not.toContain('c0');
    });

    it('should create source if it matches expected source node id', function () {
      expect(this.analysisDefinitionNodesCollection.pluck('id')).toContain('b0');
