  ...
  initialize: function() {
    ...
    this.options.geocoder.bind("geocodingComplete", function(msg) {
      ...
      if (msg.kind != "high-resolution") { // FREE
        ...
        description = "<p>" + msg.real_rows + " out of " + msg.processable_rows + " rows were succesfully turned into " + msg.geometry_type + "!</p><p>It seems that some of your rows didn't finish succesfully. Perhaps these rows contained empty values or perhaps we just didn't know what the values meant. We encourage you to take a look and try again. This job cost you $0 because we love you for being you!</p>";

      } else {

        if (user_data.geocoding.quota < user_data.geocoding.monthly_use) {
          description = "<p>" + msg.real_rows + " out of " + msg.processable_rows + " rows were succesfully turned into points!</p><p>It seems that some of your rows didn't finish succesfully. Perhaps these rows contained empty values or perhaps we just didn't know what the values ment. Unsuccessful rows don't count against your quota, so we encourage you to take a look and try again.</p><p>This job cost you $0 because we love you! You have " + user_data.geocoding.quota + " geocoding tokens remaining in your free quota this month.</p>"
