...
cdb.admin.dashboard.UserStats = cdb.core.View.extend({
  ...
  _checkTrialSuggestion: function() {
    ...
    if (this.model.get("account_type") == 'FREE' && this.model.get("table_count") > 0) {
  ...
  */
  ...
  _updateGraphs: function() {
    ...
    if (this.model.get('organization') && !this.model.get('organization').owner) {
      this._updateMapViewsGraph();
    } else {
      this._updateMapViewsBar();
    }
  ...
  _updateMapViewsGraph: function() {
    this.$("section.stats .mapviews div.stats").html('');
