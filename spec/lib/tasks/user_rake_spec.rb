...
describe 'user' do
  ...
  describe 'notifications:add_by_field' do
    it 'should not accept wrong or incomplete input' do
      ...
      end.to raise_error(RuntimeError, 'Filter field and value are needed')
      ...
      end.to raise_error(RuntimeError, 'Notification not provided. Please include it')

      expect do
        Rake::Task['user:notifications:add_by_field'].reenable
        Rake.application.invoke_task 'user:notifications:add_by_field[unknown_field,localhost,test notification!]'
      end.to raise_error(RuntimeError, 'Unknown field unknown_field for filtering. Allowed fields are database_host')
    end
