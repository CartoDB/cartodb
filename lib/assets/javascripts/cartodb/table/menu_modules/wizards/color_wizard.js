  ...
  cdb.admin.mod.ColorMapWizard = cdb.admin.mod.SimpleWizard.extend({
    ...
    _bindChanges: function() {
      ...
      var func = function(m,c) {
        var colors = self.cartoProperties.get('colors');
        if (colors && colors.length) {
          console.log("jam");
          self.cartoProperties.unbind('change:colors', func);
          // self._renderValues(colors);
        }
      }

      this.cartoProperties.bind('change:colors', func);
