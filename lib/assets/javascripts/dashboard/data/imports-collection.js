...
const ImportsModel = require('./import-model');
...
module.exports = Backbone.Collection.extend({
  ...
  parse: function (r) {
    if (r.imports.length === 0) {
      ...
      _.each(r.imports, id => {
        ...
        var imports = self.filter(mdl => mdl.imp.get('item_queue_id') === id);

        if (imports.length === 0) {
          this.add(new ImportsModel({ id: id }, { user: this.user } ));
        }
      });
    }
  ...
  canImport: function () {
    const importQuota = this.user.getMaxConcurrentImports();
