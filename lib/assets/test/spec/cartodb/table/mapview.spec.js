describe("mapview", function() {
  ...
  beforeEach(function() {
    ...
    spyOn(view, '_launchPecan');

    var featureEnable = sinon.stub(view.user, 'featureEnabled');
    featureEnable.returns(true);
  ...
  xit("should launch pecan if rows < MAX_PECAN_ROWS", function() {
    view.options.table = TestUtil.createTable('test', [['the_geom', 'geometry'], ['name','string']], []);
    var s = sinon.stub(view.options.table, 'data');
    s.returns({ length: 10 });
    view._showPecan();
    expect(view._launchPecan).toHaveBeenCalled();
  });

  it("shouldn't launch pecan if rows > MAX_PECAN_ROWS", function() {
    view.options.table = TestUtil.createTable('test', [['the_geom', 'geometry'], ['name','string']], []);
    var s = sinon.stub(view.options.table, 'data');
    s.returns({ length: view.MAX_PECAN_ROWS + 100 });
    view._showPecan();
    expect(view._launchPecan).not.toHaveBeenCalled();
  });
