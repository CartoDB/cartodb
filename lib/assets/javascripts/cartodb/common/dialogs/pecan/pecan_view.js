...
module.exports = BaseDialog.extend({
  ...
  _printStats: function(name, stats) {
    ...
    console.log('%c · type = ' + type, 'color:#666;');
    console.log('%c · distinctPercentage = ' + distinctPercentage, 'color:#666;');
    console.log('%c · count = ' + count, 'color:#666;');
    console.log('%c · null ratio = ' + null_ratio, 'color:#666;');

    if (skew) {
      console.log("%c - skew: " + skew.toFixed(2), "color:#666;");
    } else {
      console.log("%c - skew: " + skew, "color:#666;");
    }

    if (weight) {
      console.log("%c - weight: " + weight.toFixed(2), "color: #666;");
    } else {
      console.log("%c - weight: " + weight, "color:#666;");
    }
  
  },

  _rejectMessage: function(name, message, value) {
    console.log("%c"+ name + ' rejected because ' + message + ": " + value, "font-weight: bold; color:red;");
  },

  _approveMessage: function(name) {
    console.log("%c"+ name + ' is approved', "font-weight: bold; color:green;");
  },
  ...
  _analyzeString: function(name, stats) {
    ...
    if (stats.weight >= 0.8) {
      return true;
    } else if (stats.weight < 1 || !stats.weight) {
  ...
  _analyzeNumber: function(name, stats) {
    ...
    if (stats.weight < 1 || !stats.weight) {
      this._rejectMessage(name, "weight is 0", stats.weight);
      return false;
    }
  ...
  __analyzeStats: function(name, stats) {

    var type = stats.type;

    console.log("%canalyzing " + name, "text-decoration:underline;");

    if ((type === "number" || type === "string") && stats.null_ratio > 0.95) {
      console.log("%c"+name + ' rejected because the null ratio > 95%: ' + stats.null_ratio * 100, "font-weight: bold; color:red;");
      return false;
    } else if (type === 'date' && stats.null_ratio > 0.75) {
      console.log("%c"+name + ' rejected because the null ratio > 75%: ' + stats.null_ratio * 100, "font-weight: bold; color:red;");
      return false;
    } else if (type === 'boolean' && stats.null_ratio > 0.75) {
      console.log("%c"+name + ' rejected because the null ratio > 75%: ' + stats.null_ratio * 100, "font-weight: bold; color:red;");
      return false;
    ...
    console.log('%c - type = ' + stats.type, 'color:#666;');
    console.log('%c - distinctPercentage = ' + distinctPercentage, 'color:#666;');
    console.log('%c - count = ' + stats.count, 'color:#666;');
    console.log('%c - null_ratio = ' + stats.null_ratio, 'color:#666;');
    ...
    //if (type === 'string' && (distinctPercentage < 2.5) {
      //console.log("%c" + name + ' rejected because distinctPercentage < 5%: ' + distinctPercentage, "font-weight: bold; color:red;");
      //return false;
    //}
