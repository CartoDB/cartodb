describe("geocoder user stats", function() {
  ...
  it("should change progress class element when user doesn't have more rows available and text for upgrading", function() {
    ...
    expect(view.$('p').html()).toBe('Not enough geocoding credits this month. <a href="http://localhost:3000/account/staging20/upgrade">Upgrade your account</a> to get more.')
  })
  ...
  it("should change text and progress class element when doesn't have more rows available", function() {
    ...
    expect(view.$('div.progress > span.used').hasClass('danger')).toBeTruthy();
    expect(view.$('a.need').length).toBe(0);
    expect(view.$('p').html()).toBe('Not enough geocoding credits this month. <strong>From now on you will be charged $1.5 per 1,000 rows</strong>.')
  });
  ...
  it("should change text and progress class element when doesn't have more rows available", function() {
    ...
    expect(view.$('div.progress > span.used').hasClass('danger')).toBeFalsy();
    expect(view.$('a.need').length).toBe(0);
    expect(view.$('p').html()).toBe('Not enough geocoding credits this month. <strong>After limits you will be charged $1.5/1,000 rows</strong>.')
  })
