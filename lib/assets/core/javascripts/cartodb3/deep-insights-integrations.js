...
F.prototype._onLayerDefinitionAdded = function (m, c, opts) {
  ...
  if (!layerTypesAndKinds.isCartoDBType(m.get('type')) || !layerTypesAndKinds.isTorqueType(m.get('type'))) {
    return;
  }
...
var GOOGLE_MAPS_CATEGORY = 'GMaps';
var PROVIDER_LEAFLET = 'leaflet';
var PROVIDER_GOOGLE_MAPS = 'googlemaps';

var convertCategoryToProvider = function (baseLayerCategory) {
  if (baseLayerCategory && baseLayerCategory === GOOGLE_MAPS_CATEGORY) {
...
F.prototype._onBaseLayerChanged = function () {
  ...
  var previousMapProvider = convertCategoryToProvider(baseLayerDefinition.previous('category'));
  var newMapProvider = convertCategoryToProvider(baseLayerDefinition.get('category'));
  var mapProviderChanged = newMapProvider !== previousMapProvider;
  ...
  if (newBaseLayerAttrs.type) {
    ...
    cdbjsLayer.update(newBaseLayerAttrs, { silent: handleLayersSilently });
  }
  ...
  if (labelsLayerDefinition) {
    if (cdbjsHasLabelsLayer) {
      cdbjsTopLayer.update(labelsLayerDefinition.changedAttributes(), { silent: mapProviderChanged });
    } else {
