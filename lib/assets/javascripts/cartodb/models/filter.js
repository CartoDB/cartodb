...
cdb.admin.models.Filter = cdb.core.Model.extend({
  ...
  getSQLConditionForDate: function() {
    ...
    if (this.attributes.upper >= this.attributes.upper_limit) return _.template(" (<%= column %> >= date('<%= lower %>') AND <%= column %> <= date('<%= upper %>')) ")(options);
    return _.template(" (<%= column %> >= date('<%= lower %>') AND <%= column %> < date('<%= upper %>')) ")(options);
