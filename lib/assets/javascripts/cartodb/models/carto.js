...
function cluster_sql(table, zoom, props, nquartiles) {
 ...
 var sql = "       WITH metatile_extent AS ( " +
"          SELECT ext, ST_XMin(ext) xmin, ST_YMin(ext) ymin FROM (SELECT ST_SetSRID(!bbox!::box3d, 3857) as ext) a " +
"        ), " +
"        filtered_table AS ( " +
"          SELECT t.* FROM <%= table %> t, metatile_extent m WHERE t.the_geom_webmercator && m.ext " +
"        ), " +
"        big_snap AS (SELECT ST_SnapToGrid(f.the_geom_webmercator, 0, 0, greatest(!pixel_width!,!pixel_height!) * <%= size %>, greatest(!pixel_width!,!pixel_height!) * <%= size %>) the_geom_webmercator, count(*) as points_count, 1 as cartodb_id, array_agg(f.cartodb_id) AS id_list FROM filtered_table f, metatile_extent m  GROUP BY ST_SnapToGrid(f.the_geom_webmercator, 0, 0, greatest(!pixel_width!,!pixel_height!) * <%= size %>, greatest(!pixel_width!,!pixel_height!) * <%= size %>), m.xmin, m.ymin), " +
"        bigs AS (SELECT * FROM big_snap WHERE points_count > <%= size %> ), " +
"        mid_snap AS (SELECT ST_SnapToGrid(f.the_geom_webmercator, 0, 0, greatest(!pixel_width!,!pixel_height!) * 0.75 * <%= size %>, greatest(!pixel_width!,!pixel_height!) * 0.75 * <%= size %>) the_geom_webmercator, count(*) as points_count, 1 as cartodb_id, array_agg(f.cartodb_id) AS id_list FROM filtered_table f, metatile_extent m WHERE cartodb_id NOT IN (select unnest(id_list) FROM bigs) GROUP BY ST_SnapToGrid(f.the_geom_webmercator, 0, 0, greatest(!pixel_width!,!pixel_height!) * 0.75 * <%= size %>, greatest(!pixel_width!,!pixel_height!) * 0.75 * <%= size %>), m.xmin, m.ymin), " +
"        mids AS (SELECT * FROM mid_snap WHERE points_count > <%= size %> * 0.5), " +
"        small_snap AS (SELECT ST_SnapToGrid(f.the_geom_webmercator, 0, 0, greatest(!pixel_width!,!pixel_height!) * 0.5 * <%= size %>, greatest(!pixel_width!,!pixel_height!) * 0.5 * <%= size %>) the_geom_webmercator, count(*) as points_count, 1 as cartodb_id, array_agg(f.cartodb_id) AS id_list FROM filtered_table f, metatile_extent m WHERE cartodb_id NOT IN (select unnest(id_list) FROM bigs) AND cartodb_id NOT IN (select unnest(id_list) FROM mids) GROUP BY ST_SnapToGrid(f.the_geom_webmercator, 0, 0, greatest(!pixel_width!,!pixel_height!) * 0.5 * <%= size %>, greatest(!pixel_width!,!pixel_height!) * 0.5 * <%= size %>), m.xmin, m.ymin), " +
"        smalls AS (SELECT * FROM small_snap WHERE points_count > GREATEST(<%= size %> * 0.1, 2)) " +
...
"        UNION ALL " +
"        SELECT *, 'bucketA' as src, array_to_string(id_list, ',') cdb_list FROM bigs " +
"        UNION ALL " +
"        SELECT *, 'bucketB' as src, array_to_string(id_list, ',') cdb_list FROM mids " +
"        UNION ALL " +
"        SELECT *, 'bucketC' as src, array_to_string(id_list, ',') cdb_list FROM smalls ";
