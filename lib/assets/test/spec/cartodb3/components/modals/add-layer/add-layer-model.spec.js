...
describe('components/modals/add-layer/add-layer-model', function () {
  beforeEach(function () {
    ...
    this._layerDefinitionsCollection.createCartoDBLayerFromTableName = jasmine.createSpy('createCartoDBLayerFromTableName');
    ...
    describe('when an dataset is selected', function () {
      ...
      describe('when dataset is not a remote one', function () {
        ...
        it('should create layer from dataset', function () {
          expect(this._layerDefinitionsCollection.createCartoDBLayerFromTableName).toHaveBeenCalled();
        });

        it('should create layer with expected params', function () {
          expect(this._layerDefinitionsCollection.createCartoDBLayerFromTableName.calls.argsFor(0)[0]).toBe('tableName');
        });
        ...
        describe('when adding layer succeeds', function () {
          beforeEach(function () {
            ...
            this._layerDefinitionsCollection.createCartoDBLayerFromTableName.calls.argsFor(0)[1].success();
          });
        ...
        describe('when adding layer fails', function () {
          beforeEach(function () {
            this._layerDefinitionsCollection.createCartoDBLayerFromTableName.calls.argsFor(0)[1].error();
          });
    ...
    describe('.createFromScratch', function () {
      ...
      describe('when table is successfully created', function () {
        it('should add the table as new layer', function () {
          ...
          expect(this._layerDefinitionsCollection.createCartoDBLayerFromTableName).toHaveBeenCalled();
          expect(this._layerDefinitionsCollection.createCartoDBLayerFromTableName.calls.argsFor(0)[0]).toBe('name-just-for-testing-purposes');
        });
      });
    });
