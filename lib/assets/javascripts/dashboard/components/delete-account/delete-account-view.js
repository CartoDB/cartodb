...
module.exports = CoreView.extend({
  ...
  initialize: function (options) {
    ...
    this._useClientToSendRequest = _.isUndefined(options.useClientToSendRequest) ? true : options.useClientToSendRequest;

    // Make client and onError mandatory when client is used
    if (this._useClientToSendRequest) {
      this._onError = options.onError;
      this._client = options.client;

      if (!this._onError || !this._client) {
        throw new Error('onError and client are required');
      }
    }
  },
