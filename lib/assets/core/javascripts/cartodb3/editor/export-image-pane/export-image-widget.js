...
module.exports = CoreView.extend({
  ...
  _initBinds: function () {
    ...
    this._mapView.bind('dragend zoomend', this._onDrag, this);
    $('.CDB-Map').on('mousemove', this._onMouseMove.bind(this));
  },
  ...
  _onMouseMove: function (e) {
    if (this.$el.hasClass('is-disabled')) {
      ...
      if (x - 5 < left || x > left + width + 5 || y - 5 < top || y > top + height + 5) {
        this.$el.addClass('is-disabled');
      } else {
        this.$el.removeClass('is-disabled');
      }
    } else {
      if ($(e.target).hasClass('ExportHelper')) {
        this.$el.addClass('is-disabled');
      } else {
        this.$el.removeClass('is-disabled');
      }
  ...
  _onResizeWindow: function (e) {
    if (e.target === window) {
      var coordinates = this._mapView.latLngToContainerPoint({ 
