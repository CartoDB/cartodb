namespace :cartodb do
  ...
  namespace :db do
    ...
    task setup_user: :environment do
      ...
      u.builder_enabled = true if ENV['BUILDER_ENABLED'] == 'true'
    ...
    task create_dev_user:       ['cartodb:db:create_publicuser'] do
      raise 'You should provide a valid e-mail'    if ENV['EMAIL'].blank?
      raise 'You should provide a valid password'  if ENV['PASSWORD'].blank?
      raise 'You should provide a valid subdomain' if ENV['SUBDOMAIN'].blank?
      ...
      user.builder_enabled = true if ENV['BUILDER_ENABLED'] == 'true'
    ...
    task create_federated_server: :environment do
      ...
      unless system("psql -U postgres -h 127.0.0.1 -p #{port} -c \"CREATE ROLE #{user} PASSWORD '#{user}' SUPERUSER CREATEDB CREATEROLE INHERIT LOGIN;\"")
        raise('Could not create the new remote user')
      end
      unless system("psql -U postgres -h 127.0.0.1 -p #{port} -c \"CREATE DATABASE #{user} OWNER '#{user}';\"")
        raise('Could not create the new remote database')
      end
      unless system("psql -U postgres -h 127.0.0.1 -p #{port} -d #{user} -c \"CREATE EXTENSION postgis;\"")
        raise('Could not install postgis in the new remote database')
      end
    end
    ...
    task create_user: :environment do
      ...
      u.password_confirmation = ENV['PASSWORD']
      u.username = ENV['SUBDOMAIN']
      u.database_host = if ENV['DATABASE_HOST'].blank?
                          ::SequelRails.configuration.environment_for(Rails.env)['host']
                        else
                          ENV['DATABASE_HOST']
                        end
