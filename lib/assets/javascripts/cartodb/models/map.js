...
cdb.admin.CartoDBLayer = cdb.geo.CartoDBLayer.extend({
  ...
  bindLegend: function(legend) {
    ...
    this.legend.bind('change:type change:title change:show_title change:items', _.debounce(function() {
...
cdb.admin.Layers = cdb.geo.Layers.extend({
  ...
  add: function(models, options) {
    ...
    if(torque_layers > 1) {
      this.trigger('error', "only one torque layer is allowed per map")
      return;
    }
  ...
  getDataLayers: function() {
    var DATA_LAYERS = ['CartoDB', 'torque'];
    return this.filter(function(lyr) {
      return _.contains(DATA_LAYERS, lyr.get('type'));
    });
  },
    ...
    table.fetch({
      success: function() {
        ...
        map.layers.bind('reset', function() {
          ...
          self.layers.bind('error', function addingError() {
            self.layers.unbind('error', addingError);
            cdb.god.trigger('error', "only one torque layer per visualization is allowed", 'error');
          });
