...
Backbone.Form.editors.FillColor = Backbone.Form.editors.Base.extend({
  ...
  _initViews: function () {
    ...
    if (this.model.get('fillColor').range && this._tabPaneTabs.length > 1) {
  ...
  _generateValueContentView: function () {
    ...
    this._fillColorByValueView = new FillColorByValueView({
      ...
      modals: this.options.modals,
      hideTabs: this.options.phideTabs
    });
  ...
  getValue: function (param) {
    ...
    if (this._fillColorSolidView && this._fillColorByValueView) {
      return selectedTabPaneName === SOLID
        ? this._fillColorSolidView._inputCollection.getValues()
        : this._fillColorByValueView._inputCollection.getValues();
    }
