  ...
  cdb.admin.NewLayerDialog = cdb.admin.BaseDialog.extend({
    ...
    _renderDropboxPane: function($content) {
      if (cdb.config.get('oauth_dropbox')) {
        this.dropboxPane = new cdb.admin.ImportServicePane({
          service:          'dropbox',
          acceptFileTypes:  this._UPLOADER.acceptFileTypes,
          acceptSync:       this._UPLOADER.acceptSync
        });
    ...
    _renderGDrivePane: function($content) {
      if (cdb.config.get('oauth_gdrive')) {
        this.gdrivePane = new cdb.admin.ImportServicePane({
          service:          'gdrive',
          acceptFileTypes:  this._UPLOADER.acceptFileTypes,
          acceptSync:       this._UPLOADER.acceptSync
        });
    ...
    _onTablesFetched: function() {
      ...
      // If user has shared the visualization, only tables shared accross
      // visualization users are permitted, showing message!
      // if (this.vis.isVisualization() && this.vis.permission.acl.size() > 0) {
      //   this._showTablePermissionWarning();
      // }
    },
    ...
    _showTablePermissionWarning: function() { this.$('p.warning.permission').fadeIn(); },

    _hideTablePermissionWarning: function() { this.$('p.warning.permission').fadeOut(); },
    ...
    _init_uploader: function($content) {
      ...
      $upload.fileupload({
        ...
        url:                    cdb.config.prefixUrl() + this._UPLOADER.url,
        paramName:              'filename',
    ...
    _uploadData: function(file) {
      if (file.type === "file") {
        ...
        this._uploadFromService(file);
      } else {
        cdb.log.info("upload file type not supported")
      }
    },

    _uploadFromService: function(obj) {
      // Change the state
    ...
    _importSync: function(obj) {
      var self = this;
      var params = ['value', 'interval']
      
      // If it comes from a service type...
      if (obj.type === "service") {
        params = params.concat(['service_name', 'service_item_id']);
      }

      var data = {};
      _.each(obj, function(val, key) {
        if (_.contains(params, key)) {
          data[key === "value" ? 'url' : key] = val;
        }
      });

      // Create synchronization model if it is
      // necessary
      var synchronization = new cdb.admin.TableSynchronization(data);
    ...
    _onUploadError: function(e, data) {
      ...
      if (this.filePane) {
        ...
        if (data.files && data.files.length && !data.files[0].error) {
    ...
    _checkOKButton: function() {
      ...
      switch (option) {
        case 'tables':
          if (this.tableCombo.getSelected()) {
            action = 'removeClass';
          } else {
            action = 'addClass';
          }
      ...
      // Center when a change has happened
      if (this.upload_tabs) {
        this._centerInScreen();
      }
    ...
    _importTable: function(item) {
      ...
      imp.bind("importError", function(m){
        self._showError(
          m.get('error_code'),
          m.get('get_error_text').title,
          m.get('get_error_text').what_about,
          m.get('item_queue_id')
        );
      },this);
    ...
    _centerInScreen: function() {
      var pane = this.upload_panes.activePane;
      var tab = this.upload_panes.activeTab;
      var margin = 120;
      var $modal = this.$(".modal:eq(0)");

      // If tab if from a service...
      if ((tab === "dropbox" || tab === "gdrive") && (pane.model.get('state') === "retrieved")) {
        margin = 50;
      }

      // Animation is running
      if (!$modal.css('marginTop')) return false;
      ...
      switch (state) {
        case 'idle':
          if (option == 'scratch') {
            ...
            var pane = this.upload_panes.activePane;
            pane.submitUpload();
          } else if (option == 'tables' && this.tableCombo.getSelected()) {
            var table_data = this.tableCombo.getSelected();
            var vis = this.tables.find(function(i) { return i.id === table_data.vis_id });

            if (vis) {
              var table_name = vis.get('table').name || table_data.name;
              this.ok && this.ok(table_name);
              this.hide();  
            } 
          }
          break;
