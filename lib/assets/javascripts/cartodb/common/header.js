...
cdb.admin.Header = cdb.core.View.extend({
  ...
  _TEXTS: {
    ...
    rename: {
      ...
      desc_table:   _t('<strong>This change will affect you API calls.</strong> If you are accesing \
  ...
  _shareVisualization: function(e) {
    ...
    if (!this.model.isVisualization()) {
      ...
      // Check if there is any private table in the visualization
      var any_private_table = _.filter(this.model.get("related_tables"), function(table) {
          return table.privacy.toLowerCase() == "private"
        }).length > 0;

      // If so
      if (any_private_table) {
        this.share_dialog = new cdb.admin.SharePrivacyDialog({
          model: this.model,
          ok_next: this._TEXTS.share_privacy.ok_next
        });
        // After making private tables to public
        // we would like to share the visualization
        this.share_dialog.ok = function() {
          var dlg = createShare(self.model, self.options.user, self.options.config);
          dlg.appendToBody().open({ center: true });
        }
      } else {
      // If not, show the share dialog
        this.share_dialog = createShare(this.model, this.options.user, this.options.config);
        center = true;
      }
    }
    ...
    // Generic function to create a new share dialog
    function createShare(vis, user, config) {
    ...
    if (!_.isEqual(this.model.get(attr),value)) {
      ...
      this.model.save(attrs,{
        ...
        error: function(msg, resp) {
          ...
          self.globalError.showError(err, 'info', 3000);
