...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    var isLayerHidden = this._isLayerHidden() ? 'layer-hidden' : '';

    this._infoboxModel = new InfoboxModel({
      state: this._layerDefinitionModel.canBeGeoreferenced() ? 'georeference' : isLayerHidden
    });
  ...
  _infoboxState: function () {
    ...
    if (this._layerDefinitionModel.canBeGeoreferenced()) {
      ...
      this._overlayModel.set({visible: true});
    } else if (!edition && cartocss_custom && !isAutoStyleApplied) {
