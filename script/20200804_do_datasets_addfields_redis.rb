...
$users_metadata.keys("do:#{username}:datasets").each do |k|
  ...
  datasets_enriched = datasets.map do |dataset|
    ...
    if !dataset['sync_status'].present? || !dataset['type'].present?
      begin
        ...
        if !sync_data[:unsyncable_reason].nil? && (sync_data[:unsyncable_reason].include? 'exceeds the quota available')
          sync_data[:unsyncable_reason] = nil
          sync_data[:sync_status] = 'unsynced'
        end
        if sync_data.empty? # it can be a `requested` case
          sync_data[:unsyncable_reason] = nil
          sync_data[:sync_status] = 'unsynced'
        end
