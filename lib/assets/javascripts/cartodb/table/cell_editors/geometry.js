  ...
  cdb.admin.EditGeometryDialog = cdb.admin.SmallDialog.extend({
    ...
    showGeoJSONEditor: function(ev) {
      this.killEvent(ev);
      this.$('.point').slideUp();
      this.$('.geoJSON').slideDown();
      this.$('.switch').removeClass('geoJSONButton')
                       .addClass('pointButton')
                       .removeClass('disabled')
                       .addClass('enabled');
    },

    showPointEditor: function(ev) {
      this.killEvent(ev);
      this.$('.point').slideDown();
      this.$('.geoJSON').slideUp();
      this.$('.switch').addClass('geoJSONButton')
                       .removeClass('pointButton')
                       .addClass('disabled')
                       .removeClass('enabled');
    },
    ...
    _chooseGeom: function() {
      ...
      if (!geom || geom.type.toLowerCase() == "point") {
        ...
        // Fill inputs
        this.$content.find("div.point").fadeIn();
        this.$content.find("div.selector").fadeIn();
        if(geom) {
