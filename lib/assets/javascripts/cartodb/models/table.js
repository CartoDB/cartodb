  ...
  cdb.admin.Row = cdb.core.Model.extend({
    ...
    fetch: function() {
      ...
      var username = (this.options && this.options.user_data)? this.options.user_data.username :
        (window.user_data? window.user_data.username : window.username);
      var table = this.table || this.collection.table;
      ...
      var sql = 'SELECT *, ST_AsGeoJSON(the_geom,4) as geojson from ' + table.get('name') + ' WHERE cartodb_id = ' + this.get('cartodb_id');
      sqlApi.execute(sql).done(function(data){
        self.trigger('loaded');
        if(self.parse(data.rows[0])) {
          data.rows[0].the_geom = data.rows[0].geojson;
