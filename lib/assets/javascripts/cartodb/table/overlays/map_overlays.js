  ...
  setLegend: function (legend) {
    ...
    legend.$el.draggable({
      ...
      create: function(event, ui) {
        legend.$el.css({'width': legend.$el.width() + 1});

        if (legend._models && legend._models.length) {
          // an extra pixel prevents a number of draggable height and width resizing issues
          if (!legend._models[0].get('style')) {
            legend.$el.css({'width': legend.$el.width() + 1});
          }
        }
      },
      ...
      stop: function(event, ui) {
        ...
        legend.$el.css({top: topPct + '%', left: leftPct + '%'});
        legend.$el.removeClass('notransition');

        var style = legend.$el.attr('style');

        // 'legend' here is a stacked legend container view with one or more legends in it
        _.each(legend._models, function(subLegend) {
          subLegend.set('style', style);
        });
      }
   });
