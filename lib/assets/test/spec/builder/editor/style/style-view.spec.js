...
fdescribe('editor/style/style-view', function () {
  beforeEach(function () {
  ...
  describe('._launchOnboarding', function () {
    var resolveGeoreferenceFn = null;
    var resolveGeoreferencePromise = function (value) {
      if (resolveGeoreferenceFn) {
        resolveGeoreferenceFn(value);
      }
    };
    var resolveHasGeomFn = null;
    var resolveHasGeomPromise = function (value) {
      if (resolveHasGeomFn) {
        resolveHasGeomFn(value);
      }
    };

    beforeEach(function () {
      this.view._layerDefinitionModel.canBeGeoreferenced = function () {
        return new Promise(function (resolve, reject) {
          resolveGeoreferenceFn = resolve;
        });
      };
      this.view._queryGeometryModel.hasValueAsync = function () {
        return new Promise(function (resolve, reject) {
          resolveHasGeomFn = resolve;
        });
      };
    ...
    afterEach(function () {
      resolveGeoreferenceFn = null;
      resolveHasGeomFn = null;
    });
    ...
    it('should do nothing if it is editing', function (done) {
      ...
      this.view._launchOnboarding();
      resolveGeoreferencePromise(false);
      resolveHasGeomPromise(true);

      setTimeout(function () {
        expect(self.view._onboardingLauncher.launch).not.toHaveBeenCalled();
        done();
      }, 0);
    });
    ...
    it('should do nothing if it can be georeferenced', function (done) {
      ...
      this.view._launchOnboarding();
      resolveGeoreferencePromise(true);
      resolveHasGeomPromise(true);
    ...
    it('should create and launch onboarding', function (done) {
      ...
      setTimeout(function () {
        expect(self.view._onboardingLauncher.launch).toHaveBeenCalled();
        done();
      }, 0);
    });
  });
