...
module.exports = CoreView.extend({
  ...
  _initCollections: function (basemaps) {
    ...
    // google maps is not supported yet
    var basemapList = _.clone(basemaps);
    delete basemapList['GMaps'];
    delete basemapList['Google'];

    _(basemapList).each(function (categoryLayers, category) {
    ...
    _.map(this._userModel.get('layers'), function (m) {
      ...
      var name = options.name || 'Custom basemap ' + m.order;
      ...
      var tmpLayer = {
        ...
        type: options.type,
        selected: this._baseLayer.get('className') === options.className,
        val: name,
        label: name,
        template: function () {
          return basemap_thumbnail({
  ...
  _initBinds: function () {
    ...
    this._userLayersCollection.bind('change:add', this._renderSelect);
    this._userLayersCollection.bind('change:remove', this._renderSelect);
    this.add_related_model(this._userLayersCollection);
  },
  ...
  _getDefaultCategory: function () {
    ...
    if (defaultCategory === undefined) {
      defaultCategory = this._basemapsCollection.at(0);
    }
