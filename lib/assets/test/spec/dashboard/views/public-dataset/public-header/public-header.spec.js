...
describe('dashboard/views/public-dataset/public-header/public-header', function () {
  ...
  it("should render properly when authenticated users are 'empty'", function () {
    view.render();
    expect(view.$('ul.options li a').length).toBe(3);
    expect(view.$('ul.options li a.account').length).toBe(0);
    expect(view.$('ul.options li a.login').length).toBe(1);
  });
  ...
  it("should render properly when authenticated users are 'empty' and it is a mobile device", function () {
    ...
    view.render();
    expect(view.$('ul.options li a').length).toBe(2);
    expect(view.$('ul.options li a.account').length).toBe(0);
    expect(view.$('ul.options li a.login').length).toBe(1);
    expect(view.$('ul.options li a.signup').length).toBe(1);
  });
  ...
  it('should render properly when authenticated user is filled and it is in dashboard view', function () {
    ...
    expect(view.$('ul.options li a').length).toBe(1);
    expect(view.$('ul.options li a.account').length).toBe(1);
    expect(view.$('ul.options li a.login').length).toBe(0);
    expect(view.$('ul.options li a.signup').length).toBe(0);
  });
