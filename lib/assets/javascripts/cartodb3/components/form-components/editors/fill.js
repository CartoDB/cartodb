...
Backbone.Form.editors.Fill = Backbone.Form.editors.Base.extend({
  ...
  initialize: function (opts) {
    this.options = {
      ...
      query: opts.schema.query,
      configModel: opts.schema.configModel,
  ...
  _initInputFields: function () {
    ...
    /* Expected data from the category styling thing
    data = {
      color: {
        attribute: 'address',
        ramp: ['#B2DF8A', '#FF2900', '#FF0011', '#CCC'],
    ...
    this._inputCollection.each(function (inputModel) {
      ...
      if (type === 'color' && inputModel.get('ramp') && inputModel.get('domain')) {
        Klass = INPUT_TYPE_MAP['categories'];
      }
      ...
      inputKlass.bind('change', this._onInputChanged, this); // TODO: we shouldn't have to use this binding, only the inputChanged event from the collection
