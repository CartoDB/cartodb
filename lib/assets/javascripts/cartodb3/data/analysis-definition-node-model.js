...
module.exports = Backbone.Model.extend({
  ...
  /**
   * Clone node and its primary source nodes to a new node sequence, which nodes have changed to the given new letter.
   * @example intended usage
   *   var node;   // => [b3->b2->b1-> a1->…]
   *   var clonedNode = node.clonetreewithsameletter('c');
   *   clonedNode; // => [c3->c2->c1-> a1->…]
   * @param {String} newletter
   * @return {Object} instance of analysis-definition-node-model
   */
  cloneTreeWithSameLetter: function (newLetter) {
    if (!newLetter) throw new Error('newLetter is required');
    if (!_.isString(newLetter) || newLetter.length < 1) throw new Error('newLetter is required as a non-empty string');

    var newId = nodeIds.next(newLetter);
    var lastClonedNode;
    var prev;

    this._getPrimarySourcesWithSameLetter(this._letter())
      .reverse() // to start from root source and go "forward", i.e. example from above would start on a0 and end on c3
      .forEach(function (nodeDefModel) {
        if (nodeDefModel.hasPrimarySource()) {
          newId = nodeIds.next(newId);
        }

        var current = nodeDefModel.clone(newId);
        if (prev) {
          current.changeSourceIds(prev.id, lastClonedNode.id);
        }

        lastClonedNode = current;
        prev = nodeDefModel;
      }, this);
  ...
  changeSourceIds: function (currentId, newId) {
    this._sourceNames().forEach(function (sourceName) {
      if (this.get(sourceName) === currentId) {
        this.set(sourceName, newId);
      }
    }, this);
  },

  _getPrimarySourcesWithSameLetter: function (letter) {
    if (this._letter() === letter) {
      var primarySource = this.getPrimarySource();
      return primarySource
        ? [this].concat(primarySource._getPrimarySourcesWithSameLetter(letter))
        : [this];
    }
