describe("cdb.admin.Header", function() {
  ...
  it("should add tags", function() {
    ...
    this.table.trigger('change');
    expect($(this.header.el).find('.tags p').html()).toEqual('<em>cachopo</em><em>frixuelu</em>');
  })

  it("should change tags button when there are tags", function() {
    this.table.set({'tags': 'cachopo,frixuelu'});

    expect($(this.header.el).find('.tags a').html()).toEqual(this.header._TEXTS._DEFAULT_EDIT_TAG_BUTTON)
  })

  it("should remove tag button text when there are tags and there is an sql applied", function() {
    this.header.table.sqlView = 'wadus';
    this.table.set({'tags': 'cachopo,frixuelu'});
