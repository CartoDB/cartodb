...
var LegendDefinitionsCollection = require('../../../javascripts/cartodb3/data/legend-definitions-collection');
...
describe('deep-insights-integrations', function () {
  ...
  describe('_resetStylesIfNoneApplied', function () {
    beforeEach(function () {
      ...
      nodeDef.querySchemaModel.set('simple_geom', 'point', { silent: true });
    ...
    it('should fetch geometry if it is not defined until reset styles', function () {
      var nodeDef = this.layerDefModel.getAnalysisDefinitionNodeModel();
      var nodeMod = this.analysis._analysisCollection.at(1);
      nodeDef.querySchemaModel.set('simple_geom', '', { silent: true });
      nodeMod.isDone.and.returnValue(true);
      spyOn(nodeDef.querySchemaModel, 'fetch').and.callFake(function () {
        nodeDef.querySchemaModel.set('simple_geom', 'polygon', { silent: true });
      });

      this.integrations._resetStylesIfNoneApplied(this.layerDefModel);
      expect(nodeDef.querySchemaModel.fetch).toHaveBeenCalled();
      expect(this.layerDefModel.styleModel.setDefaultPropertiesByType).not.toHaveBeenCalled();
    });
  ...
  describe('legends', function () {
    ...
    it('should show legend on init', function () {
      expect(this.bubble.show).toHaveBeenCalled();
      expect(this.choropleth.show).toHaveBeenCalled();
    });
