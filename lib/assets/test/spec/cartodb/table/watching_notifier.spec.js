...
describe("Watching notifier", function() {
  ...
  describe("Model", function() {
    ...
    it('should check users when acl has any item', function() {
      spyOn(model,'pollCheck')
      vis.permission.setPermission(user, 'rw');
      model._checkPermissions();
      expect(model.pollCheck).toHaveBeenCalled();
    });
  ...
  describe("View", function() {
    ...
    beforeEach(function() {
      ...
      user = TestUtil.createUser();
      model = new cdb.admin.WatchingNotifierModel({}, { vis: vis });
      view = new cdb.admin.WatchingNotifierView({ model: model });
    });
    ...
    it('should render properly', function() {
      view.render();
      expect(view.$el.text().replace(/^\s+|\s+$/g, '')).toBe('0 people are also editing this visualization');
    ...
    it('should change text when watcher users has changed', function() {
      ...
      expect(view.$el.text().replace(/^\s+|\s+$/g, '')).toBe('3 people are also editing this visualization');
      model.set('users', ['test']);
      expect(view.$el.text().replace(/^\s+|\s+$/g, '')).toBe('test is also editing this visualization');
    });
