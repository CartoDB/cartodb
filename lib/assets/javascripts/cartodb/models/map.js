...
cdb.admin.Layers = cdb.geo.Layers.extend({
  ...
  saveLayers: function(opts) {
    ...
    function bulkLayersSave(layers) {
      var _opts = _.extend(
      ...
      layers.each(function(layer) {
        jsonLayerArray.push(layer.toJSON());
        ids.push(layer.id);
      });
      var json = { layers: jsonLayerArray };
      var url = layers.at(0).collection.url() + '/' + ids.join(',');
      //layer.save(null, _opts);
      console.log(layers);
    }

    bulkLayersSave(layers);
  },
