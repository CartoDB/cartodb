...
describe('DatasetsSection.vue', () => {
  ...
  describe('Methods', () => {
    it('applyOrder: should dispatch datasets/orderDatasets with order options', () => {
      const store = createStore();
      ...
      expect(store.dispatch).toHaveBeenCalledWith('datasets/orderDatasets', {
        order: 'updated_at',
        direction: 'desc'
      });
    });

    it('applyFilter: should dispatch datasets/filterDatasets with filter type', () => {
      const store = createStore();
      spyOn(store, 'dispatch');
      ...
      expect(store.dispatch).toHaveBeenCalledWith('datasets/filterDatasets', 'locked');
    });

    it('fetchDatasets: should dispatch datasets/fetchDatasets', () => {
      const store = createStore();
      spyOn(store, 'dispatch');
      ...
      expect(store.dispatch).toHaveBeenCalledWith('datasets/fetchDatasets');
    });
