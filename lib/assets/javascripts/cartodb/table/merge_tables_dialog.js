...
cdb.admin.MergeTablesDialog = cdb.admin.BaseDialog.extend({
  ...
  initialize: function() {
    ...
    _.bindAll(this, "_onKeyUp", "_checkKeyColumns", "_onChangeState", "_onChangeDescriptionTitle", "_onChangeEnableNext", "_onChangeEnableBack", "_loadState", "_onNextClick", "_onBackClick", "_onRadioClick", "_onToggleMergeFlavorList", "_onToggleTableSelector", "_tableSelected", "_mergeMethodSelected", "_merge", "_importCopy", "_onMergeSuccess", "_onMergeError");
    ...
    this.table = _.clone(this.options.model);
  ...
  _loadState: function() {
    ...
    if (previousState < state && previousState > 0) this.model.set("enableNext", true);
    ...
    if (state == 0) {
      ...
      this.model.set({
        ...
        description_title: "",
        description: "Merges are really usefull when you want to get data from two tables together in a single one. Select your join strategy to perform a merge."
      });
    ...
    if (stepName == "merge_tables_regular_1") {
      ...
      this.model.set({
        ...
        show_table_name_form:   false,
        title: _t("Merge with another table"),
        description_title: "Regular Join",
        description: "This let use join two tables by a common column and get a new table as a result. Select a the key column using the radiobuttons and the columns you want to preserve in your new resulting table."
      });
    ...
    if (stepName == "merge_tables_spatial_1") {
      ...
      this.model.set({
        ...
        description: "Choose a spatial method and the column to use in case you need it. Count will count the number of features intersecting. SUM and AVG will accumulate the value in a numeric column of all the intersecting features and we need more text here."
      });
  ...
  _onToggleMergeFlavorList: function() {
    ...
    if (this.model.get("show_merge_flavor_list")) {
      this.$mergeFlavorList.delay(250).slideDown(120);
    } else {
      this.$mergeFlavorList.delay(250).slideUp(120);
    }
  ...
  _onToggleTableSelector: function() {
    ...
    if (this.model.get("show_table_selector")) {
      this.$tableSelector.delay(100).slideDown(150);
    } else {
      this.$tableSelector.delay(100).slideUp(150);
    }
  ...
  _createTableColumnSelectors: function($el) {
    ...
    .render()
    .bind('keyColumnChanged',    this._checkKeyColumns)
    .bind('tableSelected',       this._tableSelected)
  ...
  */
  ...
  _merge: function() {
    console.log( "Merging", this.model.get("merge_name") );

    var self = this
    , data = {
      table_name: this.model.get("merge_name"),
      sql: this._generateQuery()
    }
    ...
    $.ajax({
      ...
      url: "/api/v1/imports",
      data: data,
      success: self._onMergeSuccess,
      error: self._onMergeError
    });
  },
