...
module.exports = Backbone.Collection.extend({
  ...
  resetByLayersData: function (layersData) {
    this.reset(layersData, {
      ...
      initialLetters: _.chain(layersData)
        ...
        .compact()
        .value(),
      initialColors: _.chain(layersData)
        .pluck('options')
        .pluck('color')
        .value()
    });
  ...
  model: function (d, opts) {
    ...
    if (layerTypesAndKinds.isCartoDBType(o.type) || layerTypesAndKinds.isTorqueType(o.type)) {
      o.color = o.color || layerColors.next(self._colors(opts.initialColors));
    }
  ...
  _letters: function (otherLetters) {
    return this._getNextValueFor('letter', otherLetters);
  },

  _colors: function (otherColors) {
    return this._getNextValueFor('color', otherColors);
  },

  _getNextValueFor: function (attribute, initialValues) {
    var valuesFromAddedModels = _.compact(this.pluck(attribute));

    // When adding multiple items the models created so far are stored in the internal object this._byId,
    // need to make sure to take them into account when returning already taken value of the attribute.
    var valuesFromModelsNotYetAdded = _.chain(this._byId).values().invoke('get', attribute).value();

    return _.union(valuesFromAddedModels, valuesFromModelsNotYetAdded, initialValues);
  },
