...
module.exports = cdb.core.View.extend({
  ...
  initialize: function (opts) {
    ...
    if (!_.isFunction(opts.createWidgetsOptionsCollection)) throw new Error('createWidgetsOptionsCollection is required');

    this._modalModel = opts.modalModel;
    this._tablesCollection = opts.tablesCollection;
    this._createWidgetsOptionsCollection = opts.createWidgetsOptionsCollection;
  ...
  _newBodyView: function (opts) {
    return new AddWidgetsBodyView({
      el: opts.el,
      collection: this._createWidgetsOptionsCollection(this._tablesCollection)
    });
  }
});
