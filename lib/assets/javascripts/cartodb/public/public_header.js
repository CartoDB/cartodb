...
cdb.open.Header = cdb.core.View.extend({
  ...
  _TEMPLATE: "<div class='inner'>\
      ...
      <ul class='options'>\
        \
        <% if (!username) { %>\
          <li><a href='http://cartodb.com/gallery'>Browse the gallery</a></li>\
        <% } %>\
        \
        ...
        <% } else { %>\
          ...
          \
          <li> \
            <a class='editor account' href='<%= urls[0] %>'>\
              <% if (avatar_url) { %><img src='<%= avatar_url %>' width='18' title='<%= username %>' alt='<%= username %>' /><% } %><span class='username'><%= username %></span><span class='icon down-arrow'></span>\
            </a>\
          </li>\
        <% } %>\
      </ul>\
