...
$(function() {
  ...
  var Dashboard = cdb.core.View.extend({
    ...
    _initViews: function() {
      ...
      // Choose sceneario
      this.StartView = new cdb.admin.dashboard.Scenario({
        el:     this.$el,
        tables: this.tables
      });
  ...
  */
  ...
  cdb.init(function() {
    ...
    if (window.dashboard.tables.options.get("tag_name") === "" && window.dashboard.tables.options.get("q") === "") {
      window.dashboard.tables.fetch();
      window.dashboard.visualizations.fetch();
      window.dashboard.tags.fetch();
    }
  });
