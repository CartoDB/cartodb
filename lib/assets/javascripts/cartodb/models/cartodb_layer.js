cdb.admin.CartoDBLayer = cdb.geo.CartoDBLayer.extend({
  ...
  initialize: function() {
    ...
    this.wizard_properties = new cdb.admin.WizardProperties({ 
      table: this.table,
      layer: this
    });
  ...
  bindWidgets: function() {
    ...
    var data = this.get('widgets');

    if (data) {
      this.widgets.add(data);
    }

    this.widgets.bind('add remove reset change', _.debounce(function() {

      // call with silent so the layer is no realoded
      // if some view needs to be changed when the widget is changed it should be
      // subscribed to the widget model not to dataLayer
      // (which is only a data container for the widget)
      
      if (!this.isNew()) {
        this.save({ widgets: this.widgets.toJSON() }, { silent: true });
      }
  ...
  bindInfowindow: function(infowindow, attr) {
    ...
    var deferredSave = _.debounce(function() {
      ...
      if (!this.isNew()) {
        var a = {};
        a[attr] = infowindow.toJSON();
        this.save(a, { silent: true });
      }
    }, 250);
