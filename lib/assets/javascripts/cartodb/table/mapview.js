  ...
  */
  enableMap: function() {
    ...
    if (!this.map_enabled) {
      ...
      var text = new cdb.admin.TextWidget({
        width: 100, height: 20,
        style: {
          color: "orange", "font-size": "15px", "font-weight": "bold", "text-align": "left"
        },
        x: 200, y: 300, text: "Consectetur neque nisi neque nisi laborum." });

      this.mapView.$el.append(text.render().$el);

      //$(".canvas").append(text.render().$el);

      var text = new cdb.admin.TextWidget({
        width: 200, height: 30,
        x: 500, y: 100, text: "I'm the"
      });
