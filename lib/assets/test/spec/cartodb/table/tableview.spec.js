...
describe("tableview", function() {
  ...
  describe('TableView', function() {
    ...
    it("should edit cell", function() {
      spyOn(tview, '_editCell');
      tview.render();
      tview.getCell(0, 0).trigger('dblclick');
      expect(tview._editCell).toHaveBeenCalled();
    });
    ...
    it("should show 'geojson' on the_Geom is  cell when we don't know the geometry type yet", function() {
      model.data().reset([
        {'id': 1, 'col1': 1, 'col2': 2, 'col3': 3, 'the_geom':
          '{"type":"Point", "coordinates": [[1,1],[2,3],[6,6]]}'}
      ]);
      ...
      expect(tview.$('td .cell').eq(5).html()).toEqual("GeoJSON");
