cdb.admin.mod.TorqueCategoryWizard = cdb.admin.mod.CategoryWizard.extend({
  ...
  render: function() {
    if(this.isValid()) {
      ...
      if (this.mapContainsTorqueLayer()) {
  ...
  isValid: function() {
    ...
    return this.validColumns().length > 0 &&
           ...
           this.torqueLayersCount() === 1 &&
           this.getTorqueLayer().isEqual(this.options.layer);
  },

  mapContainsTorqueLayer: function() {
    return this.torqueLayersCount() > 1 ||
      this.torqueLayersCount() == 1 &&
        !this.getTorqueLayer().isEqual(this.options.layer);
  },
