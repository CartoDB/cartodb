  ...
  it 'creates a new asset' do
    ...
    if File.exist?(file_path) && File.file?(file_path)
      ...
      post_json(
        ...
        response.body[:public_url].should =~ %r{/test/#{@user.username}/assets/\d+cartofante_blue}
        ...
        asset.public_url.should =~ %r{/test/#{@user.username}/assets/\d+cartofante_blue}
  ...
  it 'creates a new asset with spaces in name' do
    ...
    if File.exist?(file_path) && File.file?(file_path)
      ...
      post_json(
        ...
        response.status.should be_success
        response.body[:public_url].should =~ %r{/test/#{@user.username}/assets/\d+cartofante%20blue}
        URI(response.body[:public_url]).should be_absolute
        response.body[:kind].should == 'wadus'
        @user.reload
        @user.assets.count.should == 1
        asset = @user.assets.first
        asset.public_url.should =~ %r{/test/#{@user.username}/assets/\d+cartofante%20blue}
        asset.kind.should == 'wadus'
      end
    else
      pending("Required spec asset '#{file_path}' not found, test can't properly execute")
