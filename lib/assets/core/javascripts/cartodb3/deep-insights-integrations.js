...
var F = function (opts) {
  ...
  this.visMap().on('change', _.debounce(saveStateDefinition, 200), this);
...
F.prototype._getVisMetadata = function () {
  ...
  var zoom = this._stateDefinitionModel.getZoom();
  ...
  this._visDefinitionModel.set({
    visMetadata: imageExportMetadata,
    staticImageURLTemplate: vis.getStaticImageURL.bind(vis)
  });
};

F.prototype._setMapHelpers = function () {
  var map = this.visMap();
  this._visDefinitionModel.set({
    pixelToLatLng: map.get('pixelToLatLng'),
    latLngtoPixel: map.get('latLngtoPixel')
  });
};

F.prototype._setMapViewSize = function () {
  this._visDefinitionModel.set('mapViewSize', this.visMap().get('mapViewSize'));
};
