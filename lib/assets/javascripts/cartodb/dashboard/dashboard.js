...
$(function() {
  ...
  var Dashboard = cdb.core.View.extend({
    ...
    _initViews: function() {
      ...
      var toggleTable = this.toggleTable = new cdb.admin.dashboard.ToggleButton({
        el:      this.$el.find("section.tables"),
        kind:   "tables",
        config:  this.options.config,
        elements: [this.tableList, paginator]
      });

      var toggleVisualization = this.toggleVisualization = new cdb.admin.dashboard.ToggleButton({
      ...
      var filterTag = new cdb.admin.TagDropdown({
      ...
      this.$el.append(filterTag.render().el);

      cdb.god.bind("closeDialogs", filterTag.hide, filterTag);
  ...
  */
  cdb.admin.dashboard.ToggleButton = cdb.core.View.extend({
    ...
    _toggle: function(e) {
      ...
      if (this.options.kind == 'visualizations') this.localStorage.set({ visualizations_toggle: toggle });
      else this.localStorage.set({ tables_toggle: toggle });
    ...
    show: function() {
      ...
      this.$aside.css("display", "inline-block");
      this.$aside.animate({opacity: 1}, { duration: 250 });
      this.$order.fadeIn(250);
    ...
    hide: function() {
      ...
      this.$aside.animate({opacity: 0}, { duration: 250, complete: function() {
        $(this).hide();
      }});

      this.$order.fadeOut(250);
    ...
    initialize: function() {
      ...
      this.$aside       = this.$section.next("aside");
      this.$order       = this.$section.find(".order");
      this.$paginator   = this.$aside.next(".paginator");
