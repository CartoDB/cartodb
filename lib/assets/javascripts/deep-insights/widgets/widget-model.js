...
module.exports = cdb.core.Model.extend({
  ...
  initialize: function (attrs, models, opts) {
    ...
    this.listenTo(this.dataviewModel, 'change', this.activeAutoStyler);
  },
  ...
  isAutoStyleEnabled: function () {
    return (this.get('type') === CATEGORY_TYPE || this.get('type') === HISTOGRAM_TYPE)
      ? this._getAutostyleOption() && !this._hasGeometryChanged()
      : false;
  },

  _getAutostyleOption: function () {
    var styles = this.get('style');

    return (!styles || !styles.auto_style)
      ? this._autoStyleEnabledWhenCreated
      : styles && styles.auto_style && styles.auto_style.allowed;
  },

  _hasGeometryChanged: function () {
    var dataViewModelOptions = this.dataviewModel.get('source').get('options');
    var layerModeloptions = this.layerModel.get('source').get('options');

    return layerModeloptions &&
           dataViewModelOptions &&
           (layerModeloptions.simple_geom !== dataViewModelOptions.simple_geom);
  },
  ...
  getColor: function (name) {
    if (this.isAutoStyleEnabled() && this.isAutoStyle() && this.get('type') === CATEGORY_TYPE) {
