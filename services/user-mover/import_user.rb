  ...
  module DataMover
    class ImportJob
      ...
      def set_user_search_path(user, search_path_prefix)
        superuser_pg_conn.query("ALTER USER \"#{user}\" SET search_path= \"#{search_path_prefix}\", public, cartodb")
      end
...
if __FILE__ == $0
  ...
  parser = OptionParser.new do |opts|
    ...
    opts.on("-o", "--import-into-org ORGNAME", "Import into organization. (Will override database_host with the org one)") do |f|
      options[:target_org] = f
    end
    opts.on("-h", "--database_host IP", "Database Host to import to") do |f|
      options[:host] = f
    end
    opts.on("-r", "--rollback", "Rollback user (reverse-import). This will delete the user.") do
