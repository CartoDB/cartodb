...
def validate_view_api_key(view_name, create_query, drop_query)
  user.in_database.run(create_query)
  grants = [apis_grant(['sql']), database_grant(@table1.database_schema, view_name)]
  api_key = user.api_keys.create_regular_key!(name: 'grants_view', grants: grants)
  ...
  user.in_database.run(drop_query)
  api_key.destroy
end
...
def create_role(role_name = 'test')
  drop_role
  user.in_database(as: :superuser).execute("CREATE ROLE \"#{role_name}\"")
end

def drop_role(role_name = 'test')
  user.in_database(as: :superuser).execute("DROP ROLE IF EXISTS \"#{role_name}\"")
end
...
describe Carto::ApiKey do
  ...
  shared_examples_for 'api key' do
    before do
      ...
      @table3 = create_table(user_id: user.id)
    end
    ...
    it 'can grant only with select and update permissions' do
      ...
      api_key = user.api_keys.create_regular_key!(name: 'only_update', grants: grants)
    ...
    it 'master role is able to drop the table created by regular api with schema grants' do
      grants = [schema_grant(user.database_schema), apis_grant]
      api_key = user.api_keys.create_regular_key!(name: 'drop_by_master', grants: grants)

      with_connection_from_api_key(api_key) do |connection|
        create_select_drop_check(connection, user.database_schema, 'test_table', false)
      end

      user.in_database.execute("drop table test_table")
    end

    it 'reassigns role ownerships after deleting a regular API key' do
      grants = [schema_grant(user.database_schema), apis_grant]
      api_key = user.api_keys.create_regular_key!(name: 'drop_test', grants: grants)
      ...
      user.in_database.execute(ownership_query) do |result|
        result[0]['owner'].should eq user.database_username
      end
      user.in_database.execute("drop table test_table")
    end
    ...
    it 'fails to grant to a non-existent schema' do
      expect {
        ...
        user.api_keys.create_regular_key!(name: 'full', grants: grants)
    ...
    it 'fails to grant to a non-existent table' do
      expect {
        grants = [database_grant(user.database_schema, 'not-exists'), apis_grant]
        user.api_keys.create_regular_key!(name: 'full', grants: grants)
    ...
    it 'fails to access system tables' do
      api_key = user.api_keys.create_regular_key!(name: 'full', grants: [apis_grant])
    ...
    it 'grants to a double quoted table name' do
      ...
      user.in_database.run("ALTER TABLE #{old_name} RENAME TO \"wadus\"\"wadus\"")
      grants = [database_grant(user.database_schema, 'wadus"wadus'), apis_grant]
      api_key = user.api_keys.create_regular_key!(name: 'valid_table_name', grants: grants)
      ...
      api_key.destroy
      user.in_database.run("ALTER TABLE \"wadus\"\"wadus\" RENAME TO #{old_name}")
    end
    ...
    it 'show ownership of the tables for the user' do
      grants = [schema_grant(user.database_schema), apis_grant]
      api_key = user.api_keys.create_regular_key!(name: 'table_owner_test', grants: grants)
      ...
      with_connection_from_api_key(api_key) do |connection|
        connection.execute("drop table \"#{user.database_schema}\".test_table")
      end
    end
    ...
    it 'regular key owner with creation permission can cartodbfy tables' do
      ...
      with_connection_from_api_key(api_key) do |connection|
        connection.execute("create table \"#{user.database_schema}\".test_table(id int)")
        connection.execute("select cdb_cartodbfytable('#{user.database_schema}', 'test_table')")
        connection.execute("insert into \"#{user.database_schema}\".test_table(the_geom, id) values ('0103000020E610000001000000040000009A99999999C9524048E17A14AE873D4000000000004053400000000000003D4066666666666653400000000000803D409A99999999C9524048E17A14AE873D40'::geometry, 1)")
        connection.execute("select * from \"#{user.database_schema}\".test_table") do |result|
    ...
    describe '#destroy' do
      ...
      it 'invalidates varnish cache' do
        CartoDB::Varnish.any_instance.expects(:purge).with("#{user.database_name}.*").at_least(1)
    ...
    describe 'validations' do
      it 'fails with invalid schema permissions' do
        ...
        expect {
          user.api_keys.create_regular_key!(name: 'x', grants: grants)
      ...
      it 'validates do API grant' do
        ...
        expect {
          user.api_keys.create_regular_key!(name: 'x', grants: [apis_grants])
        }.to_not raise_error
      end
      ...
      it 'fails with several apis sections' do
        ...
        expect {
          user.api_keys.create_regular_key!(name: 'x', grants: two_apis_grant)
    ...
    describe '#table_permission_from_db' do
      before do
        @public_table = create_table(user_id: user.id)
        @public_table.table_visualization.update_attributes(privacy: 'public')
      end

      it 'loads newly created grants for role' do
        grants = [database_grant(user.database_schema, @table1.name), apis_grant(['maps', 'sql'])]
        api_key = user.api_keys.create_regular_key!(name: 'wadus', grants: grants)

        sql = "grant SELECT on table \"#{@table2.database_schema}\".\"#{@table2.name}\" to \"#{api_key.db_role}\""
        user.in_database(as: :superuser).run(sql)
      ...
      it 'doesn\'t show removed table' do
        permissions = ['insert', 'select', 'update', 'delete']
        grants = [database_grant(user.database_schema, @table1.name, permissions: permissions), apis_grant]
        api_key = user.api_keys.create_regular_key!(name: 'wadus', grants: grants)
        ...
        sql = "drop table \"#{user.database_schema}\".\"#{@table1.name}\""
        user.in_database(as: :superuser).run(sql)
      ...
      it 'shows public tables' do
        api_key = user.api_keys.default_public.first
        unless user.has_organization?
    ...
    describe '#schema_permission_from_db' do
      ...
      it 'grants creation in schema to master role' do
        ...
        master_api_key = user.api_keys.master.first
    ...
    describe 'master api key' do
      it 'user has a master key with the user db_role' do
        api_key = user.api_keys.master.first
        api_key.should be
        api_key.db_role.should eq user.database_username
        api_key.db_password.should eq user.database_password
      end

      it 'cannot create more than one master key' do
        expect {
          user.api_keys.create_master_key!
      ...
      it 'cannot create a non master api_key with master as the name' do
        expect {
          user.api_keys.create_regular_key!(name: Carto::ApiKey::NAME_MASTER, grants: [apis_grant])
        }.to raise_error(ActiveRecord::RecordInvalid)
      end
    ...
    describe 'data services api key' do
      ...
      it 'cdb_conf info with dataservices' do
        ...
        api_key = user.api_keys.create_regular_key!(name: 'dataservices', grants: grants)
      ...
      it 'cdb_conf info without dataservices' do
        grants = [apis_grant]
        api_key = user.api_keys.create_regular_key!(name: 'testname', grants: grants)
        expected = { username: user.username, permissions: [], ownership_role_name: '' }
      ...
      it 'fails with invalid data services' do
        ...
        expect {
          user.api_keys.create_regular_key!(name: 'bad', grants: grants)
        }.to raise_error(ActiveRecord::RecordInvalid)
      end
    ...
    describe 'data observatory datasets api key' do
      ...
      it 'grants with data observatory datasets' do
        ...
        api_key = user.api_keys.create_regular_key!(name: 'data-observatory', grants: grants)
    ...
    describe '#data_observatory_permissions?' do
      it 'returns true when it has the do api grant' do
        ...
        api_key = user.api_keys.create_regular_key!(name: 'x', grants: [apis_grants])
  ...
  describe 'with organization users' do
    let(:organization) { create(:organization, quota_in_bytes: 1.gigabytes) }
    let(:user) { TestUserFactory.new.create_owner(organization) }

    it_behaves_like 'api key'

    it 'fails to grant to a non-owned table' do
      other_user = TestUserFactory.new.create_test_user(unique_name('user'), organization)
    ...
    it 'drop role with grants of objects owned by other user' do
      user2 = TestUserFactory.new.create_test_user(unique_name('user'), organization)
      table_user2 = create_table(user_id: user2.id)
      schema_and_table_user2 = "\"#{table_user2.database_schema}\".#{table_user2.name}"

      table_user1 = create_table(user_id: user.id)
    ...
    it 'reassigns role ownerships after deleting an OAuth API key' do
      ...
      user.in_database(as: :superuser).execute("drop table test.wadus")
    end
  end
  ...
  describe 'org shared tables' do
    ...
    before do
      @shared_table = create_table(user_id: user.id)

      perm = @shared_table.table_visualization.permission
      perm.acl = [{ type: 'user', entity: { id: other_user.id }, access: 'rw' }]
      perm.save!
    end
    ...
    it 'should create an api key using a shared table' do
      ...
      api_key = other_user.api_keys.create_regular_key!(name: 'grants_shared', grants: grants)
    ...
    it 'should revoke permissions removing shared permissions (rw to r)' do
      ...
      perm = @shared_table.table_visualization.permission
      perm.acl = [{ type: 'user', entity: { id: other_user.id }, access: 'r' }]
      perm.save!
