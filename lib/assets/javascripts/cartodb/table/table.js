...
$(function() {
  ...
  var Table = cdb.core.View.extend({
    ...
    */
    ...
    hasAliasesFeatureFlag: function () {
      var user_data_feature_flags = user_data.feature_flags;
      this.hasAliasesFeatureFlag = false;
      for (var l = 0; l < user_data_feature_flags.length; l++) {
        if(user_data_feature_flags[l] === 'aliases') {
          this.hasAliasesFeatureFlag = true;
        }
      }
    },
    ...
    _initTableMap: function() {
      ...
      this.titleDropdown = new cdb.admin.TitleDropdown({
        user: this.user,
        position: 'position',
        horizontal_position: 'right',
        tick: 'left',
        table: this.table,
        template_base: 'table/views/title_options',
        sqlView: this.sqlView,
        vis: this.vis
      });
      this.addView(this.titleDropdown);
      ...
      this.workView.bind('tabEnabled:map', this._mapTabEnabled, this);
      this.workView.bind('tabEnabled:table', this._tableTabEnabled, this);
      ...
      this.titleDropdown.bind('renameAlias', this._saveTitleAlias, this);
      this.titleDropdown.bind('removeAlias', this._removeAlias, this);
    ...
    _initBinds: function() {
      cdb.god.on('closeDialogs', this.activeTitleOptions, this);
    ...
    setTable: function(table, sqlView) {
      ...
      if(this.table) {
        ...
        this.table.unbind('change:table_visualization', null, this);
      }
      ...
      this.table.bind('change:table_visualization', this._enableDisableExportTable, this);
      this._setSyncInfo();
      this._enableDisableExportTable();
    },
    ...
    _mapTabEnabled: function () {
        this.$el.find('.panes').removeClass('table-view');
        this.header.updateShareLabel('map');
        this._enableDisableExportTable('map');
        this.mapTab.enableMap();
    },

    _tableTabEnabled: function () {
        this.$el.find('.panes').addClass('table-view');
        this.header.updateShareLabel('table');
        this._enableDisableExportTable('table');
    },

   _saveTitleAlias: function (value) {
     var self = this;
     var current_alias_info = this.table.get('name_alias');
     current_alias_info = value;
     this.table.set('name_alias', current_alias_info);
     this.table.save(null, {
       success: function (mdl, data) {
         self.table.notice('Title alias has been renamed', 'info');
         self.table.trigger('update_layer_alias', value);
       },
       error: function (e, resp) {
         cdb.log.error("can't rename title alias");
         self.table.error('error renaming title alias', resp);
       },
       wait: true
     });
   },

    _removeAlias: function () {
      var self = this;
      this.table.set('name_alias', null);
      this.table.save(null, {
        success: function (mdl, data) {
          self.table.notice('Title alias has been removed', 'info');
          self.table.trigger('update_layer_alias', this.get('name'));
        },
        error: function (e, resp) {
          cdb.log.error("can't remove title alias");
          self.table.error('error removing title alias', resp);
        },
        wait: true });
    },

    _enableDisableExportTable: function(workView) {
        if (!this.header) {
          return;
        }
        if (!_.isString(workView)) {
          workView = table.workViewActive;
        }
        var exportable = this.table.get('table_visualization') && this.table.get('table_visualization').exportable;
        if (workView === 'map' || exportable) {
          this.header.$('a.share').removeClass("disabled")
        } else {
          this.header.clearDropdown();
        }
    },
    ...
    activeTitleOptions: function (e) {
      if(this.titleDropdown) {
        this.titleDropdown.hide();
      }
    },
    ...
    titleClick: function (e) {
      ...
      if (this.hasAliasesFeatureFlag) {
        var container = $(e.target).parent().parent();
        container.append(this.titleDropdown.el);
        this.titleDropdown.openAt(20, -5);
        this.titleDropdown.initializeState({alias: this.table.get('name_alias') || null});
        this.titleDropdown.render();
        this.titleDropdown.show();
      }
    }
  });
  ...
  cdb.init(function() {
    ...
    map_data.provider = 'openlayers';

    if ($.inArray("bbg_leaflet", user_data.feature_flags) !== -1) {
      map_data.provider = 'leaflet'
    }
