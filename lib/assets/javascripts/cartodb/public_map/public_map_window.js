  ...
  cdb.open.PublicMapWindow = cdb.core.View.extend({
    ...
    _initViews: function() {
      // Map view
      var map_options = _.defaults({ el: this.el.document.body }, this.options);
      var map = new cdb.open.MapPublic(map_options);
      this.addView(map);
    },
    ...
    _setupMapDimensions: function(animated) {
      ...
      if (animated) {
        ...
        // :S
        // this.$body.find("#map").trigger("resize");
      }

      if (this.el.vis) this.el.vis.mapView.invalidateSize();
    },
