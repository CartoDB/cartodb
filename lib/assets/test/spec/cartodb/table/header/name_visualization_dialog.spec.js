describe("Name visualization dialog", function() {
  ...
  beforeEach(function() {
    name_dialog = new cdb.admin.NameVisualization();
  });
  ...
  it("should close and send the name if it is correct", function() {
    name_dialog.render();
    name_dialog.options.res = function() {};

    var $input = name_dialog.$el.find('input');
    spyOn(name_dialog.options, 'res');
    ...
    expect(name_dialog.options.res).toHaveBeenCalled();
  ...
  it("shouldn't close and send the name", function() {
    ...
    expect(name_dialog.options.res).not.toHaveBeenCalled();
