 ...
 */
  ...
  var AutoCompleteInput = Backbone.View.extend({
    ...
    _renderNoResult: function() { return 'No results found...' },
    ...
    _changeHilitedItem: function(change) {
      ...
      if (selectedRow.length > 0) {
        $(selectedRow[0]).removeClass("search-result-row-selected");
      }

      var clsList = 'ac-list-' + this._id;
      var clsItem = clsList + '-item-' + itemIndex;

      this._hilitedItemIndex = itemIndex;
      $("." + clsItem).addClass("search-result-row-selected");
    },

    _resetSearch: function() {
      var containerId = 'ac-list-container-' + this._id;
      $('#' + containerId).remove();
    ...
    initialize: function(options) {
        ...
        if (options.renderNoResult !== undefined) {
          this._renderNoResult = options.renderNoResult;
        }
    ...
    setItems: function(items) {
      ...
      var popupHtml = '<div class="search-result-dropdown" id="' + containerId + '" style="position:absolute;top:' + position.top + 'px;left:' + position.left + 'px;z-index:' + position.zIndex + ';">' + 
           '<div>' + 
          '</div>' +
      '<div>';
      ...
      if (numItems > 0) {
        for (i = 0; i < numItems; ++i) {
            ...
            if (i == 0) {
              clsItem += ' search-result-row-selected';
            }
      ...
      $('.' + clsList).
        mouseover(function() {
          ...
          self._hilitedItemIndex = self._getItemIndexFromClass(classes);
        }).
        mouseleave(function() {
      ...
      $(".search-result-dropdown").
        mouseover(function() {
          $(this).find(".search-result-row-selected").removeClass("search-result-row-selected");
        }).
        mouseleave(function() {
          if (self._items.length > 0) {
            var rows = $(this).find(".search-result-row");
            if (rows.length > 0) {
              $(rows[0]).addClass("search-result-row-selected");
            }
          }
        });
    },
...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _addSearchBar: function() {
    ...
    var acInput = new AutoCompleteInput({
      ...
      renderListItem: function(index, item, cls) {
          ...
          var itemHtml = '<li class="search-result-row ' + cls + typeClass + '"><div class="name">' + name + '</div><div class="tags">';
          if (item.tags != null) {
              itemHtml += tags;
          }
          itemHtml += '</div></li>';
  ...
  _doDatasetSearch: function(params) {
      ...
      $.ajax({
          url: "../../api/v1/viz/search/?q=" + query + "&map_id=" + mapId + "&vis_id=" + visId + "&api_key=" + self.user.attributes.api_key
      })
