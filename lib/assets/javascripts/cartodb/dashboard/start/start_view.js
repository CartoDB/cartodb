  ...
  cdb.admin.dashboard.StartView = cdb.core.View.extend({
    ...
    initialize: function() {
      ...
      //this.$el.find("section.welcome").addClass("visible");
    ...
    _decideActiveBlock: function() {
      ...
      else if (path == "tag")                   this._setupTagsView();
      else if (path == "search") {
    ...
    _setupHomeView: function() {
      ...
      this.$no_tables[ this.tables_size > 0 ? 'removeClass' : 'addClass' ]('active');
    ...
    */
    _showTableCreationDialog: function(e) {
      ...
      var dlg = new cdb.admin.CreateTableDialog({
        ...
        files: null,
        url: url || null,
        private_tables: this.user.get("private_tables"),

        // TODO: check quota
        quota: (this.user.get("remaining_byte_quota") * (this.user.get("private_tables") != "FREE" ? 3 : 1))
      });
