...
module.exports = Backbone.Collection.extend({
  ...
  createLayerForAnalysisNode: function (nodeId, position) {
    ...
    if (isNaN(position) || position <= 0) throw new Error('position must be on top of the base layer');

    // A layer for an existing node have different side-effects depending on the context in which the node exists:
    // 1. If given node is a source layer just duplicate the source instead of creating unnecessary references
    if (!nodeDefModel.hasPrimarySource()) {
      return this.createLayerForTable(nodeDefModel.get('table_name'));
    }
    ...
    if (layerWithNodeAsHead) {
      // 2. If given node is a the head node of an layer the new layer should take over the ownership of the head node,
      // e.g. layerA[a0,a1,a2,a3] and nodeId=a3, then afterwards should be layerA[a0,a1,a2] (+ layerB[a2,b1])
      var clonedNodeAttrs = nodeDefModel.toJSON();
      clonedNodeAttrs.id = nextLetter + '1';
      var newNodeDefModel = this._analysisDefinitionNodesCollection.add(clonedNodeAttrs);

      var attrs = this._createDefaultCartoDBAttrs();
      attrs.order = position;
      attrs.options = _.extend({}, attrs.options, {
        table_name: 'paradas_metro_madrid', // TODO tmp workaround, a table_name is required to create layer
        letter: nextLetter,
      ...
      this._analysisDefinitionNodesCollection.each(function (m) {
        m.updateSourceIds(nodeId, newNodeDefModel.id);
      });
  ...
  setBaseLayer: function (basemapUrl) {
    this.getBaseLayer().set('urlTemplate', basemapUrl);
  }
