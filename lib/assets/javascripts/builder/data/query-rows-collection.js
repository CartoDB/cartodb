...
module.exports = Backbone.Collection.extend({
  ...
  isEmptyAsync: function () {
    console.log('Asking QueryRowsCollection isEmpty... [', this._tableName, ']');
    var self = this;
    if (this.isDone()) {
      return Promise.resolve(this.size() === 0);
    } else {
      return new Promise(function (resolve, reject) {
        console.log('status:', self.statusModel.get('status'), '[', self._tableName, ']');
        self.listenTo(self.statusModel, 'change:status', function (status) {
          if (self.isDone()) {
            resolve(self.size() === 0);
          }
        });
