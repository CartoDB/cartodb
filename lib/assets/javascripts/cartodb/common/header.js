...
cdb.admin.Header = cdb.core.View.extend({
  ...
  createBindings: function() {
    ...
    */
    ...
    this.model.bind('change:derived',       this.setTableInfo,    this);
  },
  ...
  _shareVisualization: function(e) {
    ...
    if (!this.model.isVisualization()) {
      dialog = new cdb.admin.NameVisualization({
        res: function(name) {
          self._changeToVisualization(name)
        }
      });
    } else {
      cdb.log.info("working on it");
      return false;
  ...
  _setVisualization: function() {
    ...
    var $vis_type = this.$('.vis_type')
      , $share = this.$('a.share')
      , is_visualization = this.model.get('derived');
