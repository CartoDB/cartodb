...
cdb.admin.ShareMapDialog = cdb.admin.BaseDialog.extend({
  ...
  _addWidget: function(widget) {
    this.addView(widget);
    this.$('.map_wrapper').append(widget.render().$el);
  },
  ...
  render_content: function() {
    ...
    this._addSwitches(['title', 'description', 'search', 'shareable', 'cartodb_logo', 'layer_selector']);
    this._addMapView();
  ...
  */
  ...
  _changePreview: function() {
    ...
    var $title          = this.$('div.header h1');
    var $description    = this.$('div.header p');
    var $social         = this.$('div.header div.social');
    var $search         = this.$('div.search_box');
