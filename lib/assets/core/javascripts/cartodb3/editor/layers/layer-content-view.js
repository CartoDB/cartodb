...
module.exports = CoreView.extend({
  ...
  render: function () {
    ...
    var renderPlaceholder = !this._getQueryRowsCollection().isDone() ||
      !this._getQueryGeometryModel().isDone() ||
      !this._getQuerySchemaModel().isDone();
  ...
  _initBinds: function () {
    ...
    this.listenTo(this._analysisDefinitionNodesCollection, 'remove', function (model) {
      if (model.get('type').indexOf('georeference') === 0) {
        ...
        if (oldParent.get('type') === 'source') {
          this.listenToOnce(layerDefinitionModel, 'change:source', function () {
            this._fetchAllQueryObjectsIfNecessary();
          });
        }
      }
    ...
    this.listenTo(querySchemaModel, 'change:status', this._fetchAllQueryObjectsIfNecessary);
    this.listenTo(queryGeometryModel, 'change:status', this._fetchAllQueryObjectsIfNecessary);
    this.listenTo(queryRowsCollection.statusModel, 'change:status', this._fetchAllQueryObjectsIfNecessary);
  ...
  _renderLayerTabPaneView: function () {
    ...
    var isEmpty = this._layerDefinitionModel.isEmpty();
    var selectedTabItem = this.options.selectedTabItem;
    var isDisabled = canBeGeoreferenced || isEmpty && isDone;

    if (isDisabled && _.contains(DISABLED_TABS, selectedTabItem)) {
  ...
  handleRoute: function (routeModel) {
    ...
    if (routeName.indexOf('add_feature_') === 0) {
      ...
      // TODO: replace stacklayout with router
