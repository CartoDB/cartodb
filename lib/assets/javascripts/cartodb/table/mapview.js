...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _addMapView: function() {
    ...
    if (this.map.get('provider') === 'googlemaps') {
      var baseType = this.map.getBaseLayer().get("base_type"); // 'roadmap', 'terrain', 'satellite', 'hybrid', 'gray_roadmap', 'dark_roadmap'
      var equivalentBaseLayer = table.baseLayers.at(1); // TODO: replace with the equivalent baselayer
      this.map.changeProvider("leaflet", equivalentBaseLayer);
    }
