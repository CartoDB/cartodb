cdb.admin.MapOptionsDropdown = cdb.ui.common.Dropdown.extend({
  ...
  _toggleOverlay: function(property) {
    ...
    if (overlay) {
      ...
      if (property === 'zoom') {
        ...
        var zoom_info  = this.collection.filter(function(m) { return m.get("type") === "zoom_info"; })[0];
        if (zoom_info) this.options.collection.remove(zoom_info);
  ...
  render: function() {
    ...
    this._addSwitches(['title', 'description', 'search', 'zoom', 'fullscreen', 'share', 'scrollwheel', 'layer_selector', 'logo', 'legends']);
