...
$(function() {
  ...
  var Dashboard = cdb.core.View.extend({
    ...
    _initModels: function() {
      ...
      // remove the style from redis
      // this is a temporal hack until this was managed by 
      // the backend
      /*
      this.tables.bind('destroy', function(table) {
        var protocol = cdb.config.get("tiler_port") == 443 ? 'https': 'http';
        var url = protocol + '://' + this.user.get('name') + "." + cdb.config.get("tiler_domain") + ':' + cdb.config.get("tiler_port") + '/tiles/' + table.get('name') + '/style';
        $.ajax({
          url: url,
          type: 'DELETE',
          crossOrigin: true,
      ...
      if ( scrolled > list_pos ) {
        if ((scrolled + aside_h) < (list_pos + list_height)) {
          $aside
            .addClass("moving")
            .css({
              marginTop: - list_pos - ($warning.hasClass("visible") ? $warning.outerHeight() : 0)
            })
            .removeClass("bottom");
        } else {
          $aside
