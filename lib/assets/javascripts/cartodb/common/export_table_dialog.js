...
cdb.admin.ExportTableDialog = cdb.admin.BaseDialog.extend({
  ...
  _CSV_FILTER: "SELECT *, CASE WHEN GeometryType(the_geom)='POINT' THEN ST_X(the_geom) WHEN ST_IsEmpty(ST_PointOnSurface(the_geom)) THEN null ELSE ST_X(ST_PointOnSurface(the_geom)) END as longitude_0, CASE WHEN GeometryType(the_geom)='POINT' THEN ST_Y(the_geom) WHEN ST_IsEmpty(ST_PointOnSurface(the_geom)) THEN null ELSE ST_Y(ST_PointOnSurface(the_geom)) END as latitude_0, ST_AsGeoJSON(the_geom,5) as geojson FROM (%%sql%%) as subq ",
  ...
  initialize: function() {
    ...
    if (this.options.force_http) {
      ...
      if (this.options.config.sql_api_protocol !== 'http') {
        this.baseUrl = 'http://' + this.options.config.sql_api_domain + '/api/v1/sql';
      }
    }
  ...
  _fetchGET: function(url) {
      ...
      var w = window.open(url);//, "export"), "height=0;width=0");
      w.onload = function() {
