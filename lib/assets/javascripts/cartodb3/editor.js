...
// prepend username to tablename in queries in order to fix map instantiation
// in map created from a shared dataset
var patchQuery = function (o) {
  var sql = o.params.query;
  var tableNameSet = o.options.table_name.split('.');
  var isOwner = tableNameSet.length === 1;

  if (!isOwner) {
    o.params.query = SQLUtils.prependTableName(sql, tableNameSet[1], tableNameSet[0]);
  }
};

// Patch the vizJSON analyses
_.each(vizJSON.analyses, patchQuery);

// Patch the analysesData
_.each(analysesData, function (analysis) {
  var analysisNode = analysis.analysis_definition;
  patchQuery(analysisNode);
});
