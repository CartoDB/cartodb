...
describe('new_dashboard/filters_view', function() {
  beforeEach(function() {
    ...
    this.lockItemsDialogStub = jasmine.createSpyObj('LockItemsDialog stub', ['appendToBody', 'bind']);
    this.LockItemsDialogSpy = jasmine.createSpy('LockItemsDialog');
    this.LockItemsDialogSpy.and.returnValue(this.lockItemsDialogStub);
    FiltersView.__set__('LockItemsDialog', this.LockItemsDialogSpy);
    this.changePrivacyDialogStub = jasmine.createSpyObj('ChangePrivacyDialog stub', ['appendToBody', 'bind']);
    this.ChangePrivacyDialogSpy = jasmine.createSpy('ChangePrivacyDialog');
    this.ChangePrivacyDialogSpy.and.returnValue(this.changePrivacyDialogStub);
    FiltersView.__set__('ChangePrivacyDialog', this.ChangePrivacyDialogSpy);
  ...
  describe('an item is selected', function() {
    ...
    describe('and click delete_items', function() {
      ...
      it('should created dialog with selected items', function() {
        expect(this.createdWith).toEqual(jasmine.objectContaining({ selectedItems: [ this.collection.at(2) ] }));
      });

      it('should created dialog with router', function() {
        expect(this.createdWith).toEqual(jasmine.objectContaining({ router: this.router }));
      });
    ...
    describe('and click lock items', function() {
      beforeEach(function() {
        ...
        this.createdWith = this.LockItemsDialogSpy.calls.argsFor(0)[0];
      });

      it('should open a lock items dialog', function() {
        expect(this.lockItemsDialogStub.appendToBody).toHaveBeenCalled();
      });

      it('should created dialog with selected items', function() {
        expect(this.createdWith).toEqual(jasmine.objectContaining({ items: [ this.collection.at(2) ] }));
      });
  ...
  describe('and click change privacy', function() {
    beforeEach(function() {
      // For now only don't support batch processing, so only select one
      this.selectedItems = [
        { selected: false },
      ...
      this.createdWith = this.ChangePrivacyDialogSpy.calls.argsFor(0)[0];
    });

    it('should open a lock items dialog', function() {
      expect(this.changePrivacyDialogStub.appendToBody).toHaveBeenCalled();
    });

    it('should created dialog with selected items', function() {
      expect(this.createdWith).toEqual(jasmine.objectContaining({ item: this.collection.at(1) }));
    });
  });
