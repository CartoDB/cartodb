...
(function() {
  ...
  cdb.admin.mod.InfowindowHTMLPane = cdb.core.View.extend({
    ...
    _apply: function(e) {
      ...
      if (html.length > 0) this.model.set("custom_html", html);
    ...
    render: function() {
      ...
      var text = "<!-- content.data contains the field info -->\n<hr>\n<h4>Ward{{content.data.ward}}</h4>           		<h4>Total Population:{{content.data.sum_totalp}}</h4>\n</hr>";
  ...
  cdb.admin.mod.InfoWindow = cdb.core.View.extend({
    ...
    _setupPane: function() {
      ...
      this.pane.bind('tabEnabled', function(tabName, tabView) {
        ...
        var left = self.$el.find(".menu li a[data-action="+tabName+"]").position().left;
