...
var TVTDefaults = require('./style-defaults/tvt-style-defaults');
...
var STYLE_MAP = {
  ...
  'simple': {
    ...
    checkIfValid: function (geometryType, styleType) {
      return !!geometryType && styleType !== 'tvt';
    },
    defaultStyles: SimpleStyleDefaults
  },
  'tvt': {
    labelTranslationKey: _('Torque Virtual Tables'),
    iconTemplate: require('./style-icons/points.tpl'),
    checkIfValid: function (geometryType, styleType) {
      return geometryType === 'point' && styleType !== 'simple';
    },
    defaultStyles: TVTDefaults
  },
  'squares': {
...
module.exports = {
  ...
  getStyleTypes: function (currentType, geometryType) {
    return _.reduce(STYLE_MAP, function (memo, val, key) {
      if (currentType === key || (val.checkIfValid ? val.checkIfValid(geometryType, currentType) : true)) {
        memo.push({
          value: key,
