    ...
    describe('.createFromScratch', function () {
      ...
      describe('when table is successfully created', function () {
        ...
        it('should place the new layer above the "CartoDB" layer on top', function () {
          ...
          this._layerDefinitionsCollection.reset([
            CartoDBLayer_1,
            CartoDBLayer_0
          ]);
          ...
          var layerAttrs = this._layerDefinitionsCollection.create.calls.argsFor(0)[0];

          // Order of new layer
          expect(layerAttrs.order).toEqual(2);
        ...
        it('should place the new layer below the "Tiled" layer on top (labels) and update the top most layer', function () {
          ...
          // Order of new layer
          expect(layerAttrs.order).toEqual(1);

          this._layerDefinitionsCollection.create.calls.argsFor(0)[1].success();

          // Order of other layers has not been changed
          expect(TiledLayer_1.get('order')).toEqual(2);
          expect(TiledLayer_1.save).toHaveBeenCalled();
          expect(CartoDBLayer_0.get('order')).toEqual(0);
        });
      });
    });
