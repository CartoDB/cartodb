...
var MapShareTab = cdb.core.View.extend({
  ...
  _changePreview: function() {
    ...
    this.$('.cartodb-header h1')[title ? 'show':'hide']();
    this.$('.cartodb-header p')[description ? 'show':'hide']();
    this.$('.cartodb-header div.social')[shareable ? 'show':'hide']();
    this.$('.cartodb-searchbox')[search ? 'show':'hide']();
    this.$('a.cartodb_logo')[cartodb_logo ? 'show':'hide']();

    if(!title && !description && !shareable) {
      this.$('.cartodb-header').hide();
    } else {
      this.$('.cartodb-header').show();
    }
  ...
  _setUrl: function() {
    ...
    this.$('input.url').val(
      "<script id='" + script_id + "' src='" +
      origin + tableUrl + ".js" + "?" + opts +
      "'></script>"
    );
  ...
  activated: function() {
    ...
    setTimeout(function() {
      ...
      this.$('.cartodb-map-wrapper').prepend(el);
  ...
  render: function() {
    ...
    this.$('.cartodb-map-wrapper').append(zoomControl.render().$el);
    ...
    this.$('.cartodb-map-wrapper').append(searchControl.render().$el);
