  ...
  cdb.admin.dashboard.VisualizationItem = cdb.core.View.extend({
    ...
    render: function() {
      ...
      if (this.model) {
        ...
        tags = this._generateTagList(tag_list, selected_tag, tags_per_item, tags_per_item, tags_per_item, tags_per_item, tags_per_item, tags_per_item, tags_per_item, tags_per_item, tags_per_item);

        // Clean up and truncate the description
        var description = this._cleanString(this.model.get("description"), 55);
        var name        = this._cleanString(this.model.get("name"), 20);

        var extra_tags_count = attrs.tags.length - tags_per_item;
        this.$el.append(this.template(_.extend(this.model.toJSON(), { name: name, description: description, empty: false, tags: tags, extra_tags_count: extra_tags_count } )));
