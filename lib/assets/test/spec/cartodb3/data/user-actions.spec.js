...
describe('cartodb3/data/user-actions', function () {
  ...
  describe('.createAnalysisNode', function () {
    ...
    describe('when given a layer that has no analysis yet', function () {
      beforeEach(function () {
        ...
        spyOn(QueryGeometryModel.prototype, 'on').and.callThrough();
      ...
      it('should listen to query-schema-model changes', function () {
        expect(QueryGeometryModel.prototype.on).toHaveBeenCalled();
        expect(QueryGeometryModel.prototype.on.calls.count()).toEqual(2);
        expect(QueryGeometryModel.prototype.on.calls.argsFor(1)[0]).toEqual('change:ready');
        expect(QueryGeometryModel.prototype.on.calls.argsFor(1)[1]).toEqual(jasmine.any(Function));
      });
      ...
      describe('when query-schema-model ready attr is changed', function () {
        beforeEach(function () {
          ...
          spyOn(this.queryGeometryModel, 'off').and.callThrough();
        ...
        it('should remove listener', function () {
          expect(this.queryGeometryModel.off).toHaveBeenCalledWith('change:ready', jasmine.any(Function));
          expect(this.queryGeometryModel.off.calls.argsFor(0)[1]).toBe(QueryGeometryModel.prototype.on.calls.argsFor(1)[1]);
        });

        it('should listen to status attr', function () {
          expect(QueryGeometryModel.prototype.on.calls.count()).toEqual(3);
          expect(QueryGeometryModel.prototype.on.calls.argsFor(2)[0]).toEqual('change:status');
          expect(QueryGeometryModel.prototype.on.calls.argsFor(2)[1]).toEqual(jasmine.any(Function));
        });
  ...
  describe('.createLayerForTable', function () {
    ...
    it('should create a new analysis for the source quoting table names if needed', function () {
      var newLayer = this.userActions.createLayerForTable('000cd294-b124-4f82-b569-0f7fe41d2db8');
      var analysisDefModel = this.analysisDefinitionsCollection.findWhere({node_id: newLayer.get('source')});
      expect(analysisDefModel).toBeDefined();
    ...
    it('should add style_properties if geometry is provided', function () {
      var layer = this.userActions.createLayerForTable('tableName', 'line');
      expect(layer.styleModel.get('type')).toBe('simple');
      var stroke = layer.styleModel.get('stroke');
      expect(stroke.color.fixed).toBe('#3EBCAE');

      var layer2 = this.userActions.createLayerForTable('tableName', 'polygon');
  ...
  describe('.deleteLayer', function () {
    ...
    describe('when given a layer w/o any dependent nodes (D)', function () {
      ...
      it('should delete the layer', function () {
        expect(this.layerDefinitionsCollection.pluck('letter')).toEqual([undefined, 'a', 'b', 'c', 'd'], 'should exclude D');
      });

      it('should delete affected widgets', function () {
        expect(this.widgetDefinitionsCollection.pluck('source')).toEqual(['b1', 'b2', 'c1', 'd1'], 'should exclude d2');
      });

      it('should persist analyses', function () {
        expect(this.analysisDefinitionsCollection.pluck('node_id')).toEqual(['a1', 'b2', 'c2', 'd0'], 'should exclude d2');
      });

      it('should delete orphaned nodes', function () {
        expect(this.analysisDefinitionNodesCollection.pluck('id').sort()).toEqual(['a0', 'a1', 'b0', 'b1', 'b2', 'c1', 'c2', 'd0'], 'should exclude [d2, d2]');
      });
    });
    ...
    describe('when given a layer with a primary dependent layer further down the linked nodes list (C)', function () {
      ...
      it('should delete affected layers', function () {
        expect(this.layerDefinitionsCollection.pluck('letter')).toEqual([undefined, 'a', 'b', 'c'], 'should exclude [C,D]');
      });
      ...
      it('should persist analyses and deleted the analysis of the ones that are no longer needed', function () {
        expect(this.analysisDefinitionsCollection.pluck('node_id')).toEqual(['a1', 'b2', 'c0']);
      });

      it('should delete orphaned nodes', function () {
        expect(this.analysisDefinitionNodesCollection.pluck('id').sort()).toEqual(['a0', 'a1', 'b0', 'b1', 'b2', 'c0', 'c1'], 'should exclude [c2, d2, d2]');
      });
    });
