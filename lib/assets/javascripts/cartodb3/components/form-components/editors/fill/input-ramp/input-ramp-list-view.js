...
module.exports = cdb.core.View.extend({
  initialize: function (opts) {
    ...
    this._rampList = this.options.rampList;

    var ramp = _.map(this.model.get('categories'), function (category) {
      return category.color;
    }, this);

    this._rampList.add({ name: this._getRampName(), val: ramp }); // TODO: remove name param?
  ...
  _initViews: function () {
    ...
    this._rampListView.bind('customize', function () {
      var rampItem = this._rampList.add({ name: this._getRampName(), val: this._getDefaultRamp() }); // TODO: name param
      this.trigger('selectItem', rampItem.get('val'), this);
    }, this);
  },
  ...
  _getRampName: function () {
    return 'ramp_' + Math.round(Math.random() * 10000);
  },
