...
(function() {
    ...
    cdb.admin.TableView = cdb.ui.common.Table.extend({
      ...
      _HIDDEN_COLUMNS: ['the_geom', 'the_geom_webmercator', 'lat', 'lon', 'lng', 'long', 'latitude', 'longitude'],
      ...
      _renderHeader: function() {
        ...
        var tableName = '';
        var tableAlias = '';
        if (this.user.featureEnabled("bbg_geom_columns")) {
          thead = cdb.ui.common.Table.prototype._renderHeader.apply(this);
        } else {
          var self = this;
          thead = $("<thead>");
          var tr = $("<tr>");
          ...
          _(this.model.get('schema')).each(function(col) {
            if (!_.contains(self._HIDDEN_COLUMNS, col[0])) {
        ...
        var table_title_alias = (this.model.get('aliases') && this.model.get('aliases').name) ? this.model.get('aliases').name : null;
        thead.prepend(cdb.templates.getTemplate('table/views/table_title')({'tablename': this.model.get('name'), 'titleAlias': table_title_alias}));
        return thead;
      },
      ...
      _getColumnName: function(idx) {
          if (this.user.featureEnabled("bbg_geom_columns")) {
            return this.model.columnName(idx-1);
          } else {
            var self = this;
            var columnNames = this.model.columnNames();
            columnNames = _.reject(columnNames, function(c) { return _.contains(self._HIDDEN_COLUMNS, c); });
      ...
      _editCell: function(e, cell, x, y) {
        ...
        var column = this._getColumnName(x);
