  ...
  cdb.admin.ShareDialog = cdb.admin.BaseDialog.extend({
    ...
    _getSublayerOptions: function() {
      ...
      var layers = this.map.layers.filter(function(layer) { return layer.get("type") == "CartoDB" });
...
_enableCopy: function(active) {
  ...
  // Check if the device is touchable, remove
  // readonly attribute from share input
  if (!!('ontouchstart' in window) || !!('onmsgesturechange' in window)) {
    this.$('input.url')
      .removeAttr('readonly')
      .attr('onkeydown', 'event.preventDefault();event.stopPropagation();return false;')
      .attr('oncut', 'event.preventDefault();event.stopPropagation();return false;');
  }
...
_setEmbedURL: function() {
  ...
  */
  ...
  var url = "<iframe width='100%' height='400' frameborder='0' src='" + origin + tableUrl + "?" + opts + "'></iframe>";
