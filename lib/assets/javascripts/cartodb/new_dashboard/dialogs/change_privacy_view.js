...
var Permissions = require('./change_privacy/permissions_collection');
...
module.exports = BaseDialog.extend({
  ...
  initialize: function() {
    ...
    if (this._user.organization) {
      this._permissions = Permissions.byPermission(this._vis.permission, this._user.organization);
    }
  ...
  _save: function() {
    ...
    if (selectedOption.canSave()) {
      ...
      selectedOption.saveToVis(this._vis)
        ...
        .fail(function() {
          self._contentPane.each(function(name, view) {
            view.delegateEvents();
          });
        });
    }
