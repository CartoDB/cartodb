...
function attributesForRedirect (attrs) {
  return _.extend({
    ...
    table_name: 'foobar',
    state: 'complete',
    original_url: 'http://example.com/foo.carto'
  }, attrs);
}
...
describe('dashboard/background_polling_model', function () {
  beforeEach(function () {
    ...
    spyOn(this.importsModel2, 'isRunning').and.returnValue(false);
    spyOn(this.importsModel3, 'isRunning').and.returnValue(false);
  });
  ...
  describe('when import has been completed', function () {
    ...
    describe('should NOT redirect', function () {
      it('if more than one table is created and file is not .carto', function () {
        this.importsModel.imp.set(attributesForRedirect({
          tables_created_count: 2,
          original_url: 'http://example.com/foo.csv'
        }));
      ...
      it('if for some reason there is no redirect URL', function () {
        this.importsModel.imp.set(attributesForRedirect({
          tables_created_count: 2,
      ...
      it('if other imports are still running', function () {
        this.importsModel2.isRunning.and.returnValue(true);
        this.importsModel3.isRunning.and.returnValue(false);
