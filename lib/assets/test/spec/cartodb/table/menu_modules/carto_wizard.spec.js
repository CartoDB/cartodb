...
describe('CartoWizard', function() {
  ...
  describe("ColorWizard", function() {
    ...
    it("should NOT apply color styles if property is not defined or colors are not specified", function() {
      view.render();
      spyOn(view, 'setFormProperties');
      spyOn(view, '_getValues');
      view.cartoProperties.trigger('change'); // Simulating when new wizard is selected -> carto_wizard.js:248
      expect(view.setFormProperties).toHaveBeenCalled();
    ...
    it("should select the first column by default if there isn't one previously selected", function() {
      ...
      spyOn(view, '_getValues');
      view.cartoProperties.trigger('change');
      expect(view.cartoProperties.get('property')).toBe('test')
      expect(view._getValues).toHaveBeenCalled();
    });
    ...
    it("should get values when property has changed", function() {
      ...
      spyOn(view, '_getValues');
      debugger;
      view.cartoProperties.set({ property: 'test10' });
      expect(view._getValues).toHaveBeenCalled();
    });
