...
var heatmapConversion = function (style, animated) {
  ...
  var type = animated === true ? 'animated' : 'heatmap';

  var attribute;
  if (type === 'animated') {
    if (style.animated && style.animated.attribute) {
      attribute = style.animated.attribute;
    }
  } else {
    attribute = attr;
  }

  if (animatedEnabled(type, attribute)) {
    style.animated = _.extend(style.animated || {}, {resolution: style.resolution});
    style.animated = _.clone(_.defaults(style.animated, defaultProperties));
  } else {
    style.animated = _.extend({}, defaultProperties, {resolution: style.resolution});
  }
...
function animatedEnabled (type, attribute) {
  return type === 'animated' && isValidAttribute(attribute); // styleDef.animated && styleDef.animated.enabled && styleDef.animated.enabled !== 'false';
}
...
function generateCartoCSS (style, geometryType) {
  ...
  var attribute;
  if (style.properties.animated && style.properties.animated.attribute) {
    attribute = style.properties.animated.attribute;
  }

  var isAnimatable = animatedEnabled(style.type, attribute);
...
function generateStyle (style, geometryType, mapContext) {
  ...
  var layerType = style.type === 'heatmap' || style.type === 'animated' ? 'torque' : 'CartoDB';
