...
module.exports = BaseDialog.extend({
  ...
  _onAnalyzeColumn: function(column) {
    ...
    if (analyzedColumnsCount === this.columns.length) {
      ...
      if (successfullColumnsCount === 0) {
        ...
        this._getPending().forEach(function(c){
          if(c.get("type") === "date"){
            ...
            var options = {torque: {} };
            for(var i = 0; i < 3; i++){
              var thisCol = self._getSuccessColumns()[i];
              // if(thisCol.get("type") === "string"){
              //   options.torque.type = "category";
              //   options.torque.dataColumn = thisCol;
              // }
              if(thisCol.get("type") === "number"){
                options.torque.type = "value";
                options.torque.dataColumn = thisCol;
              }
              self._guessColumn(c, options);
            }
          }
          else{
      ...
      var column = this.columns.find(function(column) {
        return !column.get("analyzed");
      })
      if (column) {
        this._analyzeColumn(column);
      } else {
        this._analyzeNextColumn();
      }
    }
  },
  ...
  _analyzeStats: function(column, stats) {
    ...
    if (type === 'string') {
      ...
      if (stats.bbox && this._GET_BBOX_FROM_THE_GEOM) {
        this.bbox = stats.bbox;
      }
      result = this._analyzeGeom(name, stats);
    }
  ...
  _analyzeGeom: function(name, stats) {
    if (stats.cluster_rate < 0.1) {
      this._rejectMessage(name, "cluster_rate < 10%", stats.cluster_rate  * 100);
      return this._REJECT;
    }
    return this._KEEP;
  },
  ...
  _analyzeColumns: function() {
    this._analyzeColumn(this.columns.first());
  },

  _analyzeNextColumn: function() {
    var column = this.columns.find(function(column) {
      return !column.get("analyzed");
    });
