...
describe("admin table", function() {
  ...
  describe("cbd.admin.CartoDBTableMetadata", function() {
    ...
    it("altertable", function() {
      ...
      expect(t.alterTable('truncate table')).toEqual(true);
      expect(t.alterTable('truncate schema.table')).toEqual(true);
  ...
  describe("cbd.admin.SQLViewData", function() {
    ...
    it("should replace variables like {x},{y},{z} by a 0", function() {
      ...
      expect(sqlView.getSQL()).toEqual('select 0, 0, 0 as aaa from table');
      expect(sqlView.options.get('sql')).toEqual('select {x}, {y}, {z} as aaa from table')
      expect(sqlView.getRawSQL()).toEqual('select {x}, {y}, {z} as aaa from table')
      sqlView.setSQL('select \\{x} as aaa from table');
      expect(sqlView.getSQL()).toEqual('select {x} as aaa from table');
    });

    it("should replace variables like !pixel_width! by a 0", function() {
      sqlView.setSQL('select !pixel_width! as aaa from table');
      expect(sqlView.getSQL()).toEqual('select 156543.03515625 as aaa from table');
      expect(sqlView.options.get('sql')).toEqual('select !pixel_width! as aaa from table')
      expect(sqlView.getRawSQL()).toEqual('select !pixel_width! as aaa from table')
      sqlView.setSQL('select !bbox! as aaa from table');
      expect(sqlView.getSQL()).toEqual('select ST_MakeEnvelope(-20037508.5,-20037508.5,20037508.5,20037508.5,3857) as aaa from table');
    });
