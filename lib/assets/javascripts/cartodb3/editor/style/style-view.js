...
module.exports = cdb.core.View.extend({
  ...
  initialize: function (opts) {
    ...
    this._styleModel.bind('change', function (mdl, d) {
      if (!d.unsetData) {
        this.render();
      }
    }, this);
    this._styleModel.bind('change', _.debounce(this._onStyleChange.bind(this), 50));
  ...
  _renderCarousel: function () {
    ...
    var geometryType = geom ? geom.getSimpleType() : '';
    var carouselCollection = new CarouselCollection(
      _.map(StylesFactory.getStyleTypes(this._styleModel.get('type'), geometryType), function (type) {
        return {
  ...
  clean: function () {
    this._styleModel.unbind('change', _.debounce(this._onStyleChange.bind(this), 50));
