...
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.template = cdb.templates.getTemplate('new_common/views/dashboard_header/notifications/templates/user_notifications');
  ...
  render: function() {
    ...
    // has alerts?
    this.$el[ notificationsCount > 0 ? 'addClass' : 'removeClass' ]('has--alerts');
  ...
  _generateCollection: function() {
    ...
    d.userType = 'regular';
    d.upgradeUrl = this.currentUserUrl.toUpgradeAccount();
    d.contactUrl = this.currentUserUrl.toUpgradeContactMail();
    ...
    if (!d.isInsideOrg && d.accountType === 'free' && this.user.get("table_count") > 0) {
      arr.push({
        type:   'try_trial',
        msg:    cdb.templates.getTemplate('new_common/views/dashboard_header/notifications/templates/try_trial')(d),
    ...
    if (d.remainingQuota <= 0) {
      arr.push({
        ...
        msg:    cdb.templates.getTemplate('new_common/views/dashboard_header/notifications/templates/limits_exceeded')(d),
    ...
    if ((( d.remainingQuota * 100 ) / d.bytesQuota ) < 20) {
      arr.push({
        type:   'close_limits',
        msg:    cdb.templates.getTemplate('new_common/views/dashboard_header/notifications/templates/close_limits')(d),
    ...
    if (this.user.get("show_upgraded_message")) {
      arr.push({
        ...
        msg:    cdb.templates.getTemplate('new_common/views/dashboard_header/notifications/templates/upgraded_message')(d),
    ...
    if (this.user.get("show_trial_reminder")) {
      arr.push({
        ...
        msg:    cdb.templates.getTemplate('new_common/views/dashboard_header/notifications/templates/trial_ends_soon')(d),
    ...
    arr.push({
      type:   'new_dashboard',
      msg:    cdb.templates.getTemplate('new_common/views/dashboard_header/notifications/templates/new_dashboard')(d),
      opened: this.localStorage.get('notification.new_dashboard')
    });

    // custom notification -> check it and that's it
    if (this.user.get("notification")) {
      arr.push({
        type:   'custom',
        msg:    this.user.get('notification'),
        opened: this.localStorage.get('notification.custom')
      });
    } else {
      this.localStorage.remove('notification.custom')
    }
  ...
  _openNotifications: function(e) {
    ...
    var view = this.notification = new NotificationsDropdown({
      ...
      horizontal_offset:  5,
      vertical_offset:    -5,
      template_base:      'new_common/views/dashboard_header/notifications/templates/dropdown'
    });
