...
cdb.admin.mod.Filters = cdb.core.View.extend({
    ...
    initialize: function() {
      ...
      // Update the combos when the schema changes
      this.options.table.bind("change:schema", function() {

        var comboData = self._generateComboData();

        self.columnSelector.updateData(comboData);
        self.innerColumnSelector.updateData(comboData);
