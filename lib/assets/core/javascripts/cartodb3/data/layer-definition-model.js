...
module.exports = Backbone.Model.extend({
    ...
    if (r.options.table_name) {
      ...
      if (!this.styleModel) {
        this.styleModel = new StyleDefinitionModel(r.options.style_properties, {
          // isTVTLayer: opts.userModel.featureEnabled('dot_properties') && Utils.isTVTTable(r.options.table_name),
          isTVTLayer: Utils.isTVTTable(r.options.table_name),
          parse: true
        });
  ...
  isTVTLayer: function () {
    var styleProperties = this.get('style_properties');

    return styleProperties && styleProperties.type === 'tvt';
  },
