...
namespace :carto do
  namespace :db do
    ...
    task set_carto_attribution: :environment do
      ...
      Carto::Layer.find_each do |layer|
        ...
        begin
          ...
          layer.options['category'] = 'CARTO' if category.present? && category == 'CartoDB'
          layer.save
        ...
        begin
          ...
          success = update_visualization_and_mapcap(visualization) do |vis, persisted|
            vis.user_layers.each do |l|
              ...
              next unless basemaps.keys.include?(layer_class)
    ...
    task sync_basemaps_from_app_config: :environment do
      ...
      basemaps_by_class_name = basemaps.flat_map do |category, classes|
        if category != 'GMaps'
        ...
        begin
          success = update_visualization_and_mapcap(visualization) do |vis, persisted|
            ...
            if labels_layer && attributes['labels']
              ...
              warn "WARN: Visualization #{vis.id} has label but basemap class #{class_name} does not"
            elsif attributes['labels']
              warn "WARN: Basemap class #{class_name} has label but visualization #{vis.id} does not"
            end
          ...
          errors += 1
          warn "Error updating visualization #{visualization.id}: #{e.inspect}. #{e.backtrace.join(',')}"
        end
      end
