...
var AddGroupUsersView = require('./add_group_users_view');
...
module.exports = Router.extend({
  ...
  routes: Router.supportTrailingSlashes({
    ...
    ':id': 'routeToShowGroup',
    ':id/add_users': 'routeToAddGroupUsers',
    ...
    this.model.createGroupView(id, function(group) {
      self.model.set('view',
        ViewFactory.createByList([
          self._createGroupHeader(group),
          new ShowGroupView({
            addUsersUrl: self.groupUrl(group, 'add_users'),
            group: group
          })
        ])
      );
  ...
  routeToAddGroupUsers: function(id) {
    var self = this;
    this.model.createGroupView(id, function(group) {
      self.model.set('view',
        ViewFactory.createByList([
          self._createGroupHeader(group),
          new AddGroupUsersView({
            group: group
          })
        ])
      );
  ...
  groupUrl: function(group, subpath) {
    var path = group.id;
    if (subpath) {
      path += '/' + subpath
    }
