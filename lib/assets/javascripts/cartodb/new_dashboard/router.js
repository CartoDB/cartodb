...
module.exports = Router.extend({
  ...
  initialize: function(opts) {
    ...
    this.model = new RouterModel({
      ...
      q:       this._uriItem('search', opts),
      tag:     this._uriItem('tag', opts)
    });
  },

  _uriItem: function(key, opts) {
    // Has to look on window.location because Backbone.History.fragment is not started just yet.
    var re = new RegExp(key +'\/([^\/]+)');
    var matches = re.exec((opts.window || window).location.pathname);
    if (matches) {
