...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    this._needsGeocoding = false;
    this._isLayerEmpty = false;
    this._queryGeometryHasGeom = true;

    console.log('[', this.model.get('letter'), '] data-layer-view::initialize');

    this._bindEvents();
    this._debouncedRender = _.debounce(this._render, 30);
    ...
    this._initializeGeocoding();
    this._initializeLayerEmptyness();
    this._initializeQueryGeometryHasGeom();
  },

  render: function () {
    return this._debouncedRender();
  },

  _render: function () {
    console.log('[', this.model.get('letter'), '] data-layer-view::render');
    this.clearSubViews();
  ...
  _onEditLayer: function (event) {
    ...
    if (this.model.canBeGeoreferenced()) {
      Router.goToAnalysisTab(this.model.get('id'));
      return;
    }

    Router.goToStyleTab(this.model.get('id'));
  },
  ...
  _initializeGeocoding: function () {
    var self = this;
    this.model.canBeGeoreferenced()
      .then(function (canBeGeoreferenced) {
        var needsGeocoding = canBeGeoreferenced && !self.model.hasAnalyses();
        if (self._needsGeocoding !== needsGeocoding) {
          self._needsGeocoding = needsGeocoding;
          console.log('[', self.model.get('letter'), '] Rerrenderizaremos por geocoding!');
          self.render();
        }
      });
  },

  _initializeLayerEmptyness: function () {
    var self = this;
    this.model.isEmptyAsync()
      .then(function (isEmpty) {
        if (self._isLayerEmpty !== isEmpty) {
          self._isLayerEmpty = isEmpty;
          console.log('[', self.model.get('letter'), '] Rerrenderizaremos por empty!');
          self.render();
        }
      });
  },

  _initializeQueryGeometryHasGeom: function () {
    var self = this;
    this._queryGeometryModel.hasValueAsync()
      .then(function (hasGeom) {
        if (self._queryGeometryHasGeom !== hasGeom) {
          self._queryGeometryHasGeom = hasGeom;
          console.log('[', self.model.get('letter'), '] Rerrenderizaremos por hasGeom!');
          self.render();
        }
      });
  },
