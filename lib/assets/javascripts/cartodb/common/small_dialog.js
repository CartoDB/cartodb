...
cdb.admin.EditTextDialog = cdb.admin.SmallDialog.extend({
  ...
  initialize: function() {
    _.defaults(this.options, {
      ...
      ok_title: this.ok_title,
      modal_class: this.model_class,
      clean_on_hide: this.clean_on_hide
    });
  ...
  render_content: function() {
    return '<input data-placeholder="add a value" value="' + (this.options.initial_value || "")+ '"></input>';
  },
  ...
  render: function() {
    ...
    this.super('render');
    setTimeout(function() {
      var $ioElements = self.$('select, input, textarea');
      $ioElements.get(0).focus();
      $.map($ioElements, function(e, i) {
        $e = $(e);
