...
module.exports = Backbone.Router.extend({
  ...
  /**
   * @param args {Object}
   *   baseUrl: {String} Base URL for the current router context
   *   rootPath: {String} the path to the rootPath
   */
  ...
  enableAfterMainView: function() {
    ...
    Backbone.history.start({
      pushState:  true,
      root:       this._rootPath +'/' //Yes, this trailing slash is necessary for the router to update the history state properly.
    });
  },

  _normalizeFragmentOrUrl: function(str) {
    return str.replace(this._baseUrl, '');
  }
});
