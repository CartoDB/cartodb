...
describe('data/undo-manager', function () {
  ...
  describe('public methods', function () {
    it('should add several public methods to the model', function () {
      _.each(['undo', 'redo', 'canUndo', 'canRedo', 'canCancel', 'getUndoHistory'], function (method) {
    ...
    it('should check if there is cancel action', function () {
      var model = new Backbone.Model({
        test: 1
      });
      var history = [];
      ...
      expect(model.canCancel()).toBeFalsy();

      model.set({test: 2});
      expect(model.canCancel()).toBeTruthy();
    });
