...
// Instead of passing layerDefinitionsCollection to the analysisDefinitionNodesCollection (where it is not
// created at that point), we prefer to just bind any simple_geom and store the geometry in the node
// doing a save of the analysis
analysisDefinitionNodesCollection.bind('change:simple_geom', function (nodeDefModel) {
  if (nodeDefModel.get('table_name')) { // If it is a source node, no save
    return;
  }
  var layerDefModel = layerDefinitionsCollection.findWhere({ source: nodeDefModel.get('id') });
  if (layerDefModel) {
...
if (visDefinitionModel.get('version') === 2 && modals) {
  visDefinitionModel.bind('change:version', mapCreation, this);
