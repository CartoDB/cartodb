...
module CartoDB
  module Importer2
    class Column
      ...
      DEFAULT_SRID    = 4326
      WKB_RE          = /^\d{2}/.freeze
      GEOJSON_RE      = /{.*(type|coordinates).*(type|coordinates).*}/.freeze
      WKT_RE          = /POINT|LINESTRING|POLYGON/.freeze
      KML_MULTI_RE    = /<Line|<Polygon/.freeze
      KML_POINT_RE    = /<Point>/.freeze
      ...
      def type
        db.schema(table_name, reload: true, schema: schema)
          .select do |column_details|
            column_details.first == column_name
          end.last.last.fetch(:db_type)
      end
      ...
      def empty_lines_to_nulls
        ...
        db.schema(table_name).each do |colid, coldef|
          column_type = coldef[:type] if colid == column_id
        end
