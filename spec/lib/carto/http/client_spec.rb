...
describe Carto::Http::Client do
  ...
  describe '#request' do
    it 'wraps creation of typhoeus request objects' do
      ...
      expect(Typhoeus::Request).to receive(:new).once.with(expected_url, expected_options)
      client.request(
        'www.example.com',
        method: :post,
        body: 'this is a request body',
    ...
    it 'creates a request with a default connecttimeout' do
      ...
      expect(Typhoeus::Request).to receive(:new).once.with('www.example.com', expected_options)
      client.request('www.example.com')
    end

    it 'can override the connecttimeout default value' do
      expected_options = { connecttimeout: 45 }
      expect(Carto::Http::Request).to receive(:new).once.with(anything, 'www.example.com', expected_options)
  ...
  describe 'Request#run' do
    it 'performs a request through its typhoeus request object' do
      ...
      Typhoeus.double('www.example.com').and_return(expected_response)
  ...
  describe '#set_x_request_id!' do
    ...
    it 'adds the X-Request-ID header from the CurrentRequest when present' do
      ...
      expected_response = Typhoeus::Response.new(code: 200, body: "{'name' : 'paul'}")
      Typhoeus.double('www.example.com') do |request|
  ...
  describe '#get' do
    ...
    it 'creates a request with a default connecttimeout' do
      ...
      mocked_request = double
      expect(mocked_request).to receive(:run).once
      expect(Carto::Http::Request).to receive(:new).with(anything, 'www.example.com', expected_options).and_return(mocked_request)
  ...
  describe '#post' do
    it 'creates a wrapped POST request and runs it' do
      ...
      Typhoeus.double('www.example.com/posts').and_return(expected_response)
      client
  ...
  describe '#put' do
    it 'creates a wrapped PUT request and runs it' do
      expected_response = Typhoeus::Response.new(code: 200, body: "{'success': 'true'}")
      Typhoeus.double('www.example.com/posts/1').and_return(expected_response)
