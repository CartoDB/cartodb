...
module.exports = cdb.core.View.extend({
  ...
  _initViews: function () {
    ...
    this._stackLayoutView.model.set('position', 1); // go to the 2nd page
  },
  ...
  _createRampListView: function (stackLayoutModel, opts) {
    var view = new InputRampListView({
      model: this.model,
      bins: 7, // TODO: move to defaults
      isCustomizable: true,
      showSearch: false
    });

    view.on('selectItem', function (item) {
      var categories = _.map(this.model.get('categories'), function (category, index) {
        category.color = item[index];
        return category;
      }, this);
