...
var GeoreferenceOnboardingLauncher = require('../components/onboardings/georeference/georeference-launcher');
...
module.exports = CoreView.extend({
            ...
            var infoboxSstates = [
              {
                state: 'georeference',
                createContentView: function () {
                  var layerId = GeoreferenceOnboardingLauncher.getLayerId();
                  var nonGeoreferencedLayer = self._layerDefinitionsCollection.get(layerId);
                  var name = nonGeoreferencedLayer ? nonGeoreferencedLayer.getTableName() : '';
  ...
  _initBinds: function () {
    ...
    this.listenTo(this._layerDefinitionsCollection, 'remove', this._onLayerCollectionRemove);
    this.listenTo(this._layerDefinitionsCollection, 'add', this._onLayerCollectionAdd);
  },

  _onLayerCollectionAdd: function () {
    this._infoboxModel.unset('state');
    this._findNonGeoreferencedLayer(function () {
      GeoreferenceOnboardingLauncher.launch();
    });
    this._visDefinitionModel.fetch();
  },

  _onLayerCollectionRemove: function () {
    this._infoboxModel.unset('state');
    this._findNonGeoreferencedLayer();
  },

  _findNonGeoreferencedLayer: function (onFindNonGeoLayer) {
    var onFindNonGeoreferencedLayer = function (layerDefinitionModel) {
      if (layerDefinitionModel) {
        var layerId = layerDefinitionModel.get('id');

        GeoreferenceOnboardingLauncher.setLayerId(layerId);
        AnalysesService.setLayerId(layerId);

        this._infoboxModel.set('state', 'georeference');

        onFindNonGeoLayer && onFindNonGeoLayer();
      }
    }.bind(this);
