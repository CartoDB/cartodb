...
describe('components/modals/add-widgets/add-widgets-view', function () {
  ...
  beforeEach(function () {
    ...
    analysisDefinitionNodesCollection.at(1).querySchemaModel.set({ status: 'ready', query: 'SELECT * FROM test' });
    analysisDefinitionNodesCollection.at(2).querySchemaModel.set({ status: 'ready', query: 'SELECT * FROM test2' });
    ...
    this.querySchemaModel2.set('fetched', true);
  ...
  it('should only fetch unfetched layer tables', function () {
    expect(this.querySchemaModel0.fetch).toHaveBeenCalled();
    expect(this.querySchemaModel1.fetch).toHaveBeenCalled();
    expect(this.querySchemaModel2.fetch).not.toHaveBeenCalled();
  });
  ...
  describe('when all layer tables are fetched', function () {
    beforeEach(function () {
      this.querySchemaModel0.set('fetched', true);
      this.querySchemaModel0.trigger('sync');
      this.querySchemaModel1.set('fetched', true);
      this.querySchemaModel1.trigger('sync');
    });
  ...
  describe('when a layer fails', function () {
    beforeEach(function () {
      this.querySchemaModel1.trigger('error');
    });
  ...
  describe('when an analysis is not ready', function () {
    beforeEach(function () {
      ...
      this.querySchemaModel = analysisDefinitionNodesCollection.at(0).querySchemaModel;
      this.querySchemaModel.set('status', 'failed');
