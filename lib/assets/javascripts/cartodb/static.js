...
const StaticConfig = window['StaticConfig'] || {};
...
(function (window, document) {
  ...
  client.getConfig(function (err, response, data) {
    function getAssetsVersion () {
      ...
      return StaticConfig.assetVersion || data.user_frontend_version || version;
    }

    function getAssetsBaseUrl () {
      var userAssetsHost = data.asset_host;

      return (StaticConfig.assetsBaseUrl || userAssetsHost && userAssetsHost + '/assets' || data.config.app_assets_base_url) + '/';
    }
    ...
    if (err) {
      ...
      })(window, document, assetsUrl, '/favicons/favicon.ico', StaticConfig.stylesheets, StaticConfig.scripts);
    }
  });
})(window, document);
