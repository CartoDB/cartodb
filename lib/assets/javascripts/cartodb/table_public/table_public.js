...
if (!window.addEventListener) {
  ...
  function resizeWindow(animated) {
    ...
    if (animated) {
      ...
      $(".cartodb-map-data").animate({ top: top }, 250); 
      $(".separator").animate({ top: top + 1}, 250).show(); 
      $(".separator_shadow").animate({ top: top + 2 }, 250).show();
      $(".navigation").animate({ top: top - 30 }, 250); 
    ...
    if ($("body.public").hasClass("map")) {
      ...
      if (animated) $(".panes").animate({ height: height + 69 }, 250); 
      else $(".panes").css({ height: height + 69 }); 

    } else {

      if (animated) $(".panes").css({ height: height }, 250); 
...
$(function() {
  ...
  cdb.init(function() {
    ...
    if (!window.history || !window.history.pushState) pushState = false;
      ...
      Backbone.history.start({ pushState: pushState, root: '/tables/' });
  ...
  if (!mobileDevice) renderExportOptions();
    ...
    $(".export_options span.download").hide();
  }
