...
import datasetsDOSubscription from '../../fixtures/datasetsDOSubscription';
...
describe('DatasetQuickAction.vue', () => {
  ...
  describe('computed', () => {
    ...
    it('should return "do_subscription" action name when a dataset is a DO subscription', () => {
      const dataset = datasetsDOSubscription.visualizations[0];
      const datasetQuickAction = createDatasetQuickActionComponent(dataset);
      const result = datasetQuickAction.vm.actionMode;

      expect(result).toBe('do_subscription');
    });
  });
