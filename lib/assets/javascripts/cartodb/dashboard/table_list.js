(function() {
  ...
  var TableView = cdb.core.View.extend({
    ...
    events: {
      ...
      "click a.delete:not(.disabled)": "_showDeleteConfirmation",
  ...
  var TableList = cdb.core.View.extend({
    ...
    initialize: function() {
      ...
      _.bindAll(this, "render", "partialRender", "appendTableByNumber", "_showLoader", "_hideLoader",
        "hardRender", "_updateListHeader", "_addAll", "_addTable", "_removeAllPrivacy",
    ...
    _addTable: function(m) {
      ...
      if(!this.hasTable(m.get('name'))) {
        ...
        li.bind('deleted', function(cid) {
          self.model.trigger('updated')
          self.model.trigger('reset');
          delete self._subviews[cid];
          self.refreshTable();
        })
    ...
    refreshTable: function() {
      var self = this;
      $.when(this.model.refillTableList(self._TABLES_PER_PAGE)).done(function(){
        self.partialRender();
