...
describe('cdb.admin.WizardProperties', function() {
  ...
  beforeEach(function() {
    ...
    model = new cdb.admin.WizardProperties({ table: table, layer: layer });
    layer.sync = function() {};
  });
...
describe('CartoWizard', function() {
  var view, model, table, wizard_properties;
  beforeEach(function() {
    model = new Backbone.Model();
    table = TestUtil.createTable('test');
    wizard_properties = new cdb.admin.WizardProperties({ table: table });
  ...
  describe("SimpleWizard", function() {
    ...
    describe('propertiesFromStyle', function() {
      ...
      it("should return new properties based on cartocss", function() {
        view.render();
        view.cartoProperties.set('polygon-pattern-file', 'url(https://rambo)');
        view.cartoProperties.trigger('change');
        expect(view.cartoProperties.get('polygon-fill')).toEqual('#FF6600');
        var modified = view.propertiesFromStyle('#layer { polygon-fill: #FFF; line-width: 0.1; marker-fill: red; polygon-pattern-file: url(https://s3.amazonaws.com/com.cartodb.users-assets.production/production/matallo/assets/20130808152815bullet.png);\n}');
      ...
      it("should return not change values when style is wrong", function() {
        ...
        view.cartoProperties.trigger('change');
        expect(view.cartoProperties.get('polygon-fill')).toEqual('#FF6600');
        var modified = view.propertiesFromStyle('#layer { polygon-fill: "rambo"; line-width: 0.1; marker-fill: red;}');
        expect(modified).toEqual({});
      })
  ...
  describe("CategoryWizard", function() {
    ...
    it("shouldn't generate the categories list from a colors property if categories was already defined", function() {
      view.render();
      var list = [{ title:'rojo', title_type:'string', color:'#F11810', value_type:'color' }];
    ...
    it("shouldn't add colors list if categories list has been changed", function() {
      view.render();
      var categories = [{ title:'azul', title_type:'string', color:'#4D4DFF', value_type:'color' }];
      var colors = [['rojo', '#F11810', 'string']];
      view.model.set('properties', { colors: colors, categories: categories }, { silent: true });
    ...
    it("should add all the previous properties if it came from color wizard", function() {
      view.render();
      var colors = [['rojo', '#F11810', 'string']];
      view.model.set('properties', { colors: colors, 'marker-width': 30.5, 'marker-opacity': 0.1 }, { silent: true });
      ...
      view.model.bind('change', function() {
        ...
        expect(view.cartoProperties.get('categories')[0].color).toBe('#F11810');
        expect(view.cartoProperties.get('categories')[0].value_type).toBe('color');
        expect(view.cartoProperties.get('marker-width')).toBe(30.5);
        expect(view.cartoProperties.get('marker-opacity')).toBe(0.1);
      })
