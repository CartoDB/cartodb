...
describe('new_dashboard/maps/maps_item', function() {
  ...
  describe('tags', function() {
    it('should show tags when present', function() {
      ...
      expect(this.view.$('.NoResults').text()).not.toContain('No tags');
    });

    it('should show \'No tags\' if no tags are present', function(){
      this.vis.set('tags', []);
      this.renderView();

      expect(this.view.$('.DefaultTags-item').length).toBe(0);
      expect(this.view.$('.NoResults').text()).toContain('No tags');
    })
  });
