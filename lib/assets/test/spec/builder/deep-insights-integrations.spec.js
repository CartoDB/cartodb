...
describe('deep-insights-integrations/dii', function () {
  ...
  it('should manage time-series using the animated layer when there is an animated layer', function () {
    spyOn(WidgetsIntegration, 'manageTimeSeriesForTorque');
    // Add animated layer definition mock
    this.layerDefinitionsCollection.models = [{
      styleModel: {
        get: function (attribute) {
          if (attribute === 'type') return 'simple';
        }
      }
    }, {
      styleModel: {
        get: function (attribute) {
          if (attribute === 'type') return 'animation';
        }
      }
    }];

    var simpleLayer = this.layerDefinitionsCollection.at(0);
    var animatedLayer = this.layerDefinitionsCollection.at(1);

    LayersIntegration.trigger('onLayerChanged', simpleLayer, this.layerDefinitionsCollection);
    expect(WidgetsIntegration.manageTimeSeriesForTorque).toHaveBeenCalledWith(animatedLayer);

    LayersIntegration.trigger('onLayerChanged', animatedLayer, this.layerDefinitionsCollection);
    expect(WidgetsIntegration.manageTimeSeriesForTorque).toHaveBeenCalledWith(animatedLayer);
  });

  it('should manage time-series using the provided layer when there is no animated layer', function () {
    spyOn(WidgetsIntegration, 'manageTimeSeriesForTorque');
    // Add animated layer definition mock
    this.layerDefinitionsCollection.models = [{
      styleModel: {
    ...
    var simpleLayer0 = this.layerDefinitionsCollection.at(0);
    var simpleLayer1 = this.layerDefinitionsCollection.at(1);

    LayersIntegration.trigger('onLayerChanged', simpleLayer0, this.layerDefinitionsCollection);
    expect(WidgetsIntegration.manageTimeSeriesForTorque).toHaveBeenCalledWith(simpleLayer0);

    LayersIntegration.trigger('onLayerChanged', simpleLayer1, this.layerDefinitionsCollection);
    expect(WidgetsIntegration.manageTimeSeriesForTorque).toHaveBeenCalledWith(simpleLayer1);
  });
});
