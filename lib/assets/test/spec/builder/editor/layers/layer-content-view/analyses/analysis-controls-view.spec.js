...
describe('editor/layers/layer-content-view/analyses/analysis-controls-view', function () {
  ...
  beforeEach(function () {
    ...
    canBeGeoreferencedPromise = fakePromise(this._layerDefinitionModel, 'canBeGeoreferenced');
  ...
  describe('.initialize', function () {
    beforeEach(function () {
      spyOn(AnalysisControlsView.prototype, '_renderIfCanBeGeoreferencedChanges').and.callThrough();
    ...
    describe('._renderIfCanBeGeoreferencedChanges', function () {
      it('should initialize canBeGeoreferenced', function () {
        expect(this.view._canBeGeoreferenced).toBeDefined();
        expect(AnalysisControlsView.prototype._renderIfCanBeGeoreferencedChanges).toHaveBeenCalled();
      });

      it('should render if canBeGeoreferenced has changed', function (done) {
        var self = this;
        var initialCanBeGeoreferenced = this.view._canBeGeoreferenced;
        canBeGeoreferencedPromise.resolve(!initialCanBeGeoreferenced);

        spyOn(this.view, 'render');

        this.view._renderIfCanBeGeoreferencedChanges();

        setTimeout(function () {
          expect(self.view.render).toHaveBeenCalled();
          expect(self.view._canBeGeoreferenced).toEqual(!initialCanBeGeoreferenced);
          done();
        }, 0);
      });

      it('should not render if canBeGeoreferenced has not changed', function (done) {
        var self = this;
        var initialCanBeGeoreferenced = this.view._canBeGeoreferenced;
        canBeGeoreferencedPromise.resolve(initialCanBeGeoreferenced);
