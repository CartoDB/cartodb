...
module.exports = Backbone.Model.extend({
  ...
  _createOptionsFromNodes: function (queue) {
    ...
    this._analysisDefinitionNodesCollection
      .each(function (nodeDefModel) {
        ...
        queue.defer(function (cb) {
          if (geometryModel.isFetched()) {
            self._appendNodeOption(nodeDefModel);
            return cb();
          }

          if (!geometryModel.canFetch()) {
            return cb();
          }

          var onStatusChange = function () {
            if (geometryModel.get('status') === 'fetching') return;
            geometryModel.off('change:status', onStatusChange);
            self._appendNodeOption(nodeDefModel);
            cb();
          };
          geometryModel.on('change:status', onStatusChange);
          geometryModel.fetch();
        });
      });
  },
  ...
  _appendNodeOption: function (nodeDefModel) {
    var geom = nodeDefModel.geometryModel.get('simple_geom');
