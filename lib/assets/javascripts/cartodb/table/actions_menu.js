  ...
  cdb.admin.LayersPanel = cdb.ui.common.TabPane.extend({
    ...
    _addLayers: function(layers) {
      ...
      if (!this.user.featureEnabled('bbg_rtr_search_bar')) {
    ...
    _addLayer: function(layer, pos) {
      ...
      if (layer.get('type') == 'CartoDB' ||
        ...
        var addTabOptions = this.user.featureEnabled('bbg_rtr_search_bar') ?
                              { prepend: true } : { after: 0 };
 
        self.addTab(layer.cid, v, addTabOptions);
    ...
    _manageLayers: function() {
      ...
      // Set layers offset
      var offset = this.user.featureEnabled('bbg_rtr_search_bar') ? 0 : -15;
