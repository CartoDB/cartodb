...
module.exports = Backbone.Model.extend({
  ...
  canFetch: function () {
    return this.get('query') && this.get('ready');
  },
  ...
  hasDifferentSchemaThan: function (schemaArray) {
    var foundAnyColumnNotIncluded = false;

    _.each(schemaArray, function (columnObj) {
      var columnModel = this.columnsCollection.findWhere({ name: columnObj.name });
      if (!columnModel) {
        foundAnyColumnNotIncluded = true;
      } else if (columnObj.type != null && columnModel.get('type') !== columnObj.type) {
        foundAnyColumnNotIncluded = true;
      }
    }, this);

    return foundAnyColumnNotIncluded;
  },
