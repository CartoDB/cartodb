...
describe('editor/style/style-manager', function () {
  ...
  describe('track changes', function () {
    ...
    it('should change alphamarker url when installation is custom', function () {
      ...
      expect(this.layerDefModel.get('type')).toBe('torque');
      expect(this.layerDefModel.get('cartocss').indexOf('marker-file: url(//s3.amazonaws.com/com.cartodb.assets.static/alphamarker.png);')).not.toBe(-1);

      this.configModel.set('cartodb_com_hosted', true);
      this.layerDefModel.styleModel.set('style', 'dummy');
      expect(this.layerDefModel.get('cartocss').indexOf('marker-file: url(//s3.amazonaws.com/com.cartodb.assets.static/alphamarker.png);')).toBe(-1);
      expect(this.layerDefModel.get('cartocss').indexOf('marker-file: url(/public/assets/images/alphamarker.png);')).not.toBe(-1);
      console.log(this.layerDefModel.get('cartocss'));
    });
