  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    setDataLayer: function(dataLayer) {
      ...
      if (isProUI || this.user.featureEnabled('bbg_enable_merge_datasets')) {
    ...
    _requestFiltersTabRefresh: function() {
      var self = this;
      var requestId = self._filtersTabRefreshRequestId++;

      setTimeout(function() {
        if (requestId == self._filtersTabRefreshRequestId - 1) { // prevent unnecessary refreshes
          self._refreshFiltersTab();
          self._filtersTabRefreshRequestId = 0;
        }
      }, 200);
    },
