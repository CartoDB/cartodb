...
(function() {
  ...
  var UserStats = cdb.core.View.extend({
    ...
    initialize: function() {
      // Control messages with cookies
      // - CartoDB2.0 message ->  'cartodb20'   -> Expire in a week
    ...
    _setWarning: function() {
      ...
      // tables number (cookie) and finally, if not, show the 
      // cartodb20 message (cookie).
      if (config && !config.custom_com_hosted) {
        if (this.recently_upgraded) {
          this._showWarning('upgraded');
          this.warning_type = "upgraded";

        } else if (!$.cookie('limit') && ((this.model.get('byte_quota_status') != "" || this.model.get('table_quota_status') != "")
          && this.model.get("table_quota") != null && this.model.get("byte_quota") != null)) {
          this._showWarning('limit');
          this.warning_type = "limit";

        } else if (!$.cookie('cartodb20')) {
          this._showWarning('cartodb20');
          this.warning_type = "cartodb20";
    ...
    _checkUpgradedUser: function() {
      ...
      if (config && !config.custom_com_hosted && config.cartodb_central_domain_name) {
        var self = this;
        $.ajax({
          url: '//' + config.cartodb_central_domain_name + '/account/' + this.model.get('username') + '/recently_upgraded',
          timeout: 5000,
          dataType: 'jsonp',
          success: function(r) {
    ...
    _disableWarning: function(ev) {
      ...
      } else if (!$.cookie('limit') && this.warning_type == "limit") {
        $.cookie('limit', true);
        this.warning = false;
      } else if (!$.cookie('cartodb20')) {
        // Set cookie to not show again CartoDB2.0 text
        $.cookie('cartodb20', true, { expires: 1, path: '/' });
      }
