...
cdb.admin.Header = cdb.core.View.extend({
  ...
  initialize: function(options) {
    _.bindAll(this, '_changeTitle', '_setDescription', '_setTags', '_changeToVisualization');
    var self = this;
  ...
  _setDescription: function() {
    ...
    this.$('.table_description p')
      [!this.isVisEditable() ? 'addClass' : 'removeClass' ]('static')
      .html(this.model.get('description') || default_description);
  },
  ...
  _setVisualization: function() {
    ...
    // TODO: Change router path if visualization turns into derived!!
    // IMPORTANT!
      ...
      var description_dialog = this.description_dialog = new cdb.admin.EditTextDialog({
        ...
        res: function(val) {
          ...
          if (val !== self.model.get('description')) {
            ...
            $.when(self.model.save({ description:val }, { wait: true })).done(function() {
        ...
        res: function(val) {
          if (val !== self.model.get('name') && val != '') {
            ...
            var change_confirmation = new cdb.admin.BaseDialog({
              title: "Rename this " + (self.model.isVisualization() ? "visualization" : "table"),
  ...
  _changeTags: function(e) {
    ...
    if (this.isVisEditable()) {
      ...
      var tags_dialog = this.tags_dialog = new cdb.admin.TagsDialog({
        ...
        modal_class: 'edit_name_dialog',
        res: function(val) {
          if (!_.isEqual(self.model.get('tags'),val)) {

            var rename_msg = "Applying tag changes...";
            self.globalError.showError(rename_msg, 'load', -1);

            $.when(self.model.save({ tags: val }, { wait: true })).done(function() {
              self.globalError.showError('Saved', 'info', 3000);
            }).fail(function(e) {
