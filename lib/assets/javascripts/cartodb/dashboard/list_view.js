...
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this._initBinds();
    this.originalItemsDirty = true;

    this.collection.bind('reset', function() {
      if (this.originalItemsDirty) {
        this.allItems = this.collection.models;
        this.originalItemsDirty = false;
      }
    }, this);
    ...
    searchInputEl.on('keyup', function(e) {
      ...
      if (resetSearch) {
        self.collection.reset(self.allItems);
        searchInputEl.val('');
      } else {
        q = q.toLowerCase();
        var filtered = _.filter(self.allItems, function(item, index) {
  ...
  render: function() {
    ...
    if (this.router.model.isDatasets()) {
      this.$el.append('<table class="DatasetsTable"><thead><tr><th></th><th></th><th>Name</th><th>Description</th><th>Last Opened</th><th></th><th></th></tr></thead><tbody class="TableBody"></tbody></table>');
