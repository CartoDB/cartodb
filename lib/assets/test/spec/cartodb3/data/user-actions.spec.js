...
describe('cartodb3/data/user-actions', function () {
  ...
  describe('.createAnalysisNode', function () {
    ...
    describe('when given a layer that has no analysis yet', function () {
      ...
      it('should listen to query-geometry-model changes', function () {
        ...
        expect(QueryGeometryModel.prototype.bind.calls.argsFor(1)[0]).toEqual('change:ready');
        expect(QueryGeometryModel.prototype.bind.calls.argsFor(1)[1]).toEqual(jasmine.any(Function));
      });

      describe('when query-geometry-model ready attr is changed', function () {
        beforeEach(function () {
          this.queryGeometryModel = this.analysisDefinitionNodesCollection.get('a1').queryGeometryModel;
          spyOn(this.queryGeometryModel, 'unbind').and.callThrough();
        ...
        it('should remove listener', function () {
          expect(this.queryGeometryModel.unbind).toHaveBeenCalledWith('change:ready', jasmine.any(Function));
          expect(this.queryGeometryModel.unbind.calls.argsFor(0)[1]).toBe(QueryGeometryModel.prototype.bind.calls.argsFor(1)[1]);
        });

        it('should listen to status attr', function () {
          expect(QueryGeometryModel.prototype.bind.calls.count()).toEqual(4);
          expect(QueryGeometryModel.prototype.bind.calls.argsFor(2)[0]).toEqual('change:status');
          expect(QueryGeometryModel.prototype.bind.calls.argsFor(2)[1]).toEqual(jasmine.any(Function));
        });
        ...
        describe('when query-geometry is resetted (by other request, for example)', function () {
          beforeEach(function () {
            this.queryGeometryModel.set('status', 'unfetched');
          });

          it('should unbind the status change', function () {
            expect(this.queryGeometryModel.unbind).toHaveBeenCalledWith('change:status', jasmine.any(Function));
          });
        });

        describe('when query-geometry is fetched', function () {
          beforeEach(function () {
            this.layerDefModel.save.calls.reset();
            spyOn(this.layerDefModel.styleModel, 'setDefaultPropertiesByType').and.callThrough();
          });

          describe('when data contains polygons geometry', function () {
            beforeEach(function () {
              this.queryGeometryModel.set({
                simple_geom: 'polygon',
                status: 'fetched'
            ...
            it('should set default style on layer model', function () {
              expect(this.layerDefModel.styleModel.setDefaultPropertiesByType).toHaveBeenCalledWith('simple', 'polygon', true);
            });
          ...
          describe('when the geometry is unknown', function () {
            beforeEach(function () {
              this.queryGeometryModel.set({
                simple_geom: '',
                status: 'fetched'
  ...
  describe('.saveAnalysis', function () {
    ...
    it('should do nothing if invalid', function () {
  ...
  describe('.saveAnalysisSourceQuery', function () {
    ...
    describe('setDefaultPropertiesByType', function () {
      ...
      it('should reset styles when geometry changes', function () {
        this.queryGeometryModel.set({
          ready: true,
          status: 'fetched',
          simple_geom: 'line'
      ...
      it('should not reset styles when geometry doesn\'t change', function () {
        this.queryGeometryModel.set({
          ready: true,
          status: 'fetched',
          simple_geom: 'point'
    ...
    describe('when there is no analysis', function () {
      beforeEach(function () {
        this.layerDefModel.set('source', 'a1', { silent: true });
        spyOn(this.layerDefModel, 'save').and.callThrough();
        this.userActions.saveAnalysisSourceQuery(this.query, this.a0, this.layerDefModel);
      });
      ...
      it('should create a new analysis for layer', function () {
        expect(this.analysisDefinitionsCollection.pluck('node_id')).toEqual(['a1']);
      });
  ...
  describe('.saveWidgetOption', function () {
    ...
    describe('when source of widget is already persisted', function () {
      ...
      it('should save associatd layer-definition-model', function () {
        expect(this.layerDefModel.save).toHaveBeenCalled();
      });
    });
    ...
    describe('when there is a layer with some analysis', function () {
      ...
      describe('deleting a1', function () {
        ...
        it('should not reset styles', function () {
          var queryGeometryModel = this.layerA.getAnalysisDefinitionNodeModel().queryGeometryModel;
          queryGeometryModel.set({ status: 'fetched', ready: true });
          expect(StyleModel.prototype.setDefaultPropertiesByType).not.toHaveBeenCalled();
        });
  ...
  describe('.createLayerForAnalysisNode', function () {
    ...
    describe('when given node is a source node', function () {
      ...
      it('should not reset styles, they are copied from original layer', function () {
    ...
    describe('when given node is NOT a head of any layer', function () {
        ...
        describe('when created a layer successfully', function () {
          ...
          describe('when widgets are created', function () {
            ...
            it('should reset the styles', function () {
              ...
              var queryGeometryModel = newLayerDefModel.getAnalysisDefinitionNodeModel().queryGeometryModel;
              queryGeometryModel.set({ status: 'fetched', ready: true });
              expect(StyleModel.prototype.setDefaultPropertiesByType).toHaveBeenCalled();
            });
          });
      ...
      describe('and does have saved styles', function () {
        ...
        describe('when created a layer successfully', function () {
          ...
          it('should not reset the styles', function () {
            var newLayerDefModel = this.layerDefinitionsCollection.at(1);
            var queryGeometryModel = newLayerDefModel.getAnalysisDefinitionNodeModel().queryGeometryModel;
            queryGeometryModel.set({ status: 'fetched', ready: true, simple_geom: 'point' });
            expect(StyleModel.prototype.setDefaultPropertiesByType).not.toHaveBeenCalled();
          });

          it('should copy styles from analysis node', function () {
            expect(this.newLayerDefModel.styleModel.get('type')).toEqual('wadus');
            expect(this.newLayerDefModel.get('cartocss')).toEqual('wadus');
          });
        });
      });
    });
    ...
    describe('when given node is head of a layer', function () {
      describe('without saved styles', function () {
        ...
        it('should reset styles from the previous layer', function () {
          ...
          var queryGeometryModel = prevLayerDefModel.getAnalysisDefinitionNodeModel().queryGeometryModel;
          queryGeometryModel.set({ status: 'fetched', ready: true });
          expect(StyleModel.prototype.setDefaultPropertiesByType).toHaveBeenCalled();
        });
      ...
      describe('with saved styles', function () {
        ...
        describe('when created a layer successfully', function () {
          ...
          it('should not reset the styles', function () {
            ...
            var queryGeometryModel = newLayerDefModel.getAnalysisDefinitionNodeModel().queryGeometryModel;
            StyleModel.prototype.setDefaultPropertiesByType.calls.reset();
            queryGeometryModel.set({ status: 'fetched', ready: true, simple_geom: 'polygon' });
            expect(StyleModel.prototype.setDefaultPropertiesByType).not.toHaveBeenCalled();
          });
