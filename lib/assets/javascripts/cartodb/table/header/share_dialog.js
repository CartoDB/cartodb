  ...
  cdb.admin.ShareDialog = cdb.admin.BaseDialog.extend({
    ...
    _resize: function(animated) {
      ...
      var documentHeight = $(document).height() - verticalPadding;
      var minHeight = 635;
    ...
    _setupPrivacySettings: function() {
      ...
      if (privacy == 'PUBLIC') {
        this.$el.find(".privacy .public .radiobutton").addClass("selected");
      } else if (privacy == 'PRIVATE') {
        this.$el.find(".privacy .private .radiobutton").addClass("selected");
      } else if (privacy == 'PASSWORD') {
        this.$el.find(".privacy .password_protected .radiobutton").addClass("selected");
        this.$el.find(".privacy .password_protected").addClass("open");
        this.$el.find(".privacy .password_protected .input_field").show();
      }

      this.$el.find(".radiobutton").on("click", function(e) {

        e.preventDefault();
        e.stopPropagation();

        self.$el.find(".radiobutton").removeClass("selected");

        var $radio = $(e.target).closest(".radiobutton");

        var share_mode = $radio.attr("data-mode");

        self.$el.find(".privacy .password_protected").removeClass("loading");

        if (share_mode != 'password_protected') {
          self.$el.find(".password_protected .input_field").fadeOut(250, function() {
            self.$el.find(".password_protected").removeClass("open");
            self._resize(true);
          });
        }

        if (share_mode == "private") opts.privacy = "PRIVATE";
        else if (share_mode == 'password_protected') {
          self.$el.find(".password_protected").addClass("open");
          self.$el.find(".password_protected .input_field").fadeIn(250);
          opts.privacy = "PASSWORD";
        }
        else if (share_mode == "public") opts.privacy = "PUBLIC";

        self.options.vis.save(opts);
        self._resize(true);
    ...
    _setPassword: function(e) {
      ...
      if (!cdb.Utils.isBlank(password)) {
        this.$el.find(".password_protected").addClass("loading");
        this.options.vis.save({ password: password });
      }
