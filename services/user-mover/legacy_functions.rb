module CartoDB
  module DataMover
    module LegacyFunctions
      ...
      SIGNATURE_RE = /[\d\s;]*(?<type>(?:\S+\s?['class'|'family'|'aggregate'|'domain'|'function'|'cast'|'type']*))\s+(?:[^\s]\s+)?(?<name>[^\(]+)\s*(?:\((?<arguments>.*)\))?/i.freeze
      LEGACY_FUNCTIONS = [
      ...
      def legacy_functions
        @legacy_functions ||= LEGACY_FUNCTIONS.each_with_object({}) do |line, res|
          arguments, name, type = matches(line)
