...
describe("admin table", function() {
    ...
    it("url should include sql query", function() {
      ...
      var the_geom = ["CASE",
        ...
        "ELSE",
          "'GeoJSON'",
        "END the_geom"].join(' ')
      var sql = "select \"test\",\"test2\","+ the_geom +" from (select * from testTable) __wrapped order by cartodb_id asc";
      tableData.sync();
  ...
  describe("cbd.admin.SQLViewData", function() {
    ...
    it("should set type for special geometry types", function() {
      expect(sqlView._schemaFromQueryFields({ the_geom: { type: 'string'}, the_geom_webmercator: { type: 'string' }, test: {type: 'string'} })).toEqual(
        { the_geom:'geometry', the_geom_webmercator: 'geometry' , test: 'string' }
      );
    })
  ...
  describe("cbd.admin.SQLViewDataAPI", function() {
    ...
    it("should use get schema before executing sql query", function() {
      var opt, m;
      var s = Backbone.sync;
      spyOn(sqlView, '_sqlQuery')
      sqlView.fetch();
      expect(sqlView._sqlQuery.calls.mostRecent().args[0]).toEqual('select * from (select * from a) __wrapped limit 0');
    });
  ...
  describe("Tables", function() {
    ...
    var fakeModel = function(id) {
      return {id: 'test' + id, name: 'test'+id, privacy: 'PRIVATE', rows_counted: 1, updated_at: new Date(), tags: 'a', table_size: 100, description: 'test'};
    }
    beforeEach(function() {
      tables = new cdb.admin.Tables();
