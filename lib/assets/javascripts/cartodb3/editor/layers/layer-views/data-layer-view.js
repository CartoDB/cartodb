...
module.exports = CoreView.extend({
  ...
  _confirmDeleteLayer: function () {
    var self = this;
    this._modals.create(function (modalModel) {
      var affectedItems = self._getAffectedItemsByLayer(this, self.model);
      ...
      var confirmationView = new ConfirmationView({
        ...
        renderOpts: {
          ...
          affectedItemsMessages: self._getAffectedItemsMessages(affectedItems)
        },
        runAction: function () {
          removeLayer({
  ...
  _getAffectedItemsByLayer: function (modalModel, layerModel) {
    return [
      { widgets: modalModel.widgetDefinitionsCollection.widgetsOwnedByLayer(layerModel.get('id')) },
      { analyses: layerModel.getNumberOfAnalyses() - 1 },
      { layers: layerModel.getAllDependentLayers() }
    ];
  },

  _getAffectedItemsMessages: function (affectedItems) {
