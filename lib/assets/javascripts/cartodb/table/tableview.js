...
(function() {
    ...
    cdb.admin.TableView = cdb.ui.common.Table.extend({
      ...
      addColumn: function(column){
        this.columnName = "_" + new Date().getTime();

        if (column) {
          this.columnName = column.column + this.columnName;
        }

        this.model.addColumn(this.columnName, 'string');
      ...
      _highlightColumn: function() {
        ...
        if (this.columnName) {
          var $th = $("a[href='#" + this.columnName + "']").parents("th");
          var position = $th.index();

          if (position) {
            setTimeout(function() {
              $(window).scrollLeft($th.position().left - 40);
              $("[data-x=" + position + "]").addClass("is-highlighted");
            }, 500);
