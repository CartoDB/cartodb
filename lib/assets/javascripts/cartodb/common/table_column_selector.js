...
cdb.admin.ColumnSelector = cdb.core.View.extend({
  ...
  _setKeyColumnEnabled: function() {
    ...
    if (mergeFlavor == "spatial" && name == "the_geom")  visible = false;
    if (mergeFlavor == "spatial" && source == 'origin')  showRadio = false;
    if (mergeFlavor == "spatial" && source == 'destiny') showSwitch = false;

    this.model.set({ enabled: enabled, visible: visible, showSwitch: showSwitch, showRadio: showRadio });
...
cdb.admin.TableColumnSelector = cdb.core.View.extend({
  ...
  initialize: function() {
    // Bind all
    _.bindAll(this, '_setTables', '_selectTable', '_setColumns', '_setKeyColumn', '_setMergeColumns', '_toggleMergeAllColumns', 'onClickMergeMethod', "_setupColumns");
  ...
  _addColumn: function(name, columnType) {
    ...
    if (this.options.joinType == 'spatial' && columnType == 'geometry') visible = false;
  ...
  _selectTable: function(e) {
    this._selectOption($(e.target).select2('val'));
    this.trigger("tableSelected", this);
    this._resetMergeMethods();
  },
...
_setupColumns: function() {
  ...
  if (this.mergeMethod == 'count') {
    ...
    _.each(this.subColumns, function(column) {
      column.model.set({ enabled: true, showRadio: false });
    });
...
onClickMergeMethod: function(e) {
  ...
  this.trigger("mergeMethodSelected", this);
...
_hideMergeMethods: function() {
  this.$el.find('.merge_methods').hide();
},
