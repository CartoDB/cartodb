...
cdb.admin.CartoDBTableData = cdb.ui.common.TableData.extend({
  ...
  linkToSchema: function() {
    ...
    this.table.bind('change', function() {
      if (needsFetch(self.table) && self.table.get('name') && self.table.has('schema')) {
        self.fetch();
      }
    }, this);
  },
  ...
  parse: function(d) {
    ...
    this.lastPage = false;
    this.query_schema = this._schemaFromQueryFields(d.fields);
  ...
  _schemaFromQueryFields: function(f) {
    var sc = {};
    for(k in f) {
      sc[k] = f[k].type;
    }
  ...
  sync: function(method, model, options) {
    if(!options) { options = {}; };
    var query = this._createUrlOptions();
    query += "&q=" + encodeURIComponent(this.wrappedSQL(this.table.columnTypes()));
  ...
  sqlApiUrl: function() {
    return cdb.config.get("sql_api_template").replace('{user}', cdb.config.get('user_name'));
  },
