...
describe("cbd.admin.CartoStyles", function() {
  ...
  describe("bubble generator", function() {
    it("should return valid css", function() {
      ...
      sinon.stub(table.data(), '_sqlQuery').yields({
  ...
  describe("cdb.admin.CartoStyles", function() {
    ...
    it("shoudl load params form css", function() {
      model.set({ properties: { 'polygon-fill': '#FFF' }});
      var c = new cdb.admin.CartoStyles({ table: table });
      c.loadFromCartoCSS(model.get('style'));
      expect(c.get('type')).toEqual(model.get('type'));
      expect(c.get('properties')).toEqual(model.get('properties'));
    });
