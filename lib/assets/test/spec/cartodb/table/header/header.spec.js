describe("Table/Visualization Header tests", function() {
  ...
  it("shouldn't open 'privacy dialog' when user is not owner of the table/vis", function() {
    this.user.organization = new cdb.admin.Organization({ id:1, users:[1,2,3] });
    spyOn(this.header, '_showPrivacyDialog');
    this.header.render();
    this.header.$('.privacy.share.i').click();
    expect(this.header._showPrivacyDialog).not.toHaveBeenCalled();
  })

  it("should open 'privacy dialog' when user is the owner of the table/vis", function() {
    this.user.organization = new cdb.admin.Organization({ id:1, users:[1,2,3] });
    this.vis.set('type', 'derived');
    this.vis.permission.owner = new cdb.core.Model({ username: 'staging20', avatar_url: 'http://test.com', id: 2 });

    spyOn(this.header, '_showPrivacyDialog');
    this.header.render();
    this.header.$('.privacy.share i').click();
    expect(this.header._showPrivacyDialog).toHaveBeenCalled();
  })
