...
describe('editor/layers/data-layer-view', function () {
  ...
  describe('when layer doesn\'t have a geom', function () {
    ...
    describe('.render', function () {
      it('should render properly', function () {
        expect(_.size(this.view._subviews)).toBe(4); // [inlineEditor, georeferenceTooltip, toggleTooltip, noGeometryTooltip]
      });

      it('should render warning icon', function () {
        var noGeometryIconWarning = this.view.$el.find('.js-nogeometrylayer');
        expect(noGeometryIconWarning.length).toBe(1);
      });
    });
  });
