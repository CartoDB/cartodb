...
// would an onprem break with this?
var rootLocation = (/(\/builder\/[0-z\.\-]+\/?)/).exec(location.pathname)[1];
Backbone.history.start({ pushState: true, hashChange: false, root: rootLocation });

var editorTabPaneView = createEditorMenuTabPane([
  {
    icon: 'pencilMenu',
    name: 'editor',
    onClick: () => Router.getRouter().navigate('layers', { trigger: true }),
    selected: true,
    ...
    icon: 'settings',
    name: 'settings',
    onClick: () => Router.getRouter().navigate('settings', { trigger: true }),
    createContentView: function () {
  ...
  tabPaneOptions: {
    ...
    onRouteChange: function (route) {
      var routeName = route.get('currentRoute')[0];
      ...
      } else if (routeName === 'layers' || routeName === 'root' || routeName === 'layer') {
...
window.Router = Router.getRouter();
