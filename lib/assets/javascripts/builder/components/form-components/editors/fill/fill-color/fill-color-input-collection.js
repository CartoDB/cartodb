...
module.exports = Backbone.Collection.extend({
  ...
  getValues: function () {
    ...
    if (colorModel.attributes.range && !colorModel.changed.fixed) colorOmmitAttributes.push('fixed');
    if (colorModel.attributes.range && colorModel.changed.fixed) colorOmmitAttributes.push('range');

    var colorAttributes = _.omit(colorModel.attributes, colorOmmitAttributes);

    var imageAttributes = imageModel
      ? _.omit(imageModel.attributes, imageOmmitAttributes)
      : {};

    var values = _.extend({}, imageAttributes, colorAttributes);

    colorModel.set(values);
    if (imageModel) imageModel.set(values);
