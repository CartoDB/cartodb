  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    _switchTo: function(e) {
      ...
      if (isActive && $(e.target).prop("tagName").toLowerCase() == "span"
        ...
        var self = this;
        setTimeout(function() {
          if (!self._dblClick) self.$('.layer-info a.info .name').tipsy("show");
    ...
    setLayerName: function(layer) {
      if (this.vis.isVisualization()) {
        ...
        this.$('.layer-info a.info .name').text(layer.get('table_name'));
      }
    },

    _setLayerTooltip: function() {
      var self = this;
      this.$('.layer-info a.info .name').tipsy({
        trigger:  'manual',
        fade:     true,
        gravity:  's',
        title:  function() {
    ...
    _unsetLayerTooltip: function() {
      var $name = this.$('.layer-info a.info .name');
      // Remove tipsy
      if ($name.data('tipsy')) {
    ...
    setVisibility: function(layer) {
      ...
      if(this.vis.map.layers.length <= 2) {
        sw.hide();
        close.hide();
      } else {
    ...
    setLayerOrder: function(layer) {
      ...
      this.$('.layer-info a.info .order').text(order);
    },
    ...
    setDataLayer: function(dataLayer) {
      ...
      var wizards = new cdb.admin.mod.CartoCSSWizard({
        user: this.user,
        model: this.dataLayer, // datalayer
        table: this.table,
        map: this.map,
    ...
    _initDataLayer: function(layer) {
      ...
      layer.collection.bind('add remove', this.setVisibility, this);
    ...
    _addFeature: function(mod) {
      var currentCenter = this.map.get('center');
      if (currentCenter[1] > 180 || currentCenter[1] < -180){
        this.map.setView([currentCenter[0], currentCenter[1] % 180], this.map.get("zoom"));
      }
