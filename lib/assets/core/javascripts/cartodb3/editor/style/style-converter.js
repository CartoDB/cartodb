...
function makeCartoCSS (obj, prefix) {
  ...
  prefix = prefix || '';
  for (var k in obj) {
    if (_.isObject(obj[k]) && !_.isUndefined(obj[k].key) && !_.isUndefined(obj[k].value)) {
      blocks.push(obj[k]);
    } else {
      css += prefix + k + ': ' + obj[k] + ';\n';
    }
  ...
  _.each(blocks, function (block) {
    css += '[' + block.key + '=' + block.value + '] {\n';
    css += makeCartoCSS(_.omit(block, ['key', 'value']), prefix + '  ');
    css += '}\n';
  });
...
function pointFill (props, animationType) {
  ...
  if (color) {
    ...
    if ((props.image && _.isArray(props.image)) || (color.image && _.isArray(color.image))) {
      categoryIcons(props.image || color.image, props, css);
...
function categoryIcons (images, props, css) {
  if (!_.isArray(images) || !props.color || !props.color.attribute) {
    return;
  }

  _.each(images, function (item, index) {
    var key = props.color.attribute;
    var value = props.color.domain && props.color.domain[index];
    if (value) {
      var property = key.toString() + '-' + value.toString();
      var subCss = {
        key: key,
        value: value
      };
      subCss['marker-file'] = 'url(\'' + images[index] + '\')';
      if (css[property]) {
        _.extend(css[property], subCss);
      } else {
        css[property] = subCss;
      }
    }
