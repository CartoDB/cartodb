...
var Field = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.model.bind('change:fields changeFields', this.fieldChange, this);
  },
...
cdb.admin.mod.InfoWindow = cdb.core.View.extend({
    ...
    _manageAll: function(e) {
      ...
      if (!self.selectedAll) {
        ...
        // Two ways here
        // If there is less than 30 fields to render (addField), loop the each and that's it
        // But for bigger tables let's use interval to render all the fields
        var names = this.getColumnNames()
        ...
        if (count < 30) {
          ...
          var interval = setInterval(function() {
            ...
            if (name) {
              self.model.addField(name);
              count--;
            } else {
              // Remove flag of animating-adding new fields
              self.working = false;
              // Bind again to field changes
              self.model.bind('change:fields', self.renderSelectAll, self);
              // Clear interval
              clearInterval(interval);
              // Change select switch
              self._changeSelectAll();
            }
          }, 0);
        }

        // Set a flag when we are changing all the fields to true
        self.working = true;
      }
