...
module.exports = CoreView.extend({
  initialize: function (opts) {
    ...
    if (this._imageEnabled && this._userModel.featureEnabled('icon-styling')) {
      this._setupTabPanes();
    }
  },
  ...
  _setupTabPanes: function () {
    ...
    var filePane = {
      ...
      label: this.model.get('image') ? this.model.get('image') : 'Img',
      extra: {
    ...
    this.model.bind('change:fixed', function () {
      this._tabPaneView.collection.at(0).set('label', this.model.get('fixed'));
    }, this);

    this.model.bind('change:image', function () {
      this._tabPaneView.collection.at(1).set('label', this.model.get('image'));
    }, this);
