...
module.exports = Backbone.Collection.extend({
  ...
  isEmptyAsync: function () {
    ...
    if (this.isDone()) {
      ...
      return new Promise(function (resolve, reject) {
        self.listenTo(self.statusModel, 'change:status', function (model) {
          if (self.isDone()) {
            resolve(self.size() === 0);
          }
        });
  ...
  pingMeWhenIsDone: function (callback) {
    function onStatusChanged () {
      if (this.isDone()) {
        this.statusModel.off('change:status', onStatusChanged);
        callback && callback();
      }
    }
    ...
    if (this.isDone()) {
      ...
      this.statusModel.on('change:status', onStatusChanged, this);
    }
  },
