...
var Overlay = Backbone.View.extend({
  ...
  initialize: function (options) {
    ...
    // TODO: Make this customizable hideOnPan, hideOnZoom
...
F.prototype._onFeatureClicked = function (event) {
  ...
  if (layerDefinitionModel && layerDefinitionModel.getAnalysisDefinitionNodeModel().get('type') === 'source') {
    this._editFeatureOverlay.setPosition(position);
    this._editFeatureOverlay.show();

    this._editFeatureOverlay.on('editFeature', function () {
      this._editFeatureOverlay.hide();
      ...
      featureDefinition.fetch({
        success: function () {
          if (this._mapModeModel.isEditingFeatureMode()) {
            var xfeatureDefinition = this._mapModeModel.getFeatureDefinition();
            xfeatureDefinition.set(featureDefinition.toJSON());
            this._onMapModeChanged(this._mapModeModel);
          } else {
...
F.prototype._onMapModeChanged = function (mapModeModel) {
  ...
  if (mapModeModel.isEditingFeatureMode()) {
    ...
    geometry = map.editGeoJSONGeometry(geojson);
    this._bindGeometryToFeatureDefinition(geometry, featureDefinition);
  }
};
