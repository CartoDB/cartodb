...
module.exports = CoreView.extend({
  ...
  _onContinue: function () {
    ...
    if (selectedOptionModels.length > 0) {
      this._saveSelectedWidgetsAsync(selectedOptionModels)
        .then(function () {
          self._userActions.updateWidgetsOrderAsync();
        });
  ...
  _saveSelectedWidgetsAsync: function (selectedOptionModels) {
    var saveWidgetOptionsPromises = [];

    _.map(selectedOptionModels, function (selectedOptionModel) {
      saveWidgetOptionsPromises.push(this._userActions.saveWidgetOptionAsync(selectedOptionModel));
    }, this);
