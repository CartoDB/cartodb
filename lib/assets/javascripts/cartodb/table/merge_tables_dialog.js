...
cdb.admin.MergeTablesDialog = cdb.admin.BaseDialog.extend({
  ...
  initialize: function() {
    ...
    _.extend(this.options, {
      ...
      description:           _t("Merges are useful when you want to create a new table derived from the mix of another two, for example by joining them by an attribute or as a geospatial intersection."),
      width:                 633,
      clean_on_hide:         true,
    ...
    */
    ...
    if (stepName == "merge_tables_regular_1") {
      ...
      this.model.set({
        ...
        description: "Create a new table by selecting two tables and a common column. First select the column you want to use as the matching between tables and the select the columns you want to be moved to the new resulting table. You can only merge tables with joining columns of similar type, like a string with a string."
      });
    ...
    if (stepName == "merge_tables_spatial_1") {
      ...
      this.model.set({
        ...
        nextButtonText:         "Merge tables",
        description: "Calculate the intersecting geospatial records between two tables (ex. points in polygons). You'll need to decide the operation to perform: COUNT calculates the number of intersecting records, SUM sums a value of all intersecting records, AVG provides the average value for a column of all intersecting records."
      });
