...
var AnalysisOnboarding = require('./components/onboardings/analysis/analysis-view');
...
F.prototype._recordChange = function () {
...
F.prototype._tryToSetupDefinitionNodeSync = function (m) {
  ...
  if (node.get('type') !== 'source') {
    ...
    m.querySchemaModel.bind('change:status', function (model, status) {
      console.log(status);
      if (status === 'fetched') {
        console.log(node.get('status'));
      }
    });

    node.bind('change:status', function (model, status) {
      console.log(status);
    });

    m.listenTo(node, 'change:status', function (model, status) {
      if (status === 'ready' && !LocalStorage.get(node.get('type'))) {
        self._onboardings.create(function (modalModel) {
          return new AnalysisOnboarding({
