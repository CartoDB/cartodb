...
var StackLayoutModel = require('cartodb3/components/stack-layout/stack-layout-model');
var LayerDefinitionModel = require('cartodb3/data/layer-definition-model');
var DataLayerView = require('cartodb3/editor/layers/layer-views/data-layer-view');
var AnalysisDefinitionNodeModel = require('cartodb3/data/analysis-definition-node-model');
var AnalysisDefinitionNodesCollection = require('cartodb3/data/analysis-definition-nodes-collection');
var StyleDefinitionModel = require('cartodb3/editor/style/style-definition-model');
...
describe('editor/layers/data-layer-view', function () {
  beforeEach(function () {
    ...
    spyOn(this.view, '_isLayerEmpty').and.returnValue(false);
  ...
  describe('when layer is empty', function () {
    beforeEach(function () {
      this.view._isLayerEmpty.and.returnValue(true);
    });

    afterEach(function () {
      this.view._isLayerEmpty.and.returnValue(false);
    });

    describe('.render', function () {
      it('should render warning icon', function () {
        this.view.render();
        var emptyLayerIconWarning = this.view.$el.find('.js-emptylayer');

        expect(emptyLayerIconWarning.length).toBe(1);
      });
    });
  });
  ...
  describe('when title is clicked', function () {
    ...
    it('should edit layer', function () {
      expect(Router.goToStyleTab).toHaveBeenCalledWith(this.model.get('id'));
    });
