...
describe('data/widget-definition-model', function () {
  beforeEach(function () {
    ...
    this.layerDefinitionsCollection = new LayerDefinitionsCollection([this.layerDefModel], {
      configModel: configModel,
      mapId: 'm-123',
      layersCollection: {}
    });
  ...
  it('should remove widgetModel when model is destroyed', function () {
    ...
    expect(this.widgetDefModel.widgetModel).toBeDefined();
    this.widgetDefModel.trigger('destroy');
    expect(this.widgetDefModel.widgetModel).not.toBeDefined();
  });
  ...
  describe('when it defines a category widget', function () {
    ...
    describe('when is saved', function () {
      ...
      it('should create the widget model', function () {
        ...
        expect(args[0]).toEqual(jasmine.objectContaining({ aggregation_column: 'other_col' }));
        expect(args[1]).toBe(this.layerModel);
        expect(this.widgetDefModel.widgetModel).toBeDefined();
      });
    });
