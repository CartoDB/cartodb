  ...
  cdb.admin.CreateTable = cdb.core.View.extend({
    ...
    _showDialog: function(ev,files,url) {
      ...
      var dialog = new cdb.common.CreateDialog({
        user:   this.model,
        data:   d,
        tabs:   ['file', 'gdrive', 'dropbox', 'twitter', 'scratch', 'arcgis', /*'instagram',*/ 'success', 'error'],
        option: 'file',
        where:  'table'
      })
    ...
    _refreshTables: function() {
      ...
      if (this.options.tables) {
        var order = this.options.tables.options.get("order");
        var order_obj = {};
        order_obj[order] = 'desc';
        this.options.tables.fetch({ data: { o: order_obj } });
      }
    }
