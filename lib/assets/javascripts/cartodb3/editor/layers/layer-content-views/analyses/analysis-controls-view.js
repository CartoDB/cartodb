...
module.exports = CoreView.extend({
  ...
  _html: function () {
    return template({
      ...
      isDisabled: !this._onSaveEnabled()
    });
  },
      ...
      },
      {
        ...
        createContentView: function () {
          ...
          return InfoboxFactory.createQuota({
            ...
            confirmType: 'button',
            confirmDisabled: !this._formModel.isValid() || !canRunAnalysis,
        ...
        secondAction: function () {
          if (this._formModel.isValid() && canRunAnalysis) {
  ...
  _onSaveEnabled: function () {
    var analysisNode = this._analysis.findNodeById(this._formModel.id);
    var isNew = !analysisNode;
    var status;
    if (analysisNode) {
      status = analysisNode.get('status');
      if (status === undefined) {
        analysisNode.once('change:status', this.render, this);
      }
    }
    var isDone = isNew || status === 'ready' || status === 'failed';
    return this._formModel.isValid() && isDone;
  },

  _onSaveClicked: function () {
    if (this._onSaveEnabled()) {
      this._userActions.saveAnalysis(this._formModel);
    }
  }
