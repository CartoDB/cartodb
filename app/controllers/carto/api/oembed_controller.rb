  ...
  module Api
    ...
    class OembedController < ::Api::ApplicationController
      ...
      def show
        ...
        url = if fields[:organization_name].nil?
                CartoDB.base_url(fields[:username])
              else
                CartoDB.base_url(fields[:organization_name], fields[:username])
              end
        url += CartoDB.path(self, 'public_visualizations_embed_map', id: uuid, user_domain: nil)

        # force the schema
        url = url.sub('http', 'https') if fields[:protocol] == 'https' && !url.include?('https')
      ...
      def url_fields_from_fragments(url, force_https)
        ...
        data = from_domainless_url(url_fragments, protocol) if data.nil?
      ...
      def username_from_url_fragments(url_fragments)
        ...
        if path_fragments.length < 3 || path_fragments[2].length == 0
          raise UrlFRagmentsError.new('Username not found at url')
        end
