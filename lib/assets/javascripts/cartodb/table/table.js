    ...
    _initViews: function() {
      ...
      if (this.vis.get('locked')) {
        if (this.user.featureEnabled('new_modals')) {
          var viewModel = new cdb.editor.ChangeLockViewModel([this.vis]);
          ...
          view.cancel = function() {
            var url = self.user.viewUrl().dashboard();
            window.location = self.vis.isVisualization() ? url.maps() : url.datasets();
          };
