...
class AuthenticatedClient extends PublicClient {
  ...
  likeMap (payload, callback) {
    const CONFIG_PATH = 'api/v1/viz';
    var opts = {
      data: JSON.stringify(payload),
      dataType: 'json'
    };
    return this.post([CONFIG_PATH, payload['vis_id'], 'like'], opts, callback);
  }

  deleteLikeMap (payload, callback) {
    const CONFIG_PATH = 'api/v1/viz';
    var opts = {
      data: JSON.stringify(payload),
      dataType: 'json'
    };
    return this.delete([CONFIG_PATH, payload['vis_id'], 'like'], opts, callback);
  }
}
