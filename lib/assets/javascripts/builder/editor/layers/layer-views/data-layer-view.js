...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    this._reRenderIfViewValuesHaveChanged();
  },
  ...
  _reRenderIfViewValuesHaveChanged: function () {
    var self = this;
    ...
    Promise.all([georeferencePromise, isLayerEmptyPromise, hasGeomPromise])
      .then(function (values) {
        var needsGeocoding = values[0];
        var isLayerEmpty = values[1];
        var hasGeom = values[2];
        var needsRender = false;

        if (needsGeocoding !== self._needsGeocoding) {
          self._needsGeocoding = needsGeocoding;
          needsRender = true;
        }

        if (isLayerEmpty !== self._isLayerEmpty) {
          self._isLayerEmpty = isLayerEmpty;
          needsRender = true;
        }

        if (hasGeom !== self._queryGeometryHasGeom) {
