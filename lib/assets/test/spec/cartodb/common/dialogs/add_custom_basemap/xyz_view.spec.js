  ...
  describe('when user written a URL', function() {
    ...
    describe('when finally written/pasted a valid URL', function() {
      ...
      describe('when URL has a valid XYZ format', function() {
        ...
        describe('when URL is an already added basemap', function() {
          beforeEach(function() {
            this.baseLayers.add({ urlTemplate: 'http://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}.png' });
            this.trig();
            jasmine.clock().tick(200);
          });

          it('should show error', function() {
            expect(this.innerHTML()).toContain('already added');
          });
        });

        describe('when URL is a new basemap', function() {
          beforeEach(function() {
            this.trig();
          });
          ...
          it('should create layer with url', function() {
            ...
            expect(cdb.admin.TileLayer.byCustomURL).toHaveBeenCalledWith('http://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}.png');
          });
