...
module.exports = CoreView.extend({
  ...
  _onSelectAggregation: function (mdl) {
    ...
    if (mdl.get('selected')) {
      aggregation = mdl.getValue();
      currentGeometryType = this._getCurrentSimpleGeometryType();

      isTorqueType = (aggregation === 'heatmap' || aggregation === 'animated');

      if (alreadyTorqueLayer && !isTorqueLayer && isTorqueType) {
        this._freezeTorgeAggregation(aggregation, currentGeometryType);
      } else {
        ...
        if (isTorqueType && !this._layerDefinitionsCollection.isDataLayerOnTop(this._layerDefinitionModel)) {
          this._layerDefinitionsCollection.once('layerMoved', setDefaultProperties, this);
