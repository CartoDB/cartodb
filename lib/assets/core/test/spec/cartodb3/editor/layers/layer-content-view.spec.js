...
describe('editor/layers/layer-content-view', function () {
  ...
  describe('._renderContextButtons', function () {
    ...
    beforeEach(function () {
      spyOn(this.view._tableNodeModel, 'isSync').and.returnValue(false);
      spyOn(this.view._tableNodeModel, 'hasWriteAccess').and.returnValue(true);
      analysisDefNodeModel = this.view._getAnalysisDefinitionNodeModel();
      analysisDefNodeModel.attributes.type = 'source';
      analysisDefNodeModel.querySchemaModel.attributes.query = 'SELECT * FROM foo';
    ...
    describe('when is not read-only', function () {
      it('should display edit geometry buttons properly', function () {
        expect($(dashboardCanvasEl).find('.Editor-contextSwitcherItem.js-newGeometryItem').length).toBe(3);
      });
    });

    describe('when is read-only', function () {
      it('should not display geometry buttons if is sync', function () {
        this.view._tableNodeModel.isSync.and.returnValue(true);
        this.view._renderContextButtons();
        expect($(dashboardCanvasEl).find('.Editor-contextSwitcherItem.js-newGeometryItem').length).toBe(0);
      });

      it('should not display geometry buttons if node is not source type', function () {
        analysisDefNodeModel.attributes.type = 'node';
        this.view._renderContextButtons();
        expect($(dashboardCanvasEl).find('.Editor-contextSwitcherItem.js-newGeometryItem').length).toBe(0);
      });

      it('should not display geometry buttons if user doesn\'t have write permission', function () {
        this.view._tableNodeModel.hasWriteAccess.and.returnValue(false);
        this.view._renderContextButtons();
        expect($(dashboardCanvasEl).find('.Editor-contextSwitcherItem.js-newGeometryItem').length).toBe(0);
      });

      it('should not display geometry buttons if there is a custom query applied', function () {
        analysisDefNodeModel.querySchemaModel.attributes.query = 'SELECT whatever FROM foo';
        this.view._renderContextButtons();
        expect($(dashboardCanvasEl).find('.Editor-contextSwitcherItem.js-newGeometryItem').length).toBe(0);
      });
    });
