...
LayerAnalysisViewFactory.prototype.createView = function (layerAnalysisDefinitionNodeModel, layerDefinitionModel) {
  ...
  if (nodeModel.isOwnedBy(layerDefinitionModel)) {
    ...
    var letter = nodeModel.letter();
    var otherDefinitionLayerDefinitionModel = layerDefinitionModel.collection.find(function (m) {
      return m.get('letter') === letter;
    });

    return new RefLayerAnalysisView({
      model: otherDefinitionLayerDefinitionModel
    });
  }
};
