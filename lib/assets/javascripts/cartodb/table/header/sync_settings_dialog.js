  ...
  cdb.admin.SyncSettings = cdb.admin.BaseDialog.extend({
    ...
    _TEXTS: {
      title: _t("Synchronization options"),
      ok:    _t("Save settings")
    },
    ...
    render_content: function() {
      ...
      var url = this.table.synchronization.get('url');
      ...
      if (this.table.synchronization.get('service_name') || this.table.synchronization.get('service_item_id')) {
         url = this.table.synchronization.get('service_item_id');
      }

      var service = this.table.synchronization.get('service_name');
      ...
      if (service && _.isString(service)) service = service.charAt(0).toUpperCase() + service.slice(1);
