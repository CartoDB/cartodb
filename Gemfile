source 'http://rubygems.org'

gem "rails", "3.2.0"

gem "pg", "0.11"
gem "sequel", "3.29.0"
gem "sequel_pg", "1.2.0", :require => "sequel"
#gem "sequel_column_type_array", "0.0.4"
#gem "sequel-rails", :git => "git://github.com/tokumine/sequel-rails.git", :tag => '0.3.6'
#gem "sequel-rails-cartodb", "~> 0.1.7", :require => "sequel-rails"
gem "talentbox-sequel-rails", :git => "git://github.com/Vizzuality/sequel-rails.git"
gem "rails_warden", "0.5.2" # Provides authentication for Rails applications via the Warden Rack authentication framework
gem "oauth", "0.4.5"
gem "oauth-plugin", "0.4.0.pre4"
gem "htmlentities", "4.3.1" # Facilitate encoding and decoding of named or numerical entities in HTML and XHTML documents
#gem "text-hyphen", "1.2.0"
gem "rgeo", "0.3.2" # Geospatial data library
gem "rgeo-geojson", "0.2.1", :require => "rgeo/geo_json"
gem "redis", "2.2.2"
gem "resque", "1.19.0"
gem "resque-result", "1.0.1"
gem "yajl-ruby", "1.1.0", :require => "yajl"
gem "airbrake", "3.0.9"
gem "nokogiri", "1.5.3"
gem "statsd-client", "0.0.7", :require => "statsd"
gem "jammit", "0.6.5"

# assets
group :assets do
  gem "compass-rails", "1.0.3"
end

# importer
gem "ruby-ole", "1.2.11.3"
gem "rchardet19", "1.3.5"
gem "roo", "1.9.7"
gem "spreadsheet", "0.6.5.9"
gem "google-spreadsheet-ruby", "0.1.8"
gem "rubyzip", "0.9.6.1"
gem "builder", "3.0.0"
gem "state_machine", "1.1.2"

gem "bartt-ssl_requirement", "~>1.4.0", :require => "ssl_requirement"

gem "simplecov", "0.6.4", :require => false, :group => :test
gem "spin", "0.5.3", :require => false, :group => :test

group :development, :test do
  gem "poltergeist", "0.6.0"
  gem "minitest", "2.0.2", :require => 'minitest/unit'
  gem "selenium-webdriver", "2.5.0"

  gem "mocha", "0.10.5"
  #gem "debugger", "1.2.0"

  gem "steak", "2.0.0"
  gem "rspec-rails", "2.10.1"
  gem "launchy", "2.1.0"
  gem "capybara", "1.1.2"
  gem "timecop", "0.3.5"
  gem "email_spec", "1.2.1"
  gem "rack-reverse-proxy", "0.4.4", :require => 'rack/reverse_proxy'
  gem "foreman", "0.46.0"
  gem "aws-sdk", "1.5.2"
end



