...
var TIMESPAN_QUERY_WITH_NORMALIZE = 'SELECT * FROM OBS_GetAvailableTimespans((SELECT ST_SetSRID(ST_Extent(the_geom), 4326) FROM (<%- query %>) q), NULL, {{{ measurement }}}, {{{ normalize }}}) denoms WHERE valid_numer IS TRUE AND (valid_denom IS TRUE OR {{{ normalize }}} IS NULL) ORDER BY timespan_name DESC';
