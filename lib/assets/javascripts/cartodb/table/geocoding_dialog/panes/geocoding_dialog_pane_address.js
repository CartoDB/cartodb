  ...
  cdb.admin.GeocodingDialog.Pane.Address = cdb.admin.GeocodingDialog.Pane.extend({
    ...
    _initBinds: function() {
      ...
      this.additional_columns.bind('add remove change', this._onCustomModelChange, this);
    ...
    _initViews: function() {
      ...
      // Scrollpane with gradients?
      this.scrollpane = new cdb.common.ScrollPane({
        el:         this.$('.geocoding-pane-options > ul'),
        timeout:    0,
        gradients:  true
      })
      this.scrollpane.render();
      this.addView(this.scrollpane);
    ...
    _createCustomModels: function() {
      // Estimation model
      this.estimation = new cdb.admin.GeocodingDialog.EstimationModel({ id: this.table.get('id') });
    ...
    _onCustomModelChange: function() {
      ...
      // Address?
      if (this.address_model.get('value')) {
        var address = this.address_model.get('value');
        var isText = this.address_model.get('text');

        // Should we add a comma?
        if (formatter !== "") { formatter += ', '}

        formatter += isText ? address : '{' + address + '}';
      }
      ...
      if (this.additional_columns.size() > 0) {
        this.additional_columns.each(function(m) {
          var val = m.get('value');
          var isText = m.get('text');
          formatter +=  ' ' + ( isText ? val : '{' + val + '}' );
        });
      }

      // Province?
      if (this.state_model.get('value')) {
        var state = this.state_model.get('value');
        var isText = this.state_model.get('text');

        // Should we add a comma?
        if (formatter !== "") { formatter += ', '}

        formatter += isText ? state : '{{' + state + '}}';
      }

      // Country?
      if (this.country_model.get('value')) {
        var country = this.country_model.get('value');
        var isText = this.country_model.get('text');

        // Should we add a comma?
        if (formatter !== "") { formatter += ', '}

        formatter += isText ? country : '{{' + country + '}}';
      }
