  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    initialize: function() {
      ...
      this.table.bind('update_layer_alias', this.addTableAlias, this);
    ...
    _switchTo: function(e) {
      ...
      var titleLayerOpen = this.vis.get('title_layer_open') == 'yes';

      // Preventing problem with double click
      // over span.name
      // This logic is no more valid. Properties panel being active is not taken into account
      var targetEl = isActive ? $(e.target) : null;
      if (isActive && !titleLayerOpen && targetEl.prop("tagName").toLowerCase() == "span"
        && (targetEl.hasClass("info") || targetEl.hasClass("order") || targetEl.hasClass("name"))) {
      ...
      if (!(isActive && titleLayerOpen)) {
        this._requestFiltersTabRefresh();
      }

      return false;
    },

    addTableAlias: function (layer_alias) {
      this.table_alias_name = layer_alias;
    ...
    _onChangeStatus: function() {
      ...
      if (this.view_model.get('state') == "idle") {
        ...
        $el.find('input')
          .val(this.table_alias_name || this.dataLayer.get('table_name').replace(/_/g,' '))
          .show()
          .focus();
      }
    },
    ...
    activated: function() {
      ...
      this.panels.bind('tabEnabled:filters_mod', this._requestFiltersTabRefresh, this);
    },
    ...
    setLayerName: function(layer) {
      if (this.vis.isVisualization()) {
        ...
        // table name
        this.$('.layer-info .info .name')
          .text(this.table_alias_name || layer.get('table_name').replace(/_/g,' '));

        // table name alias
        var layerUrl = layer.table && layer.table.viewUrl();
        this.$('.layer-info .info .name_info')
          .html('view of ')
          .append($('<a>').text(this.table_alias_name || layer.get('table_name')));
      } else {
        // Unset tipsy bind
        this._unsetLayerTooltip();
        this.$('.layer-info .info .name').text(this.table_alias_name || layer.get('table_name'));
      }
    },
    ...
    setDataLayer: function(dataLayer) {
      ...
      var isProUI = this.user.featureEnabled('bbg_pro_ui');
      ...
      if (isProUI || this.user.featureEnabled('bbg_sql_tab')) {
        self.addModule(sql.render(), ['table', 'tableLite', 'map', 'mapLite']);
      }
      if (isProUI || this.user.featureEnabled('bbg_cartocss_tab')) {
    ...
    allowFiltersRefresh: function() {
      var self = this;
      self._filtersRefreshAllowed = true;
      setTimeout(function() { // refreshing is not necessary after initial load of page. otherwise causes scroll to reset
        self._filtersRefreshAllowed = false;
      }, 2000);
    },
    ...
    _bindDataLayer: function() {
      ...
      this.sqlView.bind('reset', function() { // when dataset is loaded, it is necessary to refresh if they have preset filters
        if (self._filtersRefreshAllowed) {
          self._requestFiltersTabRefresh();
        }
      });
    ...
    _requestFiltersTabRefresh: function() {
      var self = this;

      clearTimeout(self._filtersRefreshTimer); // prevent unnecessary refreshes

      self._filtersRefreshTimer = setTimeout(function() {
        self._refreshFiltersTab();
      }, 200);
    },
