      ...
      _setUpPagination: function(d) {
        ...
        if (pages.length > 0 && pages[0] > 0) {
          ...
          if (this.$el.find('tfoot.page_loader.up').length == 0) {
            this.$el.prepend(this.getTemplate('table/views/table_pagination_loaders')({ direction: 'up' }));
          }
        ...
        if (!d.lastPage) {
          ...
          if (this.$el.find('tfoot.page_loader.down').length == 0) {
            this.$el.prepend(this.getTemplate('table/views/table_pagination_loaders')({ direction: 'down' }));
          }
      ...
      _moveQueryInfo: function(type) {
        if (this.model.isInSQLView()) {
          ...
          if (type == "show") {
            ...
            if (this.$('.table_panel').length > 0) {
              var opened = this.$('.table_panel').css("right").replace("px","") == 0 ? true : false;
              if (!opened) {
                $table_query_text.removeClass('displaced');
              }  
            }
