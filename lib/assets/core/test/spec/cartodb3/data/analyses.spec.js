...
describe('cartodb3/data/analyses', function () {
  ...
  describe('getAnalysesByModalCategory', function () {
    beforeEach(function () {
      ...
      this.userModel.featureEnabled = function () {};
    });
    ...
    describe('data observatory', function () {
      ...
      describe('having data-observatory-multiple-measures enabled', function () {
        ...
        describe('dataservices api existing', function () {
          it('should be enabled if user has credits and hard limit', function () {
            ...
            expect(findDataObservatory(analysisModels, 'data-observatory-multiple-measures').length).toBe(1);
          });

          it('should NOT be enabled if user have hard limit', function () {
            var DO = new Backbone.Model({
              service: 'observatory',
              monthly_quota: 100,
              soft_limit: false
          ...
          it('should NOT be enabled if user doesn\'t have credits', function () {
            var DO = new Backbone.Model({
              service: 'observatory',
              monthly_quota: 0,
              soft_limit: true
          ...
          it('should NOT be enabled if not enabled', function () {
            dataservicesApiHealth._ready = 'ready';
            dataservicesApiHealth.getService.and.returnValue(null);
            var analysisModels = analyses.getAnalysesByModalCategory('create_clean', this.configModel, this.userModel);
            expect(analysisModels.length).toBe(3);
            expect(findDataObservatory(analysisModels, 'data-observatory-measure').length).toBe(0);
            expect(findDataObservatory(analysisModels, 'data-observatory-multiple-measures').length).toBe(0);
          });
        });
      ...
      describe('NOT having data-observatory-multiple-measures enabled', function () {
        beforeEach(function () {
          spyOn(this.userModel, 'featureEnabled').and.returnValue(false);
        });
        ...
        it('dataservices api existing', function () {
          ...
          var analysisModels = analyses.getAnalysesByModalCategory('create_clean', this.configModel, this.userModel);
          expect(analysisModels.length).toBe(4);
          expect(findDataObservatory(analysisModels, 'data-observatory-measure').length).toBe(1);
          expect(findDataObservatory(analysisModels, 'data-observatory-multiple-measures').length).toBe(0);
        });
  ...
  describe('checkIfxxx several checks', function () {
    ...
    it('analyses that needs Data Observatory should check dataservices', function () {
      ...
      checkIfxxxTest(externalGeocodingAnalyses, 'observatory');
    });
  });
