...
describe('editor/layers/layers-content-view/data/data-content-view', function () {
  ...
  var renderSpy;
  var isErroredSpy;
  var renderIfHasGeomChangesSpy;
  var hasValueAsyncPromise;
  ...
  beforeEach(function () {
    ...
    renderIfHasGeomChangesSpy = spyOn(DataContentView.prototype, '_renderIfHasGeomChanges').and.callThrough();

    view = createViewFn();

    hasValueAsyncPromise = fakePromise(view._queryGeometryModel, 'hasValueAsync');
    hasValueAsyncPromise.resolve(true);
  });

  describe('initialize', function () {
    it('should initialize properly', function () {
      expect(view._hasGeom).toBeDefined();
      expect(view._renderIfHasGeomChanges).toHaveBeenCalled();
    });
  });
  ...
  describe('when query geometry model has no value', function () {
    describe('.render', function () {
      it('should render properly', function () {
        renderSpy.and.callThrough();
        hasValueAsyncPromise.resolve(false);
  ...
  describe('._renderIfHasGeomChanges', function () {
    it('should render if hasGeom has changed', function (done) {
      var initialHasGeomValue = view._hasGeom;
      hasValueAsyncPromise.resolve(!initialHasGeomValue);

      view._renderIfHasGeomChanges();

      setTimeout(function () {
        expect(view._hasGeom).toEqual(!initialHasGeomValue);
        expect(renderSpy).toHaveBeenCalled();
        done();
      }, 0);
    });

    it('should not render if hasGeom has not changed', function (done) {
      var initialHasGeomValue = view._hasGeom;
      hasValueAsyncPromise.resolve(initialHasGeomValue);
