  ...
  module Importer2
    module Factories
      class PGConnection
        ...
        def load_from_json
          json_config = File.join(File.dirname(__FILE__.to_s), 'database.json')
          ::JSON.parse(File.read(json_config)).each_with_object({}) do |(k, v), h|
            h[k.to_sym] = v
          end
        end

        def load_from_yml
          yml_config = if ENV['RAILS_DATABASE_FILE']
                         "#{File.dirname(__FILE__)}/../../../../config/#{ENV['RAILS_DATABASE_FILE']}"
                       else
                         "#{File.dirname(__FILE__)}/../../../../config/database.yml"
                       end
          yml_config = YAML.load_file(yml_config)['test'].each_with_object({}) do |(k, v), h|
            h[k.to_sym] = v
          end
