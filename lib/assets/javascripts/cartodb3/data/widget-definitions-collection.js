  ...
  addWidget: function (model, attrs) {
    ...
    this.each(function (widgetDefinitionModel, index) {
      widgetDefinitionModelSaved.push(
        widgetDefinitionModel.save({ order: index + 1 }, { wait: true }));
    });

    $.when(widgetDefinitionModelSaved).then(function () {
      return self.create(attrs, {
        wait: true,
        success: function (mdl, attrs) {
          self.trigger('successAdd', model);
        },
        error: function (mdl, e) {
          self.trigger('error', model, e);
        }
      });
    });
  },
