...
var NewSearchInput = Backbone.View.extend({
  query: "",
  _items: {},
  _clickedOnListItem : false,
  _hilitedItemIndex: 0,
  ...
  _isDatasetLocal: function(name, callback) {
    $.ajax({
      url: "../../api/v1/viz/table_type/?table=" + name
    })
    .done(function(response) {
      try {
...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _addSearchBar: function() {
    ...
    if (this.user.featureEnabled('bbg_new_search')) {
      this._searchInput = new NewSearchInput({ el: '#search-bar-input', map: this.map, user: this.user, vis: this.vis });
    }
    else {
