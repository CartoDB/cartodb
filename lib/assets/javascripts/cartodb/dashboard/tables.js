  ...
  cdb.admin.dashboard.Tables = cdb.core.View.extend({
    ...
    */
    ...
    _changeTitle: function() {
      ...
      if (this.tables.options.get("per_page") == 3) {
        title = this.model.get("default_title");
      }  else {

        var tag   = this.tables.options.get("tags");
        var count = _.filter(this.tables.models, function(table) { return table.get("type") == "table" }).length;

        if (tag) {
          title = count + " table" + ( count != 1 ? "s" : "") + " with the tag <a class='remove' href='#'>" + tag + "</a>";
        } else {
