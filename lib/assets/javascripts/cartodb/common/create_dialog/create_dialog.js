  ...
  cdb.common.CreateDialog = cdb.admin.BaseDialog.extend({
    ...
    _onCreationComplete: function() {
      // If table is created from Twitter
      if (this.model.get('option') !== "twitter") {
        this.trigger('importCompleted', this.model.get('upload'), this);  
      } else {
        // Fetch user data when twitter import finished
        // checking pending twitter credits
        this.user.fetch();
      }
    },
    ...
    _ok: function(e) {
      ...
      switch (state) {
        ...
        case 'abort':
        case 'reset':
          if (isValid) this.model.set('state', 'selected')
          break;
        case 'importing':
        case 'uploading':
        case 'unpackaging':
