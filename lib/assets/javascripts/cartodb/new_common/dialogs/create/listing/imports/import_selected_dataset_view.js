...
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.template = cdb.templates.getTemplate('new_common/views/create/listing/import_selected_dataset');
    this._initBinds();
    this._checkVisibility();
  },

  render: function() {
    var title = this.model.get('value')[this.options.fileAttrs.title];
    ...
    var currentUserUrl = urls(window.config.account_host).userUrl(this.user);
    ...
    this.$el.html(
      this.template({
        ...
        userCanSync: this.user.get('actions').sync_tables,
        showUpgrade: !customInstall && upgradeAccountUrl && (this.user.isOrgAdmin() || !this.user.isInsideOrg()),
  ...
  _onIntervalMonth: function() {
    if (this.options.acceptSync && this.user.get('actions').sync_tables) {
      this.model.set('interval', 18144000);
    }
  },
