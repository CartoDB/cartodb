...
describe('components/form-components/editors/number', function () {
  ...
  it('should create an horizontal slider', function () {
    expect(this.view.$('.js-slider').data('ui-slider')).toBeDefined();
  });
  ...
  it('should not create the horizontal slider if not desired', function () {
    ...
    view.render();
    expect(this.view.$('.js-slider').data('ui-slider')).toBeUndefined();
  });
  ...
  describe('change value', function () {
    ...
    describe('when input changes', function () {
      ...
      it('should not trigger change event if float part is not added', function () {
        var $input = this.view.$('.js-input');
        this.onChanged.calls.reset();
        $input.val('1.');
        $input.trigger('keyup');
        expect(this.onChanged.calls.count()).toBe(0);
        $input.val('1.0');
        $input.trigger('keyup');
        expect(this.onChanged.calls.count()).toBe(1);
      });

      it('should not trigger change event if float value was the same', function () {
        var $input = this.view.$('.js-input');
        this.onChanged.calls.reset();
        this.view.value = '1.20';
        $input.val('1.2');
        $input.trigger('keyup');
        expect(this.onChanged.calls.count()).toBe(0);
        $input.val('1.21');
