...
module.exports = Backbone.Collection.extend({
  ...
  addWidget: function (model, attrs) {
    var updatedWidgets = this.sliceWidgetsOrder();

    $.when(updatedWidgets)
      .then(this.createWidget(attrs, model));
  },

  createWidget: function (attrs, widgetModel) {
  ...
  sliceWidgetsOrder: function () {
    var SLICE_VALUE = 1;
    var widgetDefinitionModelSaved = [];

    this.each(function (widgetDefinitionModel, index) {
      widgetDefinitionModelSaved.push(
        widgetDefinitionModel.save({ order: index + SLICE_VALUE }, { wait: true }));
    });

    return widgetDefinitionModelSaved;
  },
