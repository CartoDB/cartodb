...
describe('editor/layers/layer-content-view/infowindows-view', function () {
  ...
  describe('when layer has infowindowModel', function () {
    ...
    it('should render placeholder if style model type is aggregated', function () {
      ...
      this.view._layerDefinitionModel.styleModel.set('type', 'heatmap');
      this.view.render();

      expect(_.size(this.view._subviews)).toBe(1);
      expect(this.view.$('.CDB-Size-huge').length).toBe(0);

      this.view._layerDefinitionModel.styleModel.set('type', 'squares');
      this.view.render();

      expect(_.size(this.view._subviews)).toBe(0);
      expect(this.view.$('.CDB-Size-huge').length).toBe(1);
      expect(this.view.$('.CDB-Size-huge').text()).toBe('editor.layers.infowindow.placeholder-text');
    });
    ...
    it('should render placeholder if style model has animated enabled', function () {
      ...
      this.view._layerDefinitionModel.styleModel.set('animated', animatedAttrs);
      this.view.render();
