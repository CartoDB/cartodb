...
cdb.admin.DropdownBasemap = cdb.ui.common.Dropdown.extend({
  ...
  // returns if google maps is enabled based on:
  // - google maps library loaded
  // - user has feature flag active
  googleMapsEnabled: function() {
    return typeof(google) !== 'undefined' && typeof(google.maps) !== 'undefined' && this.user.featureEnabled('google_maps');
  },
  ...
  add: function(lyr) {
    ...
    if (lyr.get('className') === 'wms' || proxy ) {
      ...
      if (this.googleMapsEnabled()) {
       ...
       // when google maps is enabled other kind of maps are not supported
       if (this.googleMapsEnabled()) {
         return;
       }
