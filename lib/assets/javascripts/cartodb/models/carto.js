...
function color_generator(table, props, callback) {
  ...
  if (props.colors) {
    ...
    // type of the column, number -> no quotes, date ??, string -> quotes, boolean -> no quotes

    for (var prop in props.colors) {
      css += "\n#" + table_name + "[" + props.property + "=\"" + prop + "\"] { polygon-fill: " + props.colors[prop] + "; }";
    }  
  }
...
cdb.admin.CartoParser.prototype = {
  ...
  RESERVED_VARIABLES: ['mapnik-geometry-type'],
