...
describe('data/backbone/network-interceptors/interceptors/forbidden-403', function () {
  ...
  it('should redirect to login if error response contains an unknown error', function () {
    action({
      status: 403,
      responseJSON: {
        error: 'unknown'
    ...
    expect(URLFunctions.redirectTo).toHaveBeenCalledWith(window.location.origin + '/login');
  });

  it('should redirect to login if error response contains no error', function () {
    action({
      status: 403,
      responseJSON: {
