...
module.exports = {
  ...
  _onWidgetDefinitionAdded: function (m) {
  ...
  _createWidgetModel: function (m) {
    ...
    var layerModel = this._diDashboardHelpers.getLayer(m.get('layer_id'));
  ...
  _createTimeseries: function (newLayer, animatedChanged, persist) {
    function getColumnType (source, columnName) {
    ...
    if (attribute) {
      ...
      var columnType = getColumnType.call(this, newLayer.get('source'), attribute);
      if (columnType !== 'date') {
  ...
  refreshWidgets: function () {
    var widgets = this._diDashboardHelpers.getWidgets();
    _.each(widgets, function (widget) {
      // Required to render the widged and update the source layerName
      widget.dataviewModel.trigger('change:data');
    });
  }
};
