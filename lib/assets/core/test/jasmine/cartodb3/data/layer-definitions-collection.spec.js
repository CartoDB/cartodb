...
describe('data/layer-definitions-collection', function () {
  ...
  describe('.setBaseLayer', function () {
    ...
    it('should unset previous base layer attributes that are not present in the new layer (eg: urlTemplate)', function () {
      var changeEventCallback = jasmine.createSpy('changeEventCallback');
      ...
      baseLayer.on('change', changeEventCallback);
      ...
      expect(Backbone.sync).toHaveBeenCalledWith('update', jasmine.objectContaining({
        id: 'LAYER_ID'
      }), jasmine.any(Object));
