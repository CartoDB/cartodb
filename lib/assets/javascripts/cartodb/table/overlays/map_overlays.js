...
cdb.admin.MapOverlays = cdb.core.View.extend({
  ...
  _hideToobarOptions: function(model) {
    if (this.overlayPropertiesBar) {
      if (this.overlayPropertiesBar.compareModel(model)) { // if the model is the same as the current one, hide the options bar
        this.options.mapTab.$el.find(".map_toolbar ul.options").animate({ top: -100 }, { duration: 200, easing: "easeInOutQuad" } );
  ...
  _showToolbarOptions: function() {
    var self = this;
    this.options.mapTab.$el.find(".map_toolbar ul.options").animate({ top: 0 }, { duration: 250, easing: "easeInOutQuad", complete: function() {
  ...
  _addOverlayPropertiesBar: function(model, form_data) {
    ...
    this.options.mapTab.$el.find(".map_toolbar").addClass("animated");
    ...
    if (!this.overlayPropertiesBar) {
      ...
      this.options.mapTab.$el.find(".map_toolbar").append(this.overlayPropertiesBar.render().el);
