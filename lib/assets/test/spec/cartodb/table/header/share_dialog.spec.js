describe("share_dialog", function() {
  ...
  describe("Standard user", function() {
    ...
    xit("should render a map without the CartoDB logo control", function() {
      this.share.render();
      expect(this.share.$('li.cartodb_logo').length > 0).toBeFalsy();
    });

    xit("should render the upgrade message", function() {
      this.share.render().show();
      expect(this.share.$('li.upgrade_message').length > 0).toEqual(true);
    });

    xit("shouldn't show description of the visualization as null", function() {
      this.share.render();
      expect(this.share.$('div.cartodb-header p').text()).toBe('');
    });

    xit("shouldn't show layer-selector if there is only one data layer", function(done) {
      this.share.render();
      var self = this;
      setTimeout(function() {
    ...
    xit("should show layer-selector if there are several data layers", function(done) {
      this.vis.map.layers.reset([
    ...
    xit("shouldn't show legends if there is only one data layer", function(done) {
      this.share.render();
      var self = this;
      setTimeout(function() {
    ...
    xit("should show legends if there are several data layers", function(done) {
      this.vis.map.layers.reset([
    ...
    xit("should generate correctly the vizjson url", function() {
      this.share.render();
  ...
  describe("Coronelli or greater user", function() {
    ...
    it("should render a map with controls", function() {
      ...
      expect(this.share.$('div.cartodb-header p').html()).toEqual('');
