...
describe('data/query-geometry-model', function () {
  beforeEach(function () {
    data = {
      rows: [
        { cartodb_id: 1, title: '1st'},
        { cartodb_id: 2, title: '2nd', the_geom: 'line' },
        { cartodb_id: 3, title: '3rd', the_geom: 'point' }
      ]
    };
  ...
  describe('._findValidSimpleGeom', function () {
    it('should be able to return the first row with valid geom ', function () {
      expect(this.model._findValidSimpleGeom(data)).toEqual({ cartodb_id: 2, title: '2nd', the_geom: 'line' });
    });
  });

  describe('.parse', function () {
    it('should call _findValidSimpleGeom', function () {
      data = {
        rows: []
      };

      var findValidSimpleGeomSpy = spyOn(this.model, '_findValidSimpleGeom').and.returnValue({ cartodb_id: 2, title: '2nd', the_geom: 'line' });

      this.model.parse(data);
      expect(findValidSimpleGeomSpy).toHaveBeenCalled();
    });

    it('should return a valid state if it finds a valid row', function () {
      var parseResult = this.model.parse(data);
    ...
    it('should return valid state with an empty string if it does not find a valid row', function () {
      data = {
        rows: []
      };
