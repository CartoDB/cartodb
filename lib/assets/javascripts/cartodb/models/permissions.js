...
cdb.admin.Permission = cdb.core.Model.extend({
  ...
  access: function(model) {
    ...
    var checkList = ['_ownerAccess', '_entityAccess', '_userOrganizationAccess', '_userGroupsAccess'];
  ...
  isOwner: function(model) {
    return _.result(this.owner, 'id') ? this.owner.id === _.result(model, 'id') : false;
  },
  ...
  _userOrganizationAccess: function(m) {
    if (m.organization) {
      return this._entityAccess(m.organization);
    }
  },

  _userGroupsAccess: function(m) {
    if (m.groups) {
