...
/**
 *  Custom picker for a dates range.
 *
 */
...
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    var obj = {};
    obj[this.options.property] = d;
    this.model.set(obj);

    // Generate internal model
    this._datesModel = new cdb.core.Model(d);
  ...
  render: function() {
    ...
    setTimeout(function() {
      ...
      self.addView(new cdb.common.TipsyTooltip({
        el: self.$('.js-dates'),
        title: function(e) {
  ...
  _initBinds: function() {
    ...
    this._datesModel.bind('change', function(mdl, changes){
      this.model.set(this.options.property, mdl.toJSON());
      this._setValues(mdl, changes);
    }, this);
    this.add_related_model(this._datesModel);
  ...
  _setValues: function(m, c) {
    ...
    var data = this._datesModel.attributes;

    if (data.fromDate && data.toDate) {
      text =
        'From ' +
        '<strong>' +
        this._datesModel.get('fromDate') + ' ' +
        (cdb.Utils.pad(this._datesModel.get('fromHour'),2) + ':' + cdb.Utils.pad(this._datesModel.get('fromMin'),2)) +
        '</strong>' +
        ' to ' +
        '<strong>' +
        this._datesModel.get('toDate') + ' ' +
        (cdb.Utils.pad(this._datesModel.get('toHour'),2) + ':' + cdb.Utils.pad(this._datesModel.get('toMin'),2)) +
        '</strong>' +
  ...
  _onModelValidChange: function() {
    var errorMsg = this.model.getError && this.model.getError();
    this.$('.js-dates').toggleClass('has-errors', errorMsg !== '');
  },
