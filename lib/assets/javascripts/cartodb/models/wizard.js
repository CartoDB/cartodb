...
cdb.admin.WizardProperties = cdb.core.Model.extend({
  ...
  active: function(type, props, options) {
    ...
    if (options.restore) {
      var previousWizardProperties = this.getSaved(type, currentGeom);
    } else {
      var previousWizardProperties = {};
    }

    if (options.persist && type === this.get('type')) {
      var currentWizardProperties = this.attributes;
    } else {
      var currentWizardProperties = {};
    }

    var newWizardProperties = this.defaultStyleForType(type);
    _.extend(
      newWizardProperties,
    ...
    // if current wizard, geometry_type and form is valid -> save the state
    var t = this.get('type');
