  ...
  describe('when an dataset is selected', function() {
    describe('when dataset is a remote one (from library)', function() {
      ...
      it('should pass a metadata object with necessary data to import dataset', function() {
        expect(this.remoteSelected).toEqual(jasmine.objectContaining({ shouldRedirectAfterComplete: false }));
        expect(this.remoteSelected.upload).toEqual(jasmine.objectContaining({ type: 'remote' }));
        expect(this.remoteSelected.upload).toEqual(jasmine.objectContaining({ value: 'foobar' }));
        expect(this.remoteSelected.upload).toEqual(jasmine.objectContaining({ remote_visualization_id: 'abc-123' }));
        expect(this.remoteSelected.upload).toEqual(jasmine.objectContaining({ size: 1024 }));
        expect(this.remoteSelected.upload).toEqual(jasmine.objectContaining({ create_vis: false }));
      });
    });
