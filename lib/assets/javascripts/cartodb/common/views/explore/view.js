...
module.exports = cdb.core.View.extend({
  ...
  _renderVisualizations: function() {
    ...
    if (visualizations) {
      ...
      this.model.set({ show_loader: false, show_more: true, show_mast: true });
  ...
  _fetchItems: function(params, reset) {
    ...
    if (reset) {
      this.model.set({ show_more: false, show_loader: true, show_mast: false });
      this.$('.js-items').empty();
      this._PAGE = 0;
    }
  ...
  _onClickMore: function(e) {
    this.killEvent(e);
    this.model.set({ show_loader: true, show_more: false });
    ++this._PAGE;
