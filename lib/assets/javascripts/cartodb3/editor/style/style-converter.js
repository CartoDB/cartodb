...
function generateCartoCSS (style, geometryType) {
  ...
  if (geometryType === 'point' && styleDef.animated && styleDef.animated.trails && styleDef.animated.trails > 0) {
    css += '\n' +
    _.range(1, parseInt(styleDef.animated.trails, 10) + 1)
      .map(function (t) {
        return '#layer[frame-offset=' + t + '] {\n  marker-width: ' +
          (styleDef.fill.size.fixed + 2 * t) +
          ';\n  marker-fill-opacity: ' +
          styleDef.fill.color.opacity / (2 * t) + ';\n}';
      }
    ).join('\n');
  }
