...
module.exports = BaseAnalysisFormModel.extend({
  ...
  _validateFilterColumn: function (value, formValues) {
    var filterColumn = formValues.filter_column;

    if (filterColumn === void 0) return false;

    var error = {
      message: _t('editor.layers.analysis-form.should-match')
    };

    var column = formValues.column;
    var columnType = column && this._filterColumnOptions.findColumn(column).type;
    var filterColumnType = filterColumn && this._filterColumnOptions.findColumn(filterColumn).type;

    if (column && filterColumn && columnType === filterColumnType) {
