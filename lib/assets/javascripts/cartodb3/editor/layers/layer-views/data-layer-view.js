...
module.exports = CoreView.extend({
    ...
    this._modals.create(function (modalModel) {
      var confirmationView = new ConfirmationView({
        ...
        renderOpts: {
          ...
          layerMap: this.mapDefModel._mapModel.attributes.title,
          dependentNodes: self._getDependentNodes(this, self)
        },
        runAction: function () {
          removeLayer({
      ...
      confirmationView.bind('exportLayerAction', self._exportLayer, self);
  ...
  _getDependentNodes: function (modalModel, layer) {
    var nodesList = [
      { widgets: modalModel.widgetDefinitionsCollection.widgetsOwnedByLayer(layer.model.get('id')) },
      { analyses: layer.model.getNumberOfAnalyses() - 1 },
      { layers: layer.model.getAllDependentLayers() }
    ];
    var dependentNodes = [];

    for (var i = 0; i < nodesList.length; i++) {
      for (var key in nodesList[i]) {
        if (nodesList[i][key] > 0) {
          var text = _t('editor.layers.delete.' + key, { smart_count: nodesList[i][key] });
          dependentNodes.push(text);
        }
      }
    }
