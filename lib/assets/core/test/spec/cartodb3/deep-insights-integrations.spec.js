...
describe('deep-insights-integrations/dii', function () {
  ...
  it('should refresh the widgets in case layer has changed', function () {
    var firstLayer = this.layerDefinitionsCollection.at(0);
    spyOn(WidgetsIntegration, 'refreshWidgets');
    firstLayer.set('infowindow', {whatever: 'whatever'});
    expect(WidgetsIntegration.refreshWidgets).toHaveBeenCalled();
  });
});
