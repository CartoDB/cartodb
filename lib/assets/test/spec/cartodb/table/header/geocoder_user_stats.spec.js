describe("geocoder user stats", function() {
  ...
  it("should change progress class element when user doesn't have more rows available and text for upgrading", function() {
    ...
    expect(view.$('div.progress > span.used').hasClass('danger')).toBeFalsy();
    expect(view.$('a.need').length).toBe(1);
    expect(view.$('p').html()).toBe('Not enough free geocodes for this table. <strong>Only 2 rows will be geocoded</strong>.')
  })
  ...
  it("should change text and progress class element when doesn't have more rows available to geocode full table", function() {
    ...
    expect(view.$('div.progress > span.used').hasClass('danger')).toBeFalsy();
    expect(view.$('a.need').length).toBe(0);
    expect(view.$('p').html()).toBe('You don\'t have enough geocoding credits to geocode the entire table this month. <strong>From now on you will be charged $1.5/1,000 rows</strong>.')
  })
