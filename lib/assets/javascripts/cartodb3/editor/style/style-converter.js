...
function pointFill (props, animatedType) {
  var css = {};
  var isTorqueCategory = animatedType && !props.color.fixed;
...
function animation (style, mapContext) {
  ...
  for (var i = 0, l = color.domain.length; i < l; i++) {
    ...
    if (value !== undefined) {
      if (value === null) {
        s.push('WHEN "' + columnName + '" is ' + value + ' THEN ' + (categoriesNo));
      } else {
        s.push('WHEN "' + columnName + '" = ' + value + ' THEN ' + (categoriesNo));
      }
      categoriesNo++;
    }
  }
  s.push('ELSE ' + categoriesNo + ' END) as value FROM (<%= sql %>) __wrapped');
  return s.join(' ');
}
