...
module.exports = BaseDialog.extend({
  ...
  _REJECT: 0,
  _KEEP: 1,
  _EXCLUDED_COLUMNS: ['the_geom', 'cartodb_id', 'lat', 'lon', 'lng', 'long', 'latitude', 'longitude', 'shape_length', 'shape_area', 'objectid', 'id', 'country', 'state', 'created_at', 'updated_at', 'iso2', 'iso3', 'x_coord', 'y_coord', 'xcoord', 'ycoord'],
  ...
  _printStats: function(column, stats) {
    ...
    console.log("%cAnalyzing " + name + ": " + this._getAnalyzedColumns().length + "/" + this.columns.length, "text-decoration: underline;");
  ...
  _redMessage: function(name, message, value) {
    console.log('%c > turned into category', "color:pink;");
  },

  _rejectMessage: function(name, message, value) {
    console.log('%c = rejected because ' + message + ": " + value, "color:red;");
  },
  ...
  _analyzeColumn: function(column) {
    ...
    this.sql.describe(this.query, column.get("name"), options, function(stats) {
      ...
      if (response === self._KEEP) {
        ...
        column.set({ analyzed: true, success: false });
      } else if (response === self._REDO) {
        column.set({ analyzed: true, new_type: "string", pending: true });
      }
