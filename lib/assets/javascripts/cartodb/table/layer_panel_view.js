  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    setLayerName: function(layer) {
      this.$('.layer_info a.info .name').text(layer.get('table_name_alias') || layer.get('table_name'));

      // Layer styles
      // If an alias is applied to the layer, it should change styles
      this.$el[( layer.get('table_name_alias') ? 'addClass' : 'removeClass' )]('with_alias')
      
      // table_name alias
      this.$('.layer_info a.info .name_info')
        .text('view of ' + layer.get('table_name'))
        [layer.get('table_name_alias') ? 'show' : 'hide' ]()
    },
