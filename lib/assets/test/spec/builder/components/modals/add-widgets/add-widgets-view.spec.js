...
describe('components/modals/add-widgets/add-widgets-view', function () {
  ...
  function createViewFn (options) {
    ...
    defaultOptions = {
      ...
      userActions: {
        saveWidgetOptionAsync: function (widgetOptionModels) {
          return Promise.resolve(widgetOptionModels);
        },
        updateWidgetsOrderAsync: function (widgetOptionModels) {
          return Promise.resolve(widgetOptionModels);
        }
      },
      analysisDefinitionNodesCollection: analysisDefinitionNodesCollection,
  ...
  describe('when the user clicks on continue', function () {
    ...
    it('should call _saveSelectedWidgetsAsync if there are selected widgets', function () {
      var saveSelectedWidgetsAsyncSpy = spyOn(view, '_saveSelectedWidgetsAsync').and.callThrough();
      ...
      expect(saveSelectedWidgetsAsyncSpy).toHaveBeenCalled();
    });

    it('should not call _saveSelectedWidgetsAsync if there are not selected widgets', function () {
      var saveSelectedWidgetsAsyncSpy = spyOn(view, '_saveSelectedWidgetsAsync').and.returnValue(true);
      ...
      expect(saveSelectedWidgetsAsyncSpy).not.toHaveBeenCalled();
    });
  });

  describe('when the selected widgets are selected to be saved', function () {
    it('should call saveWidgetOptionAsync for each selectedOptionModel', function () {
      var saveWidgetOptionAsyncSpy = spyOn(view._userActions, 'saveWidgetOptionAsync').and.callThrough();
      ...
      view._saveSelectedWidgetsAsync([widgetModel_1, widgetModel_2]);

      expect(saveWidgetOptionAsyncSpy).toHaveBeenCalledWith(widgetModel_1);
      expect(saveWidgetOptionAsyncSpy).toHaveBeenCalledWith(widgetModel_2);
    });
    ...
    it('should return an async response that fails if one of the widgets can not be saved', function (done) {
      ...
      view = createViewFn({userActions: {
        saveWidgetOptionAsync: function () {
          return Promise.reject(errorResponse);
        },
        updateWidgetsOrderAsync: function () {
