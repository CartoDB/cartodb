   ...
   *  TODO:
   *
   *  - Choose image fill (fix s3 assets response)
   *  - Check 'colors' parameter before apply anything and convert to 'categories' parameter,
   *  then remove 'colors' parameter (old color wizard to new category wizard).
   *
   */
  ...
  cdb.admin.mod.CategoryWizard = cdb.admin.mod.ColorMapWizard.extend({
    ...
    _TEXTS: {
      ...
      no_valid_columns: _t('There are no valid columns on your table to make a category visualization. \
      ...
      _.each(values, function(value,i) {
        categories.push(
          new cdb.core.Model({
            ...
            value: self._DEFAULTS.value_colors[i],
            value_type: 'color'
          })
        )
      });
      ...
      if (values.length >= this._DEFAULTS.max_values) {
        categories.push(
          new cdb.core.Model({
            ...
            title_type: "string",
            value: self._DEFAULTS.default_color,
            value_type: 'color',
            default: true
          })
        )
   ...
   *  - Each item needs a model with 'title', 'title_type', 'value',
   ...
   *  - It needs a model with 'title', 'title_type', 'value',
   *  'value_type' and 'default' if it is neccessary.
   *
  ...
  cdb.admin.mod.CategoryWizard.CategoriesViewItem = cdb.core.View.extend({
    ...
    render: function() {
      ...
      var view = new cdb.forms.Color({ model: this.model, property: 'value', extra: { image_property: 'file' }});
      view.bind("*", function(a,b,c) {
