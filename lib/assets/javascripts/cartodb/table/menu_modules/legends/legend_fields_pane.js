...
cdb.admin.mod.LegendFieldsPane = cdb.core.View.extend({
  ...
  _onWizardChange: function(m) {
    ...
    var wizardProperties = m.get("wizard_properties").properties;
    var previousWizardProperties = m.previousAttributes().wizard_properties.properties;

    var currentType  = m.get("wizard_properties").type;
    var previousType = m.previousAttributes().wizard_properties.type;
    ...
    if (currentType == previousType) {
      ...
      var currentProperty  = wizardProperties.property;
      ...
      if (this.model.get("type") == 'custom') {
       ...
       var difference = _.difference(wizardProperties.categories, previousWizardProperties.categories);
       if ((currentProperty == previousProperty) && (!difference || (difference && difference.length == 0))) return;
        ...
        if (wizardProperties["marker-fill"] == previousWizardProperties["marker-fill"]) return;
      }
      else if (currentType == 'density' || currentType == 'choropleth') {
        if (wizardProperties["color_ramp"] == previousWizardProperties["color_ramp"] && wizardProperties["method"] == previousWizardProperties["method"]) return;
      }
