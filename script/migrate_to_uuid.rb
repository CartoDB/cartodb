...
def usage(message = nil)
  ...
  puts ''
  puts 'Actions:'
  @actions.each { |k, v| puts format('  %10s   %s', k, v); puts '' }
  exit 1
end
...
usage 'Missing action' if ACTION.nil? || !@actions.keys.include?(ACTION)
...
def copy_redis_keys(redis_keys, id, uuid, username)
  ...
  redis_keys.each do |_k, v|
    ...
    these_redis_keys.each do |trd|
      ...
      redis.restore(new_key, 0, original_value) unless v[:no_clone]
      next unless v[:type] == 'hash'
