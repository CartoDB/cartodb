  ...
  cdb.common.CreateDialog.Actions = cdb.core.View.extend({
    ...
    _setState: function(m, c) {
      ...
      if (actions['title'] && c.changes.state) {
        var enabled = actions['title'].enabled;
      ...
      // OK BUTTON
      if (actions['ok'] && ( c.changes.state || c.changes.upload )) {
        var text = actions['ok'].text[option] || actions['ok'].text.default;
