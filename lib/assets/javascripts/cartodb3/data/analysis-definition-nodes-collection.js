...
module.exports = Backbone.Collection.extend({
  ...
  _onAnalysisModelChange: function (m) {
    var analysisDefinitionNodeModel = this.find(function (model) {
    ...
    if (analysisDefinitionNodeModel) {
      analysisDefinitionNodeModel.analysisTableModel.set({
        query: m.get('query') || analysisDefinitionNodeModel.get('query'),
        status: m.get('status')
      });
    }
  }
