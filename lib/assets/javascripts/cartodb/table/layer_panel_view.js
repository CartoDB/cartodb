  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    setDataLayer: function(dataLayer) {
      ...
      cdb.god.bind("end_narrow", filters.loadScroll, this); // load the scroll when the panel is open
    ...
    _addFeature: function(mod) {
      if (this.table.isGeoreferenced()) {
        ...
        var newGeomDropdown = new cdb.admin.NewGeometryDropdown({
          position: 'position',
        ...
        newGeomDropdown.bind('onDropdownHidden', newGeomDropdown.clean, this)

        newGeomDropdown.bind('newGeometry', function(geomType) {
          this._addGeometry(geomType);
        }, this);

        newGeomDropdown.render();
        $('body').append(newGeomDropdown.el);
        
        var pos = this.tabs.getTab(mod).offset();

        newGeomDropdown.openAt(pos.left - 165, pos.top - 77);
      }
    },
    ...
    _addGeometry: function(type) {
      ...
      this.dataLayer.trigger("startEdition", this.table.data().newRow(), type);
