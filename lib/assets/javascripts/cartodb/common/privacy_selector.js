...
cdb.admin.PrivacySelector = cdb.core.View.extend({
  ...
  render: function() {
    ...
    $el.html(this.template_base({
      ...
      show_privacy_options: (this.options.limitation && this.model.attributes.privacy == "PUBLIC") ? false : true,
  ...
  */
  show: function(target,offset) {
    ...
    var top = pos.top - el_height;

    if (this.options.direction === 'down') {
      top = pos.top + t_height + 10;
    }

    if (this.options.verticalOffset) {
      top += this.options.verticalOffset;
    }
    ...
    this.options.showCallback && this.options.showCallback();
