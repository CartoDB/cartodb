...
module.exports = CoreView.extend({
  ...
  _parseSQL: function () {
    ...
    if (altersData) {
      ...
      // Create a different query-schema-model in order to not make a mess with all
      // the models that are listening to the current query-schema-model
      var alterQueryModel = new QuerySchemaModel({
        query: query
      }, {
        configModel: this._configModel
      });
      ...
      alterQueryModel.fetch({
        success: function () {
          notification.set({
            status: 'success',
            info: _t('editor.data.notifier.sql-alter-success'),
            closable: true
          });
  ...
  _showErrors: function (model) {
    ...
    if (errors && errors.length > 0) {
      Notifier.addNotification({
        status: 'error',
        info: _t('editor.data.notifier.sql-error'),
        closable: true,
        delay: 10000
      });
    }
  ...
  _saveSQL: function () {
    ...
    Notifier.addNotification({
      status: 'success',
      info: _t('editor.data.notifier.sql-success'),
      closable: true,
      delay: 10000
    });
