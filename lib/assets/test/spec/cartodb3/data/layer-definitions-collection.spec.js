...
describe('data/layer-definitions-collection', function () {
  ...
  it('should create the layer with the next available color', function () {
    // layer example data
    ...
    var layersData = [
      d({ type: 'Tiled ' }),
      d({ color: layerColors.COLORS[0] }),
      d({ type: 'torque' }),
      d({ color: layerColors.COLORS[1] })
    ];
    this.collection.resetByLayersData(layersData);
    expect(this.collection.pluck('color')).toEqual([undefined, layerColors.COLORS[0], layerColors.COLORS[2], layerColors.COLORS[1]]);

    this.collection.remove(this.collection.at(0));
    expect(this.collection.pluck('color')).toEqual([layerColors.COLORS[0], layerColors.COLORS[2], layerColors.COLORS[1]]);

    this.collection.add(d());
    expect(this.collection.pluck('color')).toEqual([layerColors.COLORS[0], layerColors.COLORS[2], layerColors.COLORS[1], layerColors.COLORS[3]]);
