...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    if (this._geometryModel.get('status') === 'unfetched') {
      this._geometryModel.fetch();
    }
  },

  render: function () {
    var geom = this._geometryModel.get('simple_geom');

    if (this._geometryModel.get('status') === 'fetched') {
      this.$el.html(
        template({
          formats: FORMATS,
  ...
  _initBinds: function () {
    this._geometryModel.bind('change:status', function () {
      if (this._geometryModel.get('status') !== 'unavailable') {
        this.render();
      } else {
        this.showError(_t('components.modals.export-data.error.geometry-error'));
      }
    }, this);
    this.add_related_model(this._geometryModel);
  },
  ...
  getGeomFilteredSql: function () {
    var sql = this._geometryModel.get('query');
    var geom = this._geometryModel.get('simple_geom');
