...
cdb.admin.dashboard.Messages = cdb.core.View.extend({
  ...
  _MESSAGES: {
    ...
    upgraded:         { template: 'Great! Welcome to your brand new <strong><%= account_type %></strong> CartoDB. Now we love you even more than before ;)', store: true },
    trial_ends_soon:  { template: 'Just a reminder, your <strong><%= account_type %></strong> trial will finish the next <%= trial_ends_at %> (<%= moment(trial_ends_at).fromNow() %>). Happy mapping!', sticky: true }
  },
  ...
  _wasClosed: function(key) {
    ...
    var closed = _.find(this.closedMessages.get(), function(key) { return key == key; });
  ...
  add: function(text, options) {
    ...
    if (options.key && this._wasClosed(options.key)) return;
  ...
  _onRemoveMessage: function(model) {
    ...
    if (model.get("store") && model.get("key")) {
      this.closedMessages.add(model.get("key"));
    }

  },

  _resetClosedStatus: function(key) {

    console.log('reseting closed status', key);
