...
describe('components/tipsy-tooltip-view', function () {
  ...
  describe('.getAutoGravity', function () {
    ...
    it('should return first preferred gravity if there is no tooltipElement', function () {
      var gravityPosition = TipsyTooltipView.prototype.getAutoGravityPosition(0, ['n'], windowMock)();

      expect(gravityPosition).toBe('n');
    });

    it('should return first valid gravity', function () {
      var preferredGravity = ['s', 'n'];
      var tooltip = {
        offsetHeight: 50,
        offsetWidth: 100
      };
      ...
      var getAutoGravityPosition = TipsyTooltipView.prototype.getAutoGravityPosition(0, preferredGravity, windowMock);
      var autoGravityValue = getAutoGravityPosition.bind(tooltipContainer)(tooltip);
