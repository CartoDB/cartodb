  ...
  cdb.ui.common.FilterView = cdb.core.View.extend({
    ...
    _onTagSelected: function(tag) {
      ...
      var path = type + ( onlyMine ? '/mine' : '' ) + ('/tag/' + tag );
    ...
    _onSubmit: function(e) {
      ...
      var onlyMine = this.router.model.get('exclude_shared');
      var path = ( this.router.model.get('model') || 'tables' ) + ( onlyMine ? '/mine' : '' ) + ( val ? ('/' + action + '/' + val) : '' );
      this.router.navigate(path, { trigger: true });
    },

    _onCollectionFetched: function() {
      var params = this.router.model.attributes;
      var visible = params.tag || params.q || params.exclude_shared || this[params.model].total_entries > 0;
