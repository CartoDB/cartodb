  ...
  cdb.admin.mod.InfowindowFieldItem = cdb.core.View.extend({
    ...
    template: _.template('<span class="ellipsis"><%- typeof(syncName)!== "undefined"? syncName :fieldName %></span><div class="input"></div>'),

    initialize: function() {
      this.fieldName = this.options.field.name;
      this.fieldTitle = this.options.field.title;
      this.position  = this.options.position;
      this.isSync = this.options.isSync;

      if (this.isSync) {
        this.syncName = Sugar.String.titleize(this.fieldName);
      }
    },
