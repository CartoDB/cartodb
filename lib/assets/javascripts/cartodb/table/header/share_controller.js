...
// TODO: @jamon set proper code, temporally bypass to test named maps
/*
        if (this.any_private_table) {
          // If there is any private table
          // show the private tables dialog
          // We count with the model is PRIVATE ALWAYS
          this.stack.push({
            class: 'SharePrivateTablesDialog',
            step: 'makeTablesAndVizPublic',
            opts: {
              model: this.model,
              ok_next: _t('Make viz public and share')
            },
            center: false,
            callback: '_makeVisPublic'
          });
        } else if (this.model.get('privacy') == "PRIVATE") {
          // If not, just show the viz private dialog
          this.stack.push({
