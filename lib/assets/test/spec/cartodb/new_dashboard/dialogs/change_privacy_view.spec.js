...
describe('new_dashboard/dialogs/change_privacy_view', function() {
  ...
  describe('click .js-option', function() {
    ...
    it('should have selected item', function() {
      expect(this.view.options.at(2).get('selected')).toBeFalsy();
      
      this.select(2);
      expect(this.view.options.at(2).get('selected')).toBeTruthy();
    ...
    it("should set the .is-selected class on the selected item's DOM", function() {
      expect(this.innerHTML()).not.toMatch('is-selected.+data-index="2"');

      this.select(2);
      expect(this.innerHTML()).toMatch('is-selected.+data-index="2"');
