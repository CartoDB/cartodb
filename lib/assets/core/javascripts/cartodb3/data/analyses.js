...
var TTAggregateIntersectionFormModel = require('../editor/layers/layer-content-views/analyses/analysis-form-models/tt-aggregate-intersection-form-model');
...
var MAP = {
  ...
  'data-observatory-multiple-measures': {
    ...
    checkIfEnabled: function (configModel, userModel) {
      return checkIfDataObservatoryIsEnabled(configModel) && userModel.featureEnabled('data-observatory-multiple-measures');
    },
  ...
  'tt-aggregate-intersection': {
    title: _t('analyses.tt-aggregate-intersection.short-title'),
    FormModel: TTAggregateIntersectionFormModel,
    modalTitle: _t('components.modals.add-analysis.option-types.tt-aggregate-intersection.title'),
    modalDesc: _t('components.modals.add-analysis.option-types.tt-aggregate-intersection.desc'),
    modalCategory: 'data_transformation',
    onboardingTemplate: require('../components/onboardings/analysis/analyses/aggregate-intersection.tpl'),
    animationTemplate: require('../components/modals/add-analysis/animation-templates/intersect.tpl'),
    isTVTanalysis: true
...
module.exports = {
  ...
  getAnalysesByModalCategory: function (category, configModel, userModel) {
    return _.reduce(MAP, function (memo, item, analysisType) {
      if (userModel.featureEnabled('data-observatory-multiple-measures') && analysisType === DATA_OBSERVATORY_OLD) {
        return memo;
      }
      ...
      if (item.modalCategory === category) {
        if (isAnalysisValidByType(analysisType, configModel, userModel)) {
          var obj = {
            nodeAttrs: {
              type: analysisType,
              isTVTanalysis: item.isTVTanalysis
            },
