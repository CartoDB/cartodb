...
module.exports = {
  ...
  _onAnalysisDefinitionNodeChanged: function (node) {
    if (!this._hasUpdateOnlyNodeAnalysisStatus(node)) {
  ...
  _onAnalysisDefinitionNodeIdChanged: function (node) {
    console.warn('Track analysis node change:id', node);
    if (this._hasUpdateOnlyNodeAnalysisId(node)) {
  ...
  _hasUpdateOnlyNodeAnalysisStatus: function (node) {
    return node.hasChanged('status') && _.size(node.changed) === 1;
  },

  _hasUpdateOnlyNodeAnalysisId: function (node) {
