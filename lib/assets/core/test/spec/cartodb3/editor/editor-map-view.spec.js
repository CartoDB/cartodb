...
describe('editor/editor-map-view', function () {
  ...
  describe('mapModeModel changes', function () {
    ...
    it('map mode editing should go to editing view', function () {
      this.mapModeModel.attributes.mode = 'wadus';
      this.mapModeModel.enterDrawingFeatureMode();
      expect(this.view._stackLayoutView.model.goToStep).toHaveBeenCalledWith(2, this.layerDefinitionModel, false);
    });

    it('map mode viewing should go to previous', function () {
      this.mapModeModel.attributes.mode = 'wadus';
      this.mapModeModel.enterViewingMode();
      expect(this.view._stackLayoutView.model.goBack).toHaveBeenCalled();
    });

    it('map mode reset should not change stack layout model', function () {
      this.mapModeModel.attributes.mode = 'wadus';
      this.mapModeModel.resetMode();
      expect(this.view._stackLayoutView.model.goBack).not.toHaveBeenCalled();
      expect(this.view._stackLayoutView.model.goToStep).not.toHaveBeenCalled();
    });
  });
});
