  ...
  var resetStylesWhenGeometryChanges = function (nodeDefModel, layerDefModel, forceStyleUpdate) {
    ...
    onQueryGeometryReady = function () {
      ...
      if (queryGeometryModel.get('ready')) {
        queryGeometryModel.off(CHANGE_READY, onQueryGeometryReady);
      } else {
      ...
      saveDefaultStylesIfStillRelevant = function () {
        if (queryGeometryModel.get('status') !== 'fetched') return; // wait until fetched

        queryGeometryModel.off(CHANGE_STATUS, saveDefaultStylesIfStillRelevant); // only do this once
      ...
      queryGeometryModel.on(CHANGE_STATUS, saveDefaultStylesIfStillRelevant);
      queryGeometryModel.fetch();
    };
    nodeDefModel.queryGeometryModel.on(CHANGE_READY, onQueryGeometryReady);
    ...
    createLayerForAnalysisNode: function (nodeId, cfg) {
      ...
      if (prevLayer) {
        if (nodeDefModel.hasPrimarySource()) {
          ...
          var affectedWidgetsBySourceChange = widgetDefinitionsCollection.where({ source: nodeId });
          ...
          newLayerDefModel.save(null, {
            success: function () {
              var onWidgetFinished = _.after(affectedWidgetAttrsBySourceChange.length, function () {
                onNewLayerSaved(newLayerDefModel);
              });

              // Create widgets with new source and layer_id
              _.each(affectedWidgetAttrsBySourceChange, function (attrs) {
                attrs.layer_id = newLayerDefModel.id;

                widgetDefinitionsCollection.create(
                  attrs, {
                    wait: true,
                    success: onWidgetFinished,
                    error: onWidgetFinished
                  }
                );
              });

              if (!affectedWidgetAttrsBySourceChange.length) {
                onNewLayerSaved(newLayerDefModel);
              }
            }
          });
        ...
        newLayerDefModel = layerDefinitionsCollection.create(attrs, {
          ...
          success: function () {
            ...
            // Create widgets with new source
            _.each(affectedWidgetAttrsBySourceChange, function (attrs) {
              attrs.layer_id = newLayerDefModel.id;
