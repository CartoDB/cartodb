...
describe DataImport do
  ...
  it 'raises a meaningful error if cartodb_id is not valid' do
    allow_any_instance_of(Table).to receive(:cartodbfy).and_raise(CartoDB::CartoDBfyInvalidID)
    data_import = DataImport.create(
      user_id: @user.id,
  ...
  describe 'arcgis connector' do
    ...
    it 'should complete with a warning when the query batcher raises a timeout exception' do
      ...
      allow_any_instance_of(CartoDB::Importer2::QueryBatcher).to receive(:execute_update).and_raise(Sequel::DatabaseError)
