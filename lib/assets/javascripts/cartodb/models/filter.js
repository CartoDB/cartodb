  ...
  _changeDateHist: function(hist, bounds) {
    ...
    if (!this.has('lower')) {
      ...
      /*if (typeof lower == 'object') {
        lower = this.get("lower").getTime();
      } else if (typeof lower == 'string') {
        lower = parseInt(new Date(this.get("lower")).getTime(), 10);
      }
*/

      /*if (typeof lower == 'string') {
        lower = parseInt(new Date(lower).getTime(), 10);
      }

      if (typeof upper == 'string') {
        upper = parseInt(new Date(upper).getTime(), 10);
      }*/
      ...
      console.log(bounds.lower, lower);
  ...
  getSQLConditionForDate: function() {
    ...
    if (_.isNaN(lower) || _.isNaN(upper)) return;

    var lowerDate = this._getDateFromTimestamp(lower);
    var upperDate = this._getDateFromTimestamp(upper);

    var options = _.extend(this.attributes, { lower: lowerDate.toUTCString(), upper: upperDate.toUTCString() });

    var template;

    if      (!this.attributes.lower && !this.attributes.upper)     template = _.template(" (<%= column %> IS NULL) ")(options);
    else if (this.attributes.upper >= this.attributes.upper_limit) template = _.template(" (<%= column %> >= ('<%= lower %>') AND <%= column %> <= ('<%= upper %>')) ")(options);
    else template = _.template(" (<%= column %> >= ('<%= lower %>') AND <%= column %> <= ('<%= upper %>')) ")(options);
