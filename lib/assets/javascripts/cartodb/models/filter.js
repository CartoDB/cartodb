...
cdb.admin.models.Filter = cdb.core.Model.extend({
  ...
  getSQLConditionForDate: function() {
    ...
    var tz = this._getTimeZone(this.get("tz"));

    var lowerDate = moment(lower).format("YYYY-MM-DDTHH:mm:ss+" + tz).toString();
    var upperDate = moment(upper).format("YYYY-MM-DDTHH:mm:ss+" + tz).toString();
    var upperDateWithOffset = moment(upper + 1000).format("YYYY-MM-DDTHH:mm:ss+" + tz).toString();

    var attributes = _.clone(this.attributes);

    var options, sql;

    if (!this.attributes.lower && !this.attributes.upper) {

      options = _.extend(attributes, { lower: lowerDate, upper: upperDateWithOffset });
      sql = _.template(" (<%= column %> IS NULL) ")(options);

    } else if (this.attributes.upper >= this.attributes.upper_limit) {
      ...
      options = _.extend(attributes, { lower: lowerDate, upper: upperDate });
      sql = _.template(" (<%= column %> >= ('<%= lower %>') AND <%= column %> <= ('<%= upper %>')) ")(options);
  ...
  getSQLConditionForString: function() {
    ...
    if (items.length > 0) {
      ...
      if (items.length == this.items.length) {
        ...
        var query = "<%= column %> IN (<%= opts %>) OR <%= column %> IS NULL ";
