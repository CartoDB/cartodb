describe('cdb.common.Url', function() {
  beforeEach(function() {
    this.baseUrl = 'http://subdomain.cartodb.com/foobar';
    this.url = new cdb.common.Url({ base_url: this.baseUrl });
  });
  ...
  describe('.urlToPath', function() {
    describe('when given no arguments', function() {
      ...
      it('should return a new url object', function() {
        ...
        expect(this.newUrl instanceof cdb.common.Url).toBeTruthy();
      });
  ...
  describe('.hasMatchingPathname', function() {
    it('should return true if given pathname matches the one of the URL', function() {
      expect(this.url.hasMatchingPathname('/foobar')).toBe(true);
      expect(this.url.hasMatchingPathname('/foobar/')).toBe(true); // also works even if has trailing slash
      expect(this.url.hasMatchingPathname('/foobar/other')).toBe(false);
      expect(this.url.hasMatchingPathname('/notmatching')).toBe(false);
    });
  });
});
