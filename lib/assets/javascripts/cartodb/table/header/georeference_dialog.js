...
cdb.admin.GeoreferenceStyles = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.model.bind('change:style',      this._onStyleChange, this);
  ...
  _onStyleChange: function() {
    ...
    this.$el.find("li." + style).addClass("selected");
  ...
  _onStyleClick: function(e) {
    ...
    if (!this.model.get(style + "Enabled")) return;
...
cdb.admin.GeoreferenceDialog = cdb.admin.BaseDialog.extend({
  ...
  _showChooseStyleStep: function() {
    ...
    var regionsEnabled = _.contains(available_geometries, "polygons");
    var pointsEnabled  = _.contains(available_geometries, "points");

    this.styles.model.set("regionsEnabled", regionsEnabled);
    this.styles.model.set("pointsEnabled",  pointsEnabled);

    if (regionsEnabled && !pointsEnabled) this.styles.model.set("style", "regions");
    else if (!regionsEnabled && pointsEnabled) this.styles.model.set("style", "points");
    else this.styles.model.set("style", "points");
  ...
  _onAdmGetSuccess: function(res) {
    ...
    if (res.admin1[0] || res.admin1[1]) {
      options.push('<option name="adm1">adm1</option>');
      this.model.set("adm1", res.admin1);
    }

    if (res.admin2[0] || res.admin2[1]) {
      options.push('<option name="adm2">adm2</option>');
      this.model.set("adm2", res.admin2);
    }

    if (res.zip[0]    || res.zip[1]) {
      options.push('<option name="zip">zip</option>');
  ...
  _onChangeAdm: function() {
    ...
    else if (adm === 'adm2') available_geometries = this.model.get("adm2");
    else if (adm === 'zip')  available_geometries = this.model.get("zip");
  ...
  _onChangeCountry: function() {
    ...
    if (country === "world") {
      ...
      this._refreshSelect("adm", '<option name="countries">countries</option>');
  ...
  _geoCodeZip: function(column, country_code, style) {

    this.table.geocodeZip(column, country_code, style);
  ...
  _geoReference: function(ev) {
    ...
    if (this.hasContent) {
      ...
      switch (option) {
        ...
        break;
        case 2:
          if (this.model.get("adm") == "zip") this._geoCodeZip(this.model.get("column"), this.model.get("country"), this.model.get("style"));
        break;
      }
