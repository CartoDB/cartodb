...
F.prototype._onBaseLayerChanged = function () {
  ...
  // Handle all operations silently and make the instantiation at the end
  ...
  if (newBaseLayerAttrs.type) {
    cdbjsLayer.remove({ silent: true });
    this._createLayer(baseLayerDefinition, { silent: true });
  } else {
  ...
  if (labelsLayerDefinition) {
    if (cdbjsHasLabelsLayer) {
      ...
      this._createLayer(labelsLayerDefinition, { silent: true });
    }
  } else if (cdbjsHasLabelsLayer) {
    cdbjsTopLayer.remove({ silent: true });
  }
