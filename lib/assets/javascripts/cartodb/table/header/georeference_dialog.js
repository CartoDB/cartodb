...
cdb.admin.GeoreferenceStyles = cdb.core.View.extend({
  ...
  _TEXTS: {
    ...
    polygons: {
      ...
      disabled: _t('No polygon data available for your selection.'),
    }
  },
...
/*
  ...
  */
  ...
  _showChooseGeometryTypeStep: function() {
    ...
    var polygonsEnabled = _.contains(available_geometry_types, "polygon");
    var pointsEnabled   = _.contains(available_geometry_types, "point");
    ...
    this.$el.find(".inner").animate({ height: 178, left: -550 }, { easing: "easeOutQuad", duration: 200, complete: function() {
      ...
      setTimeout(function() {
        ...
        if (polygonsEnabled && !pointsEnabled) self.styles.model.set("geometry_type", "polygons");
        else if (!polygonsEnabled && pointsEnabled) self.styles.model.set("geometry_type", "points");
        else self.styles.model.set("geometry_type", "points");
  ...
  _geoCodeAddress: function() {
    ...
    if (this._checkAddressInput(address)) {
      ...
      this._hideError();
      this.options.geocoder.set({ formatter: address, kind: "high-resolution" });
    ...
    if (this.hasContent) {
      ...
      switch (option) {
        ...
        case 2:
          ...
          else if (adm == "countries")                 this._geocodeWorld(adm, column, geometry_type);
