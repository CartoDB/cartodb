  ...
  describe("New layer dialog", function() {
    ...
    it("should warn the user if table doesn't have any georeferenced data", function() {
      ...
      expect(view.$('p.warning.geo').is(':visible')).toBeFalsy();
      view.$('a.ok').click();
    ...
    it("should fetch user model when a new table is created", function() {
      spyOn(user, 'fetch');
      cdb.admin.CartoDBTableMetadata.prototype.save = function(a,b) {
        b.success(this)
      }
      view.render();
      view.model.set('option', 'scratch');
      view.$('a.ok').click();
      // This shit doesn't work when the test is running with rest of them!
      // server.respondWith('POST', '/api/v1/tables/', [200, { "Content-Type": "application/json" }, '{}']);
      // server.respond();
      expect(user.fetch).toHaveBeenCalled();
      cdb.admin.CartoDBTableMetadata.prototype.save = function(a,b) {
        cdb.admin.CartoDBTableMetadata.prototype.save.call(this, a, b);
      }
    });
