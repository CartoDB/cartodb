  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    setDataLayer: function(dataLayer) {
      ...
      var sql = new cdb.admin.mod.SQL({
        ...
        .bind('clearSQLView', this._onResetSQL, this)
        .bind('hasErrors', this._onErrorSQL, this)
        // .bind('clearError', this._onApplySQL, this)
    ...
    _setLayerState: function() {
      // Set sql view is query was applied
      if (this.dataLayer.get('query')) {
        this.table.useSQLView(this.sqlView);
        this.sqlView.setSQL(this.model.get('query'), { silent: true });
        this._onApplySQL();
      }
    },
