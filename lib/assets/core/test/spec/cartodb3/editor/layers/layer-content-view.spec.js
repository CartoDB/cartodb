...
describe('editor/layers/layer-content-view/lcv', function () {
  ...
  describe('._renderContextButtons', function () {
    ...
    beforeEach(function () {
      ...
      hasGeomSpy = spyOn(this.view, '_hasGeom');
    ...
    it('should disable tabs if no geometry', function () {
      hasGeomSpy.and.returnValue(true);
      this.view.render();
      expect(this.view.$el.find('.CDB-NavMenu-item.is-disabled').length).toBe(0);
      hasGeomSpy.and.returnValue(false);
      this.view.render();
      expect(this.view.$el.find('.CDB-NavMenu-item.is-disabled').length).toBe(4);
    });
