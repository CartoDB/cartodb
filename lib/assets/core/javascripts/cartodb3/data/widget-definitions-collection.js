...
module.exports = Backbone.Collection.extend({
  ...
  model: function (d, opts) {
    ...
    d.order = self._getWidgetOrder(d.order, !d.id);
  ...
  _getWidgetOrder: function (previousOrder, isAddedWidget) {
    if (isAddedWidget) {
      this._updatePreviousWidgetsOrder();
      return 0;
    }

    if (!_.isUndefined(previousOrder)) {
  ...
  _updatePreviousWidgetsOrder: function () {
    this.sort()
      .forEach(function (widget, index) {
        widget.set('order', index + 1);
        widget.save();
      });
  },
