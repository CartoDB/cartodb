...
function markerFill (images, color) {
  ...
  for (var i in images) {
    if (!_.isEmpty(images[i])) {
      ...
      if (color.domain[i]) {
        categoryNames.push(color.domain[i]);
      }
    }
  }
...
function polygonFill (props) {
  ...
  if (props.color) {
    ...
    if (props.color.images) {
      css['polygon-pattern-file'] = markerFill(props.color.images, props.color);
    } else if (props.color.image) {
      css['polygon-pattern-file'] = 'url(\'' + props.color.image + '\')';
    }
  }
...
function lineStroke (props) {
  ...
  if (props.color) {
    ...
    if (props.color.images) {
      css['line-pattern-file'] = markerFill(props.color.images, props.color);
    } else if (props.color.image) {
      css['line-pattern-file'] = 'url(\'' + props.color.image + '\')';
    }
  }
  return css;
}
