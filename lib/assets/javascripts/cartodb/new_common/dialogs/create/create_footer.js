...
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.model = new cdb.core.Model({ content_guessing: true });
  ...
  render: function() {
    this.$el.html(
      this.template({
        ...
        contentGuessing: this.model.get('content_guessing'),
        option: this.createModel.get('option'),
        importOption: this.createModel.get('importOption'),
  ...
  _initBinds: function() {
    ...
    this.createModel.bind('change:importOption', this.render, this);
    this.model.bind('change:content_guessing', this.render, this);
    this.add_related_model(this.createModel);
  },

  _connectDataset: function() {
    if (this._validUpload()) {
      var d = _.extend(this.createModel.getUpload(), { content_guessing: this.model.get('content_guessing') });
  ...
  _setContentGuessing: function() {
    if (this._validUpload()) {
      this.model.set('content_guessing', !this.model.get('content_guessing'));
    }
  },
