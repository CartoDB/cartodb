...
cdb.admin.PrivacyDialog = cdb.admin.BaseDialog.extend({
  ...
  _TEXTS: {
    ...
    privacy_change: {
      table:          _t("By changing the privacy of this table you will be changing or deleting the visualizations of the \
                          following users within your organization:"),
      visualization:  _t("By changing the privacy of this visualization you will be revoking access to it to the \
                          following users within your organization:"),
    },
  ...
  initialize: function() {
    ...
    this.options = _.extend({
      ...
      template_name: 'common/views/privacy_dialog/privacy_dialog',
      clean_on_hide: true,
  ...
  render_content: function() {
    ...
    if (this.user.get("actions") && this.user.get("actions").private_tables) {
      ...
      this.$('.vis_privacy_dialog .foot').append(this.privacy_password.render().el);
    }
    ...
    if (!this.model.isVisualization()) {
      mdl = this.table_data = new cdb.admin.CartoDBTableMetadata({ id: this.model.get("table").id });
      mdl.fetch();
      type = 'visualizations';
    }
    ...
    this.model.save(this.vis_model.attributes, { 
      ...
      success: function() {
        ...
        if (self.options.user.isInsideOrg()) {
          self.model.permission.acl.reset(self.vis_perm.acl.toJSON());
  ...
  _getUsersAffected: function() {
    ...
    if (this.model.isVisualization()) {
      return [] // No worries mate! (sound by @rochoa)
    }
  ...
  _getRelatedUserVisualizations: function(perm_users) {
    if (!this.model.isVisualization()) {
      ...
      return _.compact(
        this.table_data.get('dependent_visualizations').map(function(t){
          if (!_.contains(users, t.permission.owner.id)) {
            return {
