...
cdb.admin.Header = cdb.core.View.extend({
  ...
  setActiveLayer: function(layerView) {
    ...
    if (this.dataLayer) {
      ...
      this.dataLayer.table.unbind('change:isSync', this.this.setEditableInfo, this);
    }
  ...
  _setSyncInfo: function() {
    ...
    if (!this.model.isVisualization() && this.isSyncTable()) {
      this.sync_info = new cdb.admin.SyncInfo({
        model: this.dataLayer.table.synchronization
      });

      this.$('ul.actions').append(this.sync_info.render().el);
