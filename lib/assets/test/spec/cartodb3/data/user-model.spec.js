...
describe('data/user-model', function () {
  ...
  it('isInsideOrg', function () {
    this.model._organizationModel.users.reset([]);
    this.model._organizationModel.id = '';
    expect(this.model.isInsideOrg()).toEqual(false);
    this.model._organizationModel.users.add(new UserModel({}, { configModel: 'c' }));
    this.model._organizationModel.users.add(new UserModel({}, { configModel: 'c' }));
    expect(this.model.isInsideOrg()).toEqual(false);
  ...
  it('isOrgAdmin', function () {
    this.model._organizationModel.owner = this.model;
    expect(this.model.isOrgAdmin()).toEqual(true);
    this.model._organizationModel.owner = new UserModel({
      id: 'test',
      organization: {}
    }, { configModel: 'c' });
