...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    this._queryGeometryModel = this._nodeDefModel.queryGeometryModel;
    this._isSourceType = this._nodeDefModel.get('type') === 'source';

    // If primary node is a source node
    if (this._nodeDefModel.get('type') === 'source') {
      this._tableNodeModel = this._nodeDefModel.tableModel;
      this._syncModel = this._tableNodeModel && this._tableNodeModel.getSyncModel();
  ...
  _isSync: function () {
    return this._syncModel && this._syncModel.isSync();
  },
  ...
  _initBinds: function () {
    ...
    this._nodeDefModel.bind('change:query', this._renderContextButtons, this);
    this.add_related_model(this._nodeDefModel);
  ...
  _initTable: function () {
    ...
    this._tableView = TableManager.create({
      ...
      readonly: this._isSync() || !this._isSourceType,
      modals: this._modals,
      configModel: this._configModel
    });
  ...
  _isCustomQueryApplied: function () {
    ...
    var query = this._nodeDefModel.get('query');
    return sanitizeName(query) !== sanitizeName('select * from ' + this._getTableName());
  },

  _getTableName: function () {
    return this._nodeDefModel.get('table_name');
  },
  ...
  _renderContextButtons: function () {
    ...
    $('.CDB-Dashboard-canvas').append(
      changeViewButtons({
        ...
        queryGeometryModel: this._queryGeometryModel.get('simple_geom'),
        isSourceType: this._isSourceType,
  ...
  _onClickNewGeometry: function (e) {
    var $target = $(e.target).closest('.js-newGeometry');
