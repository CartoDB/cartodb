...
module.exports = Backbone.Model.extend({
  ...
  setCartoCSSFromAutoStyle: function (cartocss) {
    // If custom cartocss, we need to store it to get it back when autostyle is off
    var hasCustomCartoCSS = this.get('cartocss_custom');
    if (hasCustomCartoCSS && !this._cartoCssPreAutoStyle) {
      this._cartoCssPreAutoStyle = this.get('cartocss');
    }
  ...
  resetCartoCSSFromAutoStyle: function () {
    var hasCustomCartoCSS = this.get('cartocss_custom');
    if (hasCustomCartoCSS) {
      if (this._cartoCssPreAutoStyle) {
        this.set({
          cartocss: this._cartoCssPreAutoStyle,
          autoStyle: false
        });
        delete this._cartoCssPreAutoStyle;
      }
    } else {
      this.set('autoStyle', false);
    }
