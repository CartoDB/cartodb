...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _getOverlay: function(type) {
    ...
    var overlays = this.overlays.filter(function(m) { return m.get("type") == type });
    return overlays[0];
  ...
  _onChangeCanvasMode: function() {
    ...
    if (mode == "desktop") {
      ...
      var el = $("div.map div.cartodb-map"),
          ...
          el.animate({ height: autoHeight, width: autoWidth, left: "15px", top: "70", marginLeft: "0", marginTop: "0"}, 200, function() {
            el.css('width', 'auto'); 
            el.css('height', 'auto'); 
          });
  ...
  _renderOverlay:  function(delayed_animation, data) {
    ...
    if (type == "Search") {
      ...
      var widget = new cdb.admin.widgets[type]({
        model: data,
        map: this.map
      });

      this.search = widget.searchControl;
    ...
    if (type != "Search") { // TODO: improve this
  ...
  _onResetOverlays: function(overlays) {
    ...
    var search = this._getSearchOverlay();

    header.on("change:show_title change:show_description change:title change:description", function() {
      setTimeout(function() {
        var y = $(".header.static").outerHeight(true);
        search.set("y", y + 20);
      }, 170)
    });
