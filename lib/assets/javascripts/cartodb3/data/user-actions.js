...
module.exports = function (params) {
  ...
  return {
    ...
    deleteAnalysisNode: function (nodeId) {
      ...
      var styleHistory = layerDefModel.get('style_history');
      var oldNodeStyle = styleHistory && styleHistory[primarySourceNode.id];
    ...
    createLayerForAnalysisNode: function (nodeId, cfg) {
      ...
      if (prevLayer) {
        if (nodeDefModel.hasPrimarySource()) {
          ...
          var oldLayerStyle = prevLayer.get('style_history');
          var oldNodeStyle = oldLayerStyle && oldLayerStyle[nodeDefModel.getPrimarySource().id];
          ...
          //  |______|      |______|  |______|
          var oldLayerStyle = prevLayer.get('style_history');
          var oldNodeStyle = oldLayerStyle && oldLayerStyle[nodeId];
          tableName = prevLayer.get('table_name');
          tableNameAlias = prevLayer.get('table_name_alias');
          ownerName = prevLayer.get('user_name');
          attrs = createDefaultCartoDBAttrs(oldNodeStyle);
        ...
        var oldLayerStyle = ownerLayer.get('style_history');
        var oldNodeStyle = oldLayerStyle && oldLayerStyle[nodeId];
