 ...
 *  in the platform, as in imports and geocodings
 *
 */
...
BackgroundImporter.prototype = {
  ...
  _addGeocoding: function (mdl) {
    var geocodingItem = new GeocodingItemView({
      showGeocodingDatasetURLButton: this._model.get('showGeocodingDatasetURLButton'),
      modals: this._modals,
      importModel: mdl,
      userModel: this._userModel
    });

    geocodingItem.on('remove', this._removeGeocoding, this);
  ...
  _removeGeocoding: function (model, view) {
    this._model.removeGeocodingItem(model);
    this.removeView(view);
  }
};
