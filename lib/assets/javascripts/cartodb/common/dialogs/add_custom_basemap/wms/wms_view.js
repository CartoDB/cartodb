...
module.exports = cdb.core.View.extend({
  ...
  _initBinds: function() {
    this.model.bind('change:layers', this._onChangedLayers, this);
  ...
  _onChangedLayers: function() {
    var layersFetched = this.model.layersFetched();
    if (layersFetched) {
      if (layersFetched.length > 0) {
        this._panes.active('selectLayer');
      } else {
        this._panes.active('enterUrl');
      }
    } else {
