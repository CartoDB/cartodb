...
var F = function (opts) {
  ...
  opts.legendDefinitionsCollection.each(function (legendDefModel) {
    this._onLegendDefinitionAdded(legendDefModel);
  }, this);
...
F.prototype._resetStylesIfNoneApplied = function (layerDefModel) {
  ...
  if (isAnalysisNode && styleModel.hasNoneStyles() && isDone) {
    var simpleGeom = querySchemaModel.get('simple_geom');

    var applyDefaultStyles = function () {
      simpleGeom = querySchemaModel.get('simple_geom');
      styleModel.setDefaultPropertiesByType('simple', simpleGeom);
    };

    if (!simpleGeom) {
      querySchemaModel.once('change:simple_geom', applyDefaultStyles, this);
...
F.prototype._onLegendDefinitionChanged = function (m) {
  ...
  if (layer) {
    legend = layer.legends[type];
    legend && legend.set(_.omit(m.attributes, 'type'));
  }
};
