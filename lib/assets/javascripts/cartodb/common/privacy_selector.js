...
cdb.admin.PrivacySelector = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    /* 
      - When visualization won't get back related tables parameter, we should fetch the model
        to receive the attribute.

    else if (this.model.get('related_tables') && this.model.get('related_tables').length == 0) {
      // Get related tables if model is a visualization
      this._getRelatedTables();
    } */
  ...
  */
  ...
  _getRelatedTables: function() {
    // Fetch the model in order to get related tables... if there is any.
    this.model.fetch();
  },
