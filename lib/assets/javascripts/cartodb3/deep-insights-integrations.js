...
F.prototype._onLayerDefinitionMoved = function (m, at, c) {
  if (!m) throw new Error('moved layer model required');
  if (at === undefined) throw new Error('at position required');
...
F.prototype._createLayer = function (m, opts) {
  opts = opts || {};
  var attrs = JSON.parse(JSON.stringify(m.attributes)); // deep clone;

  if (attrs['source'] && !this._analysisDefinitionsCollection.findByNodeId(attrs['source'])) {
    // A new layer should not have an analyse to begin with,
    // even if we have it in the context of editor
    delete attrs['source'];
  }
  ...
  this.visMap()[createMethodName](attrs, opts);
};
