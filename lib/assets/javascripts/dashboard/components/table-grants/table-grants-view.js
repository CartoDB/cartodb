  ...
  _renderFormView: function () {
    ...
    if (this._userTablesModel.isFetched()) {
      if (this._userTablesModel.isEmpty()) {
        const message = this._userTablesModel.hasQuery() ? _t('dashboard.components.table_grants.table_grants_view.zero_data') : _t('dashboard.components.table_grants.table_grants_view.no_data');
