...
describe('new_dashboard/filters_view', function() {
  beforeEach(function() {
    ...
    this.lockItemsDialogStub = jasmine.createSpyObj('LockDatasetsDialog stub', ['appendToBody', 'bind']);
    this.LockDatasetsDialogSpy = jasmine.createSpy('LockDatasetsDialog');
    this.LockDatasetsDialogSpy.and.returnValue(this.lockItemsDialogStub);
    FiltersView.__set__('LockDatasetsDialog', this.LockDatasetsDialogSpy);
  ...
  describe('an item is selected', function() {
    ...
    describe('and click lock datasets', function() {
      beforeEach(function() {
        // For now only don't support batch processing, so only select one
        this.selectedItems = [
          { selected: false },
        ...
        this.createdWith = this.LockDatasetsDialogSpy.calls.argsFor(0)[0];
      });

      it('should open a lock datasets dialog', function() {
        expect(this.lockItemsDialogStub.appendToBody).toHaveBeenCalled();
      });

      it('should created dialog with selected items as datasets', function() {
        expect(this.createdWith).toEqual(jasmine.objectContaining({ datasets: [ this.collection.at(2) ] }));
      });
    });
  });
