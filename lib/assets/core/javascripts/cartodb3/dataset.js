...
var query = layerModel.get('sql') || defaultQuery;
var isDefaultQuery = SQLUtils.isSameQuery(query, defaultQuery);
...
if (!configModel.get('cartodb_com_hosted')) {
  if (userModel.get('actions').builder_enabled && Date.parse(userModel.get('created_at')) < new Date(2016, 11, 11) &&
