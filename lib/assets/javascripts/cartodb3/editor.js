...
cdb.config.set(frontendConfig); // TODO: review this change
...
window.snippets = {
  ...
  createTradeAreaAnalysisNode: function (layer) {
    if (!layer) throw new Error('layer is required, e.g. pass true for layerDefinitionsCollection.first(), or a custom layer');
    layer = _.isBoolean(layer) ? layerDefinitionsCollection.first() : layer;

    return layer.createNewAnalysisNode({
      type: 'trade-area',
      kind: 'walk',
      time: 300,
