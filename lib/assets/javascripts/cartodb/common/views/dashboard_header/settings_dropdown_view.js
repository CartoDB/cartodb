...
module.exports = cdb.admin.DropdownMenu.extend({
  ...
  shortenDisplayName: function(displayName) {
    ...
    if (displayName === 'organization user') {
      ...
      return displayName.replace(/lump-sum/gi, 'LP')
                        .replace(/academic/gi, 'aca.')
                        .replace(/ - Monthly/, ' - M')
                        .replace(/ - Annual/, ' - A')
                        .replace(/Non-Profit/, 'NP')
                        .replace(/On-premises/, 'OP')
                        .replace(/Internal use engine/, 'engine')
                        .replace(/Lite/, 'L')
                        .replace(/CARTO for /, '')
                        .replace(/CARTO /, '')
                        .toLowerCase();
  ...
  render: function() {
    ...
    var accountType = this.shortenDisplayName(user.get('account_type_display_name'));
