...
module.exports = BaseModel.extend({
  ...
  parse: function (data) {
    var simpleGeom = this._findValidSimpleGeom(data);

    return {
      status: STATUS.fetched,
      simple_geom: simpleGeom && simpleGeom.the_geom
        ? simpleGeom.the_geom
        : ''
    };
  },

  _findValidSimpleGeom: function (data) {
    return _.find(data.rows, function (row) {
      return !!(row.the_geom);
    });
  },
