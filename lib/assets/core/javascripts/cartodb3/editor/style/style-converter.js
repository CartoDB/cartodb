...
function pointFill (props, animationType) {
  ...
  if (color) {
    ...
    if ((props.images && _.isArray(props.images)) || (color.images && _.isArray(color.images))) {
      categoryIcons(props.images || color.images, color, css);
...
function categoryIcons (images, color, css) {
  if (!_.isArray(images) || !color || !color.attribute) {
    return;
  }

  if (color.domain && (color.domain.length < color.range.length)) {
    css['marker-file'] = 'url(\'' + color.images[color.range.length - 1] + '\')';
  }

  _.each(images, function (url, index) {
    var key = color.attribute;
    var value = color.domain && color.domain[index];
    if (value) {
