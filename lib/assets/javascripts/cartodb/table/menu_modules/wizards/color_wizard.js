  ...
  /*
    TODO:
      ...
      - Request analize before getting values.
      - Check with Santana some of the code.
      - Bind when new rows have been added to the map.
      - TESTS!!!!
  */
  ...
  cdb.admin.mod.ColorMapWizard = cdb.admin.mod.SimpleWizard.extend({
    ...
    _getValues: function() {
      ...
      var self = this;
      // SELECT <%= column %>, count(<%= column %>) FROM (<%= sql %>) as subq GROUP BY <%= column %> ORDER BY count DESC LIMIT 10 
  ...
  cdb.admin.mod.ColorMapWizard.ColorsViewItem = cdb.core.View.extend({
    ...
    render: function() {
      this.clearSubViews();
      this.$el.append("<span>" + (this.model.get("value") || '<i>null</i>') + "</span><span class='field'></span>");
