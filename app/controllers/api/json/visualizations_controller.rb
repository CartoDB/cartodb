...
class Api::Json::VisualizationsController < Api::ApplicationController
  ...
  def send_like_email(vis, current_viewer, vis_preview_image)
    if vis.user.is_subscribed_to?(Carto::UserNotification::TABLE_LIKE_NOTIFICATION) && vis.type == Carto::Visualization::TYPE_CANONICAL
      ::Resque.enqueue(::Resque::UserJobs::Mail::TableLiked, vis.id, current_viewer.id, vis_preview_image)
    elsif vis.user.is_subscribed_to?(Carto::UserNotification::MAP_LIKE_NOTIFICATION) && vis.type == Carto::Visualization::TYPE_DERIVED
