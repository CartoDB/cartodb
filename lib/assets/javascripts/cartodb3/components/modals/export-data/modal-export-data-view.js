...
var FORMATS = [
  {
    ...
    fetcher: 'fetchCSV',
    geomRequired: false,
    illustrationIconModifier: 'IllustrationIcon--neutral'
  }, {
    format: 'shp',
    fetcher: 'fetch',
    geomRequired: true,
    illustrationIconModifier: 'IllustrationIcon--magenta'
  }, {
    format: 'kml',
    fetcher: 'fetch',
    geomRequired: true,
    illustrationIconModifier: 'IllustrationIcon--sunrise'
  }, {
    format: 'geojson',
    label: 'geo json',
    fetcher: 'fetch',
    geomRequired: true,
    illustrationIconModifier: 'IllustrationIcon--cyan'
  }, {
    format: 'svg',
    fetcher: 'fetchSVG',
    geomRequired: true,
    illustrationIconModifier: 'IllustrationIcon--royalDark'
  }
];
...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    this.modalModel = opts.modalModel;

    this._initBinds();

    if (this.querySchemaModel.get('status') === 'unfetched') {
      this.querySchemaModel.fetch();
    }
  },

  render: function () {
    var geom = this.querySchemaModel.getGeometry();

    if (this.querySchemaModel.get('status') === 'fetched') {
      this.$el.html(
        template({
          formats: FORMATS,
          url: this.configModel.getSqlApiUrl(),
          isGeoreferenced: !!geom
        })
      );
    } else {
      this._renderLoadingContent(_t('components.export-data.loading.geometry'));
    }
  ...
  _initBinds: function () {
    this.querySchemaModel.bind('change:status', function () {
      if (this.querySchemaModel.get('status') !== 'unavailable') {
        this.render();
      } else {
        this.showError(_t('components.export-data.error.geometry-error'));
      }
    }, this);
  ...
  /**
   * Answer to button event and launch the export method associated to that format
   * @param  {Event} ev
   */
  _export: function (ev) {
  ...
  getBaseOptions: function () {
    return {
      ...
      apiKey: this.configModel.get('api_key')
    };
  },
  ...
  getGeomFilteredSql: function () {
    var sql = this.querySchemaModel.get('query');
    var geom = this.querySchemaModel.getGeometry();
  ...
  _fetch: function (options, sql) {
    ...
    this.$('.js-db').attr('disabled', 'disabled');
    this.$('.js-skipfields').attr('disabled', 'disabled');

    if (this.options.autoClose) {
      this._close();
    } else {
      this._renderLoadingContent(_t('components.export-data.loading.preparing'));
    }
  },
    ...
    function getError (content) {
      ...
      try {
        ...
        if (content && content.indexOf('error') !== -1) {
          error = _t('components.export-data.error.unknown');
        }
      }
      return error;
    }
   ...
   */
  fetchSVG: function () {
    this.$('.js-db').removeAttr('disabled');
    this.fetch('svg');
  },
