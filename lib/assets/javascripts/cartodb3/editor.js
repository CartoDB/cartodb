...
deepInsights.createDashboard('#dashboard', vizJSON, {
  ...
  analysisDefinitionNodesCollection.on('add', function (analysisDefinitionNode) {
    ...
    analysisDefinitionNode.on('change:status', function (m, status) {
      if (status === 'ready') {
        notification.set({
          status: 'success',
          info: 'Analysis ' + analysisDefinitionNode.get('id') + 'completed',
          closable: true
        });
      }
      if (status === 'failed') {
        notification.set({
          status: 'error',
          info: 'Analysis ' + analysisDefinitionNode.get('id') + 'failed',
          closable: true
        });
      }
    });

    analysisDefinitionNode.on('destroy', function () {
      notification.set({
        status: 'success',
        info: 'Analysis ' + analysisDefinitionNode.get('id') + 'was deleted',
        closable: true
      });
    });
  });
