...
module.exports = {
  File: {
    ...
    enabled: function (config, user_data) { return true; },
    name: 'file',
    title: 'Data file',
    options: {
      type: 'url',
  ...
  GDrive:   {
    ...
    enabled: function (config, user_data) { return !!config.get('oauth_gdrive'); },
    name: 'gdrive',
    title: 'Google Drive',
    options: {
      service: 'gdrive',
  ...
  Dropbox: {
    className: ImportService,
    enabled: function (config, user_data) { return !!config.get('oauth_dropbox'); },
    name: 'dropbox',
    title: 'Dropbox',
    options: {
      service: 'dropbox',
  ...
  Box: {
    className: ImportService,
    enabled: function (config, user_data) { return !!config.get('oauth_box'); },
    name: 'box',
    title: 'Box',
  ...
  Twitter: {
    ...
    enabled: function (config, user_data) { return user_data.twitter.enabled && !!config.get('datasource_search_twitter'); },
  ...
  Mailchimp: {
    className: ImportService,
    enabled: function (config, user_data) { return user_data.mailchimp.enabled && !!config.get('oauth_mailchimp'); },
  ...
  Arcgis: {
    ...
    enabled: function (config, user_data) { return config.get('arcgis_enabled'); },
  ...
  Salesforce: {
    className: ImportDataView,
    enabled: function (config, user_data) { return config.get('salesforce_enabled'); },
