  ...
  const visualizationList = csVisualizationList.reduce((acum, elem) => {
    ...
    if (!filtered) {
      acum.push({
        ...
        kind: null,
        external_source: {},
        url: `${__KEPLERGL_BASE_URL__}/demo/map/carto?mapId=${elem.id}&owner=cdb&privateMap=${elem.isPrivate}`, // TODO: Set Kepler.gl URL depending on a env var
        version: 1,
        prev_id: null,
        next_id: null,
        parent_id: null,
...
export function fetchVisualizations (context, parameters) {
  ...
  context.rootState.customStorage.getVisualizations()
    .then(data => {
      ...
      const visData = customStorageToVisualization(data, filter);
