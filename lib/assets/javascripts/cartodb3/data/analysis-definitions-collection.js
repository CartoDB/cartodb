...
module.exports = Backbone.Collection.extend({
  ...
  _onNodeAdd: function (newNodeModel) {
    if (newNodeModel.get('type') !== 'source') {
      ...
      var sourceId = newNodeModel.sourceModel().id;

      var nodesReferencingSourceId = this
        .analysisDefinitionNodesCollection
        .filter(function (m) {
          return m.get('type') !== 'source' && m.sourceModel().id === sourceId;
        });

      if (nodesReferencingSourceId.length === 1) {
