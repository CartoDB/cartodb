...
var HasContextMenu = require('../../mixins/has-context-menu');
 ...
 */
var WidgetView = CoreView.extend({
  ...
  _onMenuSelect: function (menuItem) {
    if (menuItem.get('val') === 'delete-widget') {
      this._confirmDeleteWidget();
    }
    if (menuItem.get('val') === 'rename-widget') {
      this._inlineEditor.edit();
    }
    if (menuItem.get('val') === 'edit-widget') {
      this._onEditWidget();
    }
  },
  ...
  _confirmDeleteWidget: function () {
    var self = this;
    var widgetName = this.model.get('title');
    ...
    this._modals.create(function (modalModel) {
      return new ConfirmationView({
        ...
        runAction: self._onDeleteWidget.bind(self, modalModel)
      });
    });
  },
...
_.extend(WidgetView.prototype, HasContextMenu);
