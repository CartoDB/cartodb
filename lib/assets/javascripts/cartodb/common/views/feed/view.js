...
module.exports = cdb.core.View.extend({
  ...
  _initTemplates: function() {
    ...
    this.mapTemplate = cdb.templates.getTemplate('common/views/feed/map_item_template');
    this.datasetTemplate = cdb.templates.getTemplate('common/views/feed/dataset_item_template');
    this.loaderTemplatePath = 'common/views/feed/loading';
    this.emptyTemplatePath = 'common/views/feed/empty';
    this.errorTemplatePath = 'common/views/feed/error';
  },
  ...
  /*_renderError: function() {
    this.loader = ViewFactory.createByTemplate(this.errorTemplatePath, {
      title: this._LOADER_TITLE,
      quote: randomQuote()
    });
  ...
  _renderVisualizations: function() {
    ...
    this.collection.each(function(item) {
      ...
      var el = template({
        vis: item.attributes,
        username: item.get('permission').owner.username,
        avatar_url: item.get('permission').owner.avatar_url,
        table_count: item.get('permission').owner.table_count,
        maps_count: item.get('permission').owner.maps_count,
        geomType: item.get('table') && item.get('table').geometry_types ? this._getGeometryType(item.get('table').geometry_types) : null,
  ...
  _fetchItems: function(params) {
    var data = _.extend({ types: 'table,derived', per_page: this._ITEMS_PER_PAGE, order: 'updated_at', error: this._onFetchError }, params);
