...
describe('new_common/dialogs/duplicate_vis_view', function() {
  describe('when given a map', function() {
    ...
    describe('when duplication fails', function() {
      beforeEach(function() {
  ...
  describe('when given a dataset', function() {
    beforeEach(function() {
      ...
      this.createData = {};
      spyOn(this.table, 'duplicateDataForNewImport').and.returnValue(this.createData);
      spyOn(this.vis, 'isVisualization');
      spyOn(cdb.admin.Import.prototype, 'save');
    ...
    it('should start the duplication process right away with creating an import', function() {
      expect(cdb.admin.Import.prototype.save).toHaveBeenCalled();
      expect(cdb.admin.Import.prototype.save.calls.argsFor(0)[0]).toBe(this.createData);
    });
  });
});
