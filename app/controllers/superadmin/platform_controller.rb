...
class Superadmin::PlatformController < Superadmin::SuperadminController
  ...
  def databases_info
    hosts = if params[:database_host]
              [params[:database_host]]
            else
              ::User.distinct(:database_host).select(:database_host).all.map(&:database_host)
            end
    dbs = {}
    hosts.each do |h|
      ...
      total_account_types.each do |a|
        next unless users_in_database.to_i > 0
  ...
  def check_for_database_host
    render json: { error: 'Database host parameter is mandatory' }, status: 400 unless params[:database_host]
  end
