...
module.exports = cdb.core.View.extend({
  ...
  _initBinds: function() {
    ...
    this.collection.bind('reset', function(collection) {
      ...
      this._activatePane(collection.total_entries > 0 ? 'list' : 'no_results');
      this._toggleCleanSearchBtn();
    }, this);
  ...
  _initViews: function() {
    ...
    this._panes.addTab('no_results',
      ViewFactory.createByTemplate('common/templates/no_results', {
        ...
        title: 'Oh! No results',
        msg: 'Unfortunately we could not find anything with these parameters'
      })
    );
    ...
    this._activatePane(this._chooseActivePaneName(this.collection.total_entries));
  },
