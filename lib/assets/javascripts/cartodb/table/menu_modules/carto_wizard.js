...
cdb.admin.WizardProperties = cdb.core.Model.extend({
  ...
  linkLayer: function(layer) {
    layer.bind('change:wizard_properties', function() {
      var vars = _.extend({ type: layer.get('wizard_properties').type, }, layer.get('wizard_properties').properties);
      this.set(vars);
    }, this);

    this.bind('change', function() {
      layer.set('wizard_properties', {
        type: this.get('type'),
        properties: _.omit(this.attributes, 'type')
      });
    }, layer);
  },
