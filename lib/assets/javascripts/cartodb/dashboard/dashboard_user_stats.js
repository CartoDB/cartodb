...
cdb.admin.dashboard.UserStats = cdb.core.View.extend({
  ...
  _removeLimitMessage: function() {
    ...
    if (tableLimit === null && spaceLimit === null) {
      this.messages.removeMessage("limits_exceeded");
      this.messages.resetClosedStatus("limits_exceeded");
    }

    if (mapviewsLimit === null) {
      this.messages.removeMessage("limits_mapviews_exceeded");
      this.messages.resetClosedStatus("limits_mapviews_exceeded");
    }
  ...
  _onMapViewsQuotaStatusChange: function(m) {
    ...
    if (status == "boom") {
      var overage_quota = this._chooseOverageQuota();
  ...
  _chooseOverageQuota: function() {

    var account_type = this.model.get("account_type").toLowerCase();

    if      (account_type.indexOf("john snow") != -1 ) return "0.2";
    else if (account_type.indexOf("magellan")  != -1 ) return "0.2";
    else if (account_type.indexOf("mercator")  != -1 ) return "0.2";
    else if (account_type.indexOf("coronelli") != -1 ) return "0.1";
    else if (account_type.indexOf("dedicated") != -1 ) return "0.1";
    else if (account_type.indexOf("free")      != -1 ) return "0.2";
    else return "0.2";
  ...
  _chooseBadge: function() {
    ...
    else if (account_type.indexOf("xs") != -1 || account_type.indexOf("s") != -1 || account_type.indexOf("m") != -1 || account_type.indexOf("l") != -1) return "dedicated";
  ...
  render: function() {
    ...
    this.$el.html(this.template(_.extend(this.model.toJSON(), { badge: this._chooseBadge() } )));
  ...
  _checkUpgradedUser: function() {
    ...
    if (config && !config.custom_com_hosted && config.account_host) {
      ...
      if (show_upgraded_message) {
        ...
        cdb.god.trigger('mixpanel','Account upgraded successfully');
      } else {
