namespace :user do
  namespace :deletion do
    ...
    task :by_username, [:username] => [:environment] do |_task, args|
      ...
      raise 'Deletion aborted due to bad confirmation' unless deletion_confirmed?(user)

      user.destroy
    end

    desc 'Delete a user given an email'
    task :by_email, [:email] => [:environment] do |_task, args|
  ...
  namespace :change do
    ...
    task :max_layers, [:username, :max_layers] => [:environment] do |_task, args|
    ...
    def install_ghost_tables_trigger(user_id)
      ...
      if user.nil?
        ...
        begin
          ...
          puts "ERROR creating ghost tables trigger for user #{user_id}: #{e.message}"
        end
      end
    end
  ...
  namespace :notifications do
    ...
    task :add_by_field, [:filter_field, :filter_value, :notification] => [:environment] do |_task, args|
      allowed_fields = ['database_host']
      raise 'Filter field and value are needed' if args[:filter_field].nil? || args[:filter_value].nil?
      unless allowed_fields.include?(args[:filter_field])
        raise "Unknown field #{args[:filter_field]} for filtering. Allowed fields are #{allowed_fields.join(',')}"
      end
    ...
    task :clean_by_field, [:filter_field, :filter_value] => [:environment] do |_task, args|
      allowed_fields = ['database_host']
      raise 'Filter field and value are needed' if args[:filter_field].nil? || args[:filter_value].nil?
      unless allowed_fields.include?(args[:filter_field])
        raise "Unknown field #{args[:filter_field]} for filtering. Allowed fields are #{allowed_fields.join(',')}"
