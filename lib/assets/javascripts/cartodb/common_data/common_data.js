...
$(function() {
  ...
  var CommonData = cdb.core.View.extend({
    ...
    initialize: function() {
      ...
      this.model = new cdb.admin.CommonData.Model();
    ...
    _initBinds: function() {
      ...
      this.model.bind('change:datasets', this._onRouterChange, this);
    ...
    _onRouterChange: function() {
      ...
      this.model.get('categories') && this.model.get('categories').each(function(c) {
        if (c.get('name') === self.router.model.get('tag')) {
          c.set('selected', true)
        } else {
