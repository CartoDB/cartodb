  ...
  cdb.admin.mod.InfowindowTitlePane = cdb.admin.mod.InfowindowBasePane.extend({
    ...
    renderFields: function() {
      ...
      this.nameAlias = this.getColumnAliasNames();
      var fields = [];
      ...
      _(names).each(function(f) {
        ...
        if (self.model.containsAlias(f)) {
          var pos = _.indexOf(_(self.model.get('fields')).pluck('alias'), f);
          title = self.model.get('fields')[pos] && self.model.get('fields')[pos].title;
        }

        var obj = { name: f, title: title, alias: self.nameAlias[f] || null};
