...
module.exports = BaseDialog.extend({
  ...
  _initViews: function() {
    ...
    _.bindAll(this, "_onSuccessQuery", "_addCard", "_generateThumbnail", "_analyzeColumn", "_analyzeColumns", "_refreshMapList", "_onLoadWizard", "_analyzeStats");
  ...
  _start: function() {
    ...
    this.sql.columns(this.query, this._onSuccessQuery);
  },

  _onSuccessQuery: function(columns) {
    ...
    var bbox = this.options.map.getViewBounds();
    var geometryType = this._getGeometryType();

    _(columns).each(function(type, name) {
      this.columns.add({ name: name.concat(""), type: type, geometry_type: geometryType, bbox: bbox, analyzed: false });
    }, this);
  ...
  _onCardClick: function() {
    ...
    if (this.layer) {
      ...
      this.layer.set("analyzed", true);

      this._bindDataLayer();

      var properties = this.layer.wizard_properties.propertiesFromStyle(this.model.get("response").css);
      this.layer.wizard_properties.active(this.model.get("response").visualizationType, properties); 
      console.log("%cApplying wizard: " + this.model.get("response").visualizationType, "font-weight: bold; color:brown;");
    }
  },

  _getDataLayer: function() {
    var dataLayers = this.map.layers.getDataLayers();
    return dataLayers[0];
  },
