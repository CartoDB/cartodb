...
describe('deep-insights-integrations', function () {
  ...
  describe('when a layer is moved', function () {
    beforeEach(function () {
      this.layerDefinitionsCollection.save = function (opts) {
        opts.success();
      };

      this.analysisDefinitionsCollection.findByNodeId = {};

      spyOn(this.analysisDefinitionsCollection, 'findByNodeId').and.returnValue(false);

      // Clean layers in the CartoDB.js side
      this.integrations.visMap().layers.reset([]);

      // Set two new layers in both sides
      this.layerDefinitionsCollection.add({
        id: 'integration-test0',
        type: 'plain',
        color: 'blue',
        order: 1
      });
    ...
    it('should remove model and create a new one with the same id', function () {
      var currentModel = this.integrations.visMap().layers.at(1);
      this.layerDefinitionsCollection.trigger('layerMoved', this.layerDefinitionsCollection.at(1), 0);
      var newModel = this.integrations.visMap().layers.at(1);
      expect(newModel.cid).not.toBe(currentModel.cid);
      expect(newModel.get('source')).not.toBeDefined();
    });

    it('should keep source attribute if there is any associated analysis', function () {
      this.analysisDefinitionsCollection.findByNodeId.and.returnValue(true);
      this.layerDefinitionsCollection.trigger('layerMoved', this.layerDefinitionsCollection.at(1), 0);
      var newModel = this.integrations.visMap().layers.at(1);
      expect(newModel.get('source')).toBeDefined();
      console.log(newModel, this.integrations.visMap().layers.toJSON()); // eslint-disable-line
    });
  });
});
