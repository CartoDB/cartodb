...
var getBasemapAttrs = function (basemap, category, currentClassName, assetsBaseURL) {
  var basemapAttrs = {};
  ...
  _.extend(basemapAttrs, {
    type: type,
    category: category,
    selected: currentClassName === basemap.className, // TODO: Can we use the name?
    val: basemap.className, // Why is this repeated?
    label: basemap.name,
    template: function () {
      var imgURL = getThumbnailURL(basemap, assetsBaseURL);
...
var getThumbnailURL = function (basemap, assetsBaseURL) {
  var imgURL;
  if (basemap.url) {
    imgURL = lowerXYZ(basemap.url, basemap.subdomains);
  } else {
    imgURL = [
      assetsBaseURL,
      'unversioned/google-maps-basemap-icons',
      basemap.className + '.jpg'
    ].join('/');
  }
  return imgURL;
};
  ...
  _initCollections: function (basemaps) {
    ...
    _(basemaps).each(function (categoryBasemaps, category) {
      _.map(categoryBasemaps, function (basemap) {
        var basemapAttrs = getBasemapAttrs(basemap, category, this._baseLayer.get('className'), this._configModel.get('app_assets_base_url'));
