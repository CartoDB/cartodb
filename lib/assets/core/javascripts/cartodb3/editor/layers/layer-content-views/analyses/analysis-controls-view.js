...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    var apiState = DataServicesApiCheck.get().getState();
    this._fetchQuotaIfNeeded(apiState, function () {
  ...
  _fetchQuotaIfNeeded: function (apiState, notNeededCallback) {
    var type = this._formModel.get('type');
    var apiIsReady = apiState === 'ready';
    var quotaIsReady = this._quotaInfo.isReady();
    if (apiIsReady && (AnalysesQuotaOptions.requiresQuota(type, this._quotaInfo) || !quotaIsReady)) {
      this._fetchQuota();
    } else {
      notNeededCallback();
    }
