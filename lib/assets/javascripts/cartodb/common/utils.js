  ...
  /*
   *  Trim methods
   *  input ->  string (example: '<a href="#whoknows" onClick="alert('jamon')">Jamon</a>')
   *
   *  return -> '<a href="#whoknows">Jamon</a>'
   */

  //cdb.Utils.trim = function(str){
    //return str.replace(/^\s+|\s+$/g, '');
  //};

  cdb.Utils.ltrim = function(str){
    return str.replace(/^\s+/,'');
  };

  cdb.Utils.rtrim = function(str){
    return str.replace(/\s+$/,'');
  };

  cdb.Utils.fulltrim = function(str){
    return str.replace(/(?:(?:^|\n)\s+|\s+(?:$|\n))/g,'').replace(/\s+/g,' ');
  };
