...
$(function() {
  cdb.init(function() {
    ...
    // API keys
    var regenerateApiKeyHandler = function(that, ev, scope, form_action, authenticity_token) {
    ...
    $('.js-regenerateOrganizationUsersApiKey').bind('click', function(ev) {
      regenerateApiKeyHandler($(this), ev, 'organization', '/organization/regenerate_api_key');
    });

    $('.OrganizationUser-renewApiKeyLink').bind('click', function(ev) {
      var user_id = $(this).attr('data-cdb-username');
      var current_viewer_username = $(this).attr('data-cdb-current_viewer_username');
      var authenticity_token = $('[name=authenticity_token][value]').get(0).value;
      regenerateApiKeyHandler($('.js-regenerateOrganizationUserApiKey'), ev, 'organization_user', '/u/' + current_viewer_username + '/organization/users/' + user_id + '/regenerate_api_key', authenticity_token);
    });
  });
