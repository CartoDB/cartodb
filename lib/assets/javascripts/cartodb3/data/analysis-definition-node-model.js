...
module.exports = Backbone.Model.extend({
  ...
  /**
   * Clone own node and underlying nodes (while belonging to same letter)
   * @example
   *   var b3; // => [b3->b2->b1->a1->a0]
   *   var c3 = b3.cloneTreeWithSameLetter('c') // => [c3->c2->c1->a1->a0]
   * @param {String} newLetter - e.g. 'c'
   * @return {Object} instance of analysis-definition-node-model
   */
  cloneTreeWithSameLetter: function (newLetter) {
    ...
    this._getPrimarySourcesWithSameLetter(nodeIds.letter(this.id))
  ...
  _getPrimarySourcesWithSameLetter: function (letter) {
    if (nodeIds.letter(this.id) === letter) {
      var primarySource = this.getPrimarySource();
