...
cdb.admin.Header = cdb.core.View.extend({
  ...
  _changePrivacy: function(e) {
    ...
    if (isVis) {
      if (this.options.user.isInsideOrg()) {
        if (isOwner) this._showPrivacyDialog();
      } else {
        this._shareVisualization(e);
      }
    } else {
      if (isOwner) this._showPrivacyDialog();
    }
  ...
  _setDescription: function() {
    var isEditable = this.isMetadataEditable();
    var default_description = ( isEditable ? this._TEXTS.description.normal : this._TEXTS.description.query);
    ...
    content = content.replace(/<a (.*?)a>/gi, "<span class='link' $1span>")

    if (content && !content.trim()) content = default_description;

    this.$('div.description p')
      .html(isEditable ? '<a href="#/change-description">' + content + '</a>' : content );
  },
  ...
  _changeDescription: function(e) {
    ...
    if (this.isMetadataEditable()) {
      ...
      var description_dialog = this.description_dialog = new cdb.admin.EditMarkdownDialog({
        ...
        onResponse: function(val) {
          ...
          var description = cdb.Utils.removeHTMLEvents( cdb.Utils.stripHTML(val.trim()));
      ...
      var pos = $(e.target).closest(".description p").offset();
      pos.left -= $(window).scrollLeft();
      pos.top -= $(window).scrollTop();
      var w = Math.max($(e.target).width() + 100, 480);
      description_dialog.showAt(pos.left - 5, pos.top - 15, w);
    }
  },
