...
export function createEmptyDataset (context) {
  const path = ['api/v1/tables'];
  const opts = {
    dataType: 'json',
    data: JSON.stringify({
    })
...
export async function createEmptyVisualization (context) {
  const { name } = await createEmptyDataset(context);
  return createVisualizationFromDataset(context, [name]);
}
