...
describe("Pecan", function() {
  ...
  describe("Dialog", function() {
    ...
    it("should attempt to guess the map once all the colums are analyzed", function() {

      spyOn(this.view, "_guessMaps");

      this.view.columns.each(function(col) {
        col.set({ analyzed: true, success: true})
      })

      expect(this.view._guessMaps).toHaveBeenCalled();
    });

    it("should analyze each one of the non-excluded columns", function() {
    ...
    it("should return ok if the table is valid", function() {
      expect(this.view._check()).toBe(true);
    });

    it("should return false if the table is not valid", function() {
    ...
    it("should reject string columns whose weight is < 0.1", function() {
      var name = "string";
      var stats = { weight: 0.001 }
      expect(this.view._analyzeString(name, stats)).toBe(0);
    });

    it("should reject string columns whose null_ratio is > 95%", function() {
      var name = "string";
      var stats = { null_ratio: 0.972 }
      expect(this.view._analyzeString(name, stats)).toBe(0);
    });

    it("should accept string columns whose weight is > 0.8", function() {
      var name = "string";
