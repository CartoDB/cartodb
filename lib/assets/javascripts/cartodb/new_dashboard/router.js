...
module.exports = Router.extend({
  ...
  initialize: function(opts) {
    ...
    this.model = new RouterModel({
      rootUrlForCurrentTypeFn: this.rootUrlForCurrentType.bind(this)
      // Actual attributes will be set by one of the route handlers below, upon router.enableAfterMainView()
    });
  },

  /**
   * Get the root url representation for current content-type (i.e. datasets or maps).
   * @returns {Object} instanceof new_common/urls/user/abstract_dasboard_model url
   */
  rootUrlForCurrentType: function() {
    // TODO: Some views load before router is setup, so content_type might not be set, so fallback on a default to not
    // throw errors... apparently can't set a default value on the model directly, because then nothing works, why is that?
    var type = this.model.get('content_type') || 'datasets';
    return this.currentUserUrl[type +'Url' ]();
  },
