...
var FeatureDefinitionModel = Backbone.Model.extend({
  ...
  initialize: function (attrs, options) {
    ...
    this._nodeDefModel = this._layerDefinitionModel.getAnalysisDefinitionNodeModel();
  ...
  _isCustomQueryApplied: function () {
    ...
    return sanitizeName(query) !== sanitizeName('select * from ' + this._layerDefinitionModel.getTableName());
  },

  isEditable: function () {
    var hasAnalyses = this._layerDefinitionModel.hasAnalyses();
    var isCustomQueryApplied = this._isCustomQueryApplied();

    return !hasAnalyses && !isCustomQueryApplied;
  },
  ...
  saveFeature: function (options) {
    var self = this;

    var columnsCollection = this._querySchemaModel.columnsCollection;
    ...
    this._getQueryRowModel().save(attrs, {
      ...
      error: function () {
        self.trigger('saveFeatureFailed');
      }
    });
  },
