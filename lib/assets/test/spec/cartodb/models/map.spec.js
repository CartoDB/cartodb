...
describe("cartodb.models.Map", function() {
  ...
  fdescribe('.setBaseLayer', function() {

    var baseLayerAddedCallback;
    var savingLayersFinish;

    beforeEach(function() {
      baseLayerAddedCallback = jasmine.createSpy('baseLayerAddedCallback');
      map.bind('baseLayerAdded', baseLayerAddedCallback);
    ...
    it('should set the base layer as the first layer', function(done) {
      ...
      setTimeout(function() {
        ...
        expect(baseLayerAddedCallback).toHaveBeenCalled();
        expect(savingLayersFinishCallback).toHaveBeenCalled();
        done();
      }, 0);
    ...
    fit('should add a layer with labels at the top if basemap has labels', function(done) {
      var baseLayer = new cdb.admin.CartoDBLayer({
        tile_style: 'test1',
        query: 'sql1',
        interactivity: 'int1',
      ...
      expect(map.layers.at(2).attributes).toEqual({

      });
    })

    it('should remove the layer with labels if new basemap has no labels', function() {

    })

    it('should replace layer with labels if previous basemap had labels and new basemap has labels', function() {
  ...
  describe("layers", function() {
    ...
    it("should save layers sequentially", function(done) {
      var map = new cdb.admin.Map();
      var layers = new cdb.admin.Layers();
      var layer1 = new cdb.geo.CartoDBLayer({ order: 1 });
      var layer2 = new cdb.geo.CartoDBLayer({ order: 2 });
      var steps = [];

      layer1.sync = function(a,b,opts) {
        steps.push('saving1');
        setTimeout(function(){
          steps.push('saved1');
          opts.success();
      ...
      layers.saveLayers({
        success: function() {
          ...
          expect(steps[0]).toBe('saving1');
          expect(steps[1]).toBe('saved1');
          expect(steps[2]).toBe('saving2');
          expect(steps[3]).toBe('saved2');
          done();
        }
      });
