  ...
  describe("Visualization model", function() {
    ...
    describe('> transform to visualization', function() {
      ...
      it("should turn to visualization correctly", function() {
        cdb.geo.Map.prototype.sync = function(method, m, opts) {
          m.set({ id: 69 }, { silent: true })
          opts.success({
            id: 69
          }, 200);
        }
        ...
        spyOn(vis.map, 'sync');
        vis.changeToVisualization();
        expect(vis.map.sync).toHaveBeenCalled();
      });
