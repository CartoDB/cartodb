  ...
  cdb.admin.GeocoderStats = cdb.core.View.extend({
    ...
    _TEXTS: {
      available_hard: _t('You have <strong><%= quota - monthly_use %> geocoding credit<%= quota - monthly_use > 1 ? "s" : "" %> available</strong>. Depending on your data you might run out of credits, if so consider <a href="<%= upgrade_url %>">upgrading to a higher plan</a>.'),
      available_soft: _t('You have <strong><%= quota - monthly_use %> geocoding credit<%= quota - monthly_use > 1 ? "s" : "" %> available</strong>. Depending on your data you might run out of credits and you will be charged $<%= block_price %> for every 1,000 extra geocoding credits.'),
