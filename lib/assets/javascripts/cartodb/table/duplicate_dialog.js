...
cdb.admin.DuplicateTable = cdb.admin.BaseDialog.extend({
  ...
  render_content: function() {
    ...
    $content.append(template({ queryApplied: this.model.isInSQLView() }));
  ...
  _startDuplication: function() {
    ...
    if (this.model.isInSQLView()) {
      var query = this.model.sqlView.getSQL();
      data['query'] = !query || query == "" ? query : "SELECT * FROM " + this.model.get('name');
    } else {
