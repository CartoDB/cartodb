...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _showPecan: function() {
    ...
    var hasData = this.options.table && this.options.table.data() ? true : false;

    if (!hasData) {
      return;
    }

    var geometryTypes = this.options.table.data().table && this.options.table.data().table.get("stats_geometry_types");
    var hasGeometries = geometryTypes && geometryTypes.length > 0 ? true : false;

    var row_count = this.options.table ? this.options.table.data().length : 0;
    var hasRows = row_count > 0 && row_count < this.MAX_PECAN_ROWS;

    if (hasPecan && hasNewModals && hasData && hasGeometries && hasRows) {

      var skipPencanDialog = 'pecanDialog' + this.options.table.id + '_' + this.options.table.get('map_id');
  ...
  _launchPecan: function() {

    var skipPencanDialog = 'pecanDialog' + this.options.table.id + '_' + this.options.table.get('map_id');

    this.pecanDialog = new cdb.editor.PecanView({
      clean_on_hide: true,
      query_schema: this.options.table.data().query_schema,
      table: this.options.table,
      map: this.map,
      user: this.user
    });

    this.pecanDialog.bind("skip", function() {
      localStorage[skipPencanDialog] = true;
    });

    this.pecanDialog.appendToBody();

  },

  _showScratchDialog: function() {
    if (this.user.featureEnabled('new_modals') && this.options.table && this.options.table.data().fetched && this.options.table.data().length === 0) {
  ...
  _editData: function(row) {
    ...
    if (!this.table.isReadOnly()) {
      row.fetch({ cache: false, no_geom: true, success: function() {
        ...
        if (self.user.featureEnabled('new_modals')) {
          dlg = new cdb.editor.FeatureDataView({
            row: row,
          ...
          dlg = new cdb.admin.EditFeatureFields({
            model: row,
            table: self.table,
            res: function(data) {
              self.table.notice('Saving ... ', 'load');
              // Set row model
              $.when(row.save(data)).done(function(a){
        ...
        dlg
          .appendToBody()
          .open({ center: true }); // To be removed when old dialog is deprecated
