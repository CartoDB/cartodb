...
function blending (b, geometryType, styleType) {
  var css = {};
...
var cartocssFactory = {
  ...
  blending: {
    point: function (attrs, styleType) { return blending(attrs, 'marker', styleType); },
...
function renderBlock (block, geometryType, styleType) {
  var css = {};
  for (var k in block) {
    var f = cartocssFactory[k];
    if (f) {
      css = _.extend(css, f[geometryType](block[k], styleType));
    }
  }
...
function generateCartoCSS (style, geometryType) {
  ...
  css += '#layer {\n';
  css += renderBlock(_.omit(styleDef, 'animated', 'labels'), geometryType, style.type);
  css += '}';
