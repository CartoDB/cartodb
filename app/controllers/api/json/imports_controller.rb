...
class Api::Json::ImportsController < Api::ApplicationController
  ...
  INVALID_TOKEN_MESSAGE = 'OAuth token invalid or expired'.freeze
  ...
  def create
    @stats_aggregator.timing('imports.create') do
      ...
      raise CartoDB::Importer2::UserConcurrentImportsLimitError if concurrent_import_limit.is_over_limit!
      ...
      log_info(message: 'Error: create', exception: e)
  ...
  def invalidate_service_token
    @stats_aggregator.timing('imports.invalidate-service-token') do
      ...
      raise CartoDB::Datasources::AuthError, "No oauth set for service #{params[:id]}" if oauth.nil?

      datasource = oauth.get_service_datasource
      if datasource.nil?
        raise CartoDB::Datasources::AuthError.new("Couldn't fetch datasource for service #{params[:id]}")
      end
      unless datasource.is_a? CartoDB::Datasources::BaseOAuth
      ...
      log_info(message: 'Error: invalidate_service_token', exception: e)
      render_jsonp({ errors: { imports: e.message } }, 400)
    end
  end
