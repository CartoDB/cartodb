...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _removeOverlayPropertiesBar: function(show_toolbar) {
  ...
  _showToolbarOptions: function() {
    $(".map_toolbar ul.options").show();
  },

  _hideToobarOptions: function() {
    $(".map_toolbar ul.options").hide();
  },

  _addOverlayPropertiesBar: function(model, form_data) {

    this._hideToobarOptions();
    this._removeOverlayPropertiesBar();
    ...
    if (!this.overlayPropertiesBar) {
      ...
      this.overlayPropertiesBar = new cdb.admin.OverlayPropertiesBar({
        ...
        vis: this.vis,
        form_data: form_data,
        template_base: "table/views/overlays/overlay_properties_bar",
      });
      ...
      this.overlayPropertiesBar.bind("increase-zindex", this._increaseOverlayZIndex, this);
      this.overlayPropertiesBar.bind("decrease-zindex", this._decreaseOverlayZIndex, this);

      this.overlayPropertiesBar.bind("delete-overlay", function(model) {
        self.overlays.remove(model);
      }, this);

      this.overlayPropertiesBar.bind("copy-overlay", function(model) {
        self._duplicateOverlay(model);
      }, this);

      $(".map_toolbar").append(this.overlayPropertiesBar.render().el);
  ...
  _getOverlaysZIndex: function() {
    ...
    var canvas_mode = this.canvas_mode;

    var overlays = this.overlays.filter(function(o) { return o.get("device") === canvas_mode && o.get("type") == "text" });
  ...
  _duplicateOverlay: function(model) {
    ...
    var x = model.get("x") + Math.round(Math.random()*50);
    var y = model.get("y") + Math.round(Math.random()*50);

    var indexes = this._getOverlaysZIndex();
    var zIndex   = _.max(indexes);

    var style = m.get("style");
    style["z-index"] = zIndex++;

    m.set({ id: null, x: x, y: y, style: style });
