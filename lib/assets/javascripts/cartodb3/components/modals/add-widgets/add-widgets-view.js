...
module.exports = cdb.core.View.extend({
  ...
  initialize: function (opts) {
    ...
    this._analysisDefinitionNodesCollection.each(function (m) {
      this._analysisDefinitionNodesCollection.prepareNode(m);
    }, this);

    if (!this._hasFetchedAllAnalysisNodes()) {
      var isNotFetched = _.compose(_.negate(Boolean), this._isFetched);
      this._layerTablesChain()
        .filter(isNotFetched)
        .each(function (m) {
          this.listenToOnce(m, 'sync', this._onAnalysisTableFetched);
          this.listenToOnce(m, 'error', this._onAnalysisTableFetchError);
          m.fetch();
        }, this);
    }
  ...
  _hasFetchedAllAnalysisNodes: function () {
  ...
  _layerTablesChain: function () {
    ...
    .reduce(function (memo, m) {
      var tableModel = m.analysisTableModel;
      if (tableModel && tableModel.get('query') && tableModel.get('status') === 'ready') {
        memo.push(m.analysisTableModel);
      }
      return memo;
    }, []);
  },

  _onAnalysisTableFetched: function () {
    if (this._hasFetchedAllAnalysisNodes()) {
      this.render();
    }
  },

  _onAnalysisTableFetchError: function () {
