...
describe('common/views/explore/feed_collection', function() {
  ...
  it('should generate a query', function() {
    var query = 'WITH m As (SELECT max(visualization_likes) As max_likes, max(visualization_mapviews) As max_views FROM visualizations) SELECT user_avatar_url AS avatar_url,user_username AS username,visualization_geometry_types AS geom_types,visualization_id AS id,visualization_likes AS likes,visualization_mapviews AS mapviews,visualization_mapviews::numeric / (1.0 + (now()::date - visualization_created_at::date)::numeric)^2 * 100.0 / m.max_views As mapviews_trend,visualization_likes::numeric / (1.0 + (now()::date - visualization_created_at::date)::numeric)^2 * 100.0 / m.max_likes As likes_trend,visualization_name AS name,visualization_map_tables AS map_tables,visualization_table_rows AS rows,visualization_table_size AS table_size,visualization_tags AS tags,visualization_title AS title,visualization_type AS type,visualization_created_at AS created_at,visualization_updated_at AS updated_at FROM visualizations, m  ORDER BY likes_trend DESC, created_at DESC, visualization_id DESC LIMIT 8 OFFSET 0';
