...
module.exports = Backbone.Model.extend({
  ...
  _initBinds: function () {
    ...
    this.listenTo(this.measurements.stateModel, 'change:state', this._tryToUpdateSchema);
    ...
    this.listenTo(this.measurements, 'maybeFiltersUpdated', this._fetchMeasurementsWithFilters);
  ...
  _fetchMeasurementsWithFilters: function () {
    var area = this._form.model.get('area');
    var selectedFilters = this.filters.getSelected();

    var filters = _.map(selectedFilters, function (filter) {
      return filter.getValue();
    }).concat(area).join(',');

    var fetchOptions = {
      filters: braces(filters)
    };

    // whenever the state changes, the analysis form model render the form to draw a loading in the editor.
    // But when a filter is applied, we don't want to rerender the component (it closes the dropdown)
    this._updateSchema = false;
    return this.measurements.fetch(fetchOptions);
  },

  _tryToUpdateSchema: function () {
    var state = this.measurements.stateModel.get('state');
  ...
  _fetchMeasurements: function () {
    var area = this._form.model.get('area');
    var fetchOptions = {
      filters: area && braces(area)
    };
    return $.when(this.measurements.fetch(fetchOptions), this.filters.fetch(fetchOptions));
  },
    ...
    var schema = {
      segment_name: {
        ...
        editorAttrs: {
          ...
          loading: this.measurements.stateModel.get('state') === 'fetching' // FIXME (filters as well)
        },
        dialogMode: 'float',
