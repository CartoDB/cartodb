...
module.exports = Backbone.Model.extend({
  ...
  toJSON: function () {
    ...
    if (this.get('widget_style_definition')) {
      ...
      if (this.get('auto_style_definition')) {
        ...
        if (autoStyleDefinition && autoStyleDefinition.fill) {
          autoStyleDefinition.fill.color = _.omit(autoStyleDefinition.fill.color, 'domain', 'bins');
        }
  ...
  getDefaultAutoStyle: function (type, columnName) {
    ...
    if (type === 'category') {
      ...
      attrs.fill.color.domain = widgetDomain;
    } else {
      attrs.fill.color.bins = type === 'time-series' ? 256 : defaultWidgetRampSize;
    }
