...
cdb.admin.GeoreferenceEstimations = Backbone.Collection.extend({
  ...
  url: "/api/v1/geocodings/estimation_for/",
  ...
  cdb.admin.GeocoderStats = cdb.core.View.extend({
    ...
    _TEXTS: {
      ...
      available_hard: _t('<p><%= credits %> geocoding credit<%= credits > 1 ? "s" : "" %> still available.</p>' +
                         '<% if (cost && cost > 0) { %><p><strong>We calculate that geocoding this table completely could cost you up to <span class="cost">$<%= cost %></span>.</strong></p><% } %>'),

      available_soft: _t('<p><%= credits %> geocoding credit<%= credits > 1 ? "s" : "" %> still available.</p>' + 
                         '<% if (cost && cost > 0) { %><p><strong>We calculate that geocoding this table completely could cost you up to <span class="price">$<%= cost %></span>.</strong></p><% } %>'),
