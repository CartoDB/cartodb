...
module.exports = BaseAnalysisFormModel.extend({
  ...
  _validateRightSourceColumn: function (value, formValues) {
    var rCol = formValues.right_source_column;

    if (rCol === void 0) return false;

    var error = {
      type: 'right_source_column',
      message: _t('editor.layers.analysis-form.should-match')
    };

    var lCol = formValues.left_source_column;
    var lType = lCol && this._leftColumnOptions.findColumn(lCol).type;
    var rType = rCol && this._rightColumnOptions.findColumn(rCol).type;

    if (lCol && rCol && lType === rType) {
