  ...
  cdb.admin.mod.ColorMapWizard = cdb.admin.mod.SimpleWizard.extend({
    ...
    _TEXTS: {
      ...
      no_valid_columns: _t('There are no valid columns on your table to make a color visualization. \
                            Try to add a new one or change the existant ones to a valid type.')
    },
    ...
    render: function() {
      ...
      if (this._getColorColumns().length > 0) {
        ...
        $content.append(this.column_msg.render().el);
    ...
    isValid: function() {
      ...
      return column && this._getColorColumns().length > 0;
    },
    ...
    _addViews: function() {
      ...
      // Select column message view
      this.column_msg = new cdb.admin.mod.ColorMapWizard.ColumnMsgView();
      this.addView(this.column_msg);
    ...
    _setupColors: function() {
      ...
      if (this.cartoProperties.get('property') && this.options.table.containsColumn(property)) {
        ...
        this.column_msg.hide();
      } else {
        this.column_msg.show();
      }
  ...
  /**
   *  Select column message view
   *  - When a column is not selected.
   */

  cdb.admin.mod.ColorMapWizard.ColumnMsgView = cdb.core.View.extend({
    ...
    initialize: function() {
      this.template = cdb.templates.getTemplate('table/menu_modules/wizards/views/color_wizard_column_msg');
    },
