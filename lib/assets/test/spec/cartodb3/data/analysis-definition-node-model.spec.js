...
describe('data/analysis-definition-node-model', function () {
  ...
  describe('when model is a source', function () {
    ...
    describe('validation', function () {
      ...
      it('should add an analysis table model', function () {
        expect(this.model.analysisTableModel).toBeDefined();
        expect(this.model.analysisTableModel.get('query')).toEqual('SELECT * FROM bar');
        expect(this.model.analysisTableModel.get('status')).toEqual('ready');
      });
    });
  ...
  describe('when an analysis has sources', function () {
    ...
    it('should add an analysis table model', function () {
      expect(this.model.analysisTableModel).toBeDefined();
      expect(this.model.analysisTableModel.get('query')).toEqual(null);
      expect(this.model.analysisTableModel.get('status')).toEqual('ready');
    });
  ...
  describe('when an analysis has an analysis node', function () {
    beforeEach(function () {
      this.model = new AnalysisDefinitionNodeModel({
        id: 'x1',
        type: 'trade-area',
        ...
        sqlAPI: {},
        configModel: {},
        analysisNode: new Backbone.Model(),
        collection: this.collection
      });
    });

    it('should update its analysis table model', function () {
      expect(this.model.analysisNode).toBeDefined();
    });

    it('should update its analysis table model', function () {
      this.model.analysisNode.set('query', 'SELECT * FROM foo;');
      expect(this.model.analysisTableModel.get('query')).toEqual('SELECT * FROM foo;');

      this.model.analysisNode.set('status', 'failed');
      expect(this.model.analysisTableModel.get('status')).toEqual('failed');
    });
  });
});
