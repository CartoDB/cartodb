...
describe CartoDB::DataMover::ExportJob do
  ...
  let(:first_user) do
    create_user(
      ...
      database_timeout: 123_450,
      user_timeout: 456_780,
      table_quota: nil
    )
  end
  ...
  describe 'a standalone user which has moved to an organization' do
    ...
    subject do
      ...
      CartoDB::DataMover::ImportJob.new(
        file: @tmp_path + "user_#{first_user.id}.json", mode: :rollback, drop_database: true, drop_roles: true
      ).run!
      CartoDB::DataMover::ImportJob.new(
        file: @tmp_path + "user_#{first_user.id}.json", mode: :import, host: '127.0.0.2', target_org: @org.name
      ).run!
  ...
  it 'should not touch an user metadata nor update its oids when update_metadata is not set' do
    ...
    CartoDB::DataMover::ImportJob.new(
      ...
      drop_database: true, drop_roles: true
    ).run!
