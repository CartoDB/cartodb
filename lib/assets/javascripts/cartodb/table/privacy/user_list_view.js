...
cdb.admin.UserOrgList = cdb.core.View.extend({
  ...
  _renderOrganization: function() {
    var v = new cdb.admin.UserOrgItem({
      ...
      permission: this.permission,
      vis:        this.model,
      template:   'table/views/privacy_dialog/user_list_all'
    });
    this.addView(v);
    this.$el.append(v.render().el);
  },
...
cdb.admin.UserOrgItem = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.permission = this.options.permission;
    this.template = cdb.templates.getTemplate( this.options.template || 'table/views/privacy_dialog/user_list_item_view' );
    this._initBinds();
  },
  ...
  toggleReadOnly: function(e) {
    this.killEvent(e);
    if (this.permission.getPermission(this.model)) {
  ...
  _switchFields: function() {
    ...
    if (perms) {
      ...
      $canwrite.removeClass('disabled enabled').addClass( perms === 'rw' ? 'enabled' : '' );
      $info.removeClass('disabled');
    } else {
      $canwrite.removeClass('disabled enabled').addClass( !org_privacy ? '' : 'disabled' );
