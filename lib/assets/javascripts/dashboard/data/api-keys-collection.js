...
module.exports = Backbone.Collection.extend({
  ...
  initialize: function (models, options) {
    ...
    this._paginationModel = new PaginationModel({
      per_page: 5,
      current_page: 1,
      type: options.type || [apiKeysCollectionTypes.REGULAR]
    });
  ...
  url: function () {
    const urlParams = {
      ...
      type: this._paginationModel.get('type')
    };
  ...
  getMasterKey: function () {
    return this.findWhere({ type: apiKeysCollectionTypes.MASTER });
  },

  getDefaultKey: function () {
    return this.findWhere({ type: apiKeysCollectionTypes.DEFAULT });
  },

  getRegularKeys: function () {
    return this.where({ type: apiKeysCollectionTypes.REGULAR });
  },
