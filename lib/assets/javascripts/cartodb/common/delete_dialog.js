...
cdb.admin.DeleteDialogItem = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    if (this.model.get("type") == 'dependent') {
      msg = '<a href="#" class="open"><%= visualizations.length %> <%= visualizations.length == 1 ? "visualization" : "visualizations" %></a> will be deleted since this is the only layer on them.';
    } else {
      msg = 'This layer will dissapear from <a href="#" class="open"><%= visualizations.length %> <%= visualizations.length == 1 ? "visualization" : "visualizations" %></a>';
    }

    this.model.bind("change:open", this._onToggle, this);

    this.template = _.template('<div class="msg">' + msg + '</div><div class="visualizations"><%= visualization_list %></div>');
  ...
  render: function() {
    ...
    var visualization_list = _.map(this.model.get("visualizations"), function(vis) {
      return '<a href="/viz/' + vis.id + '">' + vis.name + '</a>';
