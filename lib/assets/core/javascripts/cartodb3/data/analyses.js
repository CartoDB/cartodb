...
var checkIfInternalGeocodingIsEnabled = function (configModel) {
  if (!configModel) throw new Error('configModel is required');
  return isDataServicesReady() && !!configModel.dataServiceEnabled('geocoder_internal');
};

var checkIfRoutingIsEnabled = function (configModel) {
  if (!configModel) throw new Error('configModel is required');
  return isDataServicesReady() && !!configModel.dataServiceEnabled('routing');
};

var checkIfExternalGeocodingIsEnabled = function (configModel) {
  if (!configModel) throw new Error('configModel is required');
  return isDataServicesReady() && !!configModel.dataServiceEnabled('hires_geocoder');
};

var checkIfDataObservatoryIsEnabled = function (configModel) {
  if (!configModel) throw new Error('configModel is required');
  return isDataServicesReady() && !!configModel.dataServiceEnabled('data_observatory');
}
...
var isDataServicesReady = function () {
  return DataServicesApiCheck.get().isReady();
};
