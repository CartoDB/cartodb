...
module.exports = CoreView.extend({
  ...
  _getUsers: function (d) {
    var count;
    var users;
    if (d.users !== undefined) {
      count = d.users.length;
      if (count > 1) {
        users = _t('components.modals.publish.share.role.group.users', {
          count: count
        });
      } else {
        users = _t('components.modals.publish.share.role.group.user', {
          count: count
        });
      }
      return users;
