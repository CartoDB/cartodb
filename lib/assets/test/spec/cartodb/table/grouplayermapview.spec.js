...
describe("GrouperLayerMapView", function() {
  ...
  function GroupLayerSpecs(mapViewClass) {
    ...
    it("set active layer", function() {
      ...
      expect(mapView.groupLayer.setInteraction.callCount).toEqual(2);
      var spyCall = mapView.groupLayer.setInteraction.calls[0];
      expect(spyCall.args[0]).toEqual(0, true);
      spyCall = mapView.groupLayer.setInteraction.calls[1];
      expect(spyCall.args[0]).toEqual(1, false);
