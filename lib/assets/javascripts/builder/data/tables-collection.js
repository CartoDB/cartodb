...
module.exports = Backbone.Collection.extend({
  ...
  parse: function (res) {
    ...
    if (visualizations.length && visualizations[0].table) {
      return this._parseVisualizations(visualizations);
    } else {
      return this._parseTables(visualizations);
    }
  },

  _parseVisualizations: function (visualizations) {
    return _.map(visualizations, function (viz) {
      var table = viz.table;

      return this._getTableProps(table, viz.type);
    }, this);
  },

  _parseTables: function (visualizations) {
    return _.map(visualizations, function (table) {
      return this._getTableProps(table, table.type);
    }, this);
  },

  _getTableProps: function (table, type) {
    return {
      id: table.id,
