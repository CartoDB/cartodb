...
cdb.admin.GMapsBaseView = cdb.core.View.extend({
  ...
  render: function() {
    var back_tile = 'http://www.colourgenics.com/images/map.jpg'
  ...
  activate: function(e) {
    e.preventDefault();
    this.map.set({ provider: 'googlemaps' });
    var base = new cdb.admin.GMapsBaseLayer();
...
cdb.admin.BaseMapView = cdb.core.View.extend({
  ...
  activate: function(e) {
    ...
    if(this.map.get('provider') === 'leaflet') {
      ...
      if(layer) {
        var layerData = _.clone(this.model.attributes);
        delete layerData.id;
        layer.set(layerData);
        layer.save();
        cdb.log.debug("enabling layer: " + layer.get('urlTemplate'));
      } else {
        cdb.log.error("looks like the map does not have base layer?");
      }
    } else {
  ...
  _initDropdown: function() {
    this.dropdown = new cdb.admin.DropdownMenu({
      ...
      ev.preventDefault();
      // TODO: delete layer from the model if it is allowed...
    });

    // Add it from the beginning
  ...
  _openDropdown: function(ev) {
    ...
    cdb.god.unbind("closeDialogs", this.dropdown.hide, this.dropdown);
...
cdb.admin.BaseMapChooser = cdb.core.View.extend({
  ...
  _addGoogleMaps: function() {
    var v = new cdb.admin.GMapsBaseView({ model: this.model});
    this.addView(v);
    this.$el.append(v.render().el);
  },
