...
export async function fetchDatasetsList (context) {
  ...
  const baseURL = 'https://do-isa-admin.carto-staging.com/api/v4'; // getBaseURL(context.rootState);
...
export async function requestExtendedLicense (context, id) {
  requestAccess(context, id, { subscriptionId: 'full_access_status_bq' });
}
...
export async function requestAccess (context, { subscriptionId, requestedPlatformProperty }) {
  ...
  try {
    const baseURL = 'https://do-isa-admin.carto-staging.com/api/v4'; // getBaseURL(context.rootState);
    const apiKey = '5902bf31c2ab411b018dd4a383e8813616b81b31'; // context.rootState.user.api_key;
    ...
    const subscription = await fetch(url, { method: 'PUT' });
    context.state.currentSubscription = subscription;
  } catch (error) {
    context.state.currentSubscription[requestedPlatformProperty] = 'requested'; // FIXME
    console.error(`ERROR: ${error}`);
  }
}
