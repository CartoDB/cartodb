<template>
  <div>
    ...
    <span class="brief__text text is-small is-txtSoftGrey"> {{ $t(`SharedBrief.sharedWith`) }}&nbsp;</span>
    <span v-if="isSharedWithOrg" class="brief__text text is-small is-txtSoftGrey u-ellipsis">{{ $tc(`SharedBrief.org`) }}</span>
    <span v-else class="brief__text text is-small is-txtSoftGrey u-ellipsis">{{ sharedWithText }}</span>
  </div>
</template>
...
export default {
  ...
  computed: {
    isSharedWithOrg () {
      return this.colleagues.filter(colleague => colleague.type === 'org').length > 0;
    },
    isSharedWithGroups () {
