...
module.exports = BaseModel.extend({
  ...
  fetch: function (opts) {
    ...
    opts = opts || {};
    this.errorCallback = opts && (opts.originalError || opts.error);
    ...
    opts.error = function (model, response) {
      if (response && response.statusText !== 'abort') {
        ...
        if (error.length === 1 && error[0] === THE_GEOM_NOT_FOUND_ERROR) {
          this.fetch(_.extend({}, _.omit(opts, 'success'), { originalError: this.errorCallback, geomColumnName: 'the_geom_webmercator' }));
        } else {
