...
TableStats.prototype = {
  ...
  _getPgStats: function (table, callback) {
    ...
    if (this.tables[table]) {
      ...
      var schema = userModel.isInsideOrg() ? userModel.username : 'public';
      schemaModel.set('query', 'with a as (select reltuples from pg_class where relname = \'' + table + '\'), b as (select * from pg_stats where tablename = \'' + table + '\' and schemaname = \'' + schema + '\') select * from a,b');
