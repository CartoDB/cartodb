...
describe('editor/layers/layer-analyses-view', function () {
  ...
  describe('given a layer definition with an analysis within its own layer', function () {
    ...
    describe('when a node is changed', function () {
      beforeEach(function () {
        this.c0 = this.analysisDefinitionNodesCollection.add({
          id: 'c0',
          type: 'source',
          params: {
            query: 'SELECT * FROM another_source'
          }
        });
        this.layerDefinitionsCollection.add({
          id: 'layerC',
          ...
          options: {
            letter: 'c',
            source: 'c0',
            table_name: 'another_source'
          }
        });

        this.b1 = this.analysisDefinitionNodesCollection.get('b1');
        this.b1.set('source', 'c0');
      });

      it('should render the source as a reference layer', function () {
        expect(this.view.$el.text()).toContain('b1');
        expect(this.view.$el.text()).toContain('c0');
