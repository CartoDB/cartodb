...
cdb.admin.Header = cdb.core.View.extend({
  ...
  _TEXTS: {
    _DEFAULT_DESCRIPTION:         _t('add a description for this...'),
   ...
   */
  _setTags: function() {
    var tag_button_text = this._TEXTS._TAG._ADD
      , tags = this.model.get('tags');

    if (tags && this.isVisEditable()) {
      tag_button_text = this._TEXTS._TAG_EDIT;
    } else {
      tag_button_text = '';
    }
    ...
    function setTitle(val) {
      if (val !== self.model.get('name') && val != '') {
        ...
        var change_confirmation = new cdb.admin.BaseDialog({
          title: self._TEXTS._RENAME_NAME._TITLE.replace('{{ vis }}', ( self.model.isVisualization() ? "visualization" : "table" )),
          description: self._TEXTS._RENAME_NAME._DESCRIPTION,
          template_name: 'common/views/confirm_dialog',
          clean_on_hide: true,
          enter_to_confirm: true,
          ok_button_classes: "right button grey",
          ok_title: self._TEXTS._RENAME_NAME._OK,
