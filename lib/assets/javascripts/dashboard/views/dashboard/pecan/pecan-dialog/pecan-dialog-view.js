...
const PecanCard = require('./pecan_card');
...
const template = require('./pecan-dialog.tpl');
...
module.exports = CoreView.extend({
  ...
  render: function () {
    // TODO: Be careful
    return this._panes.getActivePane().render().el.innerHTML;
  },

  _initModels: function () {
    this.model = new Backbone.Model({ page: 1, maxPages: 0 });
  },

  _initViews: function () {
    _.bindAll(this, '_addCard', '_generateThumbnail', '_refreshMapList', '_setWizardProperties');

    this.map = this._visModel.map;
    ...
    this._panes.addTab('vis',
      ViewFactory.createByHTML(template())
    );
  ...
  _initBinds: function () {
    this.listenTo(this.model, 'change:page', this._moveTabsNavigation);
    this.listenTo(this._panes, 'tabEnabled', this.render);
  },
  ...
  _loadCard: function (column) {
    if (column.get('success')) {
      this._generateThumbnail((column, error, url) => {
        if (!error) {
          this._addCard(url, column);
        } else {
          // TODO: Centralize logger
          // cdb.log.error(error);
        }
      });
    }
  ...
  _sendAppliedStats: function () {
    this.metrics.trackEvent('applied_pecan', {
      email: window.user_data.email
    });
  },

  _sendOpenStats: function () {
    this.metrics.trackEvent('open_pecan_list', {
      email: window.user_data.email
    });
  },
  ...
  _skip: function () {
    ...
    const skipPecanDialog = `pecan_${this.options.user.get('username')}_${name}`;
    localStorage[skipPecanDialog] = true;
  },

  _nextPage: function () {
    const page = this.model.get('page');
    const maxPages = this.model.get('maxPages');
  ...
  _setupCSS: function (css, geometryType) {
    const row_count = this._visModel.tableMetadata().get('row_count');
    const removeStrokeIndex = row_count / (this._CARD_WIDTH * this._CARD_HEIGHT);
    const removeStroke = (removeStrokeIndex > this._STROKE_PX_LIMIT);
  ...
  _setupTemplate: function () {
    const template = this.map.getLayerAt(0).get('urlTemplate');

    if (template) {
      const supportedBasemap = _.find(this._SUPPORTED_BASEMAPS, function (basemap) {
  ...
  _generateLayerDefinition: function (column) {
    ...
    const type = column.get('visualizationType');
    const sql = column.get('sql');
    const css = this._setupCSS(column.get('css'), column.get('geometryType'));

    const api_key  = this.user.get('api_key');
    const maps_api_template = cdb.config.get('maps_api_template');

    const template = this._setupTemplate();
  ...
  _generateThumbnail: function (column, callback) {
    ...
    const layerDefinition = this._generateLayerDefinition(column);

    const onImageReady = function (error, url) {
      callback && callback(error, url);
    };

    const the_geom = this.columns.find(function (column) {
  ...
  _getProperties: function (column) {
    ...
    const property = column.get('column');
    const wizard = this._getWizardName(column.get('visualizationType'));
  ...
  _onCardClick: function (column) {
    ...
    const wizard = this._getWizardName(column.get('visualizationType'));
    const properties = this._getProperties(column);
  ...
  _getWizardName: function (name){
    const mappings = {'heatmap': 'torque_heat'};
  ...
  _setWizardProperties: function () { // TODO: hack, we should find a way to remove this
    const properties = this._getProperties(this.model.get('column'));
    this.layer.wizard_properties.unbind('load', this._setWizardProperties, this);
    if (properties) {
  ...
  _getChoroplethProperties: function (properties) {
    ...
    const dist     = column.get('dist_type');
    const stats    = column.get('stats');
