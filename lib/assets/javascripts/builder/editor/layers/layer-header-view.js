...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    this._uiModel = new Backbone.Model({
      zoomVisible: this._hasGeom
    });

    this._changeStyle = this._changeStyle.bind(this);
    this._bindEvents();
    this._onSourceChanged();
    this._renderIfIsLayerEmptyChanges();
    this._renderZoomIfHasGeomChanges();
  },
  ...
  _onSourceChanged: function () {
    ...
    this._topQueryGeometryModel = nodeModel.queryGeometryModel.bind('change:simple_geom', this._renderZoomIfHasGeomChanges, this);
    this._renderZoomIfHasGeomChanges();
  },
  ...
  _renderIfIsLayerEmptyChanges: function () {
    var self = this;
    this._layerDefinitionModel.isEmptyAsync()
  ...
  _showOrHideZoomVisibility: function () {
    this._getZoom().toggle(this._hasGeom);
  },

  _renderZoomIfHasGeomChanges: function () {
    var self = this;
    var nodeModel = this._getNodeModel();
    nodeModel.queryGeometryModel.hasValueAsync()
      .then(function (hasGeom) {
        if (hasGeom !== self._hasGeom) {
          self._hasGeom = hasGeom;
          self._showOrHideZoomVisibility();
        }
      });
  }
});
