...
describe('CartoWizard', function() {
  ...
  describe("ChoroplethWizard", function() {
    ...
    it("should show the form if there is any number column", function() {
      ...
      view = new cdb._ChoroplethWizard({
        table: table,
        model: model
      });

      view.render();
      expect(view.$el.find('div.content').css("display")).toEqual("block");
      expect(view.$el.find('div.no_content').css("display")).toEqual("none");
    });
    ...
    it("should not show the form if there isn't any number column", function() {
      ...
      view.render();
      expect(view.$el.find('div.content').css("display")).toEqual("none");
      expect(view.$el.find('div.no_content').css("display")).toEqual("block");
    });
  ...
  describe("BubbleWizard", function() {
    ...
    it("should render sql when it is applied", function() {
      ...
      expect(view.model.get('sql')).not.toEqual(null);
    });
  ...
  describe("DensityWizard", function() {
    ...
    it("should re-render when table schema changes", function() {
      table.set('name', 'table_jajaja')
      view.render();
      view.cartoProperties.trigger('change');
      expect(model.get('sql').indexOf('table_jajaja')).not.toEqual(-1);
    });
