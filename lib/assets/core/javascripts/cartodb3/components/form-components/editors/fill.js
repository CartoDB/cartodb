...
Backbone.Form.editors.Fill = Backbone.Form.editors.Base.extend({
  ...
  _onInputClick: function (inputModel) {
    ...
    // if the component is taller than the space below (minus the Options-bar) it toggles position
    if (this._fillDialogView.$el.outerHeight() > (window.innerHeight - this.$el.offset().top) - 70) {
      this._fillDialogView.togglePosition();
    }
  },
