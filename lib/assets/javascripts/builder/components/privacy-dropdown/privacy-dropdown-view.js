...
module.exports = CoreView.extend({
  ...
  _initBinds: function () {
    this._customCollection.on('change:selected', function (menuItem) {
      ...
      if (menuItem.get('val') === 'password') {
        ...
        this._setPrivacy(menuItem.get('val').toUpperCase());
      }
    }, this);

    this.add_related_model(this._customCollection);

    this.listenTo(this.model, 'change:privacy', function () {
      this._setPrivacy(this.model.get('privacy'));
    });
  ...
  _setPrivacy: function (newPrivacyStatus) {
    if (this._shouldShowPrivacyWarning(newPrivacyStatus)) {
      this._checkPrivacyChange(
        this._savePrivacy.bind(this, newPrivacyStatus),
        this._discardPrivacyChange.bind(this)
      );
    } else {
      this._savePrivacy(newPrivacyStatus);
    }
  },

  _savePrivacy: function (privacyStatus) {
    var self = this;
    var vis = this._visDefinitionModel;

    this.model.set({ privacy: privacyStatus }, { silent: true });
    this.model.set({ state: 'loading' });

    this._privacyCollection.searchByPrivacy(privacyStatus)
      .saveToVis(vis, {
        success: function () {
