...
module.exports = CoreView.extend({
  ...
  _generateImportButtons: function (importOptionsType) {
    ...
    _.each(filteredOptions, function (importConfig) {
      ...
      if (!_.isEmpty(importConfig) && importConfig.enabled(this._configModel, this._userModel)) {
        ...
        if (!this._checkImportIsEnabled(importConfig)) {
  ...
  _checkGoogleDriveImport: function () {
    if (!this._configModel.get('oauth_gdrive')) {
      this._disableImportButton('gdrive');
      return false;
    }
    return true;
  },
  ...
  _checkDropboxImport: function () {
    if (!this._configModel.get('oauth_dropbox')) {
      this._disableImportButton('dropbox');
      return false;
    }
    return true;
  },
  ...
  _checkTwitterImport: function () {
    if (!this._configModel.get('datasource_search_twitter') ||
      ...
      this._disableImportButton('twitter');
      return false;
    }
    return true;
  },
  ...
  _checkInstagramImport: function () {
    ...
    if (!this._configModel.get('oauth_instagram')) {
      this._disableImportButton('instagram');
      return false;
    }
    return true;
  },
  ...
  _checkMailchimpImport: function () {
    if (!this._configModel.get('oauth_mailchimp')) {
      this._disableImportButton('mailchimp');
      return false;
    }
  ...
  _checkPostgreSQLImport: function () {
    this._disableImportButton('postgresql');
    return false;
  },

  _checkMySQLImport: function () {
    this._disableImportButton('mysql');
    return false;
  },

  _checkSQLServerImport: function () {
    this._disableImportButton('sqlserver');
    return false;
  },

  _checkHiveImport: function () {
    this._disableImportButton('hive');
    return false;
  },

  _checkBigQueryImport: function () {
    this._disableImportButton('bigquery');
    return false;
  },

  _disableImportButton: function (buttonName) {
    var $button = this.$('.js-' + buttonName + 'Button');
    $button.addClass('is-disabled');

    // var tooltip = new TipsyTooltipView({
    //   el: $tab,
    //   title: function () {
    //     return _t('components.modals.add-layer.imports.tab-options-error.' + type, { name: buttonName });
    //   }
    // });
  ...
  _onImportSelected: function (selectedImport) {
    ...
    var isEnabled = this._checkImportIsEnabled(opts);
  ...
  _checkImportIsEnabled: function (opts) {
