...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _bindDataLayer: function(layerView, layer) {
    ...
    if (layerType === 'CartoDB' || layerType === 'torque') { // unbind previos stuff
      ...
      if (self.layerDataView) {
        ...
        this.map = this.mapView.layers[Object.keys(this.mapView.layers)[0]].leafletMap;
        if(self.layerDataView.type === 'torque'){
          this.map.on("mousemove", self.mouseMove.bind(self));
        }
        else{
  ...
  mouseMove: function(e) {
    var bboxSize = 50;
    var val = this.layerDataView.getValueForBBox(e.containerPoint.x - bboxSize/2, e.containerPoint.y - bboxSize/2, bboxSize, bboxSize) || 0;
    var x = e.containerPoint.x;
    var y = e.containerPoint.y;
    if(val >= 0){
      this.tooltip.show({x: x, y: y}, {"fields": [{"title": "value", "value":""+val}]});
    }
    var nw = this.map.containerPointToLatLng([x - bboxSize/2, y - bboxSize/2]);
    var se = this.map.containerPointToLatLng([x + bboxSize/2, y + bboxSize/2]);
    this.curBounds = [[nw.lat, nw.lng], [se.lat, se.lng]];
    if(this.curRect){
      this.map.removeLayer(this.curRect);
    }
    this.curRect = L.rectangle(this.curBounds, {color: "#ff7800", weight: 1}).addTo(this.map);
  },
