...
module.exports = cdb.core.View.extend({
  ...
  _renderFilterShortcut: function() {
    ...
    this._renderFilterShortcut = function() {
      ...
      var d = {
        ...
        pluralizedContents: this._pluralizedContentType(rModel, totalCount),
        url:                !rModel.get('locked') ? rModel.lockedUrl({ locked: true }) : rModel.contentUrl()
      };

      if (rModel.get('locked')) {
        html = this.filterShortcutNonLockedTemplate(d);
      } else if (totalCount > 0 && !rModel.get('shared')) {
  ...
  _pluralizedContentType: function(rModel, totalCount) {
  ...
  _setupFilterShortcut: function(collection, router) {
    ...
    router.model.bind('change', this._updateFilterShortcut, this);
  },
