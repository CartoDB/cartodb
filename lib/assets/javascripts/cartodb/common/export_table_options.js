...
cdb.admin.ExportTableOptions = cdb.core.View.extend({
  ...
  formats: [
    ...
    {format: 'api', fetcher: 'showAPIDialog' }
  ],
  ...
  initialize: function() {
    ...
    if (this.options.force_http) {
      // if the protocol is already http, let it go
      if (this.options.config.sql_api_protocol !== 'http') {
        this.baseUrl = 'http://' + this.options.config.user_name + "." + this.options.config.sql_api_domain + '/api/v1/sql';
      }
    }

    if (!this.baseUrl) {
      this.baseUrl = this.options.config.sql_api_protocol + '://' 
        + this.options.config.user_name
        + "."
        + this.options.config.sql_api_domain
        + ':'
  ...
  showAPIDialog: function() {
    ...
    var dlg = new cdb.admin.BaseDialog({
      ...
      template_name: 'common/views/api_dialog',
      clean_on_hide: true,
   ...
   */
  render: function() {
    var template =  this.getTemplate('common/views/export_table_options')({
