...
function pointFill (props, animationType) {
  var css = {};
  var isTorqueCategory = animationType && !props.color.fixed;
  var markerFillOpacity = props.color && props.color.opacity;
  ...
  if (props.color) {
    if (props.color.fixed !== undefined) {
      ...
      css['marker-fill'] = makeColorRamp(props.color, isTorqueCategory);
    }
    if (props.color.operation) {
      css['marker-comp-op'] = props.color.operation;
    }
    // if (_.isNumber(markerFillOpacity)) {
...
function isCategoryType (color) {
  return color.fixed == null;
}
...
function generateCartoCSS (style, geometryType) {
  ...
  var isCategory = isCategoryType(styleDef.fill.color);
...
function animation (style, mapContext) {
  ...
  var hasOthers = color.domain && color.domain.length > InputColorCategories.MAX_VALUES;
  var s = ['select *, (CASE'];
  ...
  // Others is specified at the beginning of the domain, so if there are more than MAX_VALUES
  // the loop will start from the next pos, 1.
  for (var i = hasOthers ? 1 : 0, l = color.domain.length; i < l; i++) {
    ...
    if (columnType !== 'string' || categoryName === null) {
      ...
      value = "\'" + _normalizeValue(categoryName.slice(1, categoryName.length - 1)) + "\'";
    }
