...
var layerOnboardingKey = require('builder/components/onboardings/layers/layer-onboarding-key');
...
describe('editor/layers/layers-content-view/data/data-view', function () {
  ...
  beforeEach(function () {
    ...
    canBeGeoreferencedPromise = fakePromise(this.layerDefinitionModel, 'canBeGeoreferenced');
  ...
  describe('._initModels', function () {
    ...
    it('should change the _infoboxModel state to georeference if can be georeferenced', function (done) {
      var self = this;
      var initialInfoboxModelState = 'initial';
      this.view._infoboxModel.set('state', initialInfoboxModelState, { silent: true });
      spyOn(this.view, '_isLayerHidden').and.returnValue(false);
      canBeGeoreferencedPromise.resolve(true);

      setTimeout(function () {
        expect(self.view._infoboxModel.get('state')).toEqual('georeference');
        done();
      }, 0);
    });

    it('should not change the _infoboxModel state if it can not be georeferenced', function (done) {
      var self = this;
      var initialInfoboxModelState = 'initial';
      this.view._infoboxModel.set('state', initialInfoboxModelState, { silent: true });
      spyOn(this.view, '_isLayerHidden').and.returnValue(false);
      ...
      setTimeout(function () {
        expect(self.view._infoboxModel.get('state')).toEqual(initialInfoboxModelState);
        done();
      }, 0);
    });

    it('should change the _infoboxModel if the layer is not hidden', function (done) {
      var self = this;
      var initialInfoboxModelState = 'initial';
      this.view._infoboxModel.set('state', initialInfoboxModelState, { silent: true });
    ...
    it('should not change the _infoboxModel if the layer is hidden', function (done) {
      var self = this;
      var initialInfoboxModelState = 'initial';
      this.view._infoboxModel.set('state', initialInfoboxModelState, { silent: true });
