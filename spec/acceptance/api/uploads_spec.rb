...
describe "Imports API uploads" do
  ...
  it 'allow to upload using the request body' do
    file = File.open(Rails.root.join('db/fake_data/estaciones.csv'), 'r+')
    ...
    response_json['file_uri'].should match(/uploads\/.*\/estaciones.csv/)
    response_json['success'].should be_true
  end

  it 'allow to upload using a file param' do

    file = Rack::Test::UploadedFile.new(Rails.root.join('db/fake_data/estaciones.csv'), 'text/plain')
