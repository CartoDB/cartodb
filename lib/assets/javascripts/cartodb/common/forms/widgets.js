...
cdb.forms.Color = cdb.core.View.extend({
  ...
  _openImagePicker: function(e) {
    ...
    /* TODO: replace with new image picker */
    ...
    if (this.user.featureEnabled('new_modals')) {
      ...
      dialog.bind('fileChosen', function(url) {
        if (this.image_property) {
          if (this.image_property == this.property) {
...
cdb.forms.SimpleNumberWithLabel = cdb.forms.SimpleNumber.extend({
  ...
  render: function(prop) {
    ...
    this.$el.html('<span class="label">' + this.options.label + '</span><div class="input"><input class="value" ' + (this.options.disabled ? 'readonly' : '') + ' value="" style="width:' + (this.options.width) + 'px!important"/><a href="#" class="plus">+</a><a href="#" class="minus">-</a></div>');
    this.$('.value').val(value);
...
cdb.forms.TextAlign = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.template = cdb.templates.getTemplate('common/views/text_align');
