...
module.exports = CoreView.extend({
  ...
  _initBinds: function () {
    ...
    this.listenTo(this._mapModeModel, 'change:mode', this._onMapModeChanged);
  },
  ...
  _onMapModeChanged: function (mapModeModel) {
    ...
    if (mapModeModel.isEditingFeatureMode() || mapModeModel.isDrawingFeatureMode()) {
      feature = mapModeModel.getFeatureDefinition();
  ...
  handleRoute: function (route) {
    ...
    if (routeName === 'layer') {
      ...
      if (layerDefinitionModel === undefined) {
        // Redirect
        Router.getRouter().navigate('/', { trigger: true, replace: true });
        return;
      }

      if (tab === 'analyses') {
        this._stackLayoutView.model.goToStep(1, layerDefinitionModel, 'layer-content', tab, route[3]);
      } else {
        this._stackLayoutView.model.goToStep(1, layerDefinitionModel, 'layer-content', tab);
      }
    } else if (routeName === 'root' || routeName === 'layers') {
      this._stackLayoutView.model.goToStep(0);
    }
  },
