  ...
  class Varnish
      ...
      ActiveSupport::Notifications.instrument('purge.varnish', what: what) do |payload|
        ...
        if conf['http_port']
          request = http_client.request(
            "http://#{conf['host']}:#{conf['http_port']}/batch", method: :purge, headers: { 'Invalidation-Match' => what }
          ).run
          return request.code
        else
    ...
    def purge_surrogate_key(key)
      ActiveSupport::Notifications.instrument('purge.varnish', what: key) do |payload|
        ...
        if conf['http_port']
          request = http_client.request(
            "http://#{conf['host']}:#{conf['http_port']}/key", method: :purge, headers: { 'Invalidation-Match' => key }
          ).run
          return request.code
        else
    ...
    def http_client
      @http_client ||= Carto::Http::Client.get('varnish')
    end
