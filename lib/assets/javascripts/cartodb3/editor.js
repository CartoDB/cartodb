...
var TableView = require('./components/table/table-view');
...
deepInsights.createDashboard('#dashboard', vizJSON, {
  ...
  window.addTableView = function () {
    $('#dashboard').html(
      new TableView({
        querySchemaModel: layerDefinitionsCollection.at(1).getAnalysisDefinitionNodeModel().querySchemaModel,
        configModel: configModel
      }).render().el
    );
  };
