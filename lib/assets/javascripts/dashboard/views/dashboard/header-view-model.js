...
module.exports = Backbone.Model.extend({
  ...
  breadcrumbTitle: function () {
    const contentType = this.router.model.get('content_type');

    if (this.isDisplayingLockedItems()) {
      return _t('dashboard.views.dashboard.header_view_model.locked_' + contentType);
    } else if (this.isDisplayingDeepInsights()) {
      return _t('dashboard.views.dashboard.header_view_model.insight');
    } else {
      // Capitalize string
      return _t('dashboard.views.dashboard.header_view_model.title_' + contentType);
    }
  },
