...
module.exports = Backbone.Collection.extend({
  ...
  fetch: function (opts) {
    ...
    opts.error = function (coll, resp) {
      ...
      if (resp.error) {
        this._onQueryRowsCollectionError(resp);
      }
  ...
  _onQueryRowsCollectionError: function (response) {
    var queryErrors = this._querySchemaModel.get('query_errors');
    if (queryErrors && queryErrors.length) {
      return;
    }

    this._querySchemaModel.set({
      query_errors: response.error,
      status: this._querySchemaModel.hasRepeatedErrors() ? STATUS.errored : STATUS.unavailable
    });
  },
