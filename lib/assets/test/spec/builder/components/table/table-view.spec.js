...
var TableBodyView = require('builder/components/table/body/table-body-view');
...
describe('components/table/table-view', function () {
  beforeEach(function () {
    ...
    spyOn(TableHeadView.prototype, 'initialize').and.callThrough();
    spyOn(TableBodyView.prototype, 'initialize').and.callThrough();
  ...
  describe('canHideColumns on TableBodyView/TableHeadView constructor', function () {
    it('should be false if simply a source node is provided', function () {
      this.view.render();
      expect(TableBodyView.prototype.initialize).toHaveBeenCalledWith(jasmine.objectContaining({
        canHideColumns: false
      }));

      expect(TableHeadView.prototype.initialize).toHaveBeenCalledWith(jasmine.objectContaining({
        canHideColumns: false
      }));
    });

    it('should be true if a trade-area analysis is somewhere in the analysis chain but no SQL analysis', function () {
      const sourceNode = new AnalysisDefinitionNodeSourceModel({
        query: 'select * from pepito',
        table_name: 'pepito',
        id: 'a0',
        type: 'source'
      ...
      const node = new AnalysisDefinitionNodeModel({
        id: 'a1',
        type: 'trade-area',
        source: 'a0'
      }, {
      ...
      new AnalysisDefinitionNodesCollection([sourceNode, node], { // eslint-disable-line no-new
        configModel: this.configModel,
        userModel: this.userModel
      });
      ...
      this.view = new TableView({
        modals: this.modals,
        columnsCollection: this.columnsCollection,
        analysisDefinitionNodeModel: node
      });
    ...
    it('should be false if a trade-area analysis is somewhere in the analysis chain but also a SQL analysis', function () {
      const sourceNode = new AnalysisDefinitionNodeSourceModel({
        query: 'select * from pepito',
        table_name: 'pepito',
        id: 'a0',
      ...
      const sqlNode = new AnalysisDefinitionNodeModel({
        id: 'a2',
      ...
      new AnalysisDefinitionNodesCollection([sourceNode, node, sqlNode], { // eslint-disable-line no-new
        configModel: this.configModel,
        userModel: this.userModel
      });
      ...
      this.view = new TableView({
        modals: this.modals,
        columnsCollection: this.columnsCollection,
        analysisDefinitionNodeModel: sqlNode
      });
