...
module.exports = cdb.core.Model.extend({
  ...
  decorateGeocodeData: function(data) {
    // Migrated from old view._onGeocodingChosen
    data.table_name = this.get('table').get('id');

    // Check if location is empty, in that case, let's take "world" as location
    var isWorld = (data.location === '' || ( data.text && data.location.search('world') !== -1 ));
    if (isWorld) {
