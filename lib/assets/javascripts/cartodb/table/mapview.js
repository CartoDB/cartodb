...
cdb.admin.MapTab = cdb.core.View.extend({
    ...
    if (layerType === 'CartoDB' || layerType === 'torque') { // unbind previos stuff
      ...
      if (self.layerDataView) {
        ...
        if(self.layerDataView.type === 'torque'){
          this.mapView.layers[Object.keys(this.mapView.layers)[0]].leafletMap.on("mousemove", self.mouseMove.bind(self));
        }
        else{
          this.mapView.layers[Object.keys(this.mapView.layers)[0]].leafletMap.off("mousemove");
        }
  ...
  mouseMove: function(e) {
    var bboxSize = 50;
    var posInfo = this.layerDataView.getValueForBBox(e.containerPoint.x - bboxSize/2, e.containerPoint.y - bboxSize/2, bboxSize, bboxSize) || 0;
    if(posInfo >= 0){
      this.tooltip.show({x: e.containerPoint.x, y: e.containerPoint.y}, {"fields": [{"title": "value", "value":""+posInfo}]});
    }
  },
