...
describe('new_common/dialogs/publish/publish_view', function() {
  ...
  describe('when map is not private', function() {
    ...
    describe('when url shortening succeeds', function() {
      ...
      it('should have set a copyableValue on all items', function() {
        // Required because option view depends on it, could not be tested due to how zclip works :(
        expect(this.view._options.all(function(model) {
          return model.get('copyableValue');
        })).toBeTruthy();
      });
    });
