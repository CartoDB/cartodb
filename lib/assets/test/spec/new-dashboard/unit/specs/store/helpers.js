...
export const testAction = (
  ...
  rootState = rootState || state; // FIXME
  ...
  const result = action({ commit, state, dispatch, rootState, rootGetters: state }, payload);
...
export function testAction2 ({ action, payload, state = {}, expectedMutations = [], expectedActions = [], rootState = {}, done = noop }) {
  if (typeof action === 'undefined') {
    throw new Error('Store Action is undefined');
  }
