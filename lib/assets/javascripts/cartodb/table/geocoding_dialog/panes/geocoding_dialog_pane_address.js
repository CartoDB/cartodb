  ...
  cdb.admin.GeocodingDialog.Pane.Address = cdb.admin.GeocodingDialog.Pane.extend({
    ...
    _onEstimationChange: function() {
      ...
      if (!googleUser && !this.user.get('geocoding').hard_limit && ( this.estimation.get('estimation') > 0 || ( this.estimation.get('estimation') === undefined && !this.user.get('geocoding').hard_limit ))) {
        this.model.set('agreement', false);
      } else if (googleUser || this.user.get('geocoding').hard_limit) {
