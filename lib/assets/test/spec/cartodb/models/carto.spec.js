...
/*
  ...
  describe("cartoparser", function() {
    ...
    it("should return the rules for main defintion", function() {
      var rules = [
       'marker-fill',
       'marker-opacity',
       'marker-width',
    ...
    it("should generate the correct list of categories when a new property is set", function() {
      sinon.stub(table_points, 'geomColumnTypes').returns(["points"]);
      ...
      cdb.admin.carto.category.category_generator(table_points, p, changed, function(style) {
        css = style;
      });

      expect(css.indexOf('cat1') !== -1).toEqual(true);
      expect(css.indexOf('cat2') !== -1).toEqual(true);
      expect(css.indexOf('cat3') !== -1).toEqual(true);
      expect(css.indexOf('cat4') !== -1).toEqual(true);
    });
