      ...
      if (!tag && !q && shared === "no" && !locked && !liked) {
        ...
        if (this.router.model.get('content_type') === "maps") {
          ...
          if (this.collection.total_shared > 0) {
            this.router.navigate(this.user.viewUrl().dashboard().datasets().urlToPath('shared'));
            return;
          } else if (cdb.config.data_library_enabled) {
            this.router.navigate(this.user.viewUrl().dashboard().datasets().dataLibrary());
            return;
          } else {
