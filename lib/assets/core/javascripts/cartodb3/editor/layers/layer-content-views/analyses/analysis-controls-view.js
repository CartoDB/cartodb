...
module.exports = CoreView.extend({
  ...
  _createQuotaView: function () {
    ...
    var quotaCallback = function (quota) {
      if (quota === false) {
        this._viewModel.set('userFetchModelState', 'notEnough');
        this._infoboxModel.set('state', 'notEnough');
      } else {
        this._quotaInfo = quota;
        this._creditsLeft = quota.totalQuota - quota.usedQuota;
        this._canRunAnalysis = this._creditsLeft > 0 || quota.hardLimit === false;
    ...
    var states = [
      ...
      },
      {
        ...
        createContentView: function () {
          ...
          var quotaMessage = '';
          var blockPrice = Math.round(this._quotaInfo.blockPrice / 100);
          if (this._creditsLeft > 0) {
            quotaMessage = _t('editor.layers.analysis-form.quota.credits-left-message', { smart_count: this._creditsLeft });
          } else {
            quotaMessage = _t('editor.layers.analysis-form.quota.no-credits-message');
          }
          this._quotaInfo.quotaMessage = quotaMessage;
          ...
          return InfoboxFactory.createQuota({
            type: 'alert',
            title: _t('editor.layers.analysis-form.quota.title'),
            body: this._creditsLeft > 0
              ? _t('editor.layers.analysis-form.quota.credits-left-body', {
                blockSize: this._quotaInfo.blockSize,
                blockPrice: blockPrice
              })
              : getNoCreditMessage(this._quotaInfo),
            confirmLabel: confirmLabel,
            confirmType: 'primary',
            confirmDisabled: !this._canSave() || !this._canRunAnalysis,
        ...
        mainAction: function () {
          if (this._canSave() && this._canRunAnalysis) {
            this._saveAnalysis();
          }
        }.bind(this),
      ...
      },
      {
        state: 'notEnough',
        createContentView: function () {
          var link = CONTACT_LINK_TEMPLATE({
            mail: _t('editor.layers.analysis-form.quota.no-credits-mail'),
            contact: _t('editor.layers.analysis-form.quota.no-credits-contact')
          });
          var message = _t('editor.layers.analysis-form.quota.no-credits-enough-body', {
            analysis: AnalysesQuotaFactory.getAnalysisName(type),
            contact: link
          });
          return InfoboxFactory.createConfirm({
            title: _t('editor.layers.analysis-form.quota.no-credits-enough-title'),
            type: 'error',
            body: message,
            cancelLabel: _t('editor.layers.analysis-form.quota.cancel')
          });
    ...
    if (this._viewModel.get('userFetchModelState') === 'idle') {
      ...
      AnalysesQuotaFactory.getQuotaInfo(type, this._userModel, quotaCallback.bind(this));
    }
