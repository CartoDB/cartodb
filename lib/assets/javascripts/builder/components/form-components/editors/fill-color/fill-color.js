...
Backbone.Form.editors.FillColor = Backbone.Form.editors.Base.extend({
  ...
  _initViews: function () {
    ...
    if (this.model.get('fillColor').range && this._tabPaneTabs.length > 1) {
      ...
      this._tabPaneTabs[0].selected = true;
    }
  ...
  _onChangeTabPaneViewTab: function () {
    ...
    var attrs = this.model.get('fillColor');

    if (selectedTabPaneName === SOLID) {
      ...
      if (attrs.fixed) {
        if (attrs.range) {
          attrs.fixed = attrs.range[0];
          attrs.range = null;
        }
      }

      var fillColor = _.extend(this.model.get('fillColor'), attrs);
      this.model.set('fillColor', fillColor);
      this.trigger('change', this.model, this);
    }
  },
