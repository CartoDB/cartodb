...
$(function() {
  ...
  var Dashboard = cdb.core.View.extend({
    ...
    _onTablesFetch: function() {
      ...
      if (this.startView.model.get("path") == 'tables') {

        if (this.tables.total_entries == 0) {
          this.searchView.hide();
        } else {

          if (this.tables.size() == 0) this._goto(null, "/tables");
    ...
    _setupVisualizations: function() {
      ...
      this.visualizationsView.bind("removeTag", function(e) {
        self.router.navigate("/dashboard/visualizations", { trigger: true });
      }, this);
    ...
    _setupTables: function() {
      ...
      this.tablesView.bind("removeTag", function(e) {
        self.router.navigate("/dashboard/tables", { trigger: true });
      }, this);
  ...
  var DashboardRouter = Backbone.Router.extend({
    ...
    search: function(model, q, page) {
      ...
      if (model == 'visualizations') {
        ...
        dashboard.tables.options.set({ q: q, page: page || 1, tags: "", per_page: dashboard.tables._TABLES_PER_PAGE, type: "table" });
    ...
    tag: function(model, tag, page) {
      ...
      if (model == 'visualizations') {
        ...
        dashboard.tablesView.showDefaultTitle(true);
        dashboard.tables.options.set({ q: "", page: page || 1, tags: tag, per_page: dashboard.tables._TABLES_PER_PAGE, type: "table" });
        dashboard.tablesView.showLoader();
        dashboard.tablesView.model.set("padding", true);
    ...
    index: function() {
      ...
      dashboard.tables.options.set({ q: "", tags: "", per_page: dashboard.tables._PREVIEW_TABLES_PER_PAGE, type: "table" });
    ...
    tables: function(page) {
      ...
      // Tables
      dashboard.tables.options.set({ q: "", page: page || 1, tags: "", per_page: dashboard.tables._TABLES_PER_PAGE, type: "table" });
      dashboard.tablesView.showLoader();
      dashboard.tablesView.model.set("padding", true);
