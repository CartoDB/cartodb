...
function generateCartoCSS (style, geometryType, configModel) {
  ...
  // Main styles
  // Omit some properties for main styles
  var omittedStyleAttrs = ['animated', 'labels'];
  if (geometryType === 'polygon' || (geometryType === 'point' && StyleUtils.canApplyDotProperties(styleType, styleDef))) {
  ...
  if (hasStroke && geometryType === 'point' && StyleUtils.canApplyDotProperties(styleType, styleDef)) {
    css += '#layer::outline {\n';
    css += renderBlock({ stroke: styleDef.stroke }, geometryType, styleType, styleDef);
    css += '}\n';
  }
