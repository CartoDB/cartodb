...
cdb.admin.models.Filter = cdb.core.Model.extend({
  ...
  _changeDateHist: function(hist, bounds) {
    ...
    if (!this.has('lower')) {
      ...
      console.log("B", bounds.lower, lower);
      ...
      console.log("A", bounds.lower, lower);
  ...
  getSQLConditionForDate: function() {
    ...
    if      (!this.attributes.lower && !this.attributes.upper)     sql = _.template(" (<%= column %> IS NULL) ")(options);
    else if (this.attributes.upper >= this.attributes.upper_limit) sql = _.template(" (<%= column %> >= ('<%= lower %>') AND <%= column %> <= ('<%= upper %>')) ")(options);
    else sql = _.template(" (<%= column %> >= ('<%= lower %>') AND <%= column %> <= ('<%= upper %>')) ")(options);
