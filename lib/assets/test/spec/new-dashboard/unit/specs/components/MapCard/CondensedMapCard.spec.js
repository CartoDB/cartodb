...
import CondensedMapCard from 'new-dashboard/components/MapCard/CondensedMapCard';
...
describe('MapCard.vue', () => {
  ...
  it('should render correct contents', () => {
    ...
    const mapCard = shallowMount(CondensedMapCard, {
  ...
  it('should render shared brief with all organisation members', () => {
    const map = {...visualization.visualizations[0]};
    var clonedMap = cloneObject(map);
    clonedMap.permission.acl = [{
      'type': 'org',
      'entity': {
        'id': 'xxxxx',
        'name': 'test-org',
    ...
    const mapCard = shallowMount(CondensedMapCard, {
      propsData: { visualization: clonedMap },
      mocks: {
        $cartoModels,
        $t,
        $tc
  ...
  describe('Methods', () => {
    ...
    it('should open quick actions', async () => {
      const map = visualization.visualizations[0];
      const mapCard = shallowMount(CondensedMapCard, {
        propsData: { visualization: map },
        mocks: {
    ...
    it('should close quick actions', async () => {
      const map = visualization.visualizations[0];
      const mapCard = shallowMount(CondensedMapCard, {
        propsData: { visualization: map },
        mocks: {
    ...
    it('should toggle mouse hover', async () => {
      const map = visualization.visualizations[0];
      const mapCard = shallowMount(CondensedMapCard, {
        propsData: { visualization: map },
        mocks: {
      ...
      mapCard.find('.cell__checkbox').trigger('mouseover');
      await nextTick();
      expect(mapCard.vm.activeHover).toBe(false);
      expect(mapCard).toMatchSnapshot();

      mapCard.find('.cell__checkbox').trigger('mouseleave');
      await nextTick();
      expect(mapCard.vm.activeHover).toBe(true);
      expect(mapCard).toMatchSnapshot();
    });

    it('should show thumbnail error', async () => {
      const map = visualization.visualizations[0];
      const mapCard = shallowMount(CondensedMapCard, {
        propsData: { visualization: map },
        mocks: {
    ...
    it('onContentChanged: should emit a contentChanged event when DatasetCard emits it', async () => {
      const map = visualization.visualizations[0];
      const mapCard = shallowMount(CondensedMapCard, {
        propsData: { visualization: map },
        mocks: {
  ...
  describe('Events', () => {
    it('should receive and process correctly the event when dropdown is open', async () => {
      const map = visualization.visualizations[0];
      const mapCard = shallowMount(CondensedMapCard, {
        propsData: { visualization: map },
        mocks: {
    ...
    it('should receive and process correctly the event when dropdown is closed', async () => {
      const map = visualization.visualizations[0];
      const mapCard = shallowMount(CondensedMapCard, {
        propsData: { visualization: map },
        mocks: {
  ...
  describe('Computed', () => {
    it('should return correct map thumbnail', () => {
      ...
      expect(mapCard.vm.mapThumbnailUrl).toBe('http://hello.example.com/api/v1/map/static/named/tpl_e97e0001_f1c2_425e_8c9b_0fb28da59200/144/144.png');
    });
    ...
    describe('showViews', () => {
      it('should return true if the map has public privacy', () => {
        const map = {...visualization.visualizations[0]};
        map.privacy = 'PUBLIC';
        const publicMap = shallowMount(CondensedMapCard, {
          propsData: { visualization: map },
          mocks: {
            $cartoModels,
            $t,
  ...
  describe('properties', () => {
    ...
    it('should select the map when the user clicks on the card and selectMode property is true', async () => {
      const mapCardWrapper = shallowMount(CondensedMapCard, {
    ...
    it('should not the map when the user clicks on the card and selectMode property is true', async () => {
      const mapCardWrapper = shallowMount(CondensedMapCard, {
