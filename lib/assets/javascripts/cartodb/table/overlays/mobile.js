cdb.admin.overlays.MobileLayers = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    _.each(this.options.layers.models, this._getLayer, this);
  ...
  _getLayer: function(layer) {
    ...
    if (layer.get("type") == 'layergroup' || layer.get('type') === 'namedmap') {
      ...
      if (layer.legend)
      layer.legend.on("change", function() {
        //this._setupLayers();
      }, this);

      //layer.wizard_properties.on("change", function() {
        //debugger;
      //}, this);
  ...
  _renderLayer: function(data) {
    ...
    var layer = new cdb.admin.overlays.MobileLayer({
      model: data,
      show_legends: this.model.get("show_legends"),
      show_title: true,
      hide_toggle: false 
    });
...
cdb.admin.overlays.MobileLayer = cdb.core.View.extend({
  ...
  _renderLegend: function() {
    ...
    var model  = new cdb.geo.ui.LegendModel(this.model.get("legend"));
    var legend = new cdb.geo.ui.Legend({ model: model });
  ...
  render: function() {
    ...
    var layer_name = this.model.get("table_name_alias");
...
cdb.admin.overlays.Mobile = cdb.core.View.extend({
  ...
  _bindOverlays: function() {
    ...
    this.overlays.on("add", function(a) {
      this._resetOverlays();
    }, this);

    this.overlays.on("remove", function(b) {
      this._resetOverlays();
    }, this);
  ...
  _reloadAside: function() {

    this.$el.find(".aside").html("");
  ...
  _addLayers: function() {
    ...
    this.layers = new cdb.admin.overlays.MobileLayers({ layers: this.map.layers, show_legends: this.map.get("legends") });
    this.addView(this.layers);
    this.$el.find(".aside").append(this.layers.render().$el);

    this.layers.bind("has-layers", function() {
      this.$el.addClass("with-layers");
    }, this);
