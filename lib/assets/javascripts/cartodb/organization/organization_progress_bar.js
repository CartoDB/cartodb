...
module.exports = cdb.core.View.extend({
  ...
  render: function() {
    ...
    this.$('.js-quotaProgress .ui-slider-handle').addClass('js-quotaProgressSlider');

    // Edit assigned literal
    this.$('.js-assignedSize').val(Utils.readablizeBytes(userQuota).replace(/\s\w*/, ""));
  ...
  _onSlideChange: function(ev, ui) {
    var userQuota = (organization_data['quota_in_bytes'] - organization_data['userQuota']) * ( ui.value / 100 );
    $('.js-assignedSize').val(Utils.readablizeBytes(userQuota).replace(/\s\w*/, ""));
