var PaginationModel = require('new_common/pagination/model');

describe('new_common/pagination/model', function() {
  describe('.pagesCount', function() {
    describe('given there are no items', function() {
      beforeEach(function() {
        this.m = new PaginationModel({
          totalCount: 0
        });
      });

      it('should return at least one page', function() {
        expect(this.m.pagesCount()).toEqual(1);
      });
    });

    describe('given there are less items than per page', function() {
      beforeEach(function() {
        this.m = new PaginationModel({
          totalCount: 6,
          perPage:    7
        });
      });

      it('should return that there is just one page', function() {
        expect(this.m.pagesCount()).toEqual(1);
      });
    });

    describe('given there are one more item than per page', function() {
      beforeEach(function() {
        this.m = new PaginationModel({
          totalCount: 4,
          perPage:    3
        });
      });

      it('should return the upper amount of pages', function() {
        expect(this.m.pagesCount()).toEqual(2);
      });
    });
  });
  
  describe('.pagesToDisplay', function() {
    describe('given there are less pages than expected to be visible', function() {
      beforeEach(function() {
        this.m = new PaginationModel({
          totalCount:   30,
          perPage:      10,
          displayCount: 5
        });
      });

      it('should return a sequence of integers starting from first page to the last available page', function() {
        expect(this.m.pagesToDisplay()).toEqual([1,2,3]);
      });
    });

    describe('given there are more pages than expected to be visible', function() {
      beforeEach(function() {
        this.m = new PaginationModel({
          totalCount:   110,
          perPage:      10,
          displayCount: 5
        });
      });

      describe('and current page is with the first visible pages', function() {
        it('should return a sequence of integers relative to current page', function() {
          var expectedSequence = [1,2,3,4,5];
          expect(this.m.set('page', 1).pagesToDisplay()).toEqual(expectedSequence);
          expect(this.m.set('page', 2).pagesToDisplay()).toEqual(expectedSequence);
          expect(this.m.set('page', 3).pagesToDisplay()).toEqual(expectedSequence);
        });
      });

      describe('and current page is in the middle range of available pages', function() {
        it('should return a sequence of integers where the current page is always in the middle of the range', function() {
          expect(this.m.set('page', 4).pagesToDisplay()).toEqual([2,3,4,5,6]);
          expect(this.m.set('page', 5).pagesToDisplay()).toEqual([3,4,5,6,7]);
          expect(this.m.set('page', 6).pagesToDisplay()).toEqual([4,5,6,7,8]);
          expect(this.m.set('page', 7).pagesToDisplay()).toEqual([5,6,7,8,9]);
          expect(this.m.set('page', 8).pagesToDisplay()).toEqual([6,7,8,9,10]);
        });
      });

      describe('and current page is within the last expected visible pages', function() {
        it('should return a sequence of integers of the last visible pages', function() {
          var expectedSequence = [7,8,9,10,11];
          expect(this.m.set('page', 9).pagesToDisplay()).toEqual(expectedSequence);
          expect(this.m.set('page', 10).pagesToDisplay()).toEqual(expectedSequence);
          expect(this.m.set('page', 11).pagesToDisplay()).toEqual(expectedSequence);
        });
      });
    });
  });

  describe('.isCurrentPage', function() {
    beforeEach(function() {
      this.m = new PaginationModel({
        page: 42
      });
    });

    it('should return true if given page is current', function() {
      expect(this.m.isCurrentPage(42)).toBeTruthy();
      expect(this.m.isCurrentPage(-1)).toBeFalsy();
    });
  });
});
