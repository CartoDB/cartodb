...
function makeColorRamp (props, isTorqueCategory) {
  ...
  attribute = props.operator ? aggToRamp(props) : '[' + attribute + ']';
...
// size
function pointSize (props, property) {
  var css = {};
  if (props.fixed !== undefined) {
    css[property + '-width'] = props.fixed;
  } else if (props.attribute) {
    css[property + '-width'] = makeWidthRamp(props);
  } else {
...
function blending (b, geometryType, styleType, animatedType) {
  var css = {};
  var isTVT = styleType === 'tvt';
  var property = (isTVT ? 'dot' : geometryType) + '-comp-op';
...
// fill
function pointFill (props, styleType, animationType) {
  var css = {};
  var color = (props && props.color) || {};
  var isTorqueCategory = animationType && !color.fixed;
  var isTVT = styleType === 'tvt';
  var property = isTVT ? 'dot' : 'marker';

  if (props.size) {
    css = pointSize(props.size, property);
  }

  if (color) {
    if (color.fixed !== undefined) {
      css[property + '-fill'] = color.fixed;
    } else if (color.attribute) {
      css[property + '-fill'] = makeColorRamp(color, isTorqueCategory);
    }
    if (color.operation) {
      css[property + '-comp-op'] = color.operation;
    }

    var opacity = color.opacity != null ? color.opacity : 1;

    if (isTVT) {
      css['dot-opacity'] = opacity;
    } else {
      css['marker-fill-opacity'] = opacity;
...
// stroke
function pointStroke (props, styleType, animationType) {
  var css = {};

  if (animationType === 'heatmap' || styleType === 'tvt') {
    return css;
  }
...
var cartocssFactory = {
  ...
  blending: {
    point: function (attrs, styleType, animationType) { return blending(attrs, 'marker', styleType, animationType); },
...
var tvtConversion = function (style) {
  // move aggregation from color color
  style.fill = _.clone(style.fill);

  style.fill.aggregation = {
    value: {
      attribute: style.fill.color.attribute,
      operator: style.fill.color.operator
    }
  };
...
var styleConversion = {
  ...
  tvt: function (style) {
    return tvtConversion(style);
  }
};

function renderBlock (block, geometryType, styleType, animationType) {
  var css = {};
  for (var k in block) {
    var f = cartocssFactory[k];
    if (f) {
      css = _.extend(css, f[geometryType](block[k], styleType, animationType));
    }
  }
...
function generateCartoCSS (style, geometryType, configModel) {
  ...
  var animationType = styleType === 'animation' && styleDef.style;
  ...
  css += '#layer {\n';
  css += renderBlock(_.omit(styleDef, omittedStyleAttrs), geometryType, styleType, animationType);
  css += '}';
...
function aggToRamp (props) {
  var attribute = (props.operator.toLowerCase() === 'count') ? 'count_vals' : props.attribute;
  return '[' + attribute + ']';
}
