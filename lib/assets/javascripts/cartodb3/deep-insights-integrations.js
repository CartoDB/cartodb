...
F.prototype._onWidgetAutoStyleChanged = function (m) {
  var cartoCSS = m.autoStyler.getStyle();
  // var styleDef = m.autoStyler.getStyleDef && m.autoStyler.getStyleDef()
  var layerId = m.dataviewModel.layer.id;
  var layerDefModel = this._layerDefinitionsCollection.findWhere({ id: layerId });
  var applyAutoStyle = m.get('autoStyle');
  var nodeDefModel = layerDefModel.getAnalysisDefinitionNodeModel();
  var simpleGeom = nodeDefModel.get('simple_geom');
  var styleModel = layerDefModel.styleModel;

  if (layerDefModel) {
    layerDefModel.set({
      autoStyle: m.get('autoStyle')
    });

    if (styleModel && applyAutoStyle) {
      console.log('come on', simpleGeom, cartoCSS);
    }
  }
};
