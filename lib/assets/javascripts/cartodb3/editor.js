...
var editorWidgetsView = new EditorWidgetsView({
  ...
  collection: mapDefModel.widgetDefinitionsCollection
});
...
dashboard.vis.done(function () {
  ...
  _.each(vizJSON.layers, function (d) {
    ...
    _.each(layersData, function (d) {
      ...
      mapDefModel.layerDefinitionsCollection.add(layerDefModel);
    });
  });

  _.each(vizJSON.widgets, function (d) {
    var layerDefModel = mapDefModel.layerDefinitionsCollection.get(d.layer_id);
    ...
    mapDefModel.widgetDefinitionsCollection.add(widgetDefModel);
  });
})
.error(function (err) {
  console.error(err);
