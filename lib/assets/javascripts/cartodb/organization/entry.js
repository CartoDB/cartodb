...
$(function() {
  cdb.init(function() {
    ...
    // API keys
    var regenerateApiKeyHandler = function(ev, type, scope, form_action, authenticity_token) {
    ...
    $('.js-renewOrgUsersApiKey').bind('click', function(ev) {
      var current_username = $(this).attr('data-current_username');
      var authenticity_token = $('[name=authenticity_token][value]').get(0).value;

      regenerateApiKeyHandler(ev, 'api', 'organization', '/u/' + current_username + '/organization/regenerate_api_keys', authenticity_token);
    });
    ...
    $('.js-renewOrgUserApiKey').bind('click', function(ev) {
      ...
      regenerateApiKeyHandler(ev, 'api', 'organization_user', '/u/' + current_username + '/organization/users/' + username + '/regenerate_api_key', authenticity_token);
    });
  });
