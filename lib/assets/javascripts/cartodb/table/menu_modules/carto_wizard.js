...
cdb.admin.WizardProperties = cdb.core.Model.extend({
  ...
  initialize: function() {
    ...
    this.bind('change', function() {
      this.cartoStylesGeneration.set('properties', _.clone(this.attributes));
    }, this);
  ...
  linkLayer: function(layer) {
    ...
    var changeLayerStyle = function(st, sql, layerType) {
      ...
      if (layer.get('tile_style_custom')) return;
    ...
    this.cartoStylesGeneration.bind('change:style change:sql', function() {
      ...
      if(st) {
        changeLayerStyle(st, this.cartoStylesGeneration.get('sql'), this.cartoStylesGeneration.get('layer_type'));
      }
    }, this);
...
cdb.admin.mod.CartoCSSWizard = cdb.admin.Module.extend({
  ...
  initialize: function() {
    ...
    // #R
    // move this to layer model
    var wizard_properties = new cdb.admin.WizardProperties({ 
  ...
  renderWizards: function() {
    ...
    el.html('');
    if(!wizard) {
      cdb.log.error("wizard type does not exists");
      return;
    }
