...
cdb.admin.Header = cdb.core.View.extend({
  ...
  _shareVisualization: function(e) {
    ...
    if (this.options.user.featureEnabled('new_modals')) {
      if (this.model.isVisualization()) {
        var publishOptions = new Backbone.Collection();
        var view = new cdb.editor.PublishView({
          publishOptions: publishOptions
        });
        view.render().appendToBody();
      } else {
        // TODO: a dataset cannot be shared, have to create a map first, what should be the flow here?
      }
      return;
    }
  ...
  _showPrivacyDialog: function(e) {
    ...
    if (this.options.user.featureEnabled('new_modals')) {
      var viewModel = new cdb.editor.ChangePrivacyModel({
        vis: this.model, //vis
        user: this.options.user
      });
