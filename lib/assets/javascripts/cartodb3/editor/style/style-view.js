...
module.exports = cdb.core.View.extend({
  ...
  _saveCartoCSS: function () {
    ...
    if (errors.length === 0) {
      ...
      // This needs to be replaced by infobox
      // this._confirmSaveCSS();
    }
  },

  _confirmSaveCSS: function () {
    this._styleModel.set({wait: true});
    this._editorModel.set({edition: false});

    // Infobox here to save on save or undo on decline
    //
    // this._cartocssModel.set('content', content);
    // this._layerDefinitionModel.save({
    //   cartocss_custom: true,
