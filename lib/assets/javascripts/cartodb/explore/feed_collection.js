...
module.exports = Backbone.Collection.extend({
  ...
  _generateQuery: function(type, orderBy, page, limit) {
    ...
    var queryTemplate = 'WITH m As (SELECT max(visualization_mapviews) As max_views FROM <%- table %>) SELECT <%= fields %> FROM <%- table %>, m <%= where %> ORDER BY <%- order_by %> DESC, created_at DESC, visualization_id DESC LIMIT <%- limit %> OFFSET <%- offset %>';
