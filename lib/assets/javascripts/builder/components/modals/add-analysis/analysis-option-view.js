...
module.exports = CoreView.extend({
  ...
  _getDeprecationWarning: function (analysisItem) {
    var deprecationWarningDate = analysisItem.deprecationWarningDate
      ? new Date(analysisItem.deprecationWarningDate).getTime()
      : null;
    var userCreation = new Date(window.userModel.get('created_at')).getTime();
    if (deprecationWarningDate && deprecationWarningDate > userCreation) {
      var deprecationDate = new Date(analysisItem.deprecationDate);
      return {
        message: _t('components.modals.add-analysis.deprecation-warning'),
        date: moment(deprecationDate).format('Do MMMM YYYY'),
