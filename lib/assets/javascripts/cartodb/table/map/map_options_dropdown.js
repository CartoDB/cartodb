cdb.admin.MapOptionsDropdown = cdb.ui.common.Dropdown.extend({
  ...
  initialize: function() {
    ...
    this.disabled_cartodb_logo = this.options.user.featureEnabled("disabled_cartodb_logo");
    ...
    this.mapOptions = new cdb.core.Model({
      ...
      fullscreen:       false,
      share:            true,
      logo:             this.disabled_cartodb_logo,
      zoom:             true,
      scrollwheel:      false,
      legends:          true
    });
  ...
  render: function() {
    ...
    if (this.options.user.get('actions').remove_logo && !this.disabled_cartodb_logo) {
      switches.push('logo');
    } else {
      this.$('.logo').remove();
    }
