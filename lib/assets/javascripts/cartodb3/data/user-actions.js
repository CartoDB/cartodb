...
module.exports = function (params) {
  ...
  return {
    ...
    createAnalysisNode: function (nodeAttrs, layerDefModel) {
      ...
      var onGeometryReady;
      onGeometryReady = function () {
        var geometryModel = nodeDefModel.geometryModel;
        if (geometryModel.get('ready')) {
          geometryModel.off(CHANGE_READY, onGeometryReady);
        } else {
        ...
        saveDefaultStylesIfStillRelevant = function () {
          if (geometryModel.get('status') !== 'fetched') return; // wait until fetched

          geometryModel.off(CHANGE_STATUS, saveDefaultStylesIfStillRelevant); // only do this once
          ...
          if ( // Only apply changes if:
            ...
            var simpleGeometryType = geometryModel.get('simple_geom');
        ...
        geometryModel.on(CHANGE_STATUS, saveDefaultStylesIfStillRelevant);
        geometryModel.fetch();
      };
      nodeDefModel.geometryModel.on(CHANGE_READY, onGeometryReady);
      onGeometryReady();
