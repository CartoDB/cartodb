...
_.extend(cdb.geo.ui.InfowindowModel.prototype, {
  toJSON: function() {
    return {
      ...
      custom_html: this.attributes.custom_html,
...
cdb.admin.CartoDBLayer = cdb.geo.CartoDBLayer.extend({
  ...
  initialize: function() {
    ...
    this._sync = _.debounce(Backbone.sync, 10);
    ...
    this.infowindow = new cdb.geo.ui.InfowindowModel({
      ...
      template_url:   'table/views/infowindow/templates'
    });
  ...
  bindLegend: function(legend) {
    ...
    this.legend.bind('change:type change:items', _.debounce(function() {
  ...
  bindInfowindow: function(infowindow) {
    ...
    this.infowindow.bind('change:fields change:custom_html change:alternative_names change:template_name', _.debounce(function() {
      // call with silent so the layer is no realoded
  ...
  applySQLFilterView: function(sql, options) {
    options = options || {
      add_to_history: true
    }
    ...
    this.sqlView.setSQL(sql, { silent: true });
    this.save({ sql_source: "filters" }, { silent: true });
...
cdb.admin.Layers = cdb.geo.Layers.extend({
  ...
  getLayerDef: function() {
    var cartodbLayers = this.getLayersByType('CartoDB');
    if(!cartodbLayers.length) return null;
    var layerDef = {
      version:'1.0.1',
      layers: []
    };
  ...
  getLayersByType: function(type) {
    if (!type || type == '' ) {
      cdb.log.info("a layer type is necessary to get layers count");
      return 0;
    }
