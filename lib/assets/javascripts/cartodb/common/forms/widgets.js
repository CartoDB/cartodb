...
cdb.forms.ComboPro = cdb.core.View.extend({
  ...
  // TODO: throw error if there's no model
  //if (!this.model) throw
    ...
    this.add_related_model(this.model);
    this.model.bind("change:" + this.options.observe, this._onUpdate);
  ...
  render: function() {
    ...
    // Sets the value
    this._setValue(this.model.get(this.options.observe));
  ...
  _changeSelection: function(e) {
    ...
    a[this.options.observe] = val;

    if (val) this.model.set(a);
    else this.model.set(a, { silent: true });

    // Send trigger
    if (val) this.trigger('change', a[this.options.observe]);
  }
