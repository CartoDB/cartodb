...
namespace :cartodb do
  namespace :vizs do
    ...
    task :delete_inconsistent, [:username] => :environment do |_t, args|
      ...
      collection.each do |viz|
        delete_with_confirmation(viz) if inconsistent?(viz)
      end
    end
    ...
    task :create_named_maps, [:order] => :environment do |_t, args|
      ...
      vqb.each do |vis|
        ...
        print '.' if current % 50 == 0
        puts "\n> #{Time.now} #{current}/#{count}" if current % 500 == 0
        vis = nil
      rescue StandardError => e
        printf 'E'
      end
    ...
    task :mapcap_builder_visualizations, [:dry] => :environment do |_, args|
      ...
      Carto::Visualization.find_each(conditions: "version = 3 and type = 'derived' and privacy != 'private'") do |visualization|
        unless visualization.mapcapped?
