...
F.prototype._manageTimeSeriesForTorque = function (m) {
  ...
  if (!typeChanged && !animatedChanged) return;
  ...
  if (!_.isEqual(this._lastTSAnimateChange, animatedChanged) || !_.isEqual(this._lastTSTypeChange, typeChanged)) {
    widgetModel = this._diDashboard.getWidgets().filter(function (m) {
      return m.get('type') === 'time-series';
    })[0];
    currentTimeseries = this._getTimeseriesDefinition();
    persistWidget = !!(currentTimeseries && currentTimeseries.get('title') !== 'time_date__t');

    if (widgetModel) {
      this._removeTimeseries();
    }

    newLayer = this._getLayer(m);
    if ((newLayer.get('type') === 'torque' && m.styleModel.get('type') === 'animation') ||
      (m.get('type') === 'torque' && m.styleModel.get('type') === 'animation') ||
      persistWidget) {
      persistName = currentTimeseries && currentTimeseries.get('title');
      this._createTimeseries(newLayer, animatedChanged, persistName);
    }
    this._lastTSAnimateChange = animatedChanged;
    this._lastTSTypeChange = typeChanged;
  }
};
