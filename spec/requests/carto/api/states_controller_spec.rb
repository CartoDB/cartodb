...
describe Carto::Api::StatesController do
  ...
  def state_should_be_correct(response)
    response.body.should eq @visualization.state
  end

  describe '#show' do
    def show_state_url(user: @user, visualization: @visualization)
      state_url(
    ...
    it 'shows a state' do
      get show_state_url, {} do |response|
    ...
    it 'returns 403 if user does not own the visualization' do
      get show_state_url(user: @intruder), {} do |response|
