...
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.currentUserUrl = urls(cdb.config.get('account_host')).userUrl(this.user);
  ...
  render: function() {
    ...
    var datasetUrl = new DatasetsUrl({ userUrl: this.currentUserUrl }).toDataset(table);

    var d = {
      isRaster:                vis.get('kind') === 'raster',
      geometryType:            table.geomColumnTypes().length > 0 ? table.geomColumnTypes()[0] : '',
      title:                   vis.get('name'),
      datasetUrl:              encodeURI(datasetUrl),
      isOwner:                 isOwner,
