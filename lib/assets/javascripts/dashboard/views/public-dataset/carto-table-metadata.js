...
const CartoTableMetadata = Base.extend({
  ...
  _TEXTS: {
    columnDeleted: _t('dashboard.views.public_dataset.col_deleted'),
    columnDeleting: _t('dashboard.views.public_dataset.col_deleting'),
    columnAdded: _t('dashboard.views.public_dataset.col_added'),
    columnAdding: _t('dashboard.views.public_dataset.col_adding')
  },
  ...
  renameColumn: function (columnName, newName, opts) {
    ...
    this.notice(_t('dashboard.views.public_dataset.col_renaming'), 'load', 0);
    c.save(null, {
      success: (mdl, data) => {
        this.notice(_t('dashboard.views.public_dataset.col_renamed'), 'info');
  ...
  saveNewColumnType: function (column, newType, opts) {
    ...
    this.notice(_t('dashboard.views.public_dataset.col_changing'), 'load', 0);
    column.save(null, {
      success: () => {
        this.notice(_t('dashboard.views.public_dataset.col_changed'), 'info');
  ...
  fetch: function (opts) {
    ...
    if (!silent) {
      this.notice(_t('dashboard.views.public_dataset.tbl_loading'), 'load', this, 0, 0);
    }
    var xhr = Base.prototype.fetch.call(this, opts);
    $.when(xhr).done(() => {
      opts && opts.success && opts.success.old_success && opts.success.old_success();
      if (!silent) {
        this.notice(_t('dashboard.views.public_dataset.tbl_loaded'));
      }
    }).fail(() => {
      if (!silent) {
