...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _addOverlaysDropdown: function() {
    ...
    if (!this.overlaysDropdown) {
      ...
      $(".add_widget").append(this.overlaysDropdown.render().el);
    }
  ...
  _setupZoomOverlay: function(type, data) {
    ...
    overlay.bind("change_y", function() {
      ...
      if (this.zoomInfo) {
        this.zoomInfo.model.set("y", overlay.model.get("y") + 70);
      }
  ...
  _setupHeaderOverlay: function(type, data) {
    ...
    this.header.bind("change_y", function() {
      ...
      if (this.header.model.get("display")) {
        ...
        if (this.layer_selector) this.layer_selector.model.set("y",  headerHeight + 15);
      } else {
        if (this.zoom)   this.zoom.model.set("y",   40);
        if (this.search) this.search.model.set("y", 40);
        if (this.share)  this.share.model.set("y",  40);
        if (this.layer_selector) this.layer_selector.model.set("y",  40);
      }
  ...
  _setupZoomInfoOverlay: function(type, data) {
    ...
    overlay.bind("change_y", function() {
      ...
      if (this.fullscreen) {
        this.fullscreen.model.set("y", overlay.model.get("y") + 60);
      }
  ...
  _renderOverlay:  function(delayed_animation, data) {
    ...
    switch(type) {
      ...
      case 'Loader':         overlay = this._setupLoaderOverlay(type, data);         break;
      case 'ZoomInfo':       overlay = this._setupZoomInfoOverlay(type, data);       break;
      case 'Share':          overlay = this._setupShareOverlay(type, data);          break;
      case 'Zoom':           overlay = this._setupZoomOverlay(type, data);           break;
      case 'LayerSelector':  overlay = this._setupLayerSelectorOverlay(type, data);  break;
      case 'Fullscreen':     overlay = this._setupFullScreenOverlay(type, data);     break;
      case 'Title':          overlay = this._setupHeaderOverlay(type, data);         break;
      case 'Search':         overlay = this._setupSearchOverlay(type, data);         break;

      // Default
      case 'Text':           overlay = new cdb.admin.overlays[type]({ model: data }); break;
      case 'Image':          overlay = new cdb.admin.overlays[type]({ model: data }); break;
