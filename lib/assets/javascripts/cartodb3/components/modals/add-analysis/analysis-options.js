 ...
 *  - {Obj} -> user model
 */
module.exports = function (Analyses, userModel) {
  var categories = {
    create_clean: {
      title: _t('analysis-category.create-clean'),
      analyses: Analyses.getAnalysesByModalCategory('create_clean', userModel)
    },
    data_transformation: {
      title: _t('analysis-category.data-transformation'),
      analyses: Analyses.getAnalysesByModalCategory('data_transformation', userModel)
    },
    analyze_predict: {
      title: _t('analysis-category.analyze-predict'),
      analyses: Analyses.getAnalysesByModalCategory('analyze_predict', userModel)
    }
  };
    ...
    categories['generated'] = {
      ...
      analyses: _.compact(
        Object
          ...
          .map(function (type) {
            if (!Analyses.isAnalysisValidByType(type, userModel)) {
              return false;
            }
