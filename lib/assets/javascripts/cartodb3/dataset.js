...
var OrganizationUsers = require('./data/organization-users-collection');
...
if (userModel.isInsideOrg()) {
  userModel.usersOrganization = new OrganizationUsers(null, {
    organizationId: userModel._organizationModel.id,
    currentUserId: userModel.id,
    configModel: configModel
  });

  userModel.usersOrganization.fetch();
}
...
var query = layerModel.get('sql') || 'SELECT * FROM ' + Utils.safeTableNameQuoting(tableName);
var isOwner = tableModel.isOwner(userModel);
var userName;

if (!isOwner) {
  userName = tableModel.get('permission').owner.username;
