...
module.exports = cdb.core.View.extend({
  ...
  _initCollections: function () {
    ...
    this.category = this._baseLayer.get('category') ? this._baseLayer.get('category') : this._getDefaultSource();

    this._sourcesCollection = new CarouselCollection(
      _.map(this._getSources(), function (source) {
        return {
          selected: this.category === source,
          val: source,
          label: source,
          template: function () {
            return source;
  ...
  _renderHeader: function () {
    ...
    this._headerView = new BasemapHeader({
