  ...
  cdb.admin.GeocodingDialog = cdb.common.CreateDialog.extend({
    ...
    _onGeocodingChosen: function() {
      ...
      // Check if World was selected as location (being free text)
      var location = ( obj.location && obj.location.toLowerCase() ) || '';
      if (obj.text && obj.kind === "admin1" && location && location.search('world') !== -1) {
        // Change kind to admin0
        obj.kind = "admin0";
      }
      
      // Send trigger to parent
      this.trigger('geocodingChosen', obj, this);

      // Not necessary to send mixpanel event due to
      // backend is doing right now
      // cdb.god.trigger('mixpanel', 'Geocoding', obj);
