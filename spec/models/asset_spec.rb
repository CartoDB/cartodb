...
describe Asset do
  ...
  def local_path(asset)
    local_url = CGI.unescape(asset.public_url).gsub(%r{/uploads}, '')
  ...
  describe '#create' do
    describe 'on local filesystem' do
      ...
      it 'should save the file when passing a full path as an argument' do
        ...
        File.exist?(local_path(asset)).should be_true
        asset.public_url.should =~ %r{.*test/#{@user.username}/assets/\d+cartofante_blue\.png.*}
      end
      ...
      it 'should import assets with spaces in their name' do
        ...
        File.exist?(local_path(asset)).should be_true
        asset.public_url.should =~ %r{/test/#{@user.username}/assets/\d+cartofante%20blue\.png}
      end
    end
  end
