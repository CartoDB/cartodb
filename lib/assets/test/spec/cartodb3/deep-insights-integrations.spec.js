...
describe('deep-insights-integrations', function () {
  ...
  describe('when analysis-definition-node is created', function () {
    ...
    it('should listen to status changes and set the status on the analysis-definition-node', function () {
      var cdbAnalysis = this.analysis.findNodeById('a0');

      expect(this.a0.get('status')).toBeUndefined();

      cdbAnalysis.set('status', 'ready');

      expect(this.a0.get('status')).toBe('ready');
    });
