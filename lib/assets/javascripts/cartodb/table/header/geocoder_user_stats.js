  ...
  cdb.admin.GeocoderStats = cdb.core.View.extend({
    ...
    _TEXTS: {
      ...
      available_hard: _t('<p><%= credits %> geocoding credit<%= credits > 1 ? "s" : "" %> still available.</p>' +
                         '<% if (cost && cost > 0) { %><p><strong>We calculate that geocoding this table completely could cost you up to $<%= cost %>.</strong></p><% } %>'),
    ...
    initialize: function() {
      ...
      this.estimations.fetch({ success : function(res) {
        self.cost = Math.ceil(res.models[0].get("estimation")/100);
        self.render();
      }});
