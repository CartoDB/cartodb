...
var CreateDatasetModel = require('./create_dataset_model');
...
module.exports = BaseDialog.extend({
  ...
  initialize: function() {
    ...
    if (this.options.type === 'dataset') {
      this.model = new CreateDatasetModel({}, modelOpts);
    } else if (this.options.type === 'map') {
      this.model = new CreateMapModel({}, modelOpts);
    } else {
      throw new Error('creation type not supported: ' + this.options.type);
    }
    this.model.bind('remoteSelected', this._onRemoteSelected, this);
