...
export async function createNewBQConnection (context, { name, ...payload }) {
  if (!context.rootState.user) {
    return;
  }
  ...
  const response = await fetch(url, { method: 'POST',
    ...
    body: JSON.stringify({ name, connector: 'bq-v2', parameters: { service_account: JSON.stringify(payload) } }) });
