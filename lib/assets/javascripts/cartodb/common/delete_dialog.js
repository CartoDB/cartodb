...
cdb.admin.DeleteDialogItem = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.template = cdb.templates.getTemplate('common/views/delete_dialog_item');
  },
...
cdb.admin.DeleteDialog = cdb.admin.BaseDialog.extend({
  ...
  _showVisualizations: function() {
    ...
    var onLoaderHidden = function() {
      ...
      var synced        = self.model.synchronization.isSync();
      ...
      if (synced) {
        var item = new cdb.admin.DeleteDialogItemModel({ type: "synced" });
        self.items.add(item); 
      }
  ...
  _showExport: function(ev) {
    ...
    var export_dialog = new cdb.admin.ExportTableDialog({
      ...
      user: this.options.user, // globals makes baby jesus cry
      autoClose: true
    });
