cdb.admin.CategoryTree = cdb.core.Model.extend({
  ...
  initialize: function(attrs, opts) {
    ...
    this._counts = attrs.counts === true;
  ...
  createFromArray: function(categoryArray) {
    ...
    for (i = 0; i < numItems; ++i) {
        ...
        if (parentCategory) {
          parentCategory.children.push({ id: item.id, name: item.name, parent_id: item.parent_id, list_order: item.list_order, count: item.count, children: [] });
        }
    }
  },
  ...
  load: function(callbackContext, onSuccess, onError) {
    var self = this;
    $.ajax({
      url: window.location.origin + '/user/' + this.user.get('username') + '/api/v1/viz/subcategories',
      data: {
        category_id: self._rootId,
        counts: self._counts
      }
    })
