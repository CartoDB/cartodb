...
cdb.admin.Layers = cdb.geo.Layers.extend({
  ...
  saveLayers: function(opts) {
    ...
    function saveLayer(layer) {
      // Layer opts: we have to remove main success and error
      // callbacks (if they exist) in order to proccess all saves
      // sequentially
      var _opts = _.extend(
        _.omit(opts, 'success', 'error', 'complete'),
        {
          success: function() {
            ++count;
            if (size > count) {
...
cdb.admin.Map = cdb.geo.Map.extend({
  ...
  _setBaseLayer: function(newBaseLayer, opts) {
    ...
    if (currentBaseLayer) {
      if (currentBaseLayer.get('type') === newBaseLayer.get('type')) {
        this._updateBaseLayer(currentBaseLayer, newBaseLayer, opts);
      } else {
        this._replaceBaseLayer(currentBaseLayer, newBaseLayer, opts);
      }
    } else {
      this._addBaseLayer(newBaseLayer, opts);
    }

    if (newBaseLayer.get('labels') && newBaseLayer.get('labels').url) {
      if (this._hasLabelsLayer()) {
        // TODO: update top layer
        // this.layers.add(topLayer, { merge: true });
      } else {
        this.layers.add({
          urlTemplate: newBaseLayer.get('labels').url,
          attribution: newBaseLayer.get('attribution'),
          minZoom: newBaseLayer.get('minZoom'),
          maxZoom: newBaseLayer.get('maxZoom'),
          subdomains: newBaseLayer.get('subdomains'),
          kind: "tiled"
        });
      }
    } else {
      if (this._hasLabelsLayer()) {
        // TODO: Remove layer with labels
      }
    }
  ...
  _updateBaseLayer: function(currentBaseLayer, newBaseLayer, opts) {
    var self = this;
    var id = currentBaseLayer.get('id');
    var order = currentBaseLayer.get('order');
    currentBaseLayer.clear({ silent: true });
    currentBaseLayer.set(_.extend({
      id: id,
      order: order
    }, _.omit(newBaseLayer.attributes, 'id', 'order'))
    );
    _.defer(function() {
  ...
  _replaceBaseLayer: function(currentBaseLayer, newBaseLayer, opts) {
    ...
    newBaseLayer.set('id', currentBaseLayer.get('id'));
    newBaseLayer.set('order', currentBaseLayer.get('order'));
    this.layers.add(newBaseLayer, { at: 0 });
    newBaseLayer.save(null, {
      success: function() {
        self.trigger('baseLayerAdded');
        opts.success && opts.success();
      },
  ...
  _addBaseLayer: function(newBaseLayer, opts) {
    this.layers.add(newBaseLayer, { at: 0 });
    this.trigger('baseLayerAdded');
    opts.success && opts.success();
  },

  _hasLabelsLayer: function() {
    return this.layers.getTiledLayers().length > 0 &&
      this.layers.last().get('type') === 'Tiled';
  },
