...
describe('data/query-rows-collection', function () {
  beforeEach(function () {
    ...
    this.querySchemaModelSetSpy = spyOn(QuerySchemaModel.prototype, 'set');
    this.querySchemaModelSetSpy.and.callThrough();
  ...
  describe('.fetch', function () {
    it('should call _onQueryRowsCollectionError when success response has property error', function () {
      ...
      spyOn(this.collection, '_onQueryRowsCollectionError');
      ...
      expect(this.collection._onQueryRowsCollectionError).toHaveBeenCalledWith(requestResponse);
    });
  });

  describe('._onQueryRowsCollectionError', function () {
    it('should set error in QuerySchemaModel if there isn\'t any other error previously', function () {
      var requestResponse = {
        error: ['Postgres Error']
      };

      this.collection._onQueryRowsCollectionError(requestResponse);

      expect(this.querySchemaModel.set).toHaveBeenCalledWith({
        query_errors: requestResponse.error,
        status: 'unavailable'
      });
    });

    it('should not set error in QuerySchemaModel if there is any other error previously', function () {
      this.querySchemaModel.set({
        query_errors: 'Test Error',
        status: 'errored'
      });

      this.querySchemaModelSetSpy.calls.reset();
      ...
      expect(this.querySchemaModel.set).not.toHaveBeenCalled();
    });
  });
