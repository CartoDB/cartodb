...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  events: {
    'click .add_widget.button':    '_onClickAddWidget',
  ...
  initialize: function() {
    ...
    _.bindAll(this, "_renderWidgets", "_renderWidget", "_fetchWidgets");
  ...
  */
  _bindDataLayer: function(layerView, layer) {
    ...
    if (layerType === 'CartoDB' || layerType === 'torque') { // unbind previos stuff
      ...
      self.widgetCollection = layer.widgets;
      self.widgetCollection.fetch({ reset: true });

      self.widgetsDropdown.collection = self.widgetCollection;
      self.widgetsDropdown.addForm();

      self.widgetCollection.bind("reset", function(widget) {

        if (self.widgetCollection.models.length > 0) {
          self.urls = [];
          self.urls = self.widgetCollection.at(0).get("overlays");
          self._fetchWidgets();
        }

      });

      self.widgetCollection.bind("remove", function(widget) {
        widget.destroy();
      });

      self.widgetCollection.bind("change", function(widget) {
        widget.save();
      });

      self.widgetCollection.bind("add", function(widget) {

        widget.save();

        self._renderWidget(false, widget);
      ...
      if (self.options.geocoder) {
        ...
        self.options.geocoder.bind('geocodingComplete geocodingError geocodingCanceled', function() {
          if(self.layerModel) {
            self.layerModel.table.fetch();
          }
        }, this);
  ...
  _cleanWidgets: function() {
    ...
    if (this.widgets) {
      _.each(this.widgets, function(widget) {
        //widget.clean(); // TODO: implement clean
      });
    }
  ...
  _onClickAddWidget: function(e) { // TODO: move this to a proper Header button tabs 
  ...
  _addWidgets: function() {
    ...
    var layers = this._getCartoDBLayers();

    _.each(layers, function(layer) {

      self.widgets = layer.widgets.models;
  ...
  _fetchWidgets: function() {
    ...
    _.each(this.urls, function(id) {
      var model = new cdb.admin.models.widgets.Text();
      model.urlRoot = self.widgetCollection.url;
      model.id = id;

      model.fetch({ success: function(model, response, options) {
        //model.set("options", JSON.parse(model.get("options")))
        //debugger;
        self._renderWidget(true, model);
      }});
  ...
  _renderWidgets: function() {
    ...
    _.each(this.widgets, function(data) {

      this._renderWidget(true, data);
  ...
  _renderWidget:  function(delayed_animation, data) {

    var self = this;

    var text = new cdb.admin.widgets.Text({ model: data });

    text.bind("remove", function(widget) {

      console.log('remove')
      //this.widgetCollection.remove(widget.model);
    ...
    this.mapView.$el.append(text.render().$el);
    ...
    if (delayed_animation) {
      ...
      setTimeout(function() {
        ...
        text.$el.show()
        text.$el.addClass('animated bounceIn');
  ...
  _addTableOptionsDropdown: function() {
    ...
    var $options = $("<a href='#show-options' class='button show-table-options'>Options</a>")
    this.mapView.$el.append($options);
