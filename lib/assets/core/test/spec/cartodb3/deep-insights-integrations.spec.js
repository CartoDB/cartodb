...
describe('deep-insights-integrations/dii', function () {
  ...
  _.each([
    {
      previousBasemapCategory: 'CartoDB',
      newBasemapCategory: 'CartoDB',
      expectedMapProvider: 'leaflet'
    },
    {
      previousBasemapCategory: 'CartoDB',
      newBasemapCategory: 'GMaps',
      expectedMapProvider: 'googlemaps'
    }
  ], function (testCase) {
    var newBasemapCategory = testCase.newBasemapCategory;
    var previousBasemapCategory = testCase.previousBasemapCategory;
    var categoryChanged = newBasemapCategory !== previousBasemapCategory;
    var expectedMapProvider = testCase.expectedMapProvider;
    var testSufix = "category hasn't changed";
    if (categoryChanged) {
      testSufix = 'category has changed';
    }

    describe('when the base layer has changed and basemap ' + testSufix, function () {
      beforeEach(function () {
        this.silentExpected = categoryChanged;
        ...
        this.layerDefinitionsCollection.at(0).set('category', previousBasemapCategory);
      ...
      it('should re-create the cdb.js layer if type has changed', function () {
        // Clear some attributes to then simulate a base layer change with a
        // different layer type. We keep the existing category attribute so that
        // we can test if operations need to be silent or not
        this.layerDefinitionsCollection.at(0).attributes = {
          category: previousBasemapCategory
        };
        this.layerDefinitionsCollection.at(0).set({
          id: 'baseLayer',
          type: 'Plain',
          color: '#FABADA',
          category: newBasemapCategory,
          order: 0
        });
        ...
        expect(this.cdbjsMap.createPlainLayer).toHaveBeenCalledWith({
          ...
          category: newBasemapCategory,
          order: 0
        }, { at: 0, silent: this.silentExpected });
      });
      ...
      it('should change the map provider', function () {
        ...
        expect(this.cdbjsMap.set).toHaveBeenCalledWith('provider', expectedMapProvider);
      });
        ...
        describe('if cdb.js does NOT have a layer with labels', function () {
          ...
          it('should create the cdb.js labels layer', function () {
            ...
            expect(this.cdbjsMap.createTileLayer).toHaveBeenCalledWith({
              ...
              type: 'Tiled',
              urlTemplate: 'urlTemplate2'
            }, { at: 1, silent: this.silentExpected });
          });
        });
      });
      ...
      describe('if new new base layer does NOT have labels', function () {
        ...
        describe('if cdb.js has a layer with labels', function () {
          ...
          it('should remove the cdb.js labels layer', function () {
            ...
            expect(this.cdbjsLabelsLayer.remove).toHaveBeenCalledWith({ silent: this.silentExpected });
          });
        });
      });
    });
