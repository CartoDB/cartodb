  ...
  enableMap: function() {
    ...
    if (!this.map_enabled) {
      ...
      this.vis.on("change:name change:description", function() {
        ...
        var overlays = self.overlays.filter(function(m) { return m.get("type") == "header" });
        var overlay  = overlays[0];

        overlay.set("title", self.vis.get("name"));
        overlay.set("description", self.vis.get("description"));
