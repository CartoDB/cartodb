...
module CartoDB
  class HiresGeocoder < HiresGeocoderInterface
    ...
    attr_reader :app_id, :token, :mailto, :status,
                :processed_rows, :successful_processed_rows, :failed_processed_rows, :empty_processed_rows,
                :total_rows, :dir, :non_batch_base_url
    ...
    def geocode_text(text)
      ...
      CartoDB.notify_debug("Non-batched geocoder couldn't parse response",
                           error: e.inspect, backtrace: e.backtrace, text: text, response_body: http_response.body)
      raise e
    end
