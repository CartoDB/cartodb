...
describe('components/form-components/editors/fill/input-color/assets-view', function () {
  ...
  function mockCreate (callback) {
    this.view._assetCollection.create = function (options, callbacks) {
      ...
      if (callback) {
        if (callback === 'error') {
          callbacks[callback].call(this, new Backbone.Model(), { responseText: '{"error": ["hi, I am an error"]}'
 });
        } else {
  ...
  describe('._onAssetUploadComplete', function () {
    it('should set go to the upload tab', function () {
      ...
      expect(this.view._stateModel.get('status')).toBe(undefined);
