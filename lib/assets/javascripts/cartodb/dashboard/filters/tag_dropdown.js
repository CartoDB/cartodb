...
cdb.admin.TagDropdown = cdb.ui.common.Dropdown.extend({
  ...
  _initBinds: function() {
    ...
    this.visualizations.bind('add remove',  this._fetchTags, this);
    this.tables.bind('add remove',          this._fetchTags, this);

    // Once tags are synced, render list
    this.tags.bind('sync', this.render, this);

    // If path changes, fetch tags
    this.router.model.bind('change', this._fetchTags, this);
  ...
  _fetchTags: function() {
    this.tags.fetch({
      data: {
        type: this.router.model.get('model') === "tables" ? 'table' : 'derived'
      }
    });
  },
  ...
  _click: function(e) {
    ...
    // if (this.model.get("type") == 'table') {
    //   this.tables.options.set({ tags: name, per_page: 3 });
    //   this.tables.fetch();
    // } else {
    //   this.visualizations.options.set({ tags: name, per_page: 3 });
