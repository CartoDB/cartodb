...
module.exports = Backbone.Collection.extend({
  ...
  createNewAnalysisNode: function (layerDefinitionModel, nodeAttrs) {
    var c = this._analysisDefinitionNodesCollection;

    var nodeModel = c.add(nodeAttrs, {parse: false});
  ...
  _onChange: function (m) {
    ...
    if (!m.isNew()) {
      if (attrs.type) {
        ...
        // TODO tmp hack until cartodb.js is updated
        if (attrs.source) {
          attrs.source = window.vis.analysis.findNodeById(attrs.source);
          attrs.cartocss = m.get('cartocss'); // need to force change to apply these styles instead of tile_style
          layer.set(attrs);
        } else {
          layer.update(attrs);
        }
