...
cdb.admin.dashboard.TableItem = cdb.core.View.extend({
  ...
  render: function() {
    ...
    this.$el.append(this.template(_.extend(attrs, { row_count: pretty_row_count, name: name, description: description, tags: tags })));
  ...
  */
  _confirmAndDelete: function(ev) {
    ...
    $.when(this.table.fetch())
    .done(this._showDeleteDialog)
    .fail(function(){
      // Handle error
    });
