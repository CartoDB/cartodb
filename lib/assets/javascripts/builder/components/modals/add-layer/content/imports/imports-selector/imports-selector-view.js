...
module.exports = CoreView.extend({
  ...
  _checkOptions: function (options) {
    return _.some(options, (option) => {
      return !_.isEmpty(option) && option.enabled(this._configModel, this._userModel);
    });
  },
  ...
  _generateContent: function () {
    ...
    const dataobservatoryOptions = this._filterImportsByType(IMPORTER_TYPES.dataobservatory);
    if (this._checkOptions(dataobservatoryOptions)) {
      this._generateImportButtons(dataobservatoryOptions);
    }

    const cloudOptions = this._filterImportsByType(IMPORTER_TYPES.cloud);
    if (this._checkOptions(cloudOptions)) {
      this.$el.append(CategoryTitleTemplate({ title: 'Cloud Storage' }));
      this._generateImportButtons(cloudOptions);
    }

    const databaseOptions = this._filterImportsByType(IMPORTER_TYPES.database);
    if (this._checkOptions(databaseOptions)) {
      this.$el.append(CategoryTitleTemplate({ title: 'Database' }));
      this._generateImportButtons(databaseOptions);
    }

    const otherOptions = this._filterImportsByType(IMPORTER_TYPES.other);
    this.$el.append(CategoryTitleTemplate({ title: 'Other' }));
    this._generateImportButtons(otherOptions, true);
  },
  ...
  _generateImportButtons: function (filteredOptions, other) {
    const importButtonsElements = $('<div/>').addClass('ImportOptions__row');

    filteredOptions.forEach(function (importConfig) {
      let connector = {};
