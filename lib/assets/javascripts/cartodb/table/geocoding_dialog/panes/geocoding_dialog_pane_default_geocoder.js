  ...
  cdb.admin.GeocodingDialog.Pane.DefaultGeocoder = cdb.admin.GeocodingDialog.Pane.extend({
    ...
    _initViews: function() {
      ...
      // Check and change pane height if it is necessary
      // setTimeout(function() {
      //   self._changePaneHeight()
      // }, 5000);
    },
    ...
    _getCountries: function() {
      if (this.countries_data.size() > 0) {
        return _.union(
          'the World',
          this.countries_data.map(function(c,i) {
    ...
    _onChangeStep: function() {
      ...
      // Move wrapper
      var pos = this.$('.geocoding-pane-step.second').position().left;
      this.$('.geocoding-pane-content-wrapper')
        .animate({
          'margin-left': step === 0 ? 0 : ( -pos + 40 /* the margin between elements */ ) 
        }, {
          duration: 350,
          queue: false
        });
