...
describe('components/form-components/editors/fill/input-color/input-ramps/input-color-ramps', function () {
  ...
  describe('with nulls', function () {
    beforeEach(function () {
      ...
      var data = [{ winner: null }, { winner: 'one' }, { winner: 'two' }];

      this.view._querySchemaModel = {};
      this.view._querySchemaModel.rowsCollection = new Backbone.Collection(data);

      this.view.render();
    });

    it('should render properly', function () {
      this.view._updateRangeAndDomain();
      this.view.render();

      expect(this.view.model.get('domain')).toContain('"one"');
      expect(this.view.model.get('domain')).toContain('"two"');
      ...
      expect(this.view.$('.js-listItem:eq(1)').text()).toContain('two');
