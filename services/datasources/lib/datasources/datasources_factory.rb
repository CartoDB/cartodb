  ...
  module Datasources
    class DatasourcesFactory
      ...
      def self.get_datasource(datasource_name, user, additional_config = {})
        additional_config[:http_timeout] = DEFAULT_HTTP_REQUEST_TIMEOUT if additional_config[:http_timeout].nil?
        additional_config[:http_connect_timeout] = HTTP_CONNECT_TIMEOUT if additional_config[:http_connect_timeout].nil?
      ...
      def self.get_config(datasource_name)
        config_source = @forced_config || Cartodb.config
        ...
        when Url::Dropbox::DATASOURCE_NAME, Url::Box::DATASOURCE_NAME, Url::GDrive::DATASOURCE_NAME, Url::InstagramOAuth::DATASOURCE_NAME,
          ...
          config ||= begin
                       config_source[:oauth.to_s]
                     rescue StandardError
                       nil
                     end
        when Search::Twitter::DATASOURCE_NAME
          config = begin
                     config_source[:datasource_search]
                   rescue StandardError
                     nil
                   end
          config ||= begin
                       config_source[:datasource_search.to_s]
                     rescue StandardError
                       nil
                     end
