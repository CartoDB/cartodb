...
cdb.admin.CartoDBTableData = cdb.ui.common.TableData.extend({
  ...
  url: function() {
    ...
    var mode = this.options.get('mode') === 'des' ? 'desc': 'asc';

    // use table.id to fetch data because if always contains the real table name
    u += "&q=" + encodeURIComponent("select " + select_columns + " from " + this.table.get('id') + " order by " + this.options.get('order_by') + " " + mode);
    return u;
  },
