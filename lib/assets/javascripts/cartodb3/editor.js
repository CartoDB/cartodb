...
cdb.deepInsights.createDashboard('#dashboard', vizJSON, {
  ...
  layerDefinitionsCollection.on('add', function(layerDef) {
   InfowindowLink(layerDef, vis.map);
  });

  function mata_gay() {
    layerDefinitionsCollection.each(function(layerDef) {
      InfowindowLink(layerDef, vis.map);
    });
  }

  layerDefinitionsCollection.on('reset', mata_gay);
