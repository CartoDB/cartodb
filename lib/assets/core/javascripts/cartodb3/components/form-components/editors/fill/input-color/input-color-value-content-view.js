...
var OperatorListView = require('./input-operators/operators-list-view');
...
module.exports = CoreView.extend({
  initialize: function (opts) {
    ...
    this._isAggregated = opts.isAggregated;
  ...
  _initViews: function () {
    ...
    var stackViewCollection = new Backbone.Collection([{
      ...
      createStackView: function (stackLayoutModel, opts) {
        return self._createOperatorsList(stackLayoutModel, opts).bind(self);
      }
    }, {
      createStackView: function (stackLayoutModel, opts) {
    ...
    if (this.model.get('attribute')) {
      ...
      if (showCategories) {
        this._stackLayoutView.model.set('position', 3);
      } else {
  ...
  _createInputColorRamps: function (stackLayoutModel, opts) {
    var position = this._isAggregated ? 1 : 0;
    ...
    view.bind('back', function (value) {
      this.model.unset('attribute', { silent: true });
      stackLayoutModel.goToStep(position);
    }, this);
  ...
  _createOperatorsList: function (stackLayoutModel, opts) {
    var columns = _.reject(this._columns, function (column) {
      return column.type === 'geometry' || _.contains(EXCLUDED_COLUMNS, column.label);
    }, this);

    var view = new OperatorListView({
      stackLayoutModel: stackLayoutModel,
      columns: columns,
      showSearch: true,
      typeLabel: 'column',
      operator: this.model.attributes.operator || 'count',
    ...
    view.bind('change', function (item) {
      this._onOperatorsChange(item, stackLayoutModel);
    }, this);
  ...
  _onOperatorsChange: function (data, stackLayoutModel) {
