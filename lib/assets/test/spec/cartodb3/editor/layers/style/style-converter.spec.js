...
},
{
  ...
  result: {
    point: { 
      cartocss: '#layer {\npolygon-fill: #000;\npolygon-opacity: 0.4;\n}',
      sql: "WITH hgrid AS (SELECT CDB_HexagonGrid(ST_Expand(!bbox!, greatest(!pixel_width!,!pixel_height!) * 100), greatest(!pixel_width!,!pixel_height!) * 100) as cell) SELECT hgrid.cell as the_geom_webmercator, count(i.test) as points_count, count(i.test)/power( 100 * CDB_XYZ_Resolution(CDB_ZoomFromScale(!scale_denominator!)), 2 ) as points_density, 1 as cartodb_id FROM hgrid, __wrapped i where ST_Intersects(i.the_geom_webmercator, hgrid.cell) GROUP BY hgrid.cell"
    }
  }
}
];
