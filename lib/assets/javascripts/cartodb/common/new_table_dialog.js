  ...
  cdb.admin.CreateTableDialog = cdb.ui.common.Dialog.extend({
    ...
    _TEXTS: {
      ...
      error: {
        default:  _t('Ooops, there was an error in the importation.'),
    ...
    initialize: function() {
      ...
      _.extend(this.options, {
        ...
        // TODO: remove all "user_data" and "config" references, they are "global variables" :(
    ...
    _importService: function(obj) {
      ...
      imp.save(null, {
        ...
        error: function(e) {
          self._showError(
            '',
            self._TEXTS.error.default,
            self._TEXTS.error.sorry,
            imp.get('item_queue_id')
          );
        }
      });
    },
    ...
    _onUploadError: function(e, data) {
      ...
      if (this.filePane) {
        ...
        // Set default error if it doesn't exist
      ...
      imp.bind("importComplete", function(e){
        ...
        if (imp.get('tables_created_count') && imp.get('tables_created_count')>1) {
          ...
          if (!imp.get("table_name")) {
            ...
            window.location.href = cdb.config.prefixUrl() + "/tables/" + imp.get("table_name") + "/";
          }
    ...
    _createTable: function() {
      ...
      this.options.tables
        .bind("add", function(m) {
          created = true;
          window.location = cdb.config.prefixUrl() + "/tables/" + m.get("name") + "/";
        },this)
