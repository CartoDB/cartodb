...
Backbone.Form.editors.Select = Backbone.Form.editors.Base.extend({
  ...
  initialize: function (opts) {
    ...
    var collectionData = opts.schema.options;

    if (opts.schema.editorAttrs && opts.schema.editorAttrs.customValue) {
      var customValue = opts.schema.editorAttrs.customValue;
      var attrExists = _.find(collectionData, function (attr) {
        return attr.val === customValue.val;
      });

      if (!attrExists) {
        collectionData.push(customValue);
        collectionData = _.sortBy(collectionData, function (d) {
          return d.val;
        });
      }
    }

    this.collection = new CustomListCollection(collectionData);
    this._initViews();
