...
class Admin::TablesController < ApplicationController
  ...
  def index
    ...
    unless params[:public]
      ...
      @tables = Table.filter(:user_id => current_user.id).order(:id).reverse.select(:id,:name,:privacy,:updated_at,:tags,:user_id).
      ...
      @tables = Table.filter(~{:user_id => current_user.id} & {:privacy => Table::PUBLIC}).order(:id).reverse.
                      select(:id,:name,:privacy,:updated_at,:tags,:user_id).paginate(current_page, per_page)
