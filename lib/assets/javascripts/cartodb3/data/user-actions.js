...
module.exports = function (params) {
  ...
  return {
    ...
    deleteAnalysisNode: function (nodeId) {
      ...
      var oldNodeStyle = layerDefModel.getStyleHistoryForNode(primarySourceNode.id);
      if (oldNodeStyle) {
        layerDefModel.set({
          source: primarySourceNode.id,
    ...
    createLayerForAnalysisNode: function (nodeId, cfg) {
      ...
      if (prevLayer) {
        if (nodeDefModel.hasPrimarySource()) {
          ...
          var oldNodeStyle = prevLayer.getStyleHistoryForNode(nodeDefModel.getPrimarySource().id);
          ...
          //  | [A0] |  =>  | [A0] |  | [A0] |
          //  |______|      |______|  |______|
          var oldNodeStyle = prevLayer.getStyleHistoryForNode(nodeId);
          tableName = prevLayer.get('table_name');
          tableNameAlias = prevLayer.get('table_name_alias');
          ownerName = prevLayer.get('user_name');
          attrs = createDefaultCartoDBAttrs(oldNodeStyle);
        ...
        var oldNodeStyle = ownerLayer.getStyleHistoryForNode(nodeId);
        ...
        // Update the style history in the old layer (A)
        var style_history = ownerLayer.get('style_history');
        if (style_history) {
          if (style_history[nodeId]) {
            style_history[newLayerHeadNode.id] = style_history[nodeId];
          }
        }
