...
module.exports = Backbone.Collection.extend({
  ...
  resetByLayerDefinition: function () {
    ...
    if (current) {
      ...
      this._walkAnalysisChain(current, this._layerDefinitionModel, function (nodeDefModel) {
        attrsList.push(_.clone(nodeDefModel.attributes));
      });
  ...
  deleteNode: function (nodeId) {
    var current = this._layerDefinitionModel.getAnalysisDefinitionNodeModel();
    var nodeDefModelToDestroy;

    this._walkAnalysisChain(current, this._layerDefinitionModel, function (nodeDefModel) {
      if (nodeDefModel.id === nodeId) {
        nodeDefModelToDestroy = nodeDefModel;
      }
    });

    // Destroy after walk to avoid exceptions being thrown
    if (nodeDefModelToDestroy) {
      nodeDefModelToDestroy.destroy();
    }
  },
