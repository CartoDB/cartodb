...
var LayerDefinitionsCollectionIntegration = require('./deep-insights-integration/layer-definitions-collection-integration');
...
var F = function (opts) {
  ...
  LayerDefinitionsCollectionIntegration.track({
  ...
  this._deepInsightsDashboard.onStateChanged(saveStateDefinition);
...
F.prototype._onWidgetDefinitionAdded = function (m) {
  var widgetModel = this._deepInsightsDashboard.getWidget(m.id);
  if (widgetModel) {
    widgetModel.set({
      show_stats: true,
      show_options: true
...
F.prototype._createWidgetModel = function (m) {
  ...
  var widgetModel = this._deepInsightsDashboard[methodName](attrs, layerModel);
...
F.prototype._manageTimeSeriesForTorque = function (m) {
  ...
  var widgetModel = this._deepInsightsDashboard.getWidgets().filter(function (m) {
...
F.prototype._saveStateDefinition = function () {
  var state = this._deepInsightsDashboard.getState();
...
F.prototype._onBoundsSet = function (bounds) {
  this._deepInsightsDashboard._dashboard.vis.map.setBounds(bounds);
};
