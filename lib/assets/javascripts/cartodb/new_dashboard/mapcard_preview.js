...
module.exports = cdb.core.View.extend({
  ...
  load: function() {
    ...
    if (this.privacy && (this.privacy === "PRIVATE" || this.privacy === "PASSWORD") ) {
      url = url + "?api_key=" + this.api_key;
    }

    if (this.options.zoom) {
      cdb.Image(url, { override_bbox: true, https: this._isHTTPS() }).zoom(this.options.zoom).size(this.width, this.height).getUrl(this._onImageCallback);
    } else {
      cdb.Image(url, { override_bbox: true, https: this._isHTTPS() }).size(this.width, this.height).getUrl(this._onImageCallback);
    }
