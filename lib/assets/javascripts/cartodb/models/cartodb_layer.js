cdb.admin.CartoDBLayer = cdb.geo.CartoDBLayer.extend({
  ...
  moveToFront: function(opts) {
    var self = this;
    var otherDataLayers = this.collection.getDataLayers().filter(function(layer) {
      return self.cid !== layer.cid;
    })
    if (otherDataLayers.length) {
      var dataLayerOnTop = otherDataLayers.splice(-1)[0];
      var newOrder = dataLayerOnTop.get('order') + 1;
    } else {
      var newOrder = 1;
    }
    this.save({
