...
module.exports = CoreView.extend({
  ...
  _onStyleChanged: function (model) {
    const fill = model.get('fill');
    const isAutoStyleApplied = Boolean(this._layerDefinitionModel.get('autoStyle'));
    const shouldPreserveAutoStyle = !fill.color.fixed && isAutoStyleApplied;

    this._styleModel.attributes.autogenerated = false;
    this._unsetExtraStyleProperties();
    this._layerDefinitionModel.save({}, { shouldPreserveAutoStyle });
