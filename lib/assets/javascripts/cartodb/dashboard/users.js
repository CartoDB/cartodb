  ...
  cdb.admin.dashboard.UserStats = cdb.core.View.extend({
    ...
    _calculateQuotas: function() {
      ...
      // Check mapviews quota status
      var sum = this._calcTotalMapViews();
      mapviews_percentage  = this._calcMapViewsPercentage(sum);

      if (mapviews_percentage < 80) {
        attrs.mapviews_quota_status = "";
      } else {
        if (mapviews_percentage < 90) {
          attrs.mapviews_quota_status = "danger";
        } else {
          attrs.mapviews_quota_status = "boom";
        }
      }
    ...
    _animateChange: function() {
      ...
      var attrs = this.model.toJSON()
        ...
        , $mapviews = this.$el.find("section.stats li:eq(2)")
        , width = 4
        , text = "";
