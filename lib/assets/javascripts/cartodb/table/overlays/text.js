cdb.admin.overlays.Text = cdb.core.View.extend({
  ...
  _savePosition: function(editable) {
    ...
    var r = $(".cartodb-map").width()  - x;
    var b = $(".cartodb-map").height() - y;

    // Default positions
    extra.portraitDirection  = "top";
    extra.landscapeDirection = "left";

    var pLeft = (x + (this.$el.width()/2)) / $(".cartodb-map").width() * 100;
    var pTop  = (y + (this.$el.height()/2)) / $(".cartodb-map").height() * 100;

    var landscapeDominantSide = x <= r ? "left" : "right";
    var portraitDominantSide =  y <= b ? "top" : "bottom";

    //console.log("pLeft: " + pLeft, "ptop: " + pTop, "landscapeDominantSide" + landscapeDominantSide, "portraitDominantSide" + portraitDominantSide);

    //console.log("x: " + x, + "y: " + y, "right" + r, "bottom" + b)

    extra.landscapeDominantSide = landscapeDominantSide;
    extra.portraitDominantSide  = portraitDominantSide;
