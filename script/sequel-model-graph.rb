...
OUTPUT_FILE = '/tmp/sequel-graph.dot'.freeze
...
Sequel::Model.subclasses.each do |model|
  ...
  associations.each do |assoc|
    ...
    association_type = INCLUDE_ASSOCIATION_TYPE ? %([ label=\"#{assoc[:type]}\" ]) : ''
    graph << "#{model.to_s.demodulize} -> #{assoc.associated_class} #{association_type};"
  end
