...
cdb.admin.GeoreferenceDialog = cdb.admin.BaseDialog.extend({
  ...
  _onGeocodingComplete: function(msg) {
    ...
    if (msg.kind != "high-resolution") { // FREE
      ...
      if (success){
        description = _.template(this._TEXTS.georeference_messages.free)(msg);
      } else if (msg.real_rows == 0) {
        description = _.template(this._TEXTS.georeference_messages.zero_free)(msg);
      }

    } else {

      if (success){
        description = _.template(this._TEXTS.georeference_messages.paid)(msg);
      } else if (msg.real_rows == 0) {
        description = _.template(this._TEXTS.georeference_messages.paid)(msg);
      }
    }
    ...
    /*
       ...
       var description = _.template(this._TEXTS.georeference_messages.paid)(msg),

       description = "<p>" + msg.real_rows + " out of " + msg.processable_rows + " rows were succesfully turned into " + msg.geometry_type + "!</p>";

       if (!success) {
       description +="<p>It seems that some of your rows didn't finish succesfully. Perhaps these rows contained empty values or perhaps we just didn't know what the values meant. We encourage you to take a look and try again.</p>"
       }

       description += "<p>This job cost you $0 because we love you for being you!</p>";

       } else {

       if (user_data.geocoding.quota < user_data.geocoding.monthly_use) {

       description = "<p>" + msg.real_rows + " out of " + msg.processable_rows + " rows were succesfully turned into points!</p>";

       if (success) {
       description += "<p>It seems that some of your rows didn't finish succesfully. Perhaps these rows contained empty values or perhaps we just didn't know what the values ment. Unsuccessful rows don't count against your quota, so we encourage you to take a look and try again.</p>";
       }

       description += "<p>This job cost you $0 because we love you! You have " + user_data.geocoding.quota + " geocoding tokens remaining in your free quota this month.</p>";
    ...
    var dlg = new cdb.admin.GeocoderMessageDialog({
      ...
      title: msg.real_rows + " rows were georeferenced",
  ...
  _geoCodeIP: function() {
    ...
    this.options.geocoder.set({ 
      formatter: this.model.get("column"),
      geometry_type: "point",
      kind: kind
    });
