    ...
    if (service !== null && !hasFetchFailed) {
      ...
      AnalysesQuotaEstimation.fetch(query)
        ...
        .then(function (enoughQuota) {
          ...
          payload.canRunAnalysis = payload.creditsLeft > 0 || payload.quotaInfo.hardLimit === false;
    ...
    var states = [
      ...
      },
      {
        ...
        createContentView: function () {
          ...
          var blockPrice = (payload.quotaInfo.blockPrice / 100);
          ...
          /*
          var data = _.extend(QuotaPresenter.make(payload), {
          ...
          */
          ...
          return InfoboxFactory.createQuota({
            type: 'alert',
            title: _t('editor.layers.analysis-form.quota.title'),
            body: payload.creditsLeft > 0
              ? _t('editor.layers.analysis-form.quota.credits-left-body', {
                blockSize: payload.quotaInfo.blockSize,
                blockPrice: blockPrice
              })
              : getNoCreditMessage(payload.quotaInfo),
            confirmLabel: confirmLabel,
            confirmType: 'primary',
            confirmDisabled: !this._canSave() || !payload.canRunAnalysis,
            confirmPosition: 'right',
