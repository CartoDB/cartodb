(function() {
  ...
  var SimpleWizard = cdb.core.View.extend({
    ...
    _bindChanges: function() {
      ...
      this.cartoProperties.bind('change', function() {
        // if the style needs to set sql
        // reset the current style before in order to 
        // get the right table schema and regenerate the sql 
        // again
        var sql = self._generateSQL()
        if(sql != null) {
          self.options.table.useSQLView(null);
        }
        sql = self._generateSQL()
        self.model.set({
          type: self.type,
  ...
  var BubbleWizard = SimpleWizard.extend({
    ...
    _generateSQL: function() {
      ...
      return _.template("select <%= columns %>, st_centroid(the_geom_webmercator) as the_geom_webmercator from <%= table %>")({
        table: table.get('name'),
        columns: _(table.columnNames()).without('the_geom_webmercator').join(', ')
      });
    }
