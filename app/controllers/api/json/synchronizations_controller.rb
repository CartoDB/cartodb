...
class Api::Json::SynchronizationsController < Api::ApplicationController
  ...
  def create
    ...
    @stats_aggregator.timing('synchronizations.create') do
      ...
      puts e.to_s
      puts e.backtrace
    rescue CartoDB::InvalidInterval => e
      render_jsonp({ errors: "#{e.detail['message']}: #{e.detail['hint']}" }, 400)
  ...
  def set_external_source
    @external_source =
      (get_external_source(params[:remote_visualization_id]) if params[:remote_visualization_id].present?)
  end
