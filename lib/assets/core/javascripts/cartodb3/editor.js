...
var georeferenceOnboardingLauncher = new GeoreferenceOnboardingLauncher({
...
var launchGeoreferenceOnboarding = function () {
  var filtered = new Backbone.Collection(layerDefinitionsCollection.reject(function (layerDefModel) {
    return _(omitLayers).contains(layerDefModel.get('id'));
  }, this));
      ...
      if (analysisDefinitionNodeModel) {
        ...
        if (queryGeometryModel.shouldFetch()) {
          queryGeometryModel.fetch({
            success: launchGeoreferenceOnboarding
          });
        }
        ...
        if (queryGeometryModel.isFetched() && queryRowsCollection.isFetched()) {
...
BuilderOnboardingLauncher.init({
  ...
  editorModel: editorModel,
  onboardingNotification: onboardingNotification,
  onboardingCallback: launchGeoreferenceOnboarding
});
