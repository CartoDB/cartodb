...
cdb.admin.Header = cdb.core.View.extend({
  ...
  _exportTable: function() {
    ...
    if (this.options.user.featureEnabled('bbg_pro_ui')) {
      view.appendToBody();
    } else {
      $('body').append('<div id="hidden-export-table-div" style="display:none"><div>');
      $('#hidden-export-table-div').append(view.render().el);
      view.fetchCSV();
      $('#hidden-export-table-div').remove();
    }
  },
  ...
  _setVisualization: function() {
    ...
    this.updateShareLabel(table.workViewActive);
  },

  updateShareLabel: function(workView) {
    var span = this.$('a.share').find("span")
    var is_visualization = this.model.isVisualization();

    if (is_visualization) {
      if (workView === "table") {
        span.text('Export Table');
        span.attr('title', 'Click to export dataset table');
      } else {
        span.text('Export Image');
        span.attr('title', 'Click to export map image');
      }
    } else {
      span.text(this._TEXTS.share.visualize);
      span.attr('title', 'Click to create map using dataset');
    }
  },
