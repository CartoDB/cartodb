...
var SearchInput = Backbone.View.extend({
  ...
  _renderListItem: function(service, index, itemMatch, selected, query) {
    var lower_query = query.toLowerCase();
    var queryLen = query.length;

    var name = itemMatch.dataset;
    var startPos = name.toLowerCase().indexOf(lower_query);
    if (startPos > -1) {
      name = name.substr(0, startPos) + '<span class="found-sub">' + name.substr(startPos, queryLen) + '</span>' + name.substr(startPos + queryLen);
    }

    var match = itemMatch.match
    if (itemMatch.match != null) {
      startPos = match.toLowerCase().indexOf(lower_query);
      match = match.substr(0, startPos) + '<span class="found-sub">' + match.substr(startPos, queryLen) + '</span>' + match.substr(startPos + queryLen);
    }
    else {
      match = '';
    }
  ...
  _getItemMatch: function(item, query) {
    var lower_query = query.toLowerCase();

    var data, startPos, match = null, matched_attribute = null;
    if (item.data != null) {
      data = item.data;
      for (var key in data) {
        ...
        if (typeof data[key] === 'string') {
          startPos = data[key].toLowerCase().indexOf(lower_query);
          if (startPos != -1) {
            match = data[key];
          ...
          for (var attr in data[key]) {
            ...
            if (typeof data[key][attr] === 'string') {
              startPos = data[key][attr].toLowerCase().indexOf(lower_query);
              if (startPos != -1) {
                match = data[key][attr];
                // Only datasets should have nested objects, so we don't need to set matched_attribute
                break;
              }
            }
          }
  ...
  _setItems: function(items) {
    ...
    var keysLength = 0, itemMatch, unique_items;
    for (var key in items) {
      ...
      for (var i in items[key]) {
        // TODO: dedup
        itemMatch = this._getItemMatch(items[key][i], this.query);
        if (itemMatch.attribute != null) {
          items[key][i].matched = itemMatch.attribute;
        }
        popupHtml += this._renderListItem(key, unique_items.length, itemMatch, keysLength === 0 && i == 0, this.query);
