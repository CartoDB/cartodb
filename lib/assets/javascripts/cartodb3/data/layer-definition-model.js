...
var OWN_ATTR_NAMES = ['id', 'order', 'infowindow', 'tooltip'];
...
module.exports = Backbone.Model.extend({
  ...
  initialize: function (attrs, opts) {
    ...
    if (this.styleModel) {
      this.styleModel.bind('change:type change:animated', function () {
        if (this.styleModel.isAggregatedType() || this.styleModel.get('animated').enabled) {
  ...
  isOwnerOfAnalysisNode: function (nodeModel) {
    return this.get('letter') === nodeModel.letter();
  },
  ...
  findPrimaryParentLayerToSource: function () {
    var nodeDefModel = this.analysisDefinitionNodeModel();
    if (!nodeDefModel) return;

    return this.collection.find(function (layerDefModel) {
      if (layerDefModel === this) return;
      var otherHeadNode = layerDefModel.analysisDefinitionNodeModel();
      if (!otherHeadNode) return;

      return otherHeadNode.getSecondarySource() !== nodeDefModel &&
        layerDefModel.containsNode(nodeDefModel);
    }, this);
  },
