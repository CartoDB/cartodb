  ...
  cdb.admin.Geocoding = cdb.core.Model.extend({
    ...
    _checkFinish: function() {
      ...
      // TODO: remove this hack preventing geocoding error
      // but not changing to failure state
      if (state !== 'failure' && error) {
        var desc = "Sorry, there is a problem with our geocoder. Please come back in a while and if the problem persists contact us at <a href='mailto:support@cartodb.com'>support@cartodb.com</a>.";
        this.destroyCheck();
        this.clear({ silent: true });
        this.trigger('geocodingError', error, this);
        this._onGeocodingError(error, desc);
        return false;
      }
