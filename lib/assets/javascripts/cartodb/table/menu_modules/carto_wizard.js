...
cdb.admin.mod.CartoCSSWizard = cdb.core.View.extend({
  ...
  enableTabs: function() {
    var _enableMap = {
      'point': ['polygon', 'bubble', 'density', 'color', 'category', 'intensity'],
      'line':['polygon', 'choropleth', 'color', 'category', 'bubble'],
      'polygon': ['polygon', 'choropleth', 'color', 'category', 'bubble']
    };
  ...
  _updateStyle: function() {
    ...
    if(this.options.table.previous('schema')) {
      ...
      if(pane.type === this.cartoStylesGeneration.get('type')) {
        ...
        this.model.save(attrs, { silence: true });
      }
    }
  }
...
cdb.admin.mod.SimpleWizard = cdb.core.View.extend({
  ...
  */
  _getColorColumns: function() {
    var self = this;
    var columns = [];
    var sc = this.options.table.get('original_schema') || this.options.table.get('schema')
