...
describe('components/modals/add-widgets/add-widgets-view', function () {
  ...
  beforeEach(function () {
    ...
    analysisDefinitionNodesCollection.at(1).analysisTableModel = new AnalysisTableModel({ status: 'ready', query: 'SELECT * FROM test' }, {
      configModel: this.configModel
    });
    analysisDefinitionNodesCollection.at(2).analysisTableModel = new AnalysisTableModel({ status: 'ready', query: 'SELECT * FROM test2' }, {
      configModel: this.configModel
    });
