...
module.exports = {
  extractErrorFromAnalysisNode: function (analysisNode, layerDefModel) {
    ...
    if ((error && error.message) || analysisNode.get('error_message')) {
      ...
      var extractedError = this.extractError(errorMessage, SAMPLE_LINK_TEMPLATE === null);
  ...
  extractError: function (errorMessage, onlyText) {
    var errorType;
    var matched = false;
    var message;
    var i = 0;
    ...
    while (!matched && i < Messages.messages.length) {
      ...
      if (errorMessage.match(regExp)) {
        if (errorMessageType) {
          body = _t('notifications.analysis.errors.' + errorMessageType, {
