...
cdb.admin.MergeTablesDialog = cdb.admin.BaseDialog.extend({
  ...
  setHeight: function(h, animated) {
    ...
    if (animated) {
      this.$(".content").animate({ height: h }, this.options.speed);
    } else {
  ...
  _onColumnChanged: function(data) {
    ...
    if (data.source === 'origin') {
      theGeomValue = this.merge_table._getTheGeom().model.get("switchSelected")
  ...
  _generateRegularQuery: function() {
    ...
    var actualTableType, mergeTableType;
    ...
    if (actualTableType == "string" && mergeTableType == "string") {
      sql += "LOWER(" + this.actual_table.table_name + "." + this.actual_table.keyColumn + ") = LOWER(" + this.merge_table.table_name + "." + this.merge_table.keyColumn + ")";
    } else {
  ...
  _importCopy: function(item_queue_id) {
    ...
    imp.bind("importComplete", function(e) {
      imp.unbind();
      window.location.href = "/tables/" + (imp.get("table_name") || imp.get("table_id")) + "/";
    }, this);
