...
cdb.admin.carto.category = {
  ...
  category_generator: function(table, props, changed, callback) {
    ...
    // When a new property is set, we have to check if any
    // custom category is applied

    var newPropertyWithoutCats = ( changed.property && !changed.categories );

    if (newPropertyWithoutCats || !props.categories || props.categories.length === 0) {
