  ...
  cdb.admin.ShareMapDialog = cdb.admin.BaseDialog.extend({
    ...
    _TEXTS: {
      _DEFAULT_TITLE: _t('Publish your visualization'),
      _DEFAULT_CLOSE: _t('Close')
    },
    ...
    _updateURL: function(obj, url) {
      if (url.toLowerCase() == "generating...") {
        this.$('input.url').addClass('loading');
      } else {
        this.$('input.url').removeClass('loading');
      }
    ...
    */
    ...
    _setAPIURL: function() {
      var url = 'http://' + location.host + '/api/v1/viz/' + this.options.vis.get("name") + '/viz.json';
