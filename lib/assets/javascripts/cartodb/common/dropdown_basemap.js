...
cdb.admin.BackgroundMapColorView = cdb.admin.BaseLayerButton.extend({
  ...
  selectButton: function() {
    if (this.model.get('color')) this.updateColorButton(this.model.get('color'));
    this.$el.parents('.dropdown.basemap').find('li.selected').removeClass('selected');
    this.$el.addClass('selected');
  }
});
...
cdb.admin.DropdownBasemap = cdb.ui.common.Dropdown.extend({
  ...
  initialize: function() {
    ...
    if (this.baseLayers) {
      ...
      // It is not necessary to listen to add event, because
      // everytime a basemap is added, reset event is emitted
  ...
  */
  ...
  _appendDefaultBaseLayerButton: function($element) {
    ...
    if ($ul.length <= 0 || ($ul.length > 0 && $ul.find("li").length >= this.defaults.basemaps_per_list - 1)) {
      var $ul = $("<ul></ul>");
  ...
  _addGoogleMaps: function() {
    ...
    var
    available = ['satellite', 'hybrid', 'gray_roadmap', 'dark_roadmap'],
