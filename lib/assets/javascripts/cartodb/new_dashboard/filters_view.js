...
module.exports = cdb.core.View.extend({
      ...
      self.addView(
        new cdb.common.TipsyTooltip({
          ...
          title: function() {
            ...
            return isFixed ? '' : $(this).attr('data-title');
          }
        })
      )
    });
  ...
  _onSearchClick: function(e) {
    ...
    hidden = !this.$('.Filters-inner').hasClass('search--enabled');
    search = this.router.model.get('q') || this.router.model.get('tag');

    this.$('.Filters-inner').toggleClass('search--enabled', hidden)

    if (hidden) this.$('.js-search-input').focus();
    if (search) this._cleanSearch(); 
  },
  ...
  _openCreateDialog: function(type) {
    ...
    createDialog.bind('datasetCreated', function(tableMetadata) {
      window.location = tableMetadata.viewUrl();
    }, this);
