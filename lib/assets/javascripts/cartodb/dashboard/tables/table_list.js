...
cdb.admin.dashboard.TableItem = cdb.core.View.extend({
  ...
  render: function() {
    ...
    var clean_name        = this._cleanString(this.model.get("name"), 36);
    ...
    if (name != clean_name) {
      $tableInfo.on("mouseenter", function() {
        console.log($tableInfo.find("hgroup.options").width())

        $title.text(name);
        $tableInfo.find(".feed").hide();
      })

      $tableInfo.on("mouseleave", function() {
        $title.html(clean_name);
        $tableInfo.find(".feed").show();
      })
    }
