...
cdb.admin.models.Filter = cdb.core.Model.extend({
  ...
  getSQLConditionForDate: function() {
    ...
    var lowerDateWithOffset = moment(lower).add({ m: tz }).format("YYYY-MM-DDTHH:mm:ssZ").toString();
    var upperDateWithOffset = moment(upper).add({ m: tz }).format("YYYY-MM-DDTHH:mm:ssZ").toString();
    ...
    if (this.get('upper') >= this.get('upper_limit')) {
      options = _.extend(attributes, {
        ...
        upper: moment(this.get('upper_limit')).add({ m: tz }).format("YYYY-MM-DDTHH:mm:ssZ").toString()
      });
