(function() {
  ...
  cdb.admin.mod.CartoCSSWizard = cdb.core.View.extend({
    ...
    initialize: function() {
      ...
      // when the table datasource changes and it was applied by a 
      // wizard the simple style should be aplied
      self.model.bind('change:query', function() {
        // the view has been cleared
        if(!self.model.get('query')  && self.model.get('query_generated')) {
          // active simple and apply its style
          self.panels.active('polygon');
          self._applyStyle();
        }
      });
    },
  ...
  var ChoroplethWizard = SimpleWizard.extend({
    ...
    initialize: function() {
      ...
      this.options.table.bind('change:schema', function() {
        this.setFormProperties();
        if(!this.options.table.containsColumn(this.cartoProperties.get('property'))) {
          var columns =  _.filter(this.options.table.columnNamesByType('number'), function(c) { return c != "cartodb_id" });
          if(columns.length) {
            this.cartoProperties.set({ 'property': columns[0] }, {silent:true});
          }
        }
    ...
    render: function() {
      ...
      var $wrapper =    $("<div>").addClass("wrapper")
        , $content =    $("<div>").addClass("content")
        , $no_columns = $("<div>").addClass("no_content").html(this.error_msg.NO_CONTENT_MSG)
      ...
      this._checkNumberColumns($wrapper);
    ...
    /**
     *  Check if there is any number column
     */
    _checkNumberColumns: function($el) {
      if (this._getNumberColumns().length == 0) {
        // Show correct content
        $el.find(".content").hide();
        $el.find(".no_content").show();
      } else {
        $el.find(".content").show();
        $el.find(".no_content").hide();
      }
    }
