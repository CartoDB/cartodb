...
cdb.admin.MapOverlays = cdb.core.View.extend({
  ...
  setLegend: function (legend) {
    this.legend = legend;
    var legendSelector = '.cartodb-legend-stack';
    ...
    $(legendSelector).children().on('dragstart', function(event) {
        event.preventDefault();
    });

    // disable map drag and map click popups while dragging legend
    $(legendSelector).on('mousedown touchstart MSPointerDown', function(event) {
    ...
    $(legendSelector).on('mouseup touchend MSPointerUp', function(event) {
      if (mapView.map_leaflet) {
    ...
    $(legendSelector).on('dblclick mousewheel DOMMouseScroll', function(event) {
      event.stopPropagation();
    });

    // make legend draggable
    $(legendSelector).draggable({
      // containment prevents dragging outside area; however, map in editor simply hides overflow
      containment: $('.cartodb-map'),
      create: function(event, ui) {
        // prevent a number of draggable height and width resizing issues
        $(legendSelector).css({'width': $(legendSelector).width() + 1});
      },
      start: function(event, ui) {
        $(legendSelector).addClass('notransition'); // disable animations while dragging
      },
      stop: function(event, ui) {
        ...
        var styles = $(legendSelector).attr('style');
        console.log('legend now has style:');
        console.log(styles);
        //$(legendSelector).attr('style', style);

        // top as percentage
        //$(legendSelector).position().top / $(legendSelector).parent().height() * 100
        event.stopPropagation();

        $(legendSelector).removeClass('notransition');
      }
   });
