...
var F = function (opts) {
  ...
  this.visMap().on('change', _.debounce(this._saveStateDefinition.bind(this), 500), this);
...
F.prototype._onFeatureClicked = function (event) {
  var layerDefinitionModel = this._layerDefinitionsCollection.get(event.layer.id);
  if (!layerDefinitionModel) {
    throw new Error('No layer definition for clicked feature');
  }

  var featureDefinition = new FeatureDefinitionModel({
    id: event.feature.cartodb_id
  }, {
  ...
  featureDefinition.fetch({
    success: function () {
      this._mapModeModel.enterEditingFeatureMode(featureDefinition);
    }.bind(this),
    error: function () {
      // TODO: Handle error
    }
...
F.prototype._onMapModeChanged = function (mapModeModel) {
  ...
  if (mapModeModel.isViewingMode()) {
    map.stopDrawing();
    map.stopEditingGeoJSONGeometry();
  }
  ...
  if (mapModeModel.isEditingFeatureMode()) {
    featureDefinition = mapModeModel.getFeatureDefinition();
    geometry = map.editGeoJSONGeometry(featureDefinition.get('the_geom'));
    this._bindGeometryToFeatureDefinition(geometry, featureDefinition);
  }
};

F.prototype._setFeatureDefinition = function (geometry, featureDefinition) {
  if (geometry.isComplete()) {
    var attrs = {
      the_geom: JSON.stringify(geometry.toGeoJSON().geometry)
    };
    if (geometry.getLatLng && geometry.getLatLng()) {
      attrs = _.extend(attrs, {
        lat: geometry.getLatLng()[0],
        lng: geometry.getLatLng()[1]
      });
    }
    featureDefinition.set(attrs);
  }
};
...
F.prototype._bindGeometryToFeatureDefinition = function (geometry, featureDefinition) {
  ...
  this._setFeatureDefinition(geometry, featureDefinition);

  geometry.on('change', function () {
    self._setFeatureDefinition(geometry, featureDefinition);
  });

  featureDefinition.on('change', function () {
    // TODO: prevent infinite loops here somehow
  });
};
