...
describe('dashboard/views/api-keys/api-keys-form-view', function () {
  ...
  describe('when an existing apiKeyModel is provided', function () {
    ...
    describe('._generateForm', function () {
      it('should render all fields properly', function () {
        ...
        expect(formView.$('input[name=create]').length).toBe(1);
        expect(formView.$('input[name=create]').attr('disabled')).toBe('disabled');

        expect(formView.$('input[name=listing]').length).toBe(1);
        expect(formView.$('input[name=listing]').attr('disabled')).toBe('disabled');
      });
    });
  });
  ...
  describe('when an existing apiKeyModel is not provided', function () {
    ...
    describe('._generateForm', function () {
      it('should render all fields properly', function () {
        ...
        expect(formView.$('input[name=create]').length).toBe(1);
        expect(formView.$('input[name=create]').attr('disabled')).not.toBe('disabled');

        expect(formView.$('input[name=listing]').length).toBe(1);
        expect(formView.$('input[name=listing]').attr('disabled')).not.toBe('disabled');
      });
    });
  });
  ...
  describe('._hasErrors', function () {
    ...
    it('should return true if name has not been filled', function () {
    ...
    it('should return true if SQL is checked and CREATE or permissions have not been selected', function () {
    ...
    it('should return true permissions have been selected but SQL or MAPS are not checked', function () {
      spyOn(view._formView, 'validate').and.returnValue(null);
      view._formView.setValue({
        apis: { maps: false, sql: false }
      });
      spyOn(view._apiKeyModel, 'hasPermissionsSelected').and.returnValue(true);

      expect(view._hasErrors()).toBe(true);
    });

    it('should return false if SQL and CREATE are checked', function () {
      spyOn(view._formView, 'validate').and.returnValue(null);
      view._formView.setValue({
        apis: { maps: false, sql: true },
      ...
      expect(view._hasErrors()).toBe(false);
    });

    it('should return false when only LISTING is checked', function () {
      spyOn(view._formView, 'validate').and.returnValue(null);
      view._formView.setValue({
        apis: { maps: false, sql: false },
        datasets: { create: false, listing: true }
      });
      spyOn(view._apiKeyModel, 'hasPermissionsSelected').and.returnValue(false);
    ...
    it('should return true when LISTING is checked and SQL or MAPS are checked without permissions', function () {
      spyOn(view._formView, 'validate').and.returnValue(null);
      view._formView.setValue({
  ...
  describe('when the form has no errors', function () {
    ...
    beforeEach(function () {
      ...
      spyOnFormChanged.and.callThrough();
      hasErrorsSpy = spyOn(view, '_hasErrors');
      view._formView.setValue({
        apis: { maps: false, sql: true },
        name: 'Wadus API key',
        token: null
    ...
    describe('._onFormChanged', function () {
      it('should not add is-disabled class to submit button', function () {
        hasErrorsSpy.and.returnValue(false);
        view._onFormChanged();

        expect(view.$('.js-submit').hasClass('is-disabled')).toBe(false);
      });

      it('should set validation tooltip to null if any', function () {
        let cleanSpy = jasmine.createSpy('clean');
        view._validationTooltip = { clean: cleanSpy };
        hasErrorsSpy.and.returnValue(false);
