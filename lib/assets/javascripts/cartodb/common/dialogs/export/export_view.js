...
module.exports = BaseDialog.extend({
  ...
  _fetch: function(options, sql) {
    ...
    if (options.format === 'csv') {
      var bbgProUI = $.inArray("bbg_pro_ui", this.options.user_data.feature_flags) !== -1;
      if (bbgProUI) {
        this.$('.skipfields').val("the_geom_webmercator");
      } else {
        var exportGeom = this.model.get('table_visualization') && this.model.get('table_visualization').export_geom;
        if (exportGeom) {
          this.$('.skipfields').val("cartodb_id,the_geom_webmercator,the_geom");
        } else {
          this.$('.skipfields').val("cartodb_id,the_geom_webmercator,the_geom,lat,lon,lng,long,latitude,longitude");
        }
      }
    } else {
    ...
    if (this.options.link_download) {
      var location = this.$('form').attr('action') + '?' + this.$('form').serialize();
      $('body').append('<div id="hidden-export-table-link-div" style="display:none"><a id="hidden-export-table-link" href="' + location + '" download>Export</a><div>');
      document.getElementById('hidden-export-table-link').click();
      $('#hidden-export-table-link-div').remove();
      this.close();
    } else {
