...
var CategoriesListView = require('./input-color-categories-list-view/input-color-categories-list-view');
var InputCategoriesRamp = require('./input-categories-ramps/input-ramp-content-view');
...
var STEPS = {
  ...
  RANGE_LIST_VIEW: 1,
  COLOR_PICKER_VIEW: 2,
  IMAGE_PICKER_VIEW: 3 // TODO needed?
};
...
module.exports = CoreView.extend({
  ...
  _generateStackLayoutView: function () {
    ...
    var stackViewCollection = new Backbone.Collection([
      {
        selected: currentStep === STEPS.RAMPS_PICKER_VIEW,
        createStackView: this._createCategoriesRampView.bind(this)
      },
      {
        selected: currentStep === STEPS.RANGE_LIST_VIEW,
    ...
    if (this._iconStylingEnabled()) {
      stackViewCollection.add({
        ...
        createStackView: this._createImagePickerView
      });
    }
  ...
  _createCategoriesRampView: function (stackLayoutModel, opts) {
    var view = new InputCategoriesRamp({
      maxValues: MAX_VALUES,
      model: this.model
    });

    view.on(InputCategoriesRamp.EVENTS.RAMP_SELECTED, function () {
      stackLayoutModel.goToStep(STEPS.RANGE_LIST_VIEW);
    }, this);

    view.on(InputCategoriesRamp.EVENTS.CUSTOM_COLOR, function () {
      stackLayoutModel.goToStep(STEPS.RANGE_LIST_VIEW);
    }, this);
