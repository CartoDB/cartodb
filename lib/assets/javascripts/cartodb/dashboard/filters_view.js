  ...
  _newMap: function(e) {
    ...
    if (this.user.featureEnabled("bbg_rtr_single_step_new_map")) {
      ...
      if (self.canCreateDatasets()) {
        window.dashboard.controllerView._hideBlocks();
        window.dashboard.controllerView._showBlocks(["main_loader"]);
        ...
        createModel.bind('datasetCreated', function(tableMetadata) {
          ...
          vis.save({
            ...
            success: function(m) {
              window.location = vis.viewUrl(self).edit();
            },
            error: function(e) {
              window.dashboard.controllerView._hideBlocks();
              window.dashboard.controllerView._showBlocks(["error"]);
