describe("New visualization dialog", function() {
  ...
  it("shouldn't show combo when there are no visualizations available", function() {
    this.dialog.visualizations.models = [];

    spyOn(this.dialog, '_showEmpyState');
    this.dialog.render().open();
    this.dialog._onReset();
    
    expect(this.dialog.$('.tables li').length == 0).toBe(true);
    expect(this.dialog._showEmpyState).toHaveBeenCalled();
  });
