cdb.admin.overlays.Annotation = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.form_data = [{
        ...
        name: 'Line',
        form: {
          'line-color':  { type: 'color', value: '#000', extra: { picker_horizontal_position: "right", picker_vertical_position: "down" }},
          'line-width':  { type: 'simple_number', value: 50, min: 5, max: 100, inc: 1 },
        }
      } , {
        name: 'Zoom (min-max)',
        form: {
          'zoom-min':  { type: 'simple_number', value: minZoomLevel, min: minZoomLevel, max: maxZoomLevel, inc: 1 },
          'zoom-max':  { type: 'simple_number', value: maxZoomLevel, min: minZoomLevel, max: maxZoomLevel, inc: 1 },
        }
      }];
  },
  ...
  _applyZoomLevelStyle: function() {
    ...
    this.$el.css("background-color", rgbaCol);
    this.$el.find(".stick").css("background-color", rgbaCol);
    this.$el.find(".ball").css("background-color", rgbaCol);
