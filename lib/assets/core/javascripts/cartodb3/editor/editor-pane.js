...
module.exports = CoreView.extend({
  ...
  render: function () {
    ...
    var count = this._getCartoLayerCount();
    var max = this._getMaxCount();

    this.clearSubViews();
    this.$el.empty();

    var infoboxStates = [this._getMaxLayerInfoBoxForCurrentUser()];

    this._maxLayerInfoboxModel = new InfoboxModel({
      state: null
    });
        ...
        var layersStackViewCollection = new Backbone.Collection([{
          createStackView: function (stackLayoutModel, opts) {
            return new PanelWithOptionsView({
              ...
              infoboxModel: self._maxLayerInfoboxModel,
  ...
  _getMaxLayerInfoBoxForCurrentUser: function () {
    ...
    var infoboxOpts = {
      type: 'alert',
      title: _t('editor.layers.max-layers-infowindow.title', { maxLayers: self._getMaxCount() })
    };
    ...
    if (this._configModel.get('cartodb_com_hosted')) {
      ...
      if (this._userModel.isInsideOrg()) {
        if (this._userModel.isOrgAdmin()) {
          infoboxOpts.body = _t('editor.layers.max-layers-infowindow.org-admin.body', { maxLayers: self._getMaxCount() });
          infoboxOpts.mainAction = { label: _t('editor.layers.max-layers-infowindow.org-admin.upgrade') };
        } else {
          infoboxOpts.body = _t('editor.layers.max-layers-infowindow.org.body', { maxLayers: self._getMaxCount() });
          infoboxOpts.mainAction = { label: _t('editor.layers.max-layers-infowindow.org.upgrade') };
        }

        baseState.mainAction = function () { window.open('mailto:' + self._userModel.upgradeContactEmail()); };
      } else {
        baseState.mainAction = function () { window.open('https://carto.com/pricing/'); };
        infoboxOpts.body = _t('editor.layers.max-layers-infowindow.regular.body', { maxLayers: self._getMaxCount() });
  ...
  _onLayerCountChange: function () {
    ...
    if (count === max) {
      ...
      this._maxLayerInfoboxModel.set('state', MAX_LAYERS_REACHED);
    } else {
      this._enableAddButton();
      this._maxLayerInfoboxModel.set('state', null);
    }
  },
  ...
  _getCartoLayerCount: function () {
    return this._layerDefinitionsCollection.models.filter(
      function (model) {
        return model.get('type') === 'CartoDB';
      }
    ).length;
  },
  ...
  _addItem: function (e) {
    if (this.$('.js-add').hasClass('is-disabled')) return false;
