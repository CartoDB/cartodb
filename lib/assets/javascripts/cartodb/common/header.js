...
cdb.admin.Header = cdb.core.View.extend({
  ...
  _TEXTS: {
    ...
    rename: {
      ...
      readonly:     _t('It is not possible to rename<br/>the table in <%= mode %> mode'),
      owner:        _t('It is not possible to rename<br/>the table if you are not the owner'),
      ok:           _t('Yes, do it')
    }
  },
  ...
  _setMetadata: function() {
    var isOwner = this.model.permission.isOwner(this.options.user);
    var isQueryApplied = this.dataLayer && this.dataLayer.table && this.dataLayer.table.isInSQLView();
    var $metadata = this.$('.metadata a');
    
    var text = this._TEXTS.metadata.edit;
    var href = "#/edit-metadata";

    if (!isOwner || isQueryApplied) {
  ...
  _changeMetadata: function(e) {
    ...
    if (this.isMetadataEditable()) {
      ...
      var dialog = new cdb.admin.MetadataDialog({

        maxLength:        this._MAX_DESCRIPTION_LENGTH,
        owner_avatar_url: this.model.get("permission").owner.avatar_url,
        owner_username:   this.model.get("permission").owner.username,
        vis_name:         this.model.get("name"),
        vis_description:  this.model.get("description"),
        vis_source:       this.model.get("source"),
        tags:             this.model.get("tags"),
        onResponse:       function(name, description, source, tags) {

          // Trim & sanitize description (html and events)
          var sanitized_description = cdb.Utils.removeHTMLEvents( cdb.Utils.stripHTML(description.trim()));
          self._onSetAttribute('description', sanitized_description);

          self._onSetAttribute('name', name);
          self._onSetAttribute('source', source);
          self._onSetAttribute('tags', tags);
  ...
  /**
   *  Check if visualization/table tags and description are editable
   *
   *  Tags and description will be editable when:
   *  
  ...
  isMetadataEditable: function() {
    if (this.model.isVisualization()) {
      ...
      if (!table) {
        cdb.log.info('Table model corrupted, there is 0 or more than one data layer added');
        return false;
      } else if (table && (table.isInSQLView() || !table.permission.isOwner(this.options.user))) {
        return false;
      } else {
        return true;
      }
