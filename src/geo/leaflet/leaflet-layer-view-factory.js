...
var LayerGroupViewConstructor = function (layerGroupModel, nativeMap, mapModel, showErrors) {
  ...
  if (renderModeResult.mode === RenderModes.VECTOR) {
    return new LeafletCartoDBWebglLayerGroupView(layerGroupModel, nativeMap, showErrors);
  }
...
LeafletLayerViewFactory.prototype.createLayerView = function (layerModel, nativeMap, mapModel, showErrors) {
  var layerType = layerModel.get('type').toLowerCase();
  var LayerViewClass = this._constructors[layerType];

  if (LayerViewClass) {
    try {
      return new LayerViewClass(layerModel, nativeMap, mapModel, showErrors);
    } catch (error) {
