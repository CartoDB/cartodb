...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _onChangeCanvasMode: function() {
    ...
    if (mode === "desktop") {
      ...
      if (this.loader && this.fullscreen) {

      setTimeout(function() {

        if (self.fullscreen.model.get("display")) self.loader.model.set("y", self.fullscreen.model.get("y") + 40);
        else self.loader.model.set("y", self.fullscreen.model.get("y") );
      }, 500);
      ...
      setTimeout(function() {
        if (self.loader) self.loader.model.set("y", $(".header.overlay-static").outerHeight(true) + 15);
      }, 300);
  ...
  _showDesktopCanvas: function(mode) {
    ...
    autoHeight = $map.css({height: 'auto', marginTop: 0,  top: "70px" }).height(); //temporarily change to auto and get the width.
    ...
    var stretchMapLandscape = function() {
      $map.animate(
        { height: autoHeight, top: "70", marginTop: "0"},
  ...
  _createSearchOverlay: function() {
    ...
    console.log('create search overlay');
  ...
  _createLayerSelectorOverlay: function() {
    ...
    console.log("creating layer selector");
  ...
  _setupShareOverlay: function(type, data) {
    ...
    console.log('setup share overlay')
  ...
  _bindShareOverlay: function(overlay) {
    ...
    var self = this;
    var hiddenXPosition = 20;
    var shownXPosition  = 60;

    var moveOverlays = function(x) {

      if (self.search) {
        self.search.model.set("x", x);
        return;
      }

      if (self.layer_selector) {
        self.layer_selector.model.set("x", x);
      }

    }

    if (this.header) overlay.model.set("y", $(".header.overlay-static").outerHeight(true) + 20);
    else overlay.model.set("y", 20);

    moveOverlays(shownXPosition);
  ...
  _bindLoaderOverlay: function(overlay) {
    ...
    if (this.fullscreen) {
      ...
      overlay.model.set("y", this.fullscreen.model.get("y") + 40);

    } else if (this.zoom) {

      overlay.model.set("y", 140);
  ...
  _bindFullScreenOverlay: function(overlay) {
    ...
    overlay.bind("change_y", function() {

      var mode = this.vis.get("canvas_mode");

      if (this.loader && mode === 'desktop') {

        if (overlay.model.get("display")) this.loader.model.set("y", overlay.model.get("y") + 40);
        else this.loader.model.set("y", overlay.model.get("y") );

      }

    }, this);

    if (this.zoom) overlay.model.set("y", 140);
    else {
  ...
  _setupLayerSelectorOverlay: function(type, data) {
    ...
    console.log("setup layer selector");
  ...
  _bindLayerSelectorOverlay: function(overlay) {
    ...
    console.log('binding layer selector');
    ...
    var onDestroy = function() {
      ...
      console.log('layer selector removed');

    };

    if (!self.search) {

      if (self.share) overlay.model.set("x", 60);
      else overlay.model.set("x", 20);

    } else {

      if (!self.share) overlay.model.set("x", 172);
  ...
  _bindZoomOverlay: function(overlay) {
    ...
    if (this.fullscreen) {
      this.fullscreen.model.set("y", 140);
    }

    var onChangeY = function() {

      if (self.zoom_info) {
        self.zoom_info.model.set("y", overlay.model.get("y") + 80);
      }
    ...
    var onDestroy = function() {
      ...
      self.zoom_info.clean();
      delete self.zoom_info;

      if (self.fullscreen) {
        self.fullscreen.model.set("y", 20);
      }

    };

    overlay.bind("change_y", onChangeY, this);
    overlay.model.bind("destroy", onDestroy, this);
  ...
  _setupHeaderOverlay: function(type, data) {
    ...
    /*// do not raise changes in initialization
    data.set({
      "title": this.vis.get("name"),
      "description": this.vis.get("description")
    }, { silent: true });
*/
  ...
  _setupSearchOverlay: function(type, data) {
    ...
    console.log('setup search overlay');
  ...
  _bindHeaderOverlay: function(overlay) {
    ...
    var mode = self.vis.get("canvas_mode");

    var move = function() {

      if (overlay.model.get("show_description") && !overlay.model.get("description")) return;

      var top = $(".header.overlay-static").outerHeight(true) + 20;

      if (self.fullscreen)     self.fullscreen.model.set("y",   top);
      if (self.zoom)           self.zoom.model.set("y",   top);
      if (self.search)         self.search.model.set("y", top);
      if (self.share)          self.share.model.set("y",  top);
      if (self.layer_selector) self.layer_selector.model.set("y",  top);

      if (self.loader && mode && mode === 'mobile') 
        self.loader.model.set("y", top);
    ...
    var onDestroy = function() {
      ...
      //if (overlay.model.get("show_title") && !overlay.model.get("description")) return;
      ...
      if (self.share)                       self.share.model.set("y",  top);
      if (self.layer_selector)              self.layer_selector.model.set("y",  top);
      if (self.loader && mode === 'mobile') self.loader.model.set("y", top);
  ...
  _bindSearchOverlay: function(overlay) {
    ...
    var onChangeX = function() {
      ...
      if (self.layer_selector) {
        ...
        if (self.share) self.layer_selector.model.set("x", 212);
        else self.layer_selector.model.set("x", 172);
    ...
    var onDestroy = function() {
      ...
      if (self.layer_selector) {
        ...
        if (self.share) self.layer_selector.model.set("x", 60);
        else self.layer_selector.model.set("x", 20);
    ...
    if (!self.share) {
      overlay.model.set("x", 20);
    }

    if (self.header) overlay.model.set("y", $(".header.overlay-static").outerHeight(true) + 20);
    else overlay.model.set("y", 20);
    ...
    overlay.model.bind("destroy", onDestroy, this);
    overlay.bind("change_x",      onChangeX, this);

  },

  _setupZoomInfoOverlay: function(type, data) {

    if (this.zoom_info) return;

    var overlay = this.zoom_info = new cdb.admin.overlays[type]({
      model: data,
      map: this.map
    });

    this._bindSetupZoomInfoOverlay(overlay);

    return overlay;

  },

  _bindSetupZoomInfoOverlay: function(overlay) {

    overlay.bind("change_y", function() {

      if (this.fullscreen) {

        if (overlay.model.get("display")) this.fullscreen.model.set("y", overlay.model.get("y") + 60);
        else this.fullscreen.model.set("y", overlay.model.get("y"));
  ...
  _renderOverlay:  function(delayed_animation, model) {
    ...
    var canvas_mode  = this.vis.get("canvas_mode");
    ...
    var display = (device && device != canvas_mode) ? false : true;
  ...
  _configureOverlay: function(model) {
    ...
    switch(type) {
      ...
      case 'ZoomInfo':       overlay = this._setupZoomInfoOverlay(type, model);        break;
  ...
  _bindMapOptions: function() {
    ...
    this.mapOptionsDropdown.bind("createOverlay", function(overlay_type, property) {
      ...
      console.log('creating overlay: ', overlay_type);
