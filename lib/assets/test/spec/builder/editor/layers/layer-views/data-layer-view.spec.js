...
describe('editor/layers/data-layer-view', function () {
  ...
  describe('_initialize', function () {
    it('initial values used in .render have the proper value', function () {
      expect(this.view._needsGeocoding).toBe(false);
      expect(this.view._isLayerEmpty).toBe(false);
      expect(this.view._queryGeometryHasGeom).toBe(true);
    });

    it('should call _reRenderIfViewValuesHaveChanged', function () {
    ...
    it('should subscribe to fetch all query objects and then call to fetchQueryRowsIfRequired', function (done) {
      ...
      setTimeout(function () {
        expect(self.view.model.fetchQueryRowsIfRequired).toHaveBeenCalled();
        done();
      }, 0);
    });
  });
  ...
  describe('.render', function () {
    ...
    describe('depending on _needsGeocoding value', function () {
      ...
      it('should render proper markup if geocoding needed', function () {
        this.view._needsGeocoding = true;
        this.view.render();
    ...
    describe('depending on _queryGeometryHasGeom value', function () {
      beforeEach(function () {
        this.view._needsGeocoding = true; // To avoid rendering `js-analyses`. We want to depend only on _queryGeometryHasGeom
      });

      it('should render proper markup if the layer has geometry', function () {
        this.view._queryGeometryHasGeom = true;
        this.view.render();
      ...
      it('should render proper markup if the layer has no geometry', function () {
        this.view._queryGeometryHasGeom = false;
        this.view.render();
  ...
  describe('._reRenderIfViewValuesHaveChanged', function () {
    ...
    beforeEach(function () {
      ...
      initialGeocodingValue = this.view._needsGeocoding;
      initialHasGeomValue = this.view._queryGeometryHasGeom;
      initialLayerEmptyValue = this.view._isLayerEmpty;
    });
    ...
    it('shoud call to render if _needsGeocoding changes', function (done) {
      var self = this;
      ...
      setTimeout(function () {
        ...
        expect(self.view._needsGeocoding).toEqual(!initialGeocodingValue);
        done();
      }, 0);
    });

    it('shoud call to render if _isLayerEmpty changes', function (done) {
      var self = this;
      ...
      setTimeout(function () {
        expect(self.view.render).toHaveBeenCalled();
        expect(self.view._isLayerEmpty).toEqual(!initialLayerEmptyValue);
        done();
      }, 0);
    });

    it('shoud call to render if _queryGeometryHasGeom changes', function (done) {
      var self = this;
      ...
      setTimeout(function () {
        expect(self.view.render).toHaveBeenCalled();
        expect(self.view._queryGeometryHasGeom).toEqual(!initialHasGeomValue);
        done();
      }, 0);
    });
  });
