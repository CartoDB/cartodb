...
var PRIVACY_OPTIONS = require('builder/data/vis-definition-model').PRIVACY_OPTIONS;
var checkAndBuildOpts = require('builder/helpers/required-opts');

var ESCAPE_KEY_CODE = 27;

// Warning: mapcapsCollection is not required, but the the privacy
// change behaviour's confirmation will change if not provided
...
module.exports = CoreView.extend({
  ...
  _initBinds: function () {
    ...
    this.model.on('change:privacy', this._onPrivacyChange, this);
  ...
  _onPrivacyChange: function () {
    var privacyStatus = this.model.get('privacy').toUpperCase();
    this.model.set({state: 'loading'});

    if (this._shouldShowPrivacyWarning(privacyStatus)) {
  ...
  _savePrivacy: function () {
    ...
    this._privacyCollection.searchByPrivacy(privacy)
      .saveToVis(vis, {
        success: function () {
  ...
  _shouldShowPrivacyWarning: function (privacyState) {
    var isPubliclyAvailable =
      privacyState === PRIVACY_OPTIONS.password ||
      privacyState === PRIVACY_OPTIONS.link ||
      privacyState === PRIVACY_OPTIONS.public;

    if (this._mapcapsCollection) {
