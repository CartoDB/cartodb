describe("cdb.admin.GeoreferenceDialog", function() {
  ...
  it("should render properly options list", function() {
    TestUtil.feedTable(table, 1);
    view.render();
    expect(view.$('ul.options > li').length).toBe(4);
    expect(view.$('ul.options > li[data-option="0"] a:eq(0)').text()).toBe("You have lon/lat columns in your table");
    expect(view.$('ul.options > li[data-option="1"] a:eq(0)').text()).toBe("You have a column identifying administrative regions");
    expect(view.$('ul.options > li[data-option="2"] a:eq(0)').text()).toBe("You have a column identifying other geo-related information");
    expect(view.$('ul.options > li[data-option="3"] a:eq(0)').text()).toBe("You have one or more columns with the address");
  });
  ...
  it("shouldn't geocode when users don't have enough rights", function() {
    ...
    spyOn(view,'hide');
    view.render();
    view.$('ul.options > li:eq(3) > a').click();
    expect(view.$('a.next').hasClass('disabled')).toBeTruthy();
    view.$('a.next').click();
    expect(view.options.geocoder.get('formatter')).toBe('');
  ...
  it("should change the button title after selecting the adm option", function() {
    ...
    view.$('.next').click();
    expect(view.$('.foot .next').text()).toEqual("Continue");

    view.$('.back').click();
    expect(view.$('.aside .next').text()).toEqual("Georeference");
