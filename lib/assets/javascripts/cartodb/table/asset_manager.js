  ...
  cdb.admin.AssetManager = cdb.admin.BaseDialog.extend({
    ...
    _renderAssetsPane: function() {
      this.assetsPane = new cdb.admin.UploadedAssetsPane({
        collection: this.collection,
        kind:       this.kind
      });

      this.assetsPane.bind('assetChosen', function() { alert("asset chosen!") }, this);
    },
    ...
    _checkOKButton: function() {
      ...
      switch (tab) {
        case 'assets':
          var asset = this.collection.find(function(m) {
            return m.get('state') === 'selected';
          });

          if (asset) value = asset.get('public_url')
          break;
        default: 
          value = pane.model.get('value');
      }
