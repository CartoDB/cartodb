...
describe('cdb.admin.Slide', function() {
  ...
  it('should fetch visualization data on activation', function() {
    ...
    // 2 visualizations and 2 overlays
  ...
  describe('cdb.admin.Slide.map', function() {
    ...
    it("should save map state on change", function(done) {
      slide.setMaster(vis);
      var center = [10, 20];
      ...
      setTimeout(function() {
        ...
        map.set('zoom', 31);
        setTimeout(function() {
          expect(slide_map.get('zoom')).toEqual(31);
          done();
        }, 1100);
      }, 1100);
    ...
    it("should change map state on change", function(done) {
      slide.setMaster(vis);
      var center = [20, 30];
  ...
  describe("cdb.admin.Slide.layers", function() {
    ...
    it("shoudl create layer to track vis changes", function() {
      ...
      lyr.set('parent_id', 'parent_id_test');
    ...
    it("should track basemap changes", function() {
      ...
      lyr.set('parent_id', 'test');
      expect(slide.visualization.map.layers.at(0).get('parent_id')).not.toEqual('test');
    ...
    it("should track layers state", function() {
      slide.visualization.map.layers.add(new cdb.admin.CartoDBLayer({ parent_id: 'layer_0', id: 'layer_0_slide' }));
      slide.setMaster(vis);
      slide.set('active', true);
      var lyr = map.layers.at(0);
    ...
    it("should track wizard props", function(done) {
      ...
      slide.visualization.map.layers.each(function(lyr, i) {
        lyr.set('parent_id', map.layers.at(i).id);
      });
      slide.setMaster(vis);
      slide.set('active', true);
      var lyr = map.layers.at(0);
