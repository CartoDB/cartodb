...
describe('deep-insights-integrations', function () {
  beforeEach(function (done) {
    ...
    deepInsights.createDashboard('#wdmtmp', vizjson, {}, function (error, dashboard) {
      ...
      this.stateDefinitionModel = jasmine.createSpyObj('state', ['updateState']);
  ...
  describe('overlaysCollection', function () {
    it('should add overlay to CDB overlays collection when a new one is created', function () {
      expect(this.integrations._vis().overlaysCollection.size()).toBe(0);
      var overlayModel = new Backbone.Model({ id: 'hello' });
      ...
      expect(this.integrations._vis().overlaysCollection.at(0).id).toBe('hello');
    });
