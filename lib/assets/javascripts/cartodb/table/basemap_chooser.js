...
cdb.admin.BaseMapChooser = cdb.core.View.extend({
  ...
  _addGoogleMaps: function() {
    var available = ['roadmap', 'gray_roadmap', 'satellite'];
    var styles = {
      roadmap: "",
      satellite: "",
    ...
    for(var i in available) {
      var base = new cdb.admin.GMapsBaseLayer({ base_type: available[i], style: styles[available[i]] });
  ...
  render: function() {
    ...
    this.$el.find('li').map(function(e,i) {
      ...
      if(!$li.attr('data-tipsy')) {
        ...
        $li.tipsy({title:"data-tipsy", fade: true}); // TODO: review
      }
    })
