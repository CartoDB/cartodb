cdb.admin.MapOptionsDropdown = cdb.ui.common.Dropdown.extend({
  ...
  _setupOptions: function() {
    ...
    var simpleOverlays = ["search" , "share", "fullscreen", "layer_selector"];
  ...
  _addSwitches: function(switches) {
    ...
    _(switches).each(function(prop) {
      ...
      if (!self.mapOptions.attributes[prop]) self.$(className);
  ...
  _onSwitchSwitched: function(property, value) {
    ...
    if (property == "search") {
      ...
      var overlay  = this.collection.filter(function(m) { return m.get("type") == "search" })[0];

      var display  = this.mapOptions.get("search");

      overlay.set({ display: display });
      overlay.save();

    } else if (property == "share") {

      var overlay = this.collection.filter(function(m) { return m.get("type") == "share" })[0];

      var display = this.mapOptions.get("share");

      overlay.set({ display: display });
      overlay.save();

    } else if (property == 'scrollwheel') {

      if (value) {
        this.options.vis.map.enableScrollWheel();
      } else {
        this.options.vis.map.disableScrollWheel();
      }

      this.options.vis.map.save();

    } else if (property == "layer_selector") {

      var overlay  = this.collection.filter(function(m) { return m.get("type") == "layer_selector" })[0];

      var display  = this.mapOptions.get("layer_selector");
      ...
      var overlay  = this.collection.filter(function(m) { return m.get("type") == "fullscreen" })[0];

      var display  = this.mapOptions.get("fullscreen");
      ...
      var overlays         = this.collection.filter(function(m) { return m.get("type") == "header" });
  ...
  render: function() {
    ...
    this._addSwitches(['title', 'description', 'search', 'zoom', 'fullscreen', 'share', 'scrollwheel', 'layer_selector', 'legends' /*, 'cartodb_logo'*/]);
