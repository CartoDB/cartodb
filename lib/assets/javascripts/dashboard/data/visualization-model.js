...
const VisualizationModel = Backbone.Model.extend({
  ...
  privacyOptions: function () {
    const privacyOptions = _.values(VisualizationModel.ALL_PRIVACY_OPTIONS);

    if (this.isVisualization()) {
      return privacyOptions;
    } else {
      return _.omit(privacyOptions, 'password');
    }
  },
  ...
  isPubliclyAvailable: function (privacyStatus) {
    return privacyStatus === this.ALL_PRIVACY_OPTIONS.password ||
           privacyStatus === this.ALL_PRIVACY_OPTIONS.link ||
           privacyStatus === this.ALL_PRIVACY_OPTIONS.public;
  }
});
