  ...
  cdb.admin.CommonTableView = cdb.core.View.extend({
    ...
    _TEXTS: {
      upgrade: _t('You don\'t have enough quota. Click to upgrade'),
      contact: _t('You don\'t have enough quota. Click to contact your admin')
    },
    ...
    render: function() {
      ...
      if (this._isOverQuota()) {
        data.tooltip = this.options.user.isInsideOrg() && !this.options.user.isOrgAdmin() ?
          this._TEXTS.contact :
          this._TEXTS.upgrade ;
      }
    ...
    _makeAction: function(e) {
      ...
      if (this.options.user.isInsideOrg() && this._isOverQuota()) {
        window.location.href = "mailto:" + ( this.options.user.isOrgAdmin() ? 'support@cartodb.com' : this.options.user.organization.owner.get('email') );
        return false;
      }
