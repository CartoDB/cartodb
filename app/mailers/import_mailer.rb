...
class ImportMailer < ActionMailer::Base
  ...
  def set_subject
    subject = if @total_tables == 1
                if @errors.nil?
                  'Your dataset import just finished'
                else
                  'There was some error while importing your dataset'
                end
              else
                if @imported_tables == 0
                  'There was some error while importing your datasets'
                else
                  'Your datasets import just finished'
                end
              end
