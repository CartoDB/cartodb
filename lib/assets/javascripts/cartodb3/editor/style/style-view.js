...
module.exports = CoreView.extend({
  ...
  _freezeTorgeAggregation: function (aggregation, currentGeometryType) {
    ...
    this._infoboxModel.once('change:state', function (mdl, state) {
      if (state === 'unfreeze') {
        this._moveTorqueLayerToTop(aggregation, currentGeometryType);
        this._infoboxModel.set({state: ''});
      }
    }, this);
  },

  _moveTorqueLayerToTop: function (aggregation, currentGeometryType) {
    ...
    this._layerDefinitionsCollection.once('layerMoved', function () {
      this._styleModel.setDefaultPropertiesByType(aggregation, currentGeometryType);
      notification.set({
        status: 'success',
    ...
    this._userActions.moveLayer({
      ...
      to: this._layerDefinitionsCollection.getTopDataLayerPosition()
    });
  },
    ...
    var infoboxSstates = [
      {
        ...
        mainAction: self._cancelTorqueAggregation.bind(self),
        secondAction: self._applyTorqueAggregation.bind(self)
      }
    ];
