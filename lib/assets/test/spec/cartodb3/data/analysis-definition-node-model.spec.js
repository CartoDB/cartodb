...
describe('data/analysis-definition-node-model', function () {
  ...
  describe('.cloneTreeWithSameLetter', function () {
    var a0;
    ...
    it('should throw error on bad input', function () {
      expect(function () { a0.cloneTreeWithSameLetter(); }).toThrowError(/required/);
      expect(function () { a0.cloneTreeWithSameLetter(null); }).toThrowError(/required/);
      expect(function () { a0.cloneTreeWithSameLetter(undefined); }).toThrowError(/required/);
      expect(function () { a0.cloneTreeWithSameLetter(true); }).toThrowError(/required/);
      expect(function () { a0.cloneTreeWithSameLetter({}); }).toThrowError(/required/);
    });

    describe('when given a source node', function () {
      beforeEach(function () {
        this.g0 = a0.cloneTreeWithSameLetter('g');
      });

      it('should clone only that model', function () {
        expect(this.g0).toBeDefined();
        expect(this.g0.id).toEqual('g0');
      });
    });
    ...
    describe('when called on a node which sources all belong to same letter', function () {
      beforeEach(function () {
        ...
        this.newTreeHeadNode = this.a2.cloneTreeWithSameLetter('h');
      });

      it('should have cloned the whole tree', function () {
        expect(this.newTreeHeadNode.id).toEqual('h2');
        expect(this.newTreeHeadNode.getPrimarySource().id).toEqual('h1');
        expect(this.newTreeHeadNode.getPrimarySource().getPrimarySource().id).toEqual('h0');
        expect(this.newTreeHeadNode.getPrimarySource().getPrimarySource().hasPrimarySource()).toBe(false);
      });
    });
    ...
    describe('when given a node which sub-tree belongs to other letter', function () {
      beforeEach(function () {
        ...
        this.newTreeHeadNode = this.c2.cloneTreeWithSameLetter('h');
      });

      it('should clone tree down to the node that no longer matches the given letter', function () {
        expect(this.newTreeHeadNode.id).toEqual('h2');
        expect(this.newTreeHeadNode.getPrimarySource().id).toEqual('h1');
        expect(this.newTreeHeadNode.getPrimarySource().getPrimarySource().id).toEqual('b1');
        expect(this.newTreeHeadNode.getPrimarySource().getPrimarySource().getPrimarySource().id).toEqual('b0');
      });
    });
  });
});
