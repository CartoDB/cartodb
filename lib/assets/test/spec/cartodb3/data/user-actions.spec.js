...
var GeometryModel = require('../../../../javascripts/cartodb3/data/geometry-model');
  ...
  describe('.createAnalysisNode', function () {
    ...
    describe('when given a layer that has no analysis yet', function () {
      beforeEach(function () {
        ...
        spyOn(GeometryModel.prototype, 'on').and.callThrough();
      ...
      it('should listen to query-schema-model changes', function () {
        expect(GeometryModel.prototype.on).toHaveBeenCalled();
        expect(GeometryModel.prototype.on.calls.count()).toEqual(2);
        expect(GeometryModel.prototype.on.calls.argsFor(1)[0]).toEqual('change:ready');
        expect(GeometryModel.prototype.on.calls.argsFor(1)[1]).toEqual(jasmine.any(Function));
      });

      describe('when query-schema-model ready attr is changed', function () {
        beforeEach(function () {
          this.geometryModel = this.analysisDefinitionNodesCollection.get('a1').geometryModel;
          spyOn(this.geometryModel, 'off').and.callThrough();
          spyOn(this.geometryModel, 'fetch');
        ...
        it('should remove listener', function () {
          expect(this.geometryModel.off).toHaveBeenCalledWith('change:ready', jasmine.any(Function));
          expect(this.geometryModel.off.calls.argsFor(0)[1]).toBe(GeometryModel.prototype.on.calls.argsFor(1)[1]);
        });

        it('should listen to status attr', function () {
          expect(GeometryModel.prototype.on.calls.count()).toEqual(3);
          expect(GeometryModel.prototype.on.calls.argsFor(2)[0]).toEqual('change:status');
          expect(GeometryModel.prototype.on.calls.argsFor(2)[1]).toEqual(jasmine.any(Function));
        });

        it('should fetch model', function () {
          expect(this.geometryModel.fetch).toHaveBeenCalled();
        });
