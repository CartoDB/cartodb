  ...
  cdb.admin.LayersPanel = cdb.ui.common.TabPane.extend({
    ...
    _addLayerDialog: function(e) {
      ...
      this.new_layer_dlg = new cdb.admin.CreateLayerDialog({
        ...
        tabs:   ['layer', 'file', 'gdrive', 'dropbox', 'twitter', 'scratch', 'success', 'error'],
        option: 'layer',
        where:  'table'
      });
    ...
    _addVisLayer: function(m, dlg) {
      ...
      if(!this.vis.isVisualization()) {
        ...
        var force_torque_conversion = false;
        // if there is some torque layer force convert the new layer to simple one
        if (self.vis.slides.length && self.vis.slides.existsTorqueLayer()) {
          force_torque_conversion = true;
        }
        // Add new layer
        this.map.addCartodbLayerFromTable(table_name, user_name, {
          vis: self.vis,
          convert_torque: force_torque_conversion,
          success: function() {
              // layers need to be saved because the order may changed
