...
cdb.admin.dashboard.MessageItem = cdb.core.View.extend({
  ...
  initialize: function() {
    this.template = _.template('<div class="inner"><%= message%><% if (!sticky) {%><a href="#/close" class="smaller close">x</a><% } %><% if (upgrade) {%><a class="button small green upgrade" href="<%= upgrade_url %>?utm_source=Dashboard_Limits_Reached&utm_medium=referral&utm_campaign=Upgrade_from_Dashboard">Upgrade now</a><% } %></div>');
  },
...
cdb.admin.dashboard.Messages = cdb.core.View.extend({
  ...
  _MESSAGES: {
    ...
    limits_mapviews_exceeded:   { template: 'Hey <strong><%= username %></strong>, you\'re over your map view limit, your current overage quota is $<%= mapviews_overage_quota %> for every 1000 extra requests.<br />Start thinking about <a href="<%= upgrade_url %>?utm_source=Dashboard_Limits_Reached&utm_medium=referral&utm_campaign=Upgrade_from_Dashboard&utm_content=upgrading%20your%20plan" class ="underline">upgrading your plan</a>.', store: true },
    upgraded:                   { template: 'Great! Welcome to your brand new <strong><%= account_type %></strong> CartoDB. Now we love you even more than before ;)', store: true },
