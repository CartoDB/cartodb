...
cdb.admin.mod.LegendEditorItem = cdb.core.View.extend({
  ...
  _addColorForm: function() {
    ...
    if (this.options.filter_type == 'category' || this.options.filter_type == 'color' || this.options.filter_type == 'custom') {
      ...
      var color = this.model.get("value").indexOf("http") == -1 ? this.model.get("value") : "#FFFFFF";
      tempModel.set({ color: color, file: this.model.get("value") });

      // Bind it to our item
      tempModel.bind("change:color change:file", function() {
        var value = this.get("color") || this.get("file");
