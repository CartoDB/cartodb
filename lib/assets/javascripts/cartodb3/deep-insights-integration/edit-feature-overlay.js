...
var EditFeatureOverlay = CoreView.extend({
  ...
  _disabledTooltip: function () {
    ...
    if (this._hasAnalyses()) {
      layers = _t('editor.edit-feature.analysis');
    } else if (this._isCustomQueryApplied()) {
      layers = _t('editor.edit-feature.custom-sql');
    } else if (this._isReadOnly()) {
      layers = _t('editor.edit-feature.sync');
    }

    return _t('editor.edit-feature.disabled', { layers: layers });
  },
