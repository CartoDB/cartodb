class InvitationsController < ApplicationController
  ...
  def invite_token_valid?
    redirect_to root_path and return if params.blank?

    invite_token = params[:invite_token] || params[:user][:invite_token]
    redirect_to root_path and return if params[:id].blank? || invite_token.blank?

    @user = User.filter({:id => params[:id]} & {:invite_token => invite_token}).first

    redirect_to root_path and return if @user.nil? || @user.invite_token_date < 30.days.ago.to_date
  rescue
