...
var ImportOptions = require('new_common/dialogs/create/listing/import_options');
var ImportFallback = require('new_common/dialogs/create/listing/imports/import_default_fallback_view');
...
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.template = cdb.templates.getTemplate('new_common/views/create/listing/import_view');
  },
  ...
  _genTabs: function() {
    var self = this;
    var tabs = "";
    var tabTemplate = cdb.templates.getTemplate('new_common/views/create/listing/import_tab');
  ...
  _genTabsNavigation: function() {
    // Set max pages
    this.model.set('maxPages', Math.ceil(this.$('.ImportOptions-tab').size() / this._TABS_PER_ROW));
  ...
  _moveTabsNavigation: function() {
    var page = this.model.get('page');
    var itemWidth = this.$('.ImportOptions-tab').width();
    var rowWidth = 800;
    ...
    _.each(this._IMPORT_OPTIONS, function(imp,i) {
      ...
      if (isEnabled || isEnabled === undefined) {
        pane = new imp.className(
          _.extend(( imp.options || {} ), { user: self.user, title: imp.title })
        );
      ...
      if (pane) {
        ...
        pane.bind('createDataset', self._createDataset, self);
  ...
  _initBinds: function() {
    this.createModel.bind('change:option', this._updateImportOption, this);
  ...
  _updateImportOption: function() {
    if (this.createModel.getListingState() === "import") {
      this.createModel.setImportState(this.importPanes.activeTab);
    }
  },
