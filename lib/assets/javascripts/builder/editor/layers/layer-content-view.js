...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    this._canBeGeoreferenced = false;
    this._showPlaceholder = this._areQueryObjectsFetching();
  ...
  _initBinds: function () {
    ...
    this.listenTo(this.model, 'change:state', this._fetchAllQueryObjects);
    ...
    this.listenTo(layerDefinitionModel, 'change:source', this._fetchAllQueryObjects);
  ...
  _renderLayerTabPaneView: function () {
    ...
    var canBeGeoreferenced = this._canBeGeoreferenced;
    var isDone = this._layerDefinitionModel.isDone();
    var isEmpty = this._isLayerEmpty && isDone;
  ...
  _fetchAllQueryObjects: function () {
    var reRenderFn = this._reRenderIfQueryObjectsChanged.bind(this);
  ...
  _reRenderIfQueryObjectsChanged: function () {
    var self = this;
    ...
    Promise.all([emptyPromise, geoReferencePromise])
      .then(function (values) {
        ...
        var canBeGeoreferenced = values[1];
        var showPlaceholder = self._areQueryObjectsFetching();

        if (isEmpty !== self._isLayerEmpty ||
          canBeGeoreferenced !== self._canBeGeoreferenced ||
          showPlaceholder !== self._showPlaceholder) {
          self._isLayerEmpty = isEmpty;
          self._canBeGeoreferenced = canBeGeoreferenced;
          self._showPlaceholder = showPlaceholder;
          // console.log('lcv Rerrenderizamos por cambios en emptyness o georreferencia.');
          self.render();
        }
      });
  },

  _areQueryObjectsFetching: function () {
    return this._getQueryRowsCollection().isFetching() ||
