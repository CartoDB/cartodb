...
describe("Watching notifier", function() {
  ...
  describe("View", function() {
    ...
    it('should render properly', function() {
      view.render();
      expect(view.$el.text().replace(/^\s+|\s+$/g, '')).toBe('0 people are also editing this table');
    ...
    it('should change text when watcher users has changed', function() {
      ...
      expect(view.$el.text().replace(/^\s+|\s+$/g, '')).toBe('3 people are also editing this table');
      model.set('users', ['test']);
      expect(view.$el.text().replace(/^\s+|\s+$/g, '')).toBe('test is also editing this table');
    });
    ...
    it('should remove current user from users array', function() {
      ...
      expect(view.$el.hasClass('active')).toBeTruthy();
      expect(view.$el.text().replace(/^\s+|\s+$/g, '')).toBe('2 people are also editing this table');
    });
  })
