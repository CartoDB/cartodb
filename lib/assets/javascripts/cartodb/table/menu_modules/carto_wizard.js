...
cdb.admin.mod.CartoCSSWizard = cdb.core.View.extend({
  ...
  _resetNavigation: function() {
    ...
    if (selectedIndex >= 3) {
      ...
      var move = this.position * 92; // LI width, it is not possible to get width if the component doesnt exist or it is not displayed
      $ul.css({ left: -move + 'px' });
...
cdb.admin.mod.SimpleWizard = cdb.core.View.extend({
  ...
  */
  ...
  /**
  *  Get columns without system columns
  */
  _getColumns: function() {
    var self = this;
    return _.filter(this.options.table.columnNames(), function(c) {
      return !_.contains(self.options.table.hiddenColumns, c);
    });
  }
