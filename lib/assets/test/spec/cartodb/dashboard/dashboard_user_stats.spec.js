describe("cdb.admin.dashboard.UserStats", function() {
  ...
  var user_stats;
  var localStorageKey = "test_user_storage_3";
  ...
  describe("Warnings", function() {
    ...
    it("should show the warning message when the user is close to the limits", function() {
      var user_stats = generateUserStats({ migrated: true, account_type: "JOHN SNOW", remaining_byte_quota: 2, table_count: 8, table_quota: 10, byte_quota: 10, username: "development" });
      user_stats.render();
      expect(user_stats.messages.$el.text()).toEqual('Hey development, looks like you\'re about to reach your account limit. Start thinking about upgrading your plan.x');
    });
    ...
    it("shouldn't show the warning message if the user is hosting the app", function() {
      ...
      user_stats.render();
      expect(user_stats.messages.$el.text()).not.toEqual('Hey development, looks like you\'re about to reach your account limit. Start thinking about upgrading your plan.x');
    });
    ...
    it("should show the warning message when the user is close to end the trial period", function() {
      ...
      user_stats.render();
      expect(user_stats.messages.$el.text()).toEqual('Just a reminder, your JOHN SNOW trial will finish the next 2013-07-21. Happy mapping!');
    });
  ...
  describe("mapviews quota", function() {
    ...
    it("should generate the mapviews quota status message when a FREE user reached the limit", function() {
      ...
      expect(user_stats.model.get("limits_mapviews_exceeded")).toEqual("mapviews");
      expect(user_stats.messages.$el.find(".limits_mapviews_exceeded").html()).toEqual('<div class="inner">Your map views for the month are over limit. Your billing month restarts on 2014-06-18. Please add your billing info as soon as possible to avoid a stop in your service. For all FREE accounts, map views over the first 50,000 cost $0.20 per 1000. If you need help or have questions, please <a href="mailto:support@cartodb.com">contact us</a><a href="#close" class="smaller close">x</a></div>');
    });
    ...
    it("should generate the mapviews quota status message when a PAID user reached the limit", function() {
      ...
      expect(user_stats.messages.messages.length).toEqual(1);
      expect(user_stats.model.get("limits_mapviews_exceeded")).toEqual("mapviews");
      expect(user_stats.messages.$el.find(".limits_mapviews_exceeded").html()).toEqual('<div class="inner">Hey <strong>dev</strong>, you\'re over your map view limit, your current overage quota is $1 for every 1000 extra requests.<br>Start thinking about <a href="?utm_source=Dashboard_Limits_Reached&amp;utm_medium=referral&amp;utm_campaign= Upgrade_from_Dashboard&amp;utm_content=upgrading%20your%20plan" class="underline">upgrading your plan</a>.<a href="#close" class="smaller close">x</a></div>');
