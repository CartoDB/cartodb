...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _addExportBar: function(model, form_data, forceReset) {
    ...
    if (!this.mapToolbar) {
      this.mapToolbar = this.$el.find(".map_toolbar");
    }
    ...
    var animatedRemoval = this._removeExportBar(false, model, forceReset);
    ...
    if (!this.exportBar) {
      ...
      this.exportBar.bind("remove", this._removeExportBar, this);
      this.exportBar.bind("copy-overlay", this._duplicate, this);
      ...
      if (!animatedRemoval) {
        ...
        this.exportBar.$el.css({ top: 0 });
        this.exportBarActions.$el.css({ top: 0 });
      }
  ...
  _removeExportBar: function(show_toolbar, model, force) {
    ...
    if (this.exportBar) {
      ...
      if (!model) { // if there's no current model, animate the bar
        ...
        this.exportBar.clean();
        delete this.exportBar;
        this.exportBarActions.clean();
        delete this.exportBarActions;
        return true;
      }
    }
  },
  ...
  _exportSizeChanged: function(selection) {
    ...
    /* TODO: show custom size text fields again when implemented to interact with resizer
    setTimeout(function() {
  ...
  _exportImageClick: function(e) {
    ...
    this._addExportBar(this.layerModel.legend, this.export_form_data, true);
  ...
  _exportImage: function() {
    ...
    var width =  this.export_size === 'custom' ? mapWidth : this.export_sizes[this.export_size][0];
    var height = this.export_size === 'custom' ? mapHeight : this.export_sizes[this.export_size][1];
    ...
    if (this.export_size !== 'custom') {
      ...
      var w = mapWidth - width;
      var h = mapHeight - height;
      exportOptions.left = w > 0 ? w / 2: 0;
      exportOptions.top = h > 0 ? h / 2 : 0;
    } else {
      // TODO: why are these not being set on options from defaults?
    ...
    cdb.god.bind("export_size_changed", function(selection) {
