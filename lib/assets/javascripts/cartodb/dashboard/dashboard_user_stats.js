...
cdb.admin.dashboard.UserStats = cdb.core.View.extend({
  ...
  _removeLimitMessage: function() {
    ...
    if (tableLimit === null && mapviewsLimit === null && spaceLimit === null) {
      this.messages.removeMessage("limits_exceeded");
      this.messages._resetClosedStatus("limits_exceeded");
    }
  ...
  _chooseBadge: function() {
    ...
    else if (account_type.indexOf("dedicated") != -1 ) return "dedicated";
  ...
  _checkTrialExpired: function() {
    ...
    if (this.model.get("trial_ends_at")) {
  ...
  /**
  *  Calculates the difference of a given date with the current date in days
  */
  _getDaysOfDifference: function(date) {

    var one_day = 1000*60*60*24;
    var today = new Date();
    return (  Math.round((today.getTime() - date.getTime())/one_day)  );
  ...
  */
  _checkUpgradedUser: function() {
    if (config && !config.custom_com_hosted && config.account_host) {
      ...
      var upgraded_at = this.model.get("upgraded_at");
      if (upgraded_at) this.messages.addMessage("upgraded", this.model.attributes);

    }
  },

      /*var upgraded_at = "2013-07-26";

      if (upgraded_at) {
        var upgradedDate = new Date(upgraded_at);
        var days = this._getDaysOfDifference(upgradedDate);
        console.log(days);
        if (days >= 15) this.messages.addMessage("upgraded", this.model.attributes);
        //self.messages.addMessage("upgraded", self.model.attributes);
      }*/
