  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    _TEXTS: {
      layer: {
        sorting: _t('Animated layers must be on top of other layers.')
      },
      error:  {
    ...
    events: {
      ...
      'click':              '_onPanelAction'
    },

    initialize: function() {
      cdb.admin.RightMenu.prototype.initialize.call(this);

      _.bindAll(this, '_editAlias', '_showTooltip', '_hideTooltip', '_moveTooltip');
      ...
      cdb.god.bind("panel_action", this._onPanelAction, this);
    ...
    render: function() {
      cdb.admin.RightMenu.prototype.render.call(this);
      this._bindTooltip();
    ...
    _onPanelAction: function(event_type) {
      this.view_model.set('state', 'idle');
    ...
    setLayerName: function(layer) {
      if (this.vis.isVisualization()) {
        // table name
        this.$('.layer_info a.info .name').text(layer.get('table_name_alias') || layer.get('table_name').replace(/_/g,' '));
    ...
    _bindTooltip: function() {
      ...
      if (this.model.get('type') !== "CartoDB") {
        this.$('.info').draggable({
          start: this._showTooltip,
          drag: this._moveTooltip,
          stop: this._hideTooltip
        });  
      }
    },

    _unbindTooltip: function() {
      this.$('.info').draggable('destroy');
    },
    
    _showTooltip: function(e, ui) {
      if (this.model.get('type') !== "CartoDB") {
        this.follow_tooltip = new cdb.admin.TooltipTrails({
          className:  'tooltip-trails',
          msg:        this._TEXTS.layer.sorting
        });

        // Get correct first drag coordinates
        var parentOffset = this.$('.info').offset();
        var position = {
          left: (e.pageX - parentOffset.left),
          top: (e.pageY - parentOffset.top)
        };

        this.$el.append(this.follow_tooltip.render().el);
        this.follow_tooltip.show(position);
        this.addView(this.follow_tooltip);
      }
    },

    _moveTooltip: function(e, ui) {
      if (this.follow_tooltip) {
        this.follow_tooltip.move(ui.position);
      }
    },

    _hideTooltip: function(e, ui) {
      if (this.follow_tooltip) {
        this.follow_tooltip.hide();
      }
    },
