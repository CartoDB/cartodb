...
module.exports = cdb.core.View.extend({
  ...
  events: {
    ...
    'click .js-create_from_sql': '_createDatasetFromSQL',
  ...
  _createDatasetFromSQL: function(e) {
    if (e) this.killEvent(e);
    ...
    if (selectedDatasets.length === 1) {
      var m = selectedDatasets[0];
      var table = m.tableMetadata();
      var owner = table.permission && table.permission.owner.get('username');
      var tableName = table.getUnqualifiedName();
      var query = encodeURIComponent("SELECT * FROM " + tableName);
      var sqlApiUrl = cdb.config.getSqlApiUrl().replace(this.user.get('username'), owner);
      var sql = sqlApiUrl + "?q=" + query + "&format=csv";

      cdb.god.trigger('importByUploadData', {
        type: 'sql',
        table_name: tableName + '_copy',
        value: sql,
        create_vis: false
      });
    }
