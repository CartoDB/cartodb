...
cdb.admin.mod.CartoCSSWizard = cdb.admin.Module.extend({
  ...
  initialize: function() {
    ...
    this.options.table.bind('change geolocated', function() {
      ...
      var geoTypes = table.changed.geometry_types;
      var prevGeoTypes = table.previousAttributes().geometry_types;
      var geoTypeChanged = geoTypes && geoTypes.length > 0 && prevGeoTypes && prevGeoTypes.length > 0;
      if (geoTypeChanged) {
  ...
  _updateForms: function() {
    ...
    if(p) {
      ...
      this.panels.active(wizard.type);
    }
  },
  ...
  activated: function() {
    ...
    var changeLayerStyle = function(st, sql) {
      ...
      self.model.save(_.extend(attrs, { tile_style: st }), {
        no_override: true
      });
    };
  ...
  _updateStyle: function() {
    ...
    if(this.options.table.previous('schema')) {
      var pane = this.panels.getActivePane();
      if(pane.type === this.cartoStylesGeneration.get('type')) {
        ...
        function updateWizardAttrs() {
          ...
          this.cartoStylesGeneration.unbind('generated', updateWizardAttrs);
        }
...
cdb.admin.mod.SimpleWizard = cdb.core.View.extend({
  ...
  */
  ...
  propertiesFromStyle: function(cartocss) {
    ...
    var rules = parser.parse(cartocss).getDefaultRules();
    var props = {};
    if (rules) {
