  ...
  cdb.admin.AssetManager = cdb.admin.BaseDialog.extend({
    ...
    initialize: function() {
      ...
      _.extend(this.options, {
        ...
        width: this.kind === "marker" ? 693 : 643
      });
    ...
    render_option_tabs: function($content) {
      ...
      // Render makis pane only if kind type
      // is a marker
      if (this.kind === "marker") {
    ...
    _renderMakisPane: function() {
      ...
      this.upload_panes.addTab('makis', this.makisPane);

      // Don't render (and request) maki images until
      // tab is enabled for the first time
      this.upload_panes.bind('tabEnabled:makis', function() {
        this.makisPane.render();
        this.upload_panes.unbind('tabEnabled:makis', null, null);
      }, this);
    },
