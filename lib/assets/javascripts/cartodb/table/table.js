  ...
  var Table = cdb.core.View.extend({
    ...
    */
    ...
    _initModels: function() {
      ...
      this.backgroundPollingModel = this.user.featureEnabled('new_modals') ?
        new cdb.editor.BackgroundPollingModel({
    ...
    _initViews: function() {
      ...
      this.menu.bind('switch', function(layerView) {
        ...
        // Not needed when new_modals is enabled
        if (!this.user.featureEnabled('new_modals')) {
    ...
    _initTableMap: function() {
      ...
      // Disabling old background geocoding tab when
      // user has new_modals enabled
      if (!this.user.featureEnabled('new_modals')) {
        // Background tab
    ...
    _onGeocodingChosen: function(data) {
      ...
      var hasNewModals = this.user.featureEnabled('new_modals');
      var isTypeLonLat = data.type === 'lonlat';

      // Lon-lat type?
      if (!hasNewModals && isTypeLonLat) {
      ...
      if (hasNewModals || !isTypeLonLat) {
        if (hasNewModals) {
          ...
          this.geocoder.set(_.omit(data, 'table_name', 'type'));
        }
        return false;
      }

      cdb.log.info('No geocoder option for ' + data.kind);
    },
