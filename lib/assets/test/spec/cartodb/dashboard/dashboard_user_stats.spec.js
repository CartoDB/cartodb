describe("cdb.admin.dashboard.UserStats", function() {
  ...
  describe("UserStats", function() {
    ...
    it("should show mapviews when user is and organization admin", function() {
      ...
      var user = new cdb.admin.User({ id:3, account_type: "JOHN SNOW", remaining_byte_quota: 0, table_count: 0, table_quota: 10, quota_in_bytes: 10, username: "development" });
      user.organization = org;
      ...
      view.render();
      expect(view.$('div.stats').length).toBe(0);
      expect(view.$('.mapviews .progress').length).toBe(1);
    });
