  ...
  cdb.admin.ShareDialog = cdb.admin.BaseDialog.extend({
    ...
    _resize: function() {
      ...
      var calcHeights = function(baseHeight) {
        ...
        //console.log(baseHeight , this.$el.find("ul.options.switches").outerHeight(true) , this.$el.find(("h4").outerHeight(true), this.$el.find(("h3").outerHeight(true), this.$el.find((".privacy.switches").outerHeight(true));
    ...
    _setupPrivacySettings: function() {
      ...
      this.$el.find(".radiobutton").on("click", function(e) {
        ...
        opts.share_mode = $radio.attr("data-mode");

        if (opts.share_mode != "public") opts.privacy = "PRIVATE";
        else opts.privacy = "PUBLIC";
