  ...
  */
  ...
  render_content: function() {
    ...
    if (this.options.files) {
      ...
      this.$import.find("input[type='text']").val(this.options.url);
      this._onInputChange();
      this._onSubmitForm();
    }
  ...
  _onUploadError: function(ev,data) {
    ...
    if (data.errorThrown != "abort") {
      ...
      msg = "You have decided cancel the upload, start mapping with other file."
    }
  ...
  _onSubmitForm: function(ev) {
    ...
    if (!this._checkURL(url)) {
      this._showUploadError(this.error_messages.urlError);
