...
var editorWidgetsView = new EditorWidgetsView({
  ...
  collection: mapDefModel.widgets
});
...
dashboard.vis.done(function () {
  ...
  _.each(vizJSON.widgets, function (d) {
    var layerDefModel = mapDefModel.layers.get(d.layer_id);
  ...
  window.w = new WidgetDefinitionModel({
    type: 'formula',
    ...
    layerDefinitionModel: mapDefModel.layers.get('0c7e1090-1a44-40dc-a78a-13e59d972a7d') // districts
  });
  mapDefModel.widgets.add(window.w);
})
.error(function (err) {
  console.error(err);
});
