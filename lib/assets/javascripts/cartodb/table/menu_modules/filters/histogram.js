    ...
    _checkEmpty: function() {
      ...
      _updateBounds:function(bounds, update) {
        ...
        if (bounds) {
          ...
          if (update) {
            this.model.set('lower', lower);
            this.model.set('upper', upper);
          }
          ...
          if (this.isDate) {
            ...
            if (max_date.toDateString() == min_date.toDateString()) {
              min = this._formatDate(min_date, true);
              max = this._formatDate(max_date, true);
            } else {
              min = this._formatDate(min_date);
              max = this._formatDate(max_date);
            }

          } else {
            var min = lower.toPrecision( Math.round(lower).toString().length + 2);
            var max = upper.toPrecision( Math.round(upper).toString().length + 2);
          }

          this.$('.range').html(min + " - " + max);

        }

      },

      _formatDate: function(date, time_based) {
        ...
        if (time_based) {
          ...
          return date.getDate() +  get_nth_suffix(date.getDate()) + " of "  + this._SHORT_MONTH_NAMES[date.getMonth() ] + ", " + date.getFullYear();
        }
