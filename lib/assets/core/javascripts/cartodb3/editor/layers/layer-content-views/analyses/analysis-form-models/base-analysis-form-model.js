...
module.exports = Backbone.Model.extend({
  ...
  _getSourceOptionsForSource: function (args) {
    var sourceAttrName = null;
    var requiredSimpleGeometryType = '*';
    var ignorePrimarySource = false;
    var onlyNodes = false;

    // Args validation and defaults
    if (args.sourceAttrName === void 0) {
      throw new Error('sourceAttrName is required.');
    } else {
      sourceAttrName = args.sourceAttrName;
    }

    if (args.requiredSimpleGeometryType !== void 0) {
      requiredSimpleGeometryType = args.requiredSimpleGeometryType;
    }

    if (args.ignorePrimarySource) {
    ...
    if (this._isFetchingOptions()) {
      ...
      return this._analysisSourceOptionsModel
        ...
        .filter(function (d) {
          return d.val !== sourceId && (onlyNodes ? d.type === 'node' : true);
        });
    }
  }
});
