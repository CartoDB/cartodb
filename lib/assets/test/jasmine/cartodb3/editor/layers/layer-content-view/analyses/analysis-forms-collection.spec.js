...
describe('cartodb3/editor/layers/layer-content-view/analyses/analysis-forms-collection', function () {
  ...
  describe('.addHead', function () {
    ...
    beforeEach(function () {
      ...
      var originalAdd = this.collection.add.bind(this.collection);
      spyOn(this.collection, 'add').and.callFake(function () {
        this.formModel = originalAdd.apply(this.collection, arguments);
        spyOn(this.formModel, 'isValid').and.returnValue(isValid);
        return this.formModel;
      }.bind(this));
    });

    describe('if form model is valid with defaults', function () {
      beforeEach(function () {
        isValid = true;
        spyOn(this.userActions, 'updateOrCreateAnalysis');
        this.res = this.collection.addHead({id: 'a3'}); // unknown
      });

      it('should return the created formModel', function () {
        expect(this.res).toBe(this.formModel);
      });

      it('should create the node-definition right away', function () {
        expect(this.userActions.updateOrCreateAnalysis).toHaveBeenCalledWith(this.formModel);
      });
    ...
    describe('if form model needs complemeting data from user', function () {
      beforeEach(function () {
        isValid = false;
        this.res = this.collection.addHead({id: 'a3'}); // unknown
      });
