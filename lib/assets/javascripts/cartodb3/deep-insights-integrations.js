...
F.prototype._onFeatureClicked = function (event) {
  ...
  // We can only display the Edit Feature Overlay if layer doesn't have any analysis
  if (layerDefinitionModel && !layerDefinitionModel.hasAnalyses()) {
    this._editFeatureOverlay.setPosition(position);
    this._editFeatureOverlay.show();
    this._editFeatureOverlay.off('editFeature');
    this._editFeatureOverlay.once('editFeature', function () {
