...
module.exports = cdb.core.View.extend({
  initialize: function () {
    ...
    this.hubspotFormEnabled = this.options.hubspotFormEnabled;
    this.user = this.options.user;
    this.template = cdb.templates.getTemplate('common/views/vendor_scripts');
    this.hubspotFormId = this.options.config.hubspot_form_ids
      ? JSON.parse(this.options.config.hubspot_form_ids)[FORM_ID]
      : null;
  },
  ...
  render: function () {
    this.$el.html(
      this.template({
        ...
        hubspotFormEnabled: this.hubspotFormEnabled || false,
