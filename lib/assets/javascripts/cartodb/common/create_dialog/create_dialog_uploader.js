  ...
  cdb.common.CreateDialog.Uploader = cdb.core.View.extend({
    ...
    _onUploadStart: function(e, data) {
      ...
      if (data.files && data.files.length > 0) {
        // Hello mixpanel!
        this._trackEvent("Uploading a file", {
    ...
    _onImportComplete: function(m) {
      ...
      if (!m.get('table_name')) {
        ...
        // Send complete event to mixpanel
        var obj = {
          item_queue_id:  this.model.get('upload').item_queue_id,
        ...
        if (this.model.get('upload').type === "service") {
          obj.value = this.model.get('upload').value;
        }
        this._trackEvent('Import completed', obj);
    ...
    _onImportError: function(m) {
      ...
      this._trackEvent('Import failed', {
        item_queue_id:  this.model.get('upload').item_queue_id,
        username:       this.user.get('username')
      });
    ...
    _createTable: function() {
      ...
      table.save({}, {
        success: function(m) {
          ...
          // Send complete event to mixpanel
          self._trackEvent('Table creation completed', {
            table_name: self.model.get('upload').table_name,
            username:   self.user.get('username'),
            option:     self.model.get('option')
          });
        ...
        error: function(m, e) {
          ...
          // Send error to mixpanel
          this._trackEvent('Table creation error', {
            username: self.user.get('username')
          });
    ...
    _trackEvent: function(eventName, eventData) {
      cdb.god.trigger('mixpanel', eventName, eventData);
    },
