  ...
  cdb.admin.ImportLayerPane = cdb.admin.ImportPane.extend({
    ...
    initialize: function() {
      ...
      this.template = cdb.templates.getTemplate(this.options.template || 'common/views/import/import_layer');
    ...
    _onTablesFetched: function() {
      ...
      this._toggleElements(['span.loader', 'p.warning.error'], false);
      this._showTablesList();
    },

    _onTablesError: function() {
      this._toggleElements(['span.loader'], false);
      this._removeTablesList();
      this._toggleElements(['p.warning.error'], true);
    },
    ...
    _onTableSelected: function(table_data) {
      ...
      // Remove all warnings
      this._toggleElements(['p.warning.geo'], false);
      ...
      table_metadata.fetch({
        success: function(m) {
          ...
          if (self.tableCombo.getSelected().name == m.get('name') && m.get('geometry_types').length == 0) {
            self._toggleElements(['p.warning.geo'], true);
          }
        }
      });
    ...
    _toggleElements: function(selectors, show) {
      var self = this;
      _.each(selectors, function(el) {
        self.$(el)[ show ? 'fadeIn' : 'fadeOut' ]()
      });
    },
