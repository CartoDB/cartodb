...
var HeaderView = cdb.admin.HeaderView = cdb.core.View.extend({
  ...
  events: {
    ...
    'focusout .col_name_edit': '_finishEdit',
    'focusout .col_alias_edit': '_finishEdit',
  ...
  initialize: function() {
    ...
    this.editing_alias = false;
    this.changing_type = false;

    if (this.column[2]) { this.alias_column = true; }
    ...
    if (HeaderView.colOptions === undefined) {
      ...
      cdb.god.bind('closeColumnDropDown', HeaderView.colOptions.hide, HeaderView.colOptions);
  ...
  render: function() {
    ...
    this.$el.append(this.template({
      col_name:         this.column[2] || this.column[0],
      col_type:         this.column[1],
      col_alias:        this.column[2],
      original_col_name:this.column[0],
      editing_name:     this.editing_name,
      editing_alias:    this.editing_alias,
      changing_type:    this.changing_type,
      read_only:        this.table.isReadOnly(),
      is_sync:          this.table.isSync(),
      hasAliasesFeatureFlag: this.table.user.featureEnabled('aliases'),
    ...
    if (this.editing_name || this.editing_alias) {
  ...
  _openColOptions: function(e) {
    ...
    colOptions.setTable(this.table, this.column[0], this.column[2]);

    colOptions.bind('addColumn', this._addColumn, this);
    colOptions.bind('renameAlias', this._renameAlias, this);
  ...
  _submitAliasEdit: function (aliasName) {
    this.table.renameAlias(this.column[0], this.column[2], aliasName);
    this._finishEdit();
  },

  _renameAlias: function (e, aliasName) {
    if (e) {
      this.killEvent(e);
    }

    this._submitAliasEdit(aliasName);
  },

  _finishEdit: function() {
    if (this.editing_name) { this.editing_name = false; }
    if (this.editing_alias) { this.editing_alias = false; }

    this.render();
  },

  renameColumnAlias: function(e){
    if (e) { this.killEvent(e); }

    this.editing_alias = true;
    this.editing_name = false;
    this.changing_type = false;
    this.render();
  },
