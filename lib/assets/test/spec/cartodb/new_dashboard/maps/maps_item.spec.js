...
describe('new_dashboard/maps/maps_item', function() {
  ...
  describe('render the URL to map', function() {
    ...
    it('should be the standard one when user doesn\'t belong to an org', function() {
      this.renderView();
      expect($(this.innerHTML()).find('.DefaultTitle-link').attr('href')).toContain('/viz/8988-8888-8888/map');
    });
  ...
  it('should render likes count', function() {
    this.renderView();
    expect($(this.innerHTML()).find('.LikesIndicator').length).toBe(1);
    expect($(this.innerHTML()).find('.LikesIndicator').text()).toContain('42');
  });

  it('should render privacy', function() {
    this.renderView();
    expect($(this.innerHTML()).find('.PrivacyIndicator').length).toBe(1);
    expect($(this.innerHTML()).find('.PrivacyIndicator').text()).toBe('public');
    expect($(this.innerHTML()).find('.PrivacyIndicator').hasClass('is-public')).toBeTruthy();
  });
  ...
  describe('render owner user info', function() {
    ...
    it('shouldn\'t render owner info if user is not in a org', function() {
      this.renderView();
      expect($(this.innerHTML()).find('.UserAvatar').length).toBe(0);
    });
    ...
    it('should render owner info if user is not the owner of the map', function() {
      ...
      this.renderView();
      expect($(this.innerHTML()).find('.UserAvatar').length).toBe(1);
      expect($(this.innerHTML()).find('.UserAvatar img').attr('title')).toBe('whoknows');
    });
  ...
  it('should render timediff', function() {
    this.renderView();
    expect($(this.innerHTML()).find('.MapCard-contentFooterTimeDiff').length).toBe(1);
    expect($(this.innerHTML()).find('.MapCard-contentFooterTimeDiff').text()).toContain('a few seconds ago');
  });

  it('should render mapviews graph', function() {
    this.renderView();
    expect($(this.innerHTML()).find('.MapCard-headerGraph').length).toBe(1);
    expect($(this.innerHTML()).find('.MapCard-headerGraph svg').length).toBe(1);
  });
  ...
  describe('description', function() {
    it('should show description if it is set', function() {
      ...
      this.renderView();
      expect($(this.innerHTML()).find('.DefaultDescription').length).toBe(1);
      expect($(this.innerHTML()).find('.DefaultDescription').text()).toBe('my desc');
    });

    it("should show 'add description...' if it is not set", function() {
      this.renderView();
      expect($(this.innerHTML()).find('.DefaultDescription').length).toBe(0);
      expect($(this.innerHTML()).find('.MapCard-contentDescription .DefaultEditInline-form').length).toBe(1);
      expect($(this.innerHTML()).find('.MapCard-contentDescription .DefaultEditInline-input').attr('placeholder')).toContain('Add a description...');
      expect($(this.innerHTML()).find('.MapCard-contentDescription .DefaultEditInline-input').val()).toBe('');
    });
  });
  ...
  describe('tags', function() {
    it('should show tags if it is set', function() {
      ...
      this.renderView();
      expect($(this.innerHTML()).find('.DefaultTags-item').length).toBe(3);
      expect($(this.innerHTML()).find('.MapCard-contentTags').text()).toContain('and 1 more');
    });

    it("should show 'add description...' if it is not set", function() {
      this.renderView();
      expect($(this.innerHTML()).find('.DefaultTags-item').length).toBe(0);
      expect($(this.innerHTML()).find('.MapCard-contentTags .DefaultEditInline-form').length).toBe(1);
      expect($(this.innerHTML()).find('.MapCard-contentTags .DefaultEditInline-input').attr('placeholder')).toContain('Add tags...');
      expect($(this.innerHTML()).find('.MapCard-contentTags .DefaultEditInline-input').val()).toBe('');
    });
  });
  ...
  describe('permission', function() {
    it('shouldn\'t show permission label when user is not in a org', function() {
      this.renderView();
      expect($(this.innerHTML()).find('.PermissionIndicator').length).toBe(0);
    });
    ...
    it("should show permission label when user is not the owner of the vis and he is in a org", function() {
      ...
      this.renderView();
      expect($(this.innerHTML()).find('.PermissionIndicator').length).toBe(1);
      expect($(this.innerHTML()).find('.PermissionIndicator').text()).toBe('READ');
    });
  });
