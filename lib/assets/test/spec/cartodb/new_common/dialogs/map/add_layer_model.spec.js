...
describe('new_common/dialogs/map/add_layer_model', function() {
  ...
  describe('.setUpload', function() {
    ...
    it('should set the given dataset', function() {
      expect(this.model.upload.set).toHaveBeenCalled();
      expect(this.model.upload.set.calls.argsFor(0)[0]).toEqual(jasmine.objectContaining({ foo: 'bar' }));
    });

    it('should omit the create_vis value since set in constructor', function() {
      expect(this.model.upload.set.calls.argsFor(0)[0]).not.toEqual(jasmine.objectContaining({ create_vis: true }));
    });
  });

  describe('.createFromImport', function() {
    beforeEach(function() {
      cdb.god.bind('datasetSelected', function(d) {
        this.datasetSelected = d;
      }, this);
      this.model.createFromImport();
    });

    it('should call cdb.god datasetSelected with expected data', function() {
      expect(this.datasetSelected).toBeTruthy();
      expect(this.datasetSelected).toEqual(jasmine.objectContaining({ state: 'idle' }));
    });
  });
});
