...
const IMPORT_OPTIONS = {
  ...
  Snowflake: {
    ...
    options: {
      ...
      placeholder_query: 'SELECT *, ST_ASWKT(ST_MAKEPOINT(lng, lat)) AS the_geom FROM table'
    }
  },
  Redshift: {
    view: ImportDatabaseView,
    ...
    options: {
      ...
      placeholder_query: 'SELECT *, ST_SetSRID(ST_MakePoint(lng, lat), 4326) AS the_geom FROM table'
    }
  },
  DataObservatory: {
    view: null,
    enabled: function (config, userModel) { return userModel.get('do_enabled'); },
