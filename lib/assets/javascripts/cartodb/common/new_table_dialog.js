   ...
   *
   *  - It doesn't need any model, just the user quota.
   *  - You can attach any file (from mamufas) or an ur (from for example)
   *  the common data.
  ...
  cdb.admin.CreateTableDialog = cdb.ui.common.Dialog.extend({
    ...
    initialize: function() {
      ...
      if (this.options.quota)
        this._UPLOADER.maxFileSize = this.options.quota
    ...
    _checkFiles: function() {
      ...
      if (this.options.files) {
        ...
        setTimeout(function(){
          ...
          self._uploadFromUrl('url', self.options.url);
        },150);
      }
    },
