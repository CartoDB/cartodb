...
describe('cdb.admin.Slide', function() {
  ...
  describe("cdb.admin.Slide.overlays", function() {
    it("should track overlays changes", function() {
      ...
      var slide_overlay = slide.visualization.overlays.at(0);
      slide.set('active', true);
      overlay.set('type', 'test');
      expect(slide_overlay.get('type')).toEqual('test');
      slide.set('active', false);
      slide_overlay.set('type', 'test2');
      slide.set('active', true);
      expect(overlay.get('type')).toEqual('test2');
    });

    it("should track overlays add", function() {
      slide.setMaster(vis);
      vis.overlays.add([
        new cdb.admin.models.Overlay({ id: 'overlay_1', type: 'zoom'}),
      ]);
    ...
    it("should track overlays destroy", function() {
      vis.overlays.add([
        new cdb.admin.models.Overlay({ id: 'overlay_1', type: 'zoom'}),
      ]);
      slide.setMaster(vis);
      expect(slide.visualization.overlays.size()).toEqual(1);
      var o = vis.overlays.at(0);
      o.url = '/test';
      o.destroy();
