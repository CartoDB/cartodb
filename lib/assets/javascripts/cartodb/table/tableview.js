...
(function() {
    ...
    cdb.admin.TableView = cdb.ui.common.Table.extend({
      ...
      _onSQLView: function() {
        ...
        this.options.sqlView.bind('reset', _renderSQLHeader);
        _renderSQLHeader();
      },
      ...
      _getEditor: function(columnType, opts) {
        if(columnType != "undefined") {
          var editors = {
            ...
            'geometry': cdb.admin.StringField, // Geometry type is needed
      ...
      _editCell: function(e, cell, x, y) {
        ...
        // Close and close previous cell editor
        this.closeEditor();

        var column = self.model.columnName(x-1);
        var columnType = this.model.getColumnType(column);

        if(this.model.isReservedColumn(column) && columnType != 'geometry') {
          return;
        }
        ...
        var dlg = this._editorsOpened = new cdb.admin.SmallEditorDialog({
          ...
          row:          row,
          rowNumber:    y,
          readOnly:     true, //this.model.data().isReadOnly(),
      ...
      */
      ...
      _renderLoading: function() {
        this._renderBodyTemplate('table/views/loading_table');
      },
      ...
      */
      rowFailed: function() {
        this.notice('Oops, there has been an error saving your changes.');
      },
