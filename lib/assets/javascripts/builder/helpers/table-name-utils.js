...
module.exports = {
  ...
  getQualifiedTableName: function (tableName, userName, inOrganization, nonOrgUserSchema = '') {
    if (nonOrgUserSchema.length > 0) {
      nonOrgUserSchema = this._quoteIfNeeded(nonOrgUserSchema) + '.';
    }

    var schemaPrefix = (inOrganization && userName) ? this._quoteIfNeeded(userName) + '.' : nonOrgUserSchema;
