  ...
  cdb.admin.GeocodingDialog.Pane.Address = cdb.admin.GeocodingDialog.Pane.extend({
    ...
    _onEstimationChange: function() {
      ...
      var hasHardLimit = this.user.get('geocoding').hard_limit;
      var hasEstimation = this.estimation.get('estimation') !== undefined;

      // Check if it is needed to accept the terms
      if (!googleUser && !hasHardLimit && ( this.estimation.get('estimation') > 0 || ( !hasEstimation && !hasHardLimit ))) {
        this.model.set('agreement', false);
      } else if (googleUser || hasHardLimit) {
