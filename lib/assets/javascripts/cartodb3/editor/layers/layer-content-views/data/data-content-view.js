...
var layerMessageActionTemplate = require('cartodb3/editor/layers/layer-views/layer-message-action.tpl');
var addFeaturePointIcon = require('cartodb3/components/icon/templates/add-feature-point.tpl');
var addFeatureLineIcon = require('cartodb3/components/icon/templates/add-feature-line.tpl');
var addFeaturePolygonIcon = require('cartodb3/components/icon/templates/add-feature-polygon.tpl');
...
module.exports = CoreView.extend({
  ...
  render: function () {
    ...
    if (this._isErrored()) {
      this._showError();
    } else if (this._isDataEmpty) {
      this._showEmptyData();
    } else if (this._isNoGeometryData()) {
  ...
  _showEmptyData: function () {
    this.$el.append(
      layerMessageActionTemplate({
        message: _t('editor.messages.no-data.message'),
        action: _t('editor.messages.no-data.action-message')
      })
    );
  },
