  ...
  cdb.admin.GeocoderStats = cdb.core.View.extend({
    ...
    _TEXTS: {
      available_hard: _t('<p>You have <strong><%= quota - monthly_use %> geocoding credit<%= quota - monthly_use > 1 ? "s" : "" %> available</strong>. Depending on your data you might run out of credits, if so consider <a href="<%= upgrade_url %>?utm_source=Georeferencing_Credits_Remaining&utm_medium=referral&utm_campaign=Upgrade_from_Dashboard&utm_content=upgrading%20to%20a%20higher%20plan">upgrading to a higher plan</a>.</p>'),
      available_soft: _t('<p>You have <strong><%= quota - monthly_use %> geocoding credit<%= quota - monthly_use > 1 ? "s" : "" %> available</strong>.</p> <p>Depending on your data you might run out of credits and you will be charged $<%= block_price %>/1,000 extra geocoding credits.</p>'),
      no_rows_available: _t('<p>You don\'t have more geocoding credits this month.</p> <p><strong>From now on you will be charged $<%= block_price %>/1,000 rows</strong>.</p>'),
      no_rows_upgrade: _t('<p>You don\'t have more geocoding credits this month. <a href="<%= upgrade_url %>?utm_source=Georeferencing_Credits_Not_Enough&utm_medium=referral&utm_campaign=Upgrade_from_Dashboard&utm_content=Upgrade%20your%20account">Upgrade your account</a> to get more.</p>'),
