  ...
  cdb.admin.EditFeatureFields = cdb.admin.BaseDialog.extend({
    ...
    editorField: {
      'date':     cdb.admin.Date,
      'number':   cdb.admin.Number,
      'boolean':  cdb.admin.Boolean,
      'geometry': cdb.admin.Geometry,
      'string':   cdb.admin.String
    },
    ...
    render_content: function() {
      ...
      _.each(this.model.attributes,function(val,attr){
        if (!_.contains(hiddenColumns,attr)) {
          ...
          // Create subview
          var subView = new self.editorField({
            label: true,
            readOnly: false,
    ...
    _isFormValid: function() {
      for (var view in this._subviews) {
        ...
        if (v.model && v.model.get('value') && !v.isValid())
          return false;
      }
      return true;
    },
