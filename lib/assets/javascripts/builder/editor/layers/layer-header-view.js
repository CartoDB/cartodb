...
module.exports = CoreView.extend({
  ...
  render: function () {
    ...
    this.$el.html(
      template({
        ...
        isEmpty: !this._isDataEmpty && this._isLayerEmpty()
      })
    );
  ...
  _onSourceChanged: function () {
    ...
    if (this._topQueryGeometryModel !== null) {
      this._topQueryGeometryModel.unbind('change:simple_geom change:status');
    }

    this._topQueryGeometryModel = nodeModel.queryGeometryModel.bind('change:simple_geom change:status', function (model) {
      if (model.isDone()) {
        this.render();
      }
  ...
  _isLayerEmpty: function () {
    var needsGeocoding = this._layerDefinitionModel.canBeGeoreferenced() && !this._layerDefinitionModel.hasAnalyses();
    return this._layerDefinitionModel.isDone() && !this._isQueryGeometryVisible() && !needsGeocoding;
  },
