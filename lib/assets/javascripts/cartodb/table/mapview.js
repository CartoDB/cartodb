  ...
  return {
    ...
    _setInteraction: function() {
      ...
      if(this.activeLayerModel) {
        ...
        if(idx < 0) {
          cdb.log.error("layer is not in layergroup");
          return;
        }
    ...
    _updateLayerDefinition: function(layer) {
      if(this.groupLayer) {
        ...
        if(layer && layer.changed && !(_.size(layer.changed) == 1 && layer.changed.extra_params)) {
    ...
    _addLayer: function(layer, layers, opts) {
      ...
      if (layer.get('type') === 'CartoDB') {
        ...
        if(!this.groupLayer) {
          ...
          var layer_view = mapViewClass.prototype._addLayer.call(this, m, layers, opts);
