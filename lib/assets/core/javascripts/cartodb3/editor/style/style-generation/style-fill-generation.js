...
function markerFill (props, styleType) {
  var css = {};
  var color = (props && props.color) || {};
  var animationType = styleType === 'animation';
  ...
  if (color) {
    ...
    if (StyleUtils.hasImagesSelected(props)) {
...
module.exports = {
  getPointCSS: function (fillDef, styleType, styleDef) {
    if (StyleUtils.canApplyDotProperties(styleType, styleDef)) {
      return dotFill(fillDef, styleType);
    } else {
      return markerFill(fillDef, styleType);
    }
  },
