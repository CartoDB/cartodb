...
cdb.admin.GeoreferenceDialog = cdb.admin.BaseDialog.extend({
  ...
  initialize: function() {
    ...
    this.options.geocoder.unbind("geocodingComplete");
    this.options.geocoder.bind("geocodingComplete", function(msg) {

      self.options.table.trigger("notice", "Table georeferenced");

      var description; 

      if (msg.model.get("kind") != "high-resolution") { // FREE

        if (msg.model.get("geometry_type") == "points") {
          description = "3,000 out of 5,321 rows were succesfully turned into points! It seems that some of your rows didn't finish succesfully. Perhaps these rows contained empty values or perhaps we just didn't know what the values ment. We encourage you to take a look and try again. This job cost you $0 because we love you for being you!";
        } else {
          description = "3,000 out of 5,321 rows were succesfully turned into polygons! It seems that some of your rows didn't finish succesfully. Perhaps these rows contained empty values or perhaps we just didn't know what the values ment. We encourage you to take a look and try again. This job cost you $0 because we love you for being you!"
        }

      } else {

description = "3,000 out of 5,321 rows were succesfully turned into points! It seems that some of your rows didn't finish succesfully. Perhaps these rows contained empty values or perhaps we just didn't know what the values ment. Unsuccessful rows don't count against your quota, so we encourage you to take a look and try again. This job cost you $0 because we love you! You have X,XXX remaining in your free quota this month.";
      }

      var dlg = new cdb.admin.GeocoderMessageDialog({
        style: msg.geometry_type,
        modal_type: "notification",
        title: msg.processed_rows + " rows were georeferenced",
