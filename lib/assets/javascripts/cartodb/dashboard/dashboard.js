...
$(function() {
  ...
  cdb.admin.dashboard.Dashboard = cdb.core.View.extend({
    ...
    _initRouter: function() {
      ...
      //this.router.bind("index",          this._tables,         this);
    ...
    _initBindings: function() {
      ...
      $("header li a.tables, article.tables .view_all, .subheader .stats .tables a").on("click", function(e) { self._goto(e, "/tables"); });
    ...
    */
    ...
    _index: function() {
      ...
      this._getVisualizations({ q: "", tags: "", per_page: this.visualizations._PREVIEW_ITEMS_PER_PAGE });
      //this._getTables({ q: "", tags: "", per_page: this.tables._PREVIEW_TABLES_PER_PAGE, type: "table", order: this.tablesSortable.getSortMethod() });
    ...
    _tables: function(page) {
      ...
      this._getTables({ q: "", page: page || 1, tags: "", per_page: this.tables._TABLES_PER_PAGE, type: "table", order: this.tablesSortable.getSortMethod() });
    ...
    _visualizations: function(page) {
      ...
      this._getVisualizations({ q: "", page: page || 1, tags: "", per_page: this.visualizations._ITEMS_PER_PAGE, type: "derived" });
    ...
    _search: function(opts) {
      ...
      if (model == 'visualizations') {
        ...
        this.tablesView.hide();
        this._getVisualizations({ q: q, page: page || 1, tags: "", per_page: this.visualizations._ITEMS_PER_PAGE, type: "derived" });

      } else if (model == 'tables') {

        this.visualizationsView.hide();
        this._getTables({ q: q, page: page || 1, tags: "", per_page: this.tables._TABLES_PER_PAGE, type: "table", order: this.tablesSortable.getSortMethod() });
      }
    ...
    _tag: function(opts) {
      ...
      if (model == 'visualizations') {
        ...
        this.tablesView.hide();
        this._getVisualizations({ q: "", page: page || 1, tags: tag, per_page: this.visualizations._ITEMS_PER_PAGE, type: "derived" });

      } else if (model == 'tables') {

        this.visualizationsView.hide();
        this._getTables({ q: "", page: page || 1, tags: tag, per_page: this.tables._TABLES_PER_PAGE, type: "table", order: this.tablesSortable.getSortMethod() });
    ...
    _getVisualizations: function(options) {
    ...
    _getTables: function(options) {

      var path = this.startView.model.get("path");

      if (path == 'tables') this.tablesView.showDefaultTitle(false);
      else this.tablesView.showDefaultTitle(true);

      this.tablesView.showLoader();

      if (path == 'tables' || path == 'search') this.tablesView.model.set("padding", true);
    ...
    _toggleAddTablesView: function() {
      ...
      if (this.tables.total_entries > 0 && this.tables.total_entries < 4) {
        this.$el.find(".more_data").show();
      } else {
        this.$el.find(".more_data").hide();
      }
