      ...
      class PubSubTracker
        ...
        def send_event(topic_key, user_id, event, properties = {})
          ...
          topic.publish_async(event, attributes) do |result|
            raise "PubSubTracker: error publishing to topic #{@topic_name} for event #{event}" unless result&.succeeded?
          end

          attributes

        rescue KeyError => e
          CartoDB::Logger.error(message: "Error:  #{topic_key} does not exist")
        rescue StandardError => e
