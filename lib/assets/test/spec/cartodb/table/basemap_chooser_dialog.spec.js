describe("Basemap adder dialog", function() {
  ...
  it("should activate mapbox pane by default and change pane", function() {
    ...
    expect(basemap_dialog.mapboxPane.$el.find("input").attr('placeholder')).toEqual('Insert your MapBox map URL or your MapBox map id');
    basemap_dialog.$el.find(".zxy").click();
    expect(basemap_dialog.basemap_panes.activeTab).toEqual("zxy");
    expect(basemap_dialog.mapboxPane.$el.css('display')).toBe('none');
    expect(basemap_dialog.zxyPane.$el.css('display')).toBe('block');
    expect(basemap_dialog.wmsPane.$el.css('display')).toBe('none');
    expect(basemap_dialog.zxyPane.$el.find("input").attr('placeholder')).toEqual('Insert your TMS URL template');
  ...
  it("should send mapbox url if user provides the mapbox edit url", function() {
    ...
    var val = basemap_dialog.mapboxPane.$el.find("input").val("https://tiles.mapbox.com/saleiva/edit/map-73mv164v#3/0.09/0.00");
    ...
    runs(function() {
      ...
      var layer = basemap_dialog._successChooser.calls[0].args[0];
      expect(layer.get('urlTemplate')).toEqual("http://a.tiles.mapbox.com/v3/saleiva.map-73mv164v/{z}/{x}/{y}.png");
    });
  });
  ...
  it("should send mapbox url if user provides the embed url", function() {
    ...
    var val = basemap_dialog.mapboxPane.$("input").val("http://a.tiles.mapbox.com/v3/saleiva.map-73mv164v/page.html");
  ...
  it("should send mapbox url if user provides the mapbox xyz url", function() {
    ...
    var val = basemap_dialog.mapboxPane.$("input").val("http://d.tiles.mapbox.com/v3/saleiva.map-73mv164v/{z}/{y}/{z}.png");
  ...
  it("should show an error if the mapbox tile doesn't exist", function() {
    spyOn(basemap_dialog, '_errorChooser');
    basemap_dialog.render();

    var val = basemap_dialog.zxyPane.$el.find("input").val("http://a.tiles.mapbox.com/v3/saleiva.map-73mv164v/{z}/{x}/{y}.png/error");
  ...
  xit("should show an error if the wms url template doesn't exist", function() {
    spyOn(basemap_dialog, '_errorChooser');
    basemap_dialog.render();
  ...
  it("should show an error if the xyz tile doesn't exist", function() {
    ...
    var val = basemap_dialog.zxyPane.$el.find("input").val("http://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{Z}/{y}/{X}/error");
