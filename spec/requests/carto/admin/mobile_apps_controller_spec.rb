...
describe Carto::Admin::MobileAppsController do
  ...
  describe '#index' do
    it 'loads apps from Central' do
      ...
      allow_any_instance_of(Cartodb::Central).to receive(:get_mobile_apps).and_return(mobile_apps: [], monthly_users: { open: 10000, private: 0 }).once
      login(@user)
  ...
  describe '#show' do
    it 'loads app from Central' do
      allow(Cartodb::Central).to receive(:sync_data_with_cartodb_central?).and_return(true)
      allow_any_instance_of(Cartodb::Central).to receive(:get_mobile_app).and_return(id: TEST_UUID, monthly_users: 0, app_type: 'open', platform: 'android').once
      login(@user)
  ...
  describe '#create' do
    ...
    it 'creates app in Central' do
      allow(Cartodb::Central).to receive(:sync_data_with_cartodb_central?).and_return(true)
      allow_any_instance_of(Cartodb::Central).to receive(:create_mobile_app).and_return({}).once
      login(@user)
    ...
    it 'validates app before sending to Central' do
      allow(Cartodb::Central).to receive(:sync_data_with_cartodb_central?).and_return(true)
      allow_any_instance_of(Cartodb::Central).to receive(:create_mobile_app).and_return({}).never
      login(@user)
  ...
  describe '#update' do
    ...
    it 'updates app in Central' do
      allow(Cartodb::Central).to receive(:sync_data_with_cartodb_central?).and_return(true)
      allow_any_instance_of(Cartodb::Central).to receive(:get_mobile_app).and_return(MOBILE_APP).once
      allow_any_instance_of(Cartodb::Central).to receive(:update_mobile_app).and_return({}).once
      login(@user)
    ...
    it 'validates app before sending to Central' do
      allow(Cartodb::Central).to receive(:sync_data_with_cartodb_central?).and_return(true)
      allow_any_instance_of(Cartodb::Central).to receive(:get_mobile_app).and_return(id: TEST_UUID, monthly_users: 0, app_type: 'open', platform: 'android').once
      allow_any_instance_of(Cartodb::Central).to receive(:update_mobile_app).and_return(mobile_app: {}).never
      login(@user)
  ...
  describe '#destroy' do
    it 'deletes app in Central' do
      allow(Cartodb::Central).to receive(:sync_data_with_cartodb_central?).and_return(true)
      allow_any_instance_of(Cartodb::Central).to receive(:delete_mobile_app).and_return({}).once
      login(@user)
