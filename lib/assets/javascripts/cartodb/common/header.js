...
cdb.admin.Header = cdb.core.View.extend({
  ...
  _setDescription: function() {
    var default_description = (this.isVisEditable() ? this._TEXTS.description.normal : this._TEXTS.description.query);
  ...
  _setVisualization: function() {
    ...
    if (is_visualization) {
      ...
      var route = ((this.options.user.isInsideOrg() || this.options.user.isOrgAdmin()) ? '/u/' + this.options.user.get('username') : '') + "/dashboard/visualizations";
      ...
      var route = ((this.options.user.isInsideOrg() || this.options.user.isOrgAdmin()) ? '/u/' + this.options.user.get('username') : '') + "/dashboard/tables";
      $back.attr("href", route );
    }
  },
      ...
      $el
        ...
        .tipsy({
          ...
          title:    function() {
            ...
            return _.template(self._TEXTS.rename.readonly)({ mode: mode })
          }
        })
        .tipsy('show')
    }
  ...
  isVisEditable: function() {
    if (this.model.isVisualization()) {
      ...
      if (!table) {
        ...
        return false;
      } else if (table && table.isReadOnly()) {
        return false;
      } else {
        return true;
      }
