...
var randomSentence = require('new_common/view_helpers/random_sentence');
...
module.exports = BaseDialog.extend({
  ...
  initialize: function() {
    ...
    if (!this.options.currentUserUrl) {
      throw new TypeError('currentUserUrl is required');
    }
  ...
  _prepareVisibleAffectedVisForTemplate: function(visibleAffectedVisData) {
    return visibleAffectedVisData.map(function(visData) {
      ...
      return {
        ...
        name: vis.get('name'),
        url: this.options.currentUserUrl.mapUrl(vis).toEdit(),
