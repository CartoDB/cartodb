  ...
  cdb.admin.mod.InfowindowHTMLPane = cdb.admin.mod.InfowindowBasePane.extend({
    ...
    _checkTemplate: function() {
      ...
      // Check if cartodb-popup class is present
      var hasPopupClass = $(html).hasClass('cartodb-popup');
      // If so, add v2 if it is not defined
      if (hasPopupClass) {
        var hasV2Class = $(html).hasClass('v2');
        if (!hasV2Class) {
          this.codeEditor.setValue(html.replace('cartodb-popup', 'cartodb-popup v2'));
          this._apply();
        }
      }
    },
