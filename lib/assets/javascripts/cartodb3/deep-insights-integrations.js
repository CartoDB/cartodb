...
F.prototype._onLayerDefinitionChanged = function (m) {
  if (!m.isNew()) {
    ...
    if (attrs.type) {
      layer.remove();
      this._createLayer(m);
      if (attrs.type === 'torque' || m.previous('type') === 'torque') {
        var widgetModel = this._diDashboard.getWidgets().filter(function (m) {
          return m.get('type') === 'time-series';
        })[0];
        var newLayer = this._getLayer(m);
        if (widgetModel) {
          if (m.previous('type') === 'torque') {
            widgetModel.remove();
          } else if (!widgetModel.get('forced')) {
            this._diDashboard.createTimeSeriesWidget(widgetModel.dataviewModel.attributes, newLayer);
          }
        } else if (attrs.type === 'torque') {
          var baseAttrs = {
            type: 'time-series',
            ...
            options: {
              column: newLayer.get('style_properties').properties.animated.attribute,
              title: 'time_date',
              bins: 256
            }
          };
