...
var StaticHeaderMetaTags = require('../helpers/static_header_meta_tags');
...
var InitPublicMap = function () {
  cdb.init(function () {
    ...
    function dataLoaded () {
      ...
      StaticHeaderMetaTags.addPublicMapMeta(
    ...
    if (window.CartoConfig.data && window.CartoConfig.vizdata) {
      ...
      var getVisualization = function () {
        ...
        var page = MapOptionsHelper.getPublicMapPage();
        var vizUrl = MapOptionsHelper.getVizUrl(page);

        client.getVisualization(vizUrl, params, function (err, response, vizdata) {
          if (err) {
            console.error(err);
            return err;
          } else {
