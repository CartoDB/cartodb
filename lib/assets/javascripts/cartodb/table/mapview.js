  ...
  */
  _bindDataLayer: function(layerView, layer) {
    ...
    if (layerType === 'CartoDB' || layerType === 'torque') { // unbind previos stuff
      ...
      self._addLegends();
      self._addTimeline(layerView, layer);
  ...
  _addLegends: function() {
    ...
    if(!this.map.layers.any(function(lyr) { return lyr.get('type') === 'torque'; })) {
  ...
  _addTimeline: function(layerView, layer) {
      if(layer.get('type') === 'torque') {
        if (this.timeline) this.timeline.clean();
        this.timeline = new cdb.admin.TimeSlider({
          layer: this.mapView.getLayerByCid(layer.cid)
        });
