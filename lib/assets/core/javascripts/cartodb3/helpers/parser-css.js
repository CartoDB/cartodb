...
var CartoParser = function (cartocss, isTorqueLayer) {
  this.parse_env = null;
  this.ruleset = null;
  if (cartocss) {
    this.parse(cartocss, isTorqueLayer);
  }
};
...
CartoParser.prototype = {
  ...
  parse: function (cartocss, isTorqueLayer) {
    var reference = isTorqueLayer ? carto.torque_reference.version.latest : carto.default_reference.version.latest;
    /**
     * using fallback reference when the default reference or torque reference (in case it is a torque layer) throws an error
     * in order to be more restrictive isTorqueLayer argument should be used and use the fallback only for the default reference
     * or just remove the fallback reference alltogether
     */

    try {
      this._parse(cartocss, reference);
    } catch(e) {
      this._parse(cartocss, carto.fallback_reference.version.latest, true);
    }
    return this;
  },

  _parse: function (cartocss, reference, isFallbackReference) {
    this.parse_env = {
    ...
    if (ruleset) {
      ...
      symbolizers = carto.torque_reference.version.latest.layer;
    ...
    if (!isFallbackReference && this.parse_env.errors.length > 0) {
      throw new Error('There was an error parsing cartocss');
    }
