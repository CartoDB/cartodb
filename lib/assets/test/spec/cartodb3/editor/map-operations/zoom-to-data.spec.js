...
describe('map-operations/zoom-to-data', function () {
  ...
  it('should show notifications and call stateModel.setBounds with the bounds response', function () {
    ...
    notifications = Notifier.getCollection();
    notification = Notifier.getCollection().models[0];
    assertNotification(notification, 'loading', '- SOME TEXT -', false);

    // Respond to call
    request = jasmine.Ajax.requests.mostRecent().respondWith(responses.success);
    jasmine.clock().tick(10);

    expect(result[0][1]).toBe(bounds.rows[0].maxx);
    assertNotification(notification, 'success', '- SOME TEXT -', true);
  });
  ...
  it('should show error notifications if call fails', function () {
    ...
    request = jasmine.Ajax.requests.mostRecent().respondWith(responses.error);
    jasmine.clock().tick(10);

    assertNotification(notification, 'error', '- SOME TEXT -', true);
  });
