  ...
  _initViews: function () {
    ...
    function completeAfter(cm, pred) {
      ...
      if (!pred || pred()) setTimeout(function() {
        if (!cm.state.completionActive && str.length > autocompleteChars) {
    ...
    function completeIfAfterCtrlSpace (cm) {
      return completeAfter(cm, function() {
        ...
        // if (!cm.state.completionActive && str.length > autocompleteChars && (listHints.length > 0 || listTriggers.length > 0)) {

        return cm.getRange(CodeMirror.Pos(cur.line, cur.ch - 2), cur) === '{{';
      });
    }
    ...
    if (NOHINT.indexOf(code) === -1) {
      ...
      this._autocompleteTimeout = setTimeout(function () {
        ...
        if (str.length > autocompleteChars) {
          ...
          if (!cm.state.completionActive && (listHints.length > 0)) {
            self._showAutocomplete(cm);
          }
        }
      }, 150);
