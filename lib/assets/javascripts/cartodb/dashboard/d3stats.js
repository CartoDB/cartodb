...
cdb.admin.D3Stats = cdb.core.View.extend({
  ...
  initialize: function() {
    // Get data
    this.requests = this.options.api_calls || [1, 4, 10, 20, 5, 2, 4, 5];
  ...
  render: function() {
    ...
    // Put total api calls
    this.$el.find("p").html(
      "<strong>" + _.reduce(this.requests, function(memo, num){ return memo + num; }, 0) + "</strong> map views the last " + _.size(this.requests) + " days"
    )
