...
cdb.admin.mod.CartoCSSWizard = cdb.core.View.extend({
  ...
  _resetNavigation: function() {
    ...
    if (selectedIndex >= 3) {
      ...
      var move = $ul.find("a.selected").parent().outerWidth();
      $ul.animate({ left: -move + 'px' }, 200);

      $left.removeClass("disabled");
      $right.addClass("disabled");

      self.position  = selectedIndex - 2;
      self.animation = false;

      return;
    }

    $ul.animate({ left: 0 }, { duration: 0, complete: function() {
      ...
      // Check arrows
      if (self.position + gap >= list_size) {
        $right.addClass("disabled");
      } else {
  ...
  _moveNavigation: function(side) {
    ...
    var
    $ul           = this.$el.find("ul.vis_options")
    , gap         = 3
    , list_size   = $ul.find("li").size()
  ...
  enableTabs: function() {
    var _enableMap = {
      'point': ['polygon', 'bubble', 'density', 'intensity'],
