...
module.exports = Backbone.Collection.extend({
  ...
  createLayerForAnalysisNode: function (nodeId, newLayerOrder) {
    ...
    if (layerWithNodeAsHead) {
      ...
      layerWithNodeAsHead.set({
        order: newOrderForLayerWithNodeAsHead,
        letter: tmpLetter,
        source: nodeDefModel.clone(tmpLetter + '1').id
      }, {
        silent: true // saved after layer's creation save below
      });

      newNodeDefModel = nodeDefModel.getPrimarySource();
    } else {
      newNodeDefModel = nodeDefModel.cloneTree(nextLetter);
    }

    var attrs = this._createDefaultCartoDBAttrs();
    attrs.order = newLayerOrder;
    attrs.options = _.extend({}, attrs.options, {
      table_name: 'paradas_metro_madrid', // TODO temporary workaround, a table_name is required to create layer
      letter: nextLetter,
    ...
    // Finally save the new layer and reset orders afterwards
