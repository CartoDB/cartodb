...
/**
 *  Deprecated color wizard generator
 */
 ...
 */
function category_generator(table, props, callback) {
  var type = table.geomColumnTypes() && table.geomColumnTypes()[0] || "polygon";
  var table_name = table.get('name');
  ...
  for (var i in props) {
    if (i!="colors" && i!="property" && i!="categories") {
      css +=  "   " + i + ": " + props[i] + ";\n";
    }
  }
  ...
  // It could happen when user deletes the column
  // applied
  if (props.categories && table.containsColumn(props.property)) {
    // type of the column, number -> no quotes, string -> quotes, boolean -> no quotes
    for (var i in props.categories) {
      // Get fill property
      var fill = 'polygon-fill';
      switch (type) {
        case 'polygon': fill = (props.categories[i].value_type == "file") ? 'polygon-pattern-file' : 'polygon-fill'; break;
        case 'line':    fill = (props.categories[i].value_type == "file") ? 'line-pattern-file' : 'line-color'; break;
        default:        fill = (props.categories[i].value_type == "file") ? 'marker-file' : 'marker-fill';
      }

      // Generate custom category
      if (props.categories[i].default) {
        // Default category
        css += "\n#" + table_name + " {\n   " + fill + ": " + props.categories[i].value + ";\n}";
      } else {
        // Set correct title type
        var title = '';
        if (props.categories[i].title_type != "string" || props.categories[i].title === null) {
          title = props.categories[i].title;
        } else {
          title = "\"" + props.categories[i].title.replace(/\n/g,'\\n') + "\""; // Replace linebreaks in values for \n
        }

        // Custom category
        css += "\n#" + table_name + "[" + props.property + "=" + title + "] {\n   " + fill + ": " + props.categories[i][props.categories[i].value_type] + ";\n";
