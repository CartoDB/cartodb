...
describe('dataview/base', function () {
  ...
  it('.setColumn should throw an error if the argument is not string or undefined', function () {
    expect(function () { base.setColumn(); }).toThrow(new TypeError('Column property is required.'));
    expect(function () { base.setColumn(12); }).toThrow(new TypeError('Column property must be a string.'));
    expect(function () { base.setColumn(''); }).toThrow(new TypeError('Column property must be not empty.'));
  });
  ...
  it('.getData should not be defined in the base dataview', function () {
    expect(function () { base.getData(); }).toThrow(new Error('getData must be implemented by the particular dataview.'));
  });
});
