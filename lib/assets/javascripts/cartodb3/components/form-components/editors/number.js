...
Backbone.Form.editors.Number = Backbone.Form.editors.Base.extend({
  ...
  initialize: function (opts) {
    ...
    if (this.validators) {
      this.options.min = this.validators[0].min;
      this.options.max = this.validators[0].max;
      this.options.step = this.validators[0].step;
    }
  ...
  _onInputKeyUp: function () {
    ...
    if (!isFloatNotCompleted.test(value) && isDifferent && this._hasSlider()) {
