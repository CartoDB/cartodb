...
var LockItemsDialog = require('new_dashboard/dialogs/lock_items_view');
var DeleteItemsDialog = require('new_dashboard/dialogs/delete_items/view');
var ConnectDataset = require('new_common/dialogs/connect_dataset/connect_dataset_view');
...
module.exports = cdb.core.View.extend({
  ...
  events: {
    ...
    'click .js-lock':           '_openLockItemsDialog',
    ...
    this.$('.Filters-inner').html(
      this.template(
        _.extend({
            ...
            pluralizedContentType: pluralizeString(this.router.model.get('content_type') === "datasets" ? 'dataset' : 'map', changedContentType ? 0 : this.collection.total_entries),
  ...
  _openLockItemsDialog: function(e) {
    this.killEvent(e);
    var lockDialog = new LockItemsDialog({
      items: this._selectedItems(),
      contentType: this.router.model.get('content_type')
    });
    lockDialog.bind('done', function() {
      this.collection.fetch();
    }, this);
    lockDialog.appendToBody();
  },
  ...
  _connectDataset: function(e) {
    ...
    if (this.user.canCreateDatasets()) {
      var connectDataset = new ConnectDataset({
        user: this.user
      });
      
      connectDataset.bind('done', function() {
        console.log("show background importer");
      }, this);
      connectDataset.appendToBody()
    }
  },
