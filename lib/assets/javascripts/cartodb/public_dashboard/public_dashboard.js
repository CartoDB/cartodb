  ...
  $(function() {
    ...
    var PublicDashboard = cdb.core.View.extend({
      ...
      events: {
        'click .visualizations .edit':  '_onVisEditClick',
        'click .map-wrapper':           '_onClickBackdrop',
        'click .datasets .edit':        '_onDatasetEditClick',
      ...
      _onDatasetEditClick: function(e) {
        if (e) this.killEvent(e);
        var $link = $(e.target).closest('.title').find('h2 a');
        var location = $link.attr('href');
        window.location = location.replace('/public', '');
      },

      _onVisEditClick: function(e) {
        if (e) this.killEvent(e);
        var $link = $(e.target).closest('.map-wrapper').find('.title a');
        var location = $link.attr('href');
        window.location = location.replace('/public_map', '');
      },
