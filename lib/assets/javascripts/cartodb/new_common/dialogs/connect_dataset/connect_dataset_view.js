var cdb = require('cartodb.js');
var BaseDialog = require('new_common/views/base_dialog/view');
var pluralizeString = require('new_common/view_helpers/pluralize_string');
var ConnectDatasetModel = require('new_common/dialogs/connect_dataset/connect_dataset_model');
var ConnectDatasetDefaults = require('new_common/dialogs/connect_dataset/connect_dataset_defaults');
var ConnectDatasetContent = require('new_common/dialogs/connect_dataset/connect_dataset_content');

/**
 *  Connect dataset dialog
 */

var View = BaseDialog.extend({

  className: 'Dialog ConnectDialog',

  // Default options for connect dataset dialog
  options: ConnectDatasetDefaults,
    
  initialize: function() {
    this.elder('initialize');
    this.user = this.options.user;
    this.template = cdb.templates.getTemplate('new_common/views/connect_dataset/dialog_template');

    this.model = new ConnectDatasetModel({
      option: this.options.selected_option
    });

    console.log(this.model.toJSON());
    console.log(this.options);

    // TODO: check external data
    // TODO: check window size at the beginning

    // var self = this;
    // setTimeout(function() {
    //   self._onWindowResize();
    // }, 10);
  },

  render_content: function() {
    var $el = this.template();

    var connectDatasetContent = new ConnectDatasetContent({

    })

    // Options tabs
    // Dialog body -> Options content
    // Dialog footer

    return $el;
  },

  _initBinds: function() {
    $(window).on('resize', this._onWindowResize);
  },

  _disableBinds: function() {
    $(window).off('resize', this._onWindowResize);
  },

  _onWindowResize: function() {
    // Make content body min height enough
    this.$('.ConnectDialog-body').css('min-height', $(window).height() - 169);
  },

  clean: function() {
    this._disableBinds();
    this.elder('clean');
  }

});

module.exports = View;
