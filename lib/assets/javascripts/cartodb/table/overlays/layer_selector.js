cdb.admin.overlays.LayerSelector = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    // Re-render the control if we ever change the sync model, since our prettify method
    // depends on the complete sync state existing
    _.each(this.mapView.map.layers.models, function(layer) {
      if (layer.get("type") === "CartoDB" || layer.get('type') === 'torque') {
        layer.table.bind('change:synchronization', self.render, self);
      }
    });
  },
