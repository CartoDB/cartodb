...
cdb.admin.HeaderDropdown = cdb.admin.DropdownMenu.extend({
  ...
  filterColumn: function(e) {
    ...
    if (column_type == 'boolean' || column_type == 'string' || column_type == 'number') this._addFilter(this.column);
    else this._showFilterDialog();

  },

  _addFilter: function(column_name) {
    table.menu.show("filters_mod")
    table.menu.filters.filters.add({ column: column_name });
  },
