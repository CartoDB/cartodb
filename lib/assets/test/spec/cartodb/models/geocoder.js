...
describe("cbd.admin.CartoStyles", function() {
  ...
  it("should get the number of rows to georefernece", function()  {
    var called = false;
    this.server.respondWith('/api/v1/queries/?sql=SELECT%20count(cartodb_id)%20from%20test%20where%20cartodb_georef_status%20is%20NULL',
  ...
  it("should feed the queue", function() {
    ...
    this.server.respondWith('/api/v1/queries/?sql=SELECT%20*%20from%20test%20where%20(cartodb_georef_status%20is%20%20NULL)%20AND%20cartodb_id%20%3E%200%20order%20by%20cartodb_id%20limit%20100%20',
  ...
  it("should feed the queue again if there not more rows to proccess, updating the lastCartoDbId on the request", function() {
    ...
    this.server.respondWith('/api/v1/queries/?sql=SELECT%20*%20from%20test%20where%20(cartodb_georef_status%20is%20%20NULL)%20AND%20cartodb_id%20%3E%207%20order%20by%20cartodb_id%20limit%20100%20',
      TestUtil.response('{"time":0.0028295516967773438,"total_rows":0,"rows":[],"results":true,"modified":false}'));
    this.geocoder._feedQueue();
