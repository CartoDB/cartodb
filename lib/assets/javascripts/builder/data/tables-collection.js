...
module.exports = Backbone.Collection.extend({
  ...
  parse: function (res) {
    ...
    if (visualizations.length && visualizations[0].table) {
      return this._parseVisualizationTable(visualizations);
    } else {
      return this._parseTable(visualizations);
    }
  },

  _parseVisualizationTable: function (visualizations) {
  ...
  _parseTable: function (visualizations) {
    return _.map(visualizations, function (viz) {
      return {
        id: viz.id,
        name: viz.name
