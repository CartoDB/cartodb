...
cdb.admin.Visualization = cdb.core.Model.extend({
  ...
  urlRoot: function() {
    var version = cdb.config.urlVersion('visualization');
    return '/api/' + version + '/viz';
  },
  ...
  overlaysUrl: function() {
    var version = cdb.config.urlVersion('overlays');
    return '/api/' + version + '/viz/' + "/" + this.id + "/overlays";
  },
  ...
  initialize: function() {
    ...
    this.overlays.url = this.overlaysUrl();
  ...
  vizjsonURL: function() {
    var url = this.permission.owner.viewUrl();
    var version = cdb.config.urlVersion('vizjson', 'v2');
  ...
  viewUrl: function(currentUser) {
    ...
    if (this.isVisualization() || _.isUndefined(this.get('type'))) {
      return new cdb.common.MapUrl({
        base_url: userUrl.urlToPath('viz', this.get('id'))
      });
    } else {
      if (currentUser) {
...
cdb.admin.Visualizations = Backbone.Collection.extend({
  ...
  initialize: function() {
    ...
    //TODO remove then then flag is removed
    var user = cdb.config.get('user')
    if (user && user.featureEnabled('cached_dashboard')) {
      this.sync = Backbone.cachedSync('vis-' + window.user_data.username)
    }
  ...
  url: function() {
    var version = cdb.config.urlVersion('visualizations');
    var u = '/api/' + version + '/viz/';
