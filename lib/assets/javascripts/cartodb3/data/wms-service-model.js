...
module.exports = Backbone.Model.extend({
  ...
  initialize: function (attrs, opts) {
    if (opts && opts.layersCollection) {
      this.layersCollection = opts.layersCollection;
    }
  },
  ...
  parse: function (r) {
    ...
    this.layersCollection.reset(r.layers);
    this.layersCollection.each(function (model) {
      model.set('type', self.get('type')); // wms/wmts
    });
  ...
  url: function (method) {
    ...
    if (method === 'create') {
      if (this.get('layer') && this.get('srs')) {
        ...
        req += '&layer=' + this.get('layer');
        req += '&matrix_set=' + this.supportedMatrixSets(this.get('matrix_sets' || []))[0];
      }
    }
  ...
  newCustomBaselayerModel: function () {
    if (!this.get('mapproxy_id')) {
      throw new Error('mapproxy_id must be set');
    }

    var url = this._PROXY_TILES + '/' + this.get('mapproxy_id') + '/wmts/map/webmercator/{z}/{x}/{y}.png';
