...
describe('deep-insights-integrations', function () {
  ...
  describe('when analysis-definition-node is created', function () {
    beforeEach(function () {
      ...
      this.nodeDefModel.analysisTableModel = jasmine.createSpyObj('analysisTableModel', ['set', 'listenTo']);
    });
    ...
    describe('when an analysis-definition is added', function () {
      ...
      it('should setup query schema model of node-definition', function () {
        expect(this.nodeDefModel.analysisTableModel.set).toHaveBeenCalled();
        expect(this.nodeDefModel.analysisTableModel.set).toHaveBeenCalledWith({
          query: 'SELECT * FROM foobar'
        });
      });
