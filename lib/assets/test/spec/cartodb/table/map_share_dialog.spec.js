describe("ShareDialog", function() {
  ...
  describe("dialog", function() {
    ...
    beforeEach(function() {
      ...
      map.layers.add(new cdb.geo.CartoDBLayer({cartodb_logo: 'false', query: 'select * from rambo', tile_style: 'style', table_name: 'test', user_name: 'test'} ));
    ...
    it("should render a map with the CartoDB logo", function() {
      ...
      runs(function(){
        expect(view.mapView.$('div.cartodb-logo').length > 0).toEqual(true);
      });
    });

    it("should render a map with controls", function() {
      view.render().show();
      expect(view.$('.cartodb-zoom').length).toEqual(1);
      expect(view.$('div.cartodb-header h1').html()).toEqual('test');
      expect(view.$('div.cartodb-header p').html()).toEqual('test description');
      expect(view.$('.form_switch').length).toEqual(7); // With cartodb_logo there are 7
    });
