...
module.exports = CoreView.extend({
  ...
  _initTipsy: function () {
    ...
    if (options.gravity === 'auto') {
      options.gravity = this.getAutoGravityPosition(0, ['s', 'e', 'w', 'n'], window);
    }
  ...
  getAutoGravityPosition: function (margin, preferredGravityValues, browserWindow) {
    return function (tooltipElement) {
      if (!tooltipElement) {
        return preferredGravityValues[0] || 'n';
      }
      ...
      var optimalGravity = _.find(preferredGravityValues, function (gravityValue) {
        var position = gravityOptions[gravityValue];
      ...
      return optimalGravity ? optimalGravity + tooltipBodyGravity : preferredGravityValues[0];
    };
  },
