describe("mapview", function() {
  ...
  it("should trigger the no-geo warning when there is data without geom info and it is in read-only mode", function() {
    view.table = TestUtil.createTable('test', [['the_geom', 'geometry'], ['name','string']], [], '12345');
    spyOn(view.table, 'isReadOnly').and.returnValue(true);
  ...
  it('should render attributions of each layer and default CartoDB attribution', function() {
    ...
    expect(attributions).toEqual('attribution1, attribution2, CartoDB <a href="http://cartodb.com/attributions" target="_blank">attribution</a>');
  });
