...
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.template = cdb.templates.getTemplate('common/dialogs/create/listing/templated_workflows/templated_workflows_form');
    this._initBinds();
  },
  ...
  render: function() {
    ...
    this.$el.html(
      this.template({
        description: this.options.form_description || ''
      })
    );
  ...
  _initBinds: function() {
    ...
    this.collection.bind('error valid', this._checkFormError, this)
  },
  ...
  _checkFormError: function() {
    var error = this.collection.getError();
    this.$('.js-formErrorText').text(error);
    this.$('.js-formError').toggle(error !== '');
  },
