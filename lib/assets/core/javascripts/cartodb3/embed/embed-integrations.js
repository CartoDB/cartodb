...
var StylesCollection = require('./style-collection');
...
F.prototype._onWidgetAutoStyleChanged = function (m) {
  ...
  if (isAutoStyleApplied) {
    style = m.getAutoStyle();
    LegendManager.updateLegends(layerId, style);
  } else {
    if (!isAnyAutoStyleApplied) {
      layer = this.visMap().getLayerById(layerId);
      layerStyle = this._layerStyleCollection.findById(layerId);
      layer && layer.set({cartocss: layerStyle.get('cartocss')});
      LegendManager.resetLegends(layerId, layerStyle.get('style'));
    }
  }
};
