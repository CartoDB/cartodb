...
cdb.admin.MapOverlays = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.vertical_overlays   = ["zoom", "fullscreen", "loader", "inset_map"];
  ...
  _bindInsetMapOverlay: function (overlay) {
    ...
    overlay.on('reposition', this._positionOverlaysHorizontally, this);

    var onDestroy = function() {
      if (self.inset_map) {
        self.inset_map.off('reposition', self._positionOverlaysHorizontally);
  ...
  _positionInsetMapOverlay: function(overlay_model) {
    ...
    overlay_model.set("x", x - horizontalPadding);
    overlay_model.set("y", y);
  }
