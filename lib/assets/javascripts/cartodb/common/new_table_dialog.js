...
cdb.admin.CreateTableDialog = cdb.ui.common.Dialog.extend({
  ...
  */
  ...
  events: {
    'click .ok'                     : 'ok',
    'mouseleave .switch'            : '_hideTipsy',
    'mouseenter .switch'            : '_showTipsy',
    'click .privacy label'          : '_switchPrivacy',
    'click .privacy .switch'        : '_switchPrivacy',
  ...
  initialize: function() {
    ...
    this.model = new cdb.core.Model({
      private: self.options.private_tables
    });
    ...
    _.extend(this.options, {
      ...
      ok_title_empty: "Create empty table",
      modal_type: "creation",
      width: 600,
      error_messages: {
  ...
  _hideTipsy: function() {
    if (this.options.private_tables) return;

    var self = this;
    this.tipsyTimeout = setTimeout(function() {
      self.$el.find(".switch").tipsy("hide");
    }, 500);
  ...
  _showTipsy: function() {
    ...
    clearTimeout(this.tipsyTimeout);
    this.$el.find(".switch").tipsy("show");
  },
  ...
  render_content: function() {
    ...
    $(document).on("mouseenter", ".tipsy", function() {
      clearTimeout(self.t);
    });

    $(document).on("mouseleave", ".tipsy", function() {
      self._hideTipsy()
    });
  ...
  _hideUploader: function() { console.log(this.$holder); this.$holder.hide() },
  ...
  _switchPrivacy: function(ev) {
    ev.preventDefault();
    ev.stopPropagation();

    if (!this.options.private_tables) return;

    this.model.set("private", !this.model.get("private"));

    var enabled = this.model.get('private');
  ...
  _changeOption: function(ev) {
    ...
    if (this.option == 2) this.$el.find(".button.ok").text(this.options.ok_title_empty);
    else this.$el.find(".button.ok").text(this.options.ok_title);
