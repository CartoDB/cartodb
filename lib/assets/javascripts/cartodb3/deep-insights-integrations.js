...
var F = function (opts) {
  ...
  opts.layerDefinitionsCollection.each(function (layerDefModel) {
    ...
    this._checkStyle(layerDefModel);
  }, this);
...
F.prototype._checkStyle = function (layerDefModel) {
  var nodeDefModel = layerDefModel.getAnalysisDefinitionNodeModel();
  var nodeModel = this._analysis().findNodeById(layerDefModel.get('source'));
  var isAnalysisNode = nodeModel && nodeModel.get('type') !== 'source';
  var isRunning = nodeModel && nodeModel.get('status') !== 'ready';
  var styleType = layerDefModel.styleModel && layerDefModel.styleModel.get('type');

  if (isAnalysisNode && styleType === 'none' && !isRunning) {
    var self = this;
    var simpleGeom = nodeDefModel.querySchemaModel.get('simple_geom');

    var applyDefaultStyles = function () {
      simpleGeom = nodeDefModel.querySchemaModel.get('simple_geom');
      layerDefModel.styleModel.setDefaultPropertiesByType('simple', simpleGeom);
      nodeDefModel.querySchemaModel.unbind(null, null, self);
    };

    if (!simpleGeom) {
      nodeDefModel.querySchemaModel.bind('change:simple_geom', applyDefaultStyles, this);
    } else {
      applyDefaultStyles();
    }
  }
