...
cdb.admin.Permission = cdb.core.Model.extend({
  ...
  _genetateAcl: function() {
    ...
    if (this.get('acl')) {
      ...
      for (var i = 0; i < acl.length; ++i) {
        ...
        } else if (aclitem.type === 'org') {
          var org = new cdb.admin.Organization(aclitem.entity);
          this.setPermission(org, aclitem.access);
        } else if (aclitem.type === 'group') {
          var group = new cdb.admin.Group(aclitem.entity);
          this.setPermission(group, aclitem.access);
        } else {
  ...
  _createACLItem: function(u, access) {
    ...
    } else if (u instanceof cdb.admin.Organization) {
      return new cdb.admin.ACLItem({ type: 'org', entity: u, access: access});
    } else if (u instanceof cdb.admin.Group) {
      return new cdb.admin.ACLItem({ type: 'group', entity: u, access: access});
    } else {
      throw new Error("Unknown entity type: " + u);
    }
  },
  ...
  getPermission: function(user) {
    ...
    if (aclItem) {
      ...
      if (orgAcl) {
        ...
        var groupAcl = _.find(this.acl.where({
          type: 'group'
        }), function(group) {
          return user.groups && user.groups.map(function(g) { return g.id }).indexOf(group.get('entity').id) != -1;
        });

        if (groupAcl) {
          return groupAcl.get('access');
        }
      }
    }
    return null;
