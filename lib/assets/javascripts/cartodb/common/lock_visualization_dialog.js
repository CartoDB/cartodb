  ...
  cdb.admin.LockVisualizationDialog = cdb.admin.BaseDialog.extend({
    ...
    _TEXTS: {
      ...
      // },
      // description: {
      //   lock:   _t("Locking your <strong><%= name %></strong> <%= type %> will hide it from your <%= type %>s dashboard, but it will be \
      //             visible clicking over the link at the bottom of that section. \
      //             <% if (sharedPeople > 0 && type === 'table') { %><br/><br/>Also, people with write permission over this <%= type %> \
      //             will still be able to edit it.<% } %>"),
    ...
    initialize: function() {
      ...
      // var belongsOrg = this.options.user.isInsideOrg();
      // var sharedPeople = this.model.permission.acl.size();

      // Extend options
      _.extend(this.options, {
        // title:              _.template(this._TEXTS.title[ !isLock ? 'lock' : 'unlock' ])({ name: name, type: type }),
        // description:        _.template(this._TEXTS.description[ !isLock ? 'lock' : 'unlock' ])({ name: name, type: type, sharedPeople: sharedPeople }),
        width:              500,
        clean_on_hide:      true,
