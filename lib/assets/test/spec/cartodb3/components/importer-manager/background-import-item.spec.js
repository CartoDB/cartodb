...
describe('editor/components/importer-manager/background-import-item', function () {
  beforeEach(function () {
    ...
    spyOn(BackgroundImportItemView.prototype, 'updateNotification');
    spyOn(BackgroundImportItemView.prototype, 'clean');
  ...
  it('should show display_name when it is available from import model', function () {
    ...
    expect(this.importerView._getInfo()).toContain('hello-id');
    this.model._importModel.set('display_name', 'table_name_test');
    this.model.set('state', 'geocoding');
    expect(this.importerView._getInfo()).toContain('table_name_test');
    expect(this.importerView._getInfo()).not.toContain('hello-id');
  });
});
