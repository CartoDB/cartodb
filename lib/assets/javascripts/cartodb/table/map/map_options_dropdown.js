cdb.admin.MapOptionsDropdown = cdb.ui.common.Dropdown.extend({
  ...
  _setupOptions: function() {
    ...
    _.each(overlays, function(overlay) {
      ...
      }  else if (overlay.get("type") == "search") {

        var display = overlay.get("display");
        self.mapOptions.set({ search: display });

        if (display) self.$el.find(".search").removeClass("disabled");
  ...
  _onSwitchSwitched: function(property, value) {
    ...
    if (property == "search") {
      ...
      var overlays   = this.collection.filter(function(m) { return m.get("type") == "search" });
