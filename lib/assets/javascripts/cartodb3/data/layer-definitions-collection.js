...
module.exports = Backbone.Collection.extend({
  ...
  _colors: function (otherColors) {
    var colorsFromAddedModels = _.compact(this.pluck('color'));

    // When adding multiple items the models created so far are stored in the internal object this._byId,
    // need to make sure to take them into account when returning already taken colors.
    var colorsFromModelsNotYetAdded = _.chain(this._byId).values().invoke('get', 'color').value();

    return _.union(colorsFromAddedModels, colorsFromModelsNotYetAdded, otherColors);
  },
