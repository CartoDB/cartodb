...
(function() {
    ...
    cdb.admin.TableView = cdb.ui.common.Table.extend({
      ...
      initializeBindings: function() {
        ...
        // Geocoder binding
        this.options.geocoder.bind('geocodingComplete geocodingError geocodingCanceled', this.render, this);
      ...
      headerView: function(column) {
        ...
        if(column[1] !== 'header') {
          var v = new cdb.admin.HeaderView({
            ...
            var dlg;
            if (!self.options.geocoder.isGeocoding()) {
              dlg = new cdb.admin.GeoreferenceDialog({
                table: self.model,
                user: self.user,
                option: 0, /** geocode using a column */
