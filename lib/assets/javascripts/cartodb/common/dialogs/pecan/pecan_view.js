...
module.exports = BaseDialog.extend({
  ...
  _analyzeStats: function(name, stats) {
    ...
    if (stats.type === 'date' && stats.null_ratio > 0.75) {
      console.log("%c"+name + ' rejected because the null ratio > 75%: ' + stats.null_ratio * 100, "font-weight: bold; color:red;");
      return false;
    ...
    if (distinctPercentage < 5 || distinctPercentage > 90) {
      console.log("%c" + name + ' rejected because the distinctPercentage > 90% || distinctPercentage < 5%: ' + distinctPercentage, "font-weight: bold; color:red;");
      return false;
    }
  ...
  _addCard: function(url, response) {
    ...
    var null_count = response.stats.null_ratio * response.stats.null_count;
