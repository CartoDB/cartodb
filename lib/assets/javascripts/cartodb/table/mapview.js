...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _showGMapsDeprecationDialog: function() {
    ...
    if (this.user.featureEnabled('new_modals')) {
      dialog = cdb.editor.ViewFactory.createDialogByTemplate('common/dialogs/confirm_gmaps_basemap_to_leaflet_conversion');
    } else {
      dialog = new cdb.admin.BaseDialog({
        width: 600,
  ...
  _exportImage: function() {
    ...
    if (this.user.featureEnabled('new_modals')) {
      ...
      this.exportImageView.bind("finish", function(response) {
        this.exportImageView = null;
    ...
    if (!this.table.isReadOnly()) {
      ...
      var remove_confirmation = new cdb.admin.BaseDialog({
        title: "Delete feature",
          descriptionSafeHtml: "Are you sure you want to delete this feature and all its associated data?",
