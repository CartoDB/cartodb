...
var TVTDefaults = require('../editor/style/style-defaults/tvt-style-defaults');
  ...
  return {
    ...
    saveAnalysis: function (analysisFormModel) {
      ...
      if (nodeDefModel) {
        ...
        // ugliest hack
        if (analysisFormModel.get('type') === 'tt-aggregate-intersection') {
          layerDefModel.save({
    ...
    createLayerFromTable: function (tableModel, options) {
      ...
      if (tableGeometry) {
        // var styleType = (Utils.isTVTTable(tableName) && userModel.featureEnabled('dot_properties')) ? 'tvt' : 'simple';
        var styleType = 'simple';
        var styleDefaults = SimpleStyleDefaults;

        if (Utils.isTVTTable(tableName)) {
          styleType = 'tvt';
          styleDefaults = TVTDefaults;
          attrs.options.tile_style = camshaftReference.getDefaultCartoCSSForType(styleType);
          attrs.options.cartocss = camshaftReference.getDefaultCartoCSSForType(styleType);
        }

        attrs.options.style_properties = {
          type: styleType,
          properties: styleDefaults.generateAttributes(tableGeometry)
        };
      }
