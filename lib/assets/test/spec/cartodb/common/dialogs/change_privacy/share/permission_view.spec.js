...
describe('common/dialogs/change_privacy/share/permission_view', function() {
  beforeEach(function() {
    ...
    spyOn(this.permission, 'grantAccess').and.callThrough();
  ...
  describe('when can change write access', function() {
    ...
    describe('when click write toggle', function() {
      ...
      it('should toggle write access for each click', function() {
        expect(this.permission.grantAccess.calls.argsFor(0)).toEqual([ this.model, cdb.admin.Permission.READ_WRITE ]);
        expect(this.permission.grantAccess.calls.argsFor(1)).toEqual([ this.model, cdb.admin.Permission.READ_ONLY ]);
        expect(this.permission.grantAccess.calls.argsFor(2)).toEqual([ this.model, cdb.admin.Permission.READ_WRITE ]);
      });
    });
  });
  ...
  describe('when cannot change write access', function() {
    ...
    it('should not render write toggler', function() {
      ...
      expect(this.permission.grantAccess).not.toHaveBeenCalled();
    });
  });
  ...
  describe('when click read toggle', function() {
    ...
    it('should toggle read access', function() {
      expect(this.permission.grantAccess.calls.argsFor(0)).toEqual([ this.model, cdb.admin.Permission.READ_ONLY ]);
      expect(this.permission.revokeAccess.calls.argsFor(0)).toEqual([ this.model ]);
      expect(this.permission.grantAccess.calls.argsFor(1)).toEqual([ this.model, cdb.admin.Permission.READ_ONLY ]);
    });
  });
