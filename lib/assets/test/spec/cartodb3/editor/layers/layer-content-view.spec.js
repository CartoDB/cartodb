...
var LayerContentView = require('cartodb3/editor/layers/layer-content-view');
...
describe('editor/layers/layer-content-view/layer-content-view', function () {
  ...
  describe('._addContextButtonsTooltips', function () {
    var isLayerEmptySpy;
    var isLayerEmptyTooltip;
    ...
    it('should not add a disabledButtonTooltip if isLayerEmpty is false', function () {
      isLayerEmptySpy = spyOn(view, '_isLayerEmpty').and.returnValue(true);
      view._addContextButtonsTooltips();

      expect(isLayerEmptySpy).toHaveBeenCalled();
      expect(_.size(view._subviews)).toBe(4); // [Header, Placeholder, Geometry Toolips]
    });

    it('should add a disabledButtonTooltip if isLayerEmpty is true', function () {
      isLayerEmptySpy = spyOn(view, '_isLayerEmpty').and.returnValue(false);
      view._addContextButtonsTooltips();

      expect(_.size(view._subviews)).toBe(6); // [Header, Placeholder, Disabled Button Tooltip, Geometry Toolips]
    });
  });

  describe('._isLayerEmpty', function () {
    var analysisDefNodeModel;
    var layerDefinitionModel;
    ...
    describe('should return true', function () {
      it('when _layerDefinitionModel is not visible', function () {
        analysisDefNodeModel.isReadOnly.and.returnValue(false);
        layerDefinitionModel.set('visible', false);
      ...
      it('when analysisDefinitionNodeModel is read only', function () {
        analysisDefNodeModel.isReadOnly.and.returnValue(true);
        layerDefinitionModel.set('visible', true);
  ...
  describe('._isDataEmpty', function () {
    ...
    beforeEach(function () {
      ...
      isDataEmptySpy = spyOn(view.model, 'isDataEmpty');
    });

    it('should be able to check if there is data', function () {
      isDataEmptySpy.and.returnValue(false);

      expect(view._isDataEmpty()).toBe(false);
    });

    it('should be able to check if there is no data', function () {
      isDataEmptySpy.and.returnValue(true);

      expect(view._isDataEmpty()).toBe(true);
    });
  });
