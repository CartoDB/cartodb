...
var DISABLED_TABS = ['style', 'popups', 'legends'];
...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    this._layerDefinitionModel.fetchQueryRowsIfRequired();
  ...
  render: function () {
    ...
    var fetching = this._getQueryRowsCollection().statusModel.get('status') === 'fetching';
    console.log(this._getQueryRowsCollection().statusModel.get('status'));

    if (fetching) {
      this._renderPlaceholder();
      console.log('_renderPlaceholder');
    } else {
      this._renderLayerTabPaneView();
      console.log('_renderLayerTabPaneView');
    }
  ...
  _initBinds: function () {
    ...
    this.listenTo(querySchemaModel, 'change:query', this._renderContextButtons);
      ...
      if (this._editorModel.isEditing()) {
        this.listenToOnce(querySchemaModel, 'sync', function (e) {
          ...
          queryRowsCollection.fetch({
            success: function () {
              self.options.selectedTabItem = 'data';
              self.render();
            }
          });
        });
    ...
    this.listenTo(queryRowsCollection.statusModel, 'change:status', this.render);

    this.listenTo(this._analysisDefinitionNodesCollection, 'remove', function (model) {
      if (model.get('type').indexOf('georeference') === 0) {
        var oldParent = this._analysisDefinitionNodesCollection.find({ id: model.get('source') });
        if (oldParent.get('type') === 'source') {
          this.listenToOnce(this._layerDefinitionModel, 'change:source', function () {
            // Immediately force another georeference analyisis
            this.options.selectedTabItem = 'analyses';
            this.render();
          });
        }
      }
    ...
    this.listenTo(Router.getRouteModel(), 'change:currentRoute', this.handleRoute);
  },
  ...
  _renderLayerTabPaneView: function () {
    ...
    var isDisabled = canBeGeoreferenced || this._layerDefinitionModel.isEmpty() && isDone;
    ...
    var tabPaneTabs = [{
      ...
      name: 'analyses',
      selected: (selectedTabItem && selectedTabItem === 'analyses'),
      disabled: false,
      ...
      //     self.listenTo(self._analysisDefinitionNodesCollection, 'add', function (newAnalysisNode) {
      //       var nodeSource = newAnalysisNode.getPrimarySource();
      ...
      //           newAnalysisNode.get('type').indexOf('georeference') === 0 &&
      //           canBeGeoreferenced) {
      //         var callback = function (model) {
      //           if (model.isDone() || model.hasFailed()) {
      //             this.stopListening(newAnalysisNode, 'change:status', callback);
      //           }

      //           if (model.isDone()) {
      //             self._stackLayoutModel.goToStep(1, self._layerDefinitionModel, 'layer-content', 'style', null);
      //           }
      ...
      //         self.listenTo(newAnalysisNode, 'change:status', callback);
      //       }
      //     });
      ...
      name: 'infowindow',
      selected: selectedTabItem && selectedTabItem === 'infowindow',
      onClick: function () { Router.goToInteractionTab(layerId); },
      disabled: isDisabled,
      createContentView: function () {
      ...
      name: 'legends',
      selected: selectedTabItem && selectedTabItem === 'legends',
      onClick: function () { Router.goToLegendTab(layerId); },
      disabled: isDisabled,
      createContentView: function () {
        var nodeDefModel = self._layerDefinitionModel.getAnalysisDefinitionNodeModel();
