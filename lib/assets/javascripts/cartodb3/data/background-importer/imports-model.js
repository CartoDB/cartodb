...
module.exports = cdb.core.Model.extend({
  ...
  getWarnings: function () {
    ...
    return step === 'import' ? this._importModel.get('warnings') : '';
  },
  ...
  getError: function () {
    if (this.hasFailed()) {
      ...
      return _.extend(
        {
          error_code: this[step === 'upload' ? '_uploadModel' : '_importModel'].get('error_code'),
          item_queue_id: step === 'import' ? this._importModel.get('id') : '',
          original_url: step === 'import' ? this._importModel.get('original_url') : '',
          data_type: step === 'import' ? this._importModel.get('data_type') : '',
          http_response_code: step === 'import' ? this._importModel.get('http_response_code') : '',
          http_response_code_message: step === 'import' ? this._importModel.get('http_response_code_message') : ''
        },
  ...
  _createVis: function (attrs) {
    ...
    // vis.permission.owner = this._userModel; // TODO: update this
    return vis;
  },

  setError: function (opts) {
    var step = this.get('step');
    var stepModel = this[ step === 'upload' ? '_uploadModel' : '_importModel' ];
  ...
  get: function (attr) {
    if (attr === 'upload') return this._uploadModel.toJSON();
    if (attr === 'import') return this._importModel.toJSON();
