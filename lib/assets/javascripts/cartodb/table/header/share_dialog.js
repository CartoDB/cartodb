  ...
  cdb.admin.ShareDialog = cdb.admin.BaseDialog.extend({
    ...
    _TEXTS: {
      ...
      privacy_names: {
        LINK: "Anyone with a link",
        PASSWORD: "Anyone with a password",
        PRIVATE: "Private"
      }
    ...
    initialize: function() {
      ...
      var show_disabled_privacy = (free_account && !has_private_maps && (privacy != 'PRIVATE' && privacy != 'PUBLIC')) || (!has_private_maps && paid_account && privacy != 'PUBLIC' && privacy != 'LINK');
      ...
      this.options = _.extend({
        ...
        show_disabled_privacy: show_disabled_privacy,
        disabled_privacy: this.options.vis.get("privacy").toLowerCase(),
        disabled_privacy_name: this._TEXTS.privacy_names[this.options.vis.get("privacy")],

        upgrade_message: this._getUpgradeMessage(user_data.account_type),
    ...
    _addShareWidget: function() {
      ...
      var embed_url      = origin + tableURL + "?" + opts;
      var public_map_url = origin + publicURL + "?" + opts;
    ...
    render_content: function() {
      ...
      // Send mixpanel event
      cdb.god.trigger('mixpanel', 'Click Publish Visualization', { account_type: this.options.user.get('account_type') });
    ...
    _getUpgradeMessage: function(account_type) {
      ...
      if (account_type == 'free') {
        upgrade_message = "Privacy and visibility options are only available on paid accounts."
      } else {
        upgrade_message = "Private maps are only available on Coronelli plans or greater."
      }
    ...
    _setShortPublicURL: function() {
      ...
      this.options.vis.save({ url_options: opts });
