...
const template = require('./templates/user-notifications');
const dropdownTemplate = require('./templates/dropdown');
const LocalStorage = require('../../../util/local-storage');
...
module.exports = CoreView.extend({
  ...
  _generateCollection: function () {
    ...
    data.isInsideOrg = this.user.isInsideOrg();
    data.isOrgOwner = this.user.isOrgOwner();
    data.accountType = this.user.get('account_type').toLowerCase();
    data.remainingQuota = this.user.get('remaining_byte_quota');
    data.publicProfileUrl = userUrl.publicProfile();
    data.bytesQuota = this.user.get('quota_in_bytes');
    data.userType = 'regular';
    data.upgradeUrl = window.upgrade_url || '';
    data.upgradeContactEmail = this.user.upgradeContactEmail();
    data.trialEnd = this.user.get('trial_ends_at') && moment(this.user.get('trial_ends_at')).format('YYYY-MM-DD');
    data.userName = this.user.get('name') || this.user.get('username');
    ...
    // try_trial -> trial_end_at is null && user is not paid user
    if (!comHosted && !data.isInsideOrg && data.accountType === 'free' && this.user.get('table_count') > 0) {
      arr.push({
    ...
    // limits_exceeded -> check table quota size
    if (!comHosted && data.remainingQuota <= 0) {
      arr.push({
    ...
    // close_limits -> check table quota size < 80%
    if (!comHosted && ((data.remainingQuota * 100) / data.bytesQuota) < 20) {
      arr.push({
        iconFont: 'CDB-IconFont-barometer',
  ...
  _openNotifications: function (e) {
    ...
    var view = this.notification = new NotificationsDropdown({
      ...
      horizontal_offset: 5,
      vertical_offset: -5,
      template_base: dropdownTemplate
    });
