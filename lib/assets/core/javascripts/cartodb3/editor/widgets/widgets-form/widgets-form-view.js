...
var TIME_SERIES_VALUE = 'time-series';
var REQUIRED_OPTS = [
  'userActions',
...
module.exports = CoreView.extend({
  ...
  _renderCarousel: function () {
    var containsTimeSeries = _.any(this._widgetDefinitionModel.collection.models, function (model) {
      return model.get('type') === TIME_SERIES_VALUE;
    });
    var filteredDataTypes = _.filter(WidgetFormFactory.getDataTypes(this._querySchemaModel), function (type) {
      // Do not allow to change the widget to TimeSeries if there is already a TimeSeries widget
      return (!containsTimeSeries ||
              type.value !== TIME_SERIES_VALUE ||
              this._widgetDefinitionModel.get('type') === TIME_SERIES_VALUE);
    }, this);
