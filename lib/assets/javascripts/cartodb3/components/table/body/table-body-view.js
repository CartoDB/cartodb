...
module.exports = CoreView.extend({
  ...
  _showContextMenu: function (ev) {
    ...
    var $tableCell = $(ev.target).closest('.Table-cell');
    var modelCid = $(ev.target).closest('.Table-row').data('model');
    var menuItems = [{
      label: _t('components.table.rows.options.copy'),
      val: 'copy',
      action: function () {
        // Work-around for Clipboard \o/
        this._clipboard = new Clipboard($tableCell.get(0));
        $tableCell.click();
    ...
    var triggerElementID = 'context-menu-trigger-' + modelCid;
