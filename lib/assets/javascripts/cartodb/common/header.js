...
cdb.admin.Header = cdb.core.View.extend({
  ...
  tableInfo: function() {
    ...
    this.$('.table_description p').text(this.table.get('description') || 'add a description for this table...');
        ...
        res: function(val) {
          if(val !== self.model.get('name')) {
            ...
            $.when(promise).done(function(newName) {
              ...
              self.options.map.get('dataLayer').save({'tile_style': tileStyle});
              // self.options.map.get('dataLayer').save();
            });
