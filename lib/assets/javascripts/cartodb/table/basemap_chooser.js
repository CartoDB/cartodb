...
cdb.admin.BaseLayerButton = cdb.core.View.extend({
  ...
  */
  checkIfSelected: function() {
    ...
    if (this.options && this.options.map && this.model) {
      var baseLayer = this.options.map.getBaseLayer();
      var modelBaseLayer = this.model;
      if(baseLayer.get('name') === modelBaseLayer.get('name')) {
  ...
  */
  checkIfSelected: function() {
    if (this.options && this.options.map) {
      var baseLayer = this.options.map.getBaseLayer();
      if(baseLayer.attributes.type === 'Plain') {
        $("." + baseLayer.get("className")).parent().addClass("selected");
        this.updateColorButton(baseLayer.attributes.color);
      }
    }
  },
