  ...
  module Synchronization
    ...
    class Member
      ...
      def interval=(seconds=3600)
        super(seconds.to_i)
        self.run_at = Time.now + (seconds.to_i || 3600) if seconds
        seconds
      end

      def store
        raise CartoDB::InvalidMember unless valid?
      ...
      def can_manually_sync?
        ...
        (self.state == STATE_SUCCESS ||
          ...
          (self.state == STATE_QUEUED && updated_at + SYNC_NOW_TIMESPAN < Time.now)
      ...
      def run
        ...
        raise "Couldn't instantiate synchronization user. Data: #{self}" if user.nil?

        raise "Can't run a synchronization for inactive user: #{user.username}" unless user.reload.active?

        raise CartoDB::Datasources::AuthError.new('User is not authorized to sync tables') unless authorize?(user)
        ...
        if e.is_a? CartoDB::Datasources::NotFoundDownloadError
          log_warning(exception: e, synchronization_member: { id: id })
        else
          log_error(exception: e, synchronization_member: { id: id })
        end
        log.append_and_store e.message, truncate = false
        log.append e.backtrace.join("\n"), truncate = false

        if importer.nil?
          if e.is_a?(NotFoundDownloadError)
            set_general_failure_state_from(e, 1017, 'File not found, you must import it again')
          elsif e.is_a?(CartoDB::Importer2::FileTooBigError)
            set_general_failure_state_from(e, e.error_code,
                                           CartoDB::IMPORTER_ERROR_CODES[e.error_code][:title])
          elsif e.is_a?(AuthError)
            set_general_failure_state_from(e, 1011, 'Unauthorized')
          else
            set_general_failure_state_from(e)
          end
        else
          set_failure_state_from(importer)
        end
        ...
        if e.is_a?(TokenExpiredOrInvalidError)
          begin
            user.oauths.remove(e.service_name)
          rescue StandardError => e
      ...
      def notify
        sync_log = {
          'name' => name,
          'sync_time' => updated_at - created_at,
      ...
      def get_downloader
        ...
        self.service_item_id = url if service_item_id.nil? || service_item_id.size == 0
        ...
        log.append "Fetching datasource #{datasource_provider} metadata for item id #{service_item_id} from user #{user.id}"
      ...
      def set_success_state_from(importer)
        ...
        log_error(
          exception: e, message: 'Error updating state for sync table',
      ...
      def set_failure_state_from(importer)
        ...
        self.log_trace = importer.runner_log_trace
        log.append "*** Runner log: #{log_trace} \n***" unless log_trace.nil?
        self.state          = STATE_FAILURE
        ...
        if error_message.blank? && !error_code.nil?
          default_message = CartoDB::IMPORTER_ERROR_CODES.fetch(error_code, {})
        ...
        if self.retried_times < MAX_RETRIES
          ...
          ::Resque.enqueue(::Resque::UserJobs::Mail::Sync::MaxRetriesReached, user_id,
                           visualization_id, name, error_code, error_message)
        end

        track_failure
      end

      def set_general_failure_state_from(exception, error_code = 99_999, error_message = 'Unknown error, please try again')
        ...
        if self.retried_times < MAX_RETRIES
          ...
          ::Resque.enqueue(::Resque::UserJobs::Mail::Sync::MaxRetriesReached, user_id,
                           visualization_id, name, self.error_code, self.error_message)
        end
      ...
      def track_failure
        ...
        Carto::Tracking::Events::FailedSync.new(user_id, properties).report
      end
      ...
      def table
        @table = ::Table.new(name: name, user_id: user.id) if @table.nil?
        @table
      end
