...
module.exports = cdb.core.View.extend({
  ...
  events: {
    'click .js-add-selected-users': '_onClickAddSelectedUsers'
  },

  initialize: function() {
    _.each(['orgUsers', 'processUsers'], function(name) {
  ...
  _initBinds: function() {
    this.options.orgUsers.on('loading', this._setStateToLoadingUsers, this);
  ...
  _setStateToLoadingUsers: function() {
    this.model.set({
      state: 'loading',
  ...
  _onChangeSelectedUser: function() {
    var selectedUsersCount = this.options.orgUsers.where({ selected: true }).length;
    if (selectedUsersCount > 0) {
      this.$('.js-no-selected-users').hide();
      this.$('.js-add-selected-users').show();
      this.$('.js-add-selected-users span').text(
        pluralizeStr('Add 1 user', 'Add ' + selectedUsersCount + ' users', selectedUsersCount)
      );
    } else {
      this.$('.js-add-selected-users').hide();
      this.$('.js-no-selected-users').show();
    }
  },
  ...
  _onClickAddSelectedUsers: function() {
    var selectedUsers = this.options.orgUsers.where({ selected: true });
    this.model.set({
      state: 'loading',
      loadingText: 'Adding users to group'
    });
    var self = this;
    this.options.processUsers(selectedUsers, this.model.set.bind(this.model, 'state', 'error'));
  },
