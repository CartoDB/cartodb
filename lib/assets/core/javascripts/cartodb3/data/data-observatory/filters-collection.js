...
var FILTERS_QUERY_WITHOUT_REGION = "SELECT count(*) num_measurements, tag.key subsection_id, tag.value subsection_name FROM OBS_GetAvailableNumerators((SELECT ST_SetSRID(ST_Extent(the_geom), 4326) FROM (<%- query %>) q)) numers, Jsonb_Each(numers.numer_tags) tag WHERE tag.key like 'subsection%' GROUP BY tag.key, tag.value";
...
module.exports = BaseCollection.extend({
  ...
  buildQuery: function (options) {
    return options && options.region && options.region != null
            ? _.template(FILTERS_QUERY_WITH_REGION)
            : _.template(FILTERS_QUERY_WITHOUT_REGION);
  },
