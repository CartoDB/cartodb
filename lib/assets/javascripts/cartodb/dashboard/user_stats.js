...
cdb.admin.dashboard.UserStats = cdb.core.View.extend({
  ...
  _setupLocalStorage: function() {
    ...
    // Init localStorage
    var key = this.options.localStorageKey || 'cartodb_user_stats';
    this.localStorage   = new cdb.admin.localStorage(key);
  ...
  _onTablesQuotaStatusChange: function(m) {
    ...
    if (status == "boom" || status == "danger")
      this.model.set("limits_exceeded", "tables");
  },
  ...
  _onSpaceQuotaStatusChange: function(m) {
    ...
    if (status == "boom" || status == "danger")
      this.model.set("limits_exceeded", "space");
  },
  ...
  _onMapViewsQuotaStatusChange: function(m) {
    ...
    if (status == "boom" || status == "danger")
      this.model.set("limits_exceeded", "mapviews");
  },
  ...
  */
  _toggleWarnings: function() {
    ...
    if (!this.model.get("migrated") && !this.localStorage.get('migrated')) {
    ...
    if (!this.localStorage.get('limit') && this.model.get("limits_exceeded")) {
  ...
  */
  _disableWarning: function(ev) {
    ...
    } else if (!this.localStorage.get('migrated') && this.warning_type == "migrated") {

      this.localStorage.set({ migrated: true });
      this.warning_type = null;
  ...
  */
  _showWarning: function(type) {
    ...
    .removeClass("upgraded migrated limit cartodb20")
