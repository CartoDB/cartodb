...
cdb.admin.Header = cdb.core.View.extend({
  ...
  onSQLView: function() {
    if(this.table.isInSQLView()) {
      ...
      this.$('.status').hide();
      this.$('.georeference').hide();
    } else {
      this.$('.clearview').hide();
      this.$el.removeClass('query');
      this.$('.status').show();
      this.$('.georeference').show();
    }
  },
  ...
  _changeTags: function(e) {
    ...
    var tags_dialog = this.tags_dialog = new cdb.admin.EditTextDialog({
    ...
    pos.top -= $(window).scrollTop()
    var w = $(e.target).width() + 20;
    tags_dialog.showAt(pos.left - 20, pos.top - 13,w);
  }
});
