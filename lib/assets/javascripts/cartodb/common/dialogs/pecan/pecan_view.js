...
module.exports = BaseDialog.extend({
  ...
  events: {
    "click .js-goPrev": "_goPrev",
    "click .js-goNext": "_goNext"
  },
  ...
  initialize: function() {
    ...
    this.model = new cdb.core.Model({ page: 0, maxPages: 0 });
  ...
  _initViews: function() {
    ...
    _.bindAll(this, "_guessMap", "_generateThumbnail", "_analyzeColumn", "_analyzeColumns");
  ...
  _moveTabsNavigation: function() {
    ...
    this.$('.PecanMap-MapsList').css('margin-left', '-' + p + 'px');
      ...
      if (response) {
        ...
        self._generateThumbnail(column, function(error, url) {
          ...
          if (!error) {
            ...
            var w = $el.width();
            var l = self.$(".MapsList-item").length;
            self.$(".PecanMap-MapsList").width(w * l + (l - 1) * 30);
            self.model.set('maxPages', Math.ceil(self.$('.MapsList-item').size() / self._TABS_PER_ROW));
            self._checkTabsNavigation();
          }
        });
