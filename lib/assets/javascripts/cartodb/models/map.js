...
var attrHistory = function(model, attr) {
  ...
  var save = model['save' + attrHistory] = function(m, attrVal) {
    ...
    // If we save a value in the position 3, we should remove
    // any value greater than the history position.
    // if (m[attrHistoryPos] < model.MAX_HISTORY && m[attrHistoryPos] > 0) {
    //   h = h.slice(0,(m[attrHistoryPos] - 1));
    // }

    // if the new val is the same that the last one, we don't save it
    if(h.length == 0 || h[h.length-1] != attrVal) {
      h.push(attrVal);
