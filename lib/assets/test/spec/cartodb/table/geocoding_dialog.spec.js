...
describe("Geocoding dialog", function() {
  ...
  // Mixpanel events is sent from the back end!
  // 
  ...
  it("should disable/enable address pane depending geocoding attributes", function() {
    ...
    // Quota 0 and hard limit -> disable
    dialog.user.set({
    ...
    // quota 0 but no hard limit -> enable
    dialog.user.set({
  ...
  it("should not have tabs city, admin, postal, ip and address available if on_premise flag (feature flags) is enabled", function() {
    var feature_flags = [];
    feature_flags.push('on_premise');
  ...
  describe("Address pane", function() {
    ...
    it("should let add additional columns and formatter should be ok", function() {
      ...
      pane.render();
      pane.address_model.set({ value: 'testing' });
      expect(pane.$('.ok').hasClass('disabled')).toBeFalsy();
      pane.$('.ok').click();
