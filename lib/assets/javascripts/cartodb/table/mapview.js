  ...
  return {
    ...
    _addLayer: function(layer, layers, opts) {
      ...
      if (layer.get('type') === 'CartoDB') {
        ...
        layer.bind('change:tile_style change:query change:interactivity change:visible', this._updateLayerDefinition, this);
      } else {
...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    _.bindAll(this, "_addOverlays", "_bindMapOptions");
  ...
  _addSlides: function() {
    ...
    if (this.vis.get("type") === "table") return;

    var sd = new cdb.admin.Slides(null, { visualization: this.vis});
  ...
  _addSQLViewHeader: function() {
    ...
    setTimeout(function() {
      self.overlays._positionOverlaysVertically(true);
    }, 200)
  ...
  _toggleSlides: function(e) {
    ...
    this.killEvent(e);
    var visible = this.slidesPanel.toggle();

    if (visible) {
      this.$el.find(".toggle_slides h5").html("Hide")
      this.$el.find(".toggle_slides").addClass("active");
    } else {
      this.$el.find(".toggle_slides h5").html("Show");
      this.$el.find(".toggle_slides").removeClass("active");
    }
