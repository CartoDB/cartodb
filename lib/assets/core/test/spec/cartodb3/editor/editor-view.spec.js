...
describe('editor/editor-view', function () {
  ...
  describe('._onLayerCollectionRemove', function () {
    it('should unset infobox model state and find non georeference layer', function () {
      expect(this.view._infoboxModel.get('state')).toBeDefined();
      spyOn(this.view, '_findNonGeoreferencedLayer');
      this.layerDefinitionsCollection.remove(this.layerDefinitionsCollection.at(0));
      expect(this.view._infoboxModel.get('state')).toBeUndefined();
      expect(this.view._findNonGeoreferencedLayer).toHaveBeenCalled();
    });
  });

  describe('._onLayerCollectionAdd', function () {
    it('should fetch visualization privacy and unset infobox model state if a new layer is added', function () {
    ...
    it('should launch georeference onboarding when a new non georeferenced layer is added', function () {
      ...
      spyOn(this.view, '_findNonGeoreferencedLayer').and.callFake(function (callback) {
        callback();
      });
