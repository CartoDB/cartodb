...
class Geocoding < Sequel::Model
  ...
  ALLOWED_KINDS = %w(admin0 admin1 namedplace postalcode high-resolution ipaddress).freeze
  ...
  SANITIZED_FORMATTER_REGEXP = /\A[[[:word:]]\s\,\']*\z/.freeze
  ...
  def run!
    ...
    if processable_rows == 0
      update(state: 'finished', real_rows: 0, used_credits: 0, processed_rows: 0, cache_hits: 0)
      report
      return
    end

    rows_geocoded_before = begin
    ...
    run_geocoding!(processable_rows, rows_geocoded_before)
  rescue StandardError => e
  ...
  def run_geocoding!(processable_rows, rows_geocoded_before = 0)
    ...
    update state: 'started', processable_rows: processable_rows
    @started_at ||= Time.now
    ...
    update(table_geocoder.update_geocoding_status)
  ...
  def report(error = nil)
    ...
    payload.delete_if { |k, _v| %w{distinct_id email table_id}.include?(k.to_s)}
  ...
  def cost
    ...
    begin
      processed_rows.to_i * CartoDB::GeocoderConfig.instance.get['cost_per_hit_in_cents']
    rescue StandardError
      0
    end
  end
  ...
  def max_geocodable_rows
    ...
    return 50_000 if user.soft_geocoding_limit?
  ...
  def handle_geocoding_success(rows_geocoded_before)
    update(cache_hits: table_geocoder.cache.hits) if table_geocoder.respond_to?(:cache)
    stats_aggregator = CartoDB::Stats::Geocoding.instance
    begin
      stats_aggregator.gauge('requests', "+#{processed_rows}")
    rescue StandardError
      nil
    end
    begin
      stats_aggregator.gauge('cache_hits', "+#{cache_hits}")
    rescue StandardError
      nil
    end
    ...
    update(state: 'finished', real_rows: geocoded_rows, used_credits: calculate_used_credits)
    send_report_mail(state, table_name, nil, processable_rows, geocoded_rows)
    log.append_and_store 'Geocoding finished'
    # In the import table_service could be nil
    unless table_service.nil?
  ...
  def handle_geocoding_failure(raised_exception, rows_geocoded_before)
    ...
    update(state: 'failed', processed_rows: 0, cache_hits: 0)
    CartoDB.notify_exception(raised_exception, user: user)
    geocoded_rows = total_geocoded_rows(rows_geocoded_before)
    send_report_mail(state, table_name, error_code, processable_rows, geocoded_rows)
    log.append_and_store "Unexpected exception: #{raised_exception}"
    log.append_and_store raised_exception.backtrace
    report(raised_exception)
  end

  def total_geocoded_rows(rows_geocoded_before)
    rows_geocoded_after = begin
                            table_service.owner.in_database.select.from(table_service.sequel_qualified_table_name).where('cartodb_georef_status is true and the_geom is not null').count
                          rescue StandardError
                            0
                          end
  ...
  def store_geocoder_info(pid)
    ...
    self.pid = pid
    self.geocoder_type = table_geocoder.name if defined?(@table_geocoder)
    save
  end
