...
F.prototype._manageTimeSeriesForTorque = function (m) {
  ...
  if (!typeChanged || _.isEqual(this._lastTSChange, typeChanged)) return;
  ...
  var isAnimated = m.styleModel.get('type') === 'animated';
  var animatedAttrs = m.styleModel.changedAttributes().animated;
  var persistName;

  if (isAnimated || persistWidget) {
    persistName = currentTimeseries && currentTimeseries.get('title');
    this._createTimeseries(newLayer, animatedAttrs, persistName);
  }
  this._lastTSChange = typeChanged;
};
