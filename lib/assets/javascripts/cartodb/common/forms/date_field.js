  ...
  cdb.admin.DateField = cdb.admin.StringField.extend({
    ...
    default_options: {
      template_name: 'common/views/forms/date_field'  
    },

    events: {
      'change input.time': '_onChange',
      'keyup input.time':  '_onKeyDown'
    },

    initialize: function() {
      _.defaults(this.options, this.default_options);

      _.bindAll(this, '_onChange', '_onKeyDown', '_onChangeModel');
      ...
      // Get date
      var date = this._splitDate((this.model.get('value')).toString());

      // Generate a new date Model (later it will be removed and destroyed)
      this.date_model = new cdb.core.Model(date);
    ...
    _toDate: function(date) {
      return date.year + "-" + date.month + "-" + date.day + "T" + date.time + "+0" + this.timezone + ":00"
    },
