  ...
  cdb.admin.dashboard.StartView = cdb.core.View.extend({
    ...
    _decideActiveBlock: function() {
      ...
      var current_route = this.model.get("path"); //Backbone.history.fragment;

      var tables_size = this.tables.size();
      var vis_size    = this.visualizations.size();

      console.log(current_route, tables_size, vis_size, current_route.indexOf("tag"));
      ...
      if (current_route == "visualizations") {
        this.$no_tables.removeClass("active");
        $("article.tables").hide();
      } else if (current_route == "tables") {
        this.$no_vis.removeClass("active");
        $("visualizations.tables").hide();
      } else if (current_route == 'tag') {
        this.$no_tables.removeClass("active");
        this.$no_vis.removeClass("active");

        if (tables_size == 0)  $("article.tables").hide();
        else $("articles.tables").show();

        if (vis_size == 0)     $("article.visualizations").hide();
        else $("article.visualizations").show();
      }
