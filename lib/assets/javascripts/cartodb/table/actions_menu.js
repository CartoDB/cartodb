  ...
  cdb.admin.LayersPanel = cdb.ui.common.TabPane.extend({
    ...
    _changeTitle: function(e) {
      ...
      if (!!this.panelOpen) {
        ...
        if (this.isVisEditable()) {
          ...
          title_dialog.showAt(pos.left, pos.top + 6, w);
    ...
    *******/
    ...
    _addLayer: function(layer, pos) {
      var self = this;
      var emptyDataset = layer.get('table_name') === cdb.config.get('shared_empty_dataset_name');
    ...
    _removeLayer: function(dataLayerCid) {
      ...
      if (!this._sharedEmptyDatasetDisabled) {
        if (self.map.layers.getDataLayers().length === 2) {
    ...
    _manageLayers: function() {
      ...
      if (activePane) {
        if (this.metadataOpen) {
          ...
          $el.find('.title_layer').css({height: 30});
      ...
      // Set layers offset
      var offset = this.metadataOpen ? 0 : -30;
