...
module.exports = BaseDialog.extend({
  ...
  _analyzeStats: function(name, stats) {
    ...
    if (type === 'boolean' && stats.null_ratio > 0.75) {
      console.log("%c"+name + ' rejected because the null ratio > 75%: ' + stats.null_ratio * 100, "font-weight: bold; color:red;");
      return false;
    ...
    console.log(stats.distinct, stats.count)
    ...
    if (skew && weight) {
      console.log("%c - weight: " + weight.toFixed(2) + "; skew: " + skew.toFixed(2), "color:#666;");
    } else if (weight) {
    ...
    //if (type === 'string' && (distinctPercentage < 5 || distinctPercentage > 95)) {
      //console.log("%c" + name + ' rejected because the distinctPercentage > 95% || distinctPercentage < 5%: ' + distinctPercentage, "font-weight: bold; color:red;");
      //return false;
    //}
