...
cdb.admin.NewVisualizationDialog = cdb.admin.BaseDialog.extend({
  ...
  _onAddTableItem: function(e) {
    ...
    var table = $(this.$("select").select2('data').element).data();
  ...
  _addCombo: function() {
    ...
    // Generate the desired list :)
    var table_list = this.visualizations.map(function(mdl){
      var obj = {
        vis_id:     mdl.get('id'),
        name:       mdl.get('name'),
        username:   '',
      ...
      if (this.user.isInsideOrg()) {
        ...
        obj.avatar = d.avatar_url;
        obj.permission = mdl.permission.getPermission(this.user) === cdb.admin.Permission.READ_ONLY ? 'READ': null
      }
    ...
    // Create combo
    this.tableCombo = new cdb.admin.VisualizationsCombo({
