...
var MAP = {
  ...
  'data-observatory-measure': {
    ...
    checkIfValid: function (userModel) {
      if (!userModel) throw new Error('userModel is required');
      return userModel.dataServiceEnabled('data_observatory');
    },
  ...
  'georeference-city': {
    ...
    checkIfValid: function (userModel) {
      return checkIfInternalGeocodingIsEnabled(userModel);
    },
  ...
  'georeference-street-address': {
    ...
    checkIfValid: function (userModel) {
      if (!userModel) throw new Error('userModel is required');
      return userModel.dataServiceEnabled('hires_geocoder');
    },
    onboardingTemplate: require('../components/onboardings/analysis/analyses/georeference.tpl'),
    genericType: 'georeference'
  },
  ...
  'routing-sequential': {
    ...
    checkIfValid: function (userModel) {
      return checkIfRoutingIsEnabled(userModel);
    }
  },
  ...
  'trade-area': {
    ...
    checkIfValid: function (userModel) {
      if (!userModel) throw new Error('userModel is required');
      return !!userModel.dataServiceEnabled('isolines');
    },
    onboardingTemplate: require('../components/onboardings/analysis/analyses/area-of-influence.tpl'),
    genericType: 'area-of-influence'
  },
...
var checkIfInternalGeocodingIsEnabled = function (userModel) {
  if (!userModel) throw new Error('userModel is required');
  return !!userModel.dataServiceEnabled('geocoder_internal');
};

var checkIfRoutingIsEnabled = function (userModel) {
  if (!userModel) throw new Error('userModel is required');
  return !!userModel.dataServiceEnabled('routing');
};
...
var isAnalysisValidByType = function (type, userModel) {
  var analysis = MAP[type];
  return analysis && analysis.checkIfValid ? analysis.checkIfValid(userModel) : true;
};
...
module.exports = {
  ...
  getAnalysesByModalCategory: function (category, userModel) {
    return _.reduce(MAP, function (memo, item, analysisType) {
      if (item.modalCategory === category) {
        if (isAnalysisValidByType(analysisType, userModel)) {
          var obj = {
            nodeAttrs: {
              type: analysisType
            },
  ...
  isAnalysisValidByType: function (type, userModel) {
    return isAnalysisValidByType(type, userModel);
  },
