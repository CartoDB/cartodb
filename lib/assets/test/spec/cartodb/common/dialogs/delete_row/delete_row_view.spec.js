...
describe('common/dialogs/delete_row/delete_row_view', function() {
  ...
  describe('when click ok to delete row and it fails', function() {
    it('should call to destroy the model', function() {
      var server = sinon.fakeServer.create();
      server.respondWith("DELETE", "/api/v1/tables/" + this.table.get('name') + "/records/100", [ 500, { "Content-Type": "application/json" }, '{ "error": "bad request" }' ]);

      var succeded = false;

      this.table.bind('remove:row', function() {
        succeded = true;
      });
      ...
      expect(succeded).toBeFalsy();
      expect(this.view._panes.activeTab).toEqual("fail");
    });
  });
