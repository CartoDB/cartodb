...
module.exports = Backbone.Collection.extend({
  ...
  setBaseLayer: function (layer) {
    ...
    var layerHasLabels = layer.labels && layer.labels.url;

    if (currentBaseLayer) {
      if (currentBaseLayer.get('type') === layer.type) {
        this._updateBaseLayer(currentBaseLayer, layer);
      } else {
        this._replaceBaseLayer(currentBaseLayer, layer);
      }
    } else {
    ...
    if (layerHasLabels) {
      if (this._hasLabelsLayer()) {
        this._updateLabelsLayer(layer);
      } else {
        this._addLabelsLayer(layer);
      }
    } else {
      if (this._hasLabelsLayer()) {
  ...
  _updateBaseLayer: function (currentBaseLayer, layer) {
    var newAttributes = _.extend(_.clone(layer), {
    ...
    currentBaseLayer.save();
  },

  _replaceBaseLayer: function (currentBaseLayer, layer) {
    this.remove(currentBaseLayer);

    var newBaseLayer = this.createBaseLayer(layer);
    newBaseLayer.set({
      id: currentBaseLayer.get('id'),
      order: currentBaseLayer.get('order')
    });
    this.add(newBaseLayer, { at: 0 });
    newBaseLayer.save();
  },
  ...
  _hasLabelsLayer: function () {
    return this.size() > 1 && this.getLayerOnTop().get('type') === 'Tiled';
  },

  _updateLabelsLayer: function (layer) {
    var labelsLayer = this.getLayerOnTop();
    labelsLayer.set({
      name: this._labelsLayerNameFromLayer(layer),
      urlTemplate: layer.labels.url,
  ...
  _addLabelsLayer: function (layer) {
    var labelsLayer = this.createBaseLayer({
      name: this._labelsLayerNameFromLayer(layer),
      urlTemplate: layer.labels.url,
      attribution: layer.attribution,
      minZoom: layer.minZoom,
    ...
    this.add(labelsLayer, { silent: true });
    labelsLayer.save();
  },
