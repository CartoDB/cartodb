...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    if (this._firstNode.isSourceType()) {
      ...
      this._tableNodeModel.once('change:synchronization', function () {
        if (this._tableNodeModel && this._tableNodeModel.isSync()) {
          this._initTable();
        }
      }, this);
    }
  ...
  _initBinds: function () {
    ...
    this.listenTo(this._layerDefinitionModel, 'destroy', this._onClickBack);
    this.listenTo(this._layerDefinitionModel, 'change:source', this._initTable);
    this.listenTo(this._layerDefinitionModel, 'change:source', this._renderContextButtons);
