...
TableStats.prototype = {
  ...
  getFormulaStats: function (tableName, column, callback) {
    // var schemaModel = new QuerySchemaModel(null, {
    //   configModel: configModel
    // });
    // schemaModel.set('query', 'select * from pg_stats where tablename = \'' + table + '\'');
    var self = this;
    this._getPgStats(tableName, function (stats) {
      var graph = new ColumnGraph(self.stats[column]);
      callback(graph);
    });
  },
