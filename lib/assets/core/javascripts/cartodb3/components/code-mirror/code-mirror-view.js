...
module.exports = CoreView.extend({
  ...
  _initViews: function () {
    ...
    var autocompletePrefix = options.autocompletePrefix;

    function completeIfAfterOpeningCurly (cm) {
    ...
    function completeIfAfterCtrlSpace (cm) {
      var autocompleteChars = self._autocompleteChars - 1;
      var triggers = self._autocompleteTriggers;
    ...
    var extraKeys = this._autocompletePrefix ? _.defaults(_.extend({}, {
      "'{'": completeIfAfterOpeningCurly,
      'Ctrl-Space': completeIfAfterCtrlSpace
  ...
  _onKeyUpEditor: function (cm, event) {
    ...
    if (NOHINT.indexOf(code) === -1) {
      ...
      return self._completeAfter(cm, function() {
        ...
        if (str.length > autocompleteChars) {
          if (self._autocompletePrefix) {
            var listTriggers = _.filter(triggers, autocompleteHandler);
            return listTriggers.length > 0;
          }

          var listHints = _.filter(hints, autocompleteHandler);
          return listHints.length > 0;
        }
      });
    }
  },

  _completeAfter(cm, pred) {
    var self = this;
