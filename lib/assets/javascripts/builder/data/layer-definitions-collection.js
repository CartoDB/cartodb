...
module.exports = Backbone.Collection.extend({
  ...
  isThereAnyGeometryData: function () {
    ...
    return new Promise(function (resolve, reject) {
      var dataLayers = self.filter(function (layerDefModel) {
        return layerDefModel.isDataLayer();
      });
      var allQueryGeometryModels = _.compact(_.map(dataLayers, function (dataLayer) {
        return dataLayer.getAnalysisDefinitionNodeModel().queryGeometryModel;
      }));
      var allPromises = _.map(allQueryGeometryModels, function (queryGeometryModel) {
        return queryGeometryModel.hasValueAsync();
      });
      Promise.all(allPromises)
        .then(function (hasGeomValues) {
          resolve(_.some(hasGeomValues));
        });
    });
  },
