...
TableStats.prototype = {
  ...
  _getPgStats: function (table, callback) {
    ...
    if (this.tables[table]) {
      ...
      var url = 'https://' + this.user + '.cartodb.com/api/v2/sql?q=select * from pg_stats where tablename = \'' + table + '\'&api_key=' + this.apiKey;
      var r = new XMLHttpRequest();
      r.open("GET", url, true);
      r.onreadystatechange = function () {
        if (r.readyState !== 4 || r.status !== 200) return;
        var rows = JSON.parse(r.responseText).rows.filter(function (r) { return r.histogram_bounds; }).map(function (r) {
          return {
            column: r.attname, 
