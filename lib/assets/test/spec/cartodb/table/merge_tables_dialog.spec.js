...
describe("[CT] TableColumnSelectorEvents", function() {
...
describe("[CT] TableColumnSelector", function() {
  ...
  it("should show or hide the merge methods list when the joinType changes", function() {
    ...
    selector.setJoinType("spatial");
    expect(selector.$el.find(".merge_methods")).toBeVisible();
  ...
  it("should have a column selected by default in a regular selector", function() {
    ...
    expect(selector.$el.find(".columns li:first a").hasClass("selected")).toBeTruthy();
  ...
  it("should have a column selected by default in a spatial selector", function() {
    ...
    selector.setJoinType("spatial");
    selector._selectKeyByDefault();
    console.log(selector.$el.find(".columns li:first a"));
    expect(selector.$el.find(".columns li:first a").hasClass("selected")).toBeTruthy();
...
describe("[CT] MergeTablesDialog", function() {
  ...
  beforeEach(function() {
    ...
    table = TestUtil.createTable('table_a', schemaA);

    tableB = TestUtil.createTable('table_b', schemaB);
  ...
  it("should generate a query", function() {
    ...
    var query = dialog._generateSQL();

    //dialog.model.set("merge_flavor", "regular");
    console.log(tableB);

    //dialog.merge_table._selectOption(1");

    console.log(dialog.actual_table.subColumns);
    console.log(dialog.merge_table.subColumns);

    //dialog.actual_table.subColumns[0].model.set("keySelected", true); // fielda
    //dialog.merge_table.subColumns[0].model.set("keySelected", true); // fieldd

    expect(query).toEqual("SELECT table_a.the_geom, table_a.fielda, table_a.fieldb, table_a.fieldc, table_b.the_geom AS table_b_the_geom, table_b.fieldd  FROM table_a FULL OUTER JOIN table_b ON table_a.fielda = table_b.fieldd");
  ...
  */
  ...
  it("should enable the next button when the user selects a destination table", function() {
    ...
    console.log("val", val, dialog.merge_table.$el.find("option").length);
    dialog.merge_table._selectOption(val);
