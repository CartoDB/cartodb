...
module.exports = BaseDialog.extend({
  ...
  _addCard: function(url, response) {
    ...
    this._addHistogram($el.find(".js-graph"));
  ...
  _addHistogram: function(el) {
    var data = [
      { count: 0,  when: "2015-07-12" },
      { count: 10, when: "2015-07-13" },
      { count: 20, when: "2015-07-14" },
      { count: 10, when: "2015-07-15" },
      { count: 5,  when: "2015-07-16" },
      { count: 3,  when: "2015-07-17" },
      { count: 0,  when: "2015-07-18" },
      { count: 4,  when: "2015-07-19" },
      { count: 3,  when: "2015-07-20" },
      { count: 5,  when: "2015-07-21" },
      { count: 13, when: "2015-07-22" },
    ];
    ...
    svg.selectAll(".bar")
      ...
      .enter().append("rect")
        ...
        .attr('data-title', function(d) {
          return Utils.formatNumber(d.count) 
        })
        .attr("x", function(d) { return x(d.when); })
        .attr("width", 5)
        .attr("y", function(d) {
          var value = height - y(d.count);
          var yPos = y(d.count);
