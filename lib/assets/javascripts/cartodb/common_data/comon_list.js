  ...
  cdb.admin.CommonTableView = cdb.core.View.extend({
    ...
    render: function() {
      ...
      // Over quota?
      data.over_quota = data.size > +this.options.user.get('remaining_byte_quota') || +this.options.user.get("remaining_table_quota") == 0;
    ...
    _copyTable: function(e) {
      ...
      var _e =  ((this.model.get("size") > +this.options.user.get('remaining_byte_quota')) || (+this.options.user.get("remaining_table_quota") == 0)) ? 'upgrade' : 'create';
      this.trigger(_e, e);
    },
