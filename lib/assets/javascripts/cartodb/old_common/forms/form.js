...
cdb.forms.Form = cdb.core.View.extend({
  ...
  _pro_ui_fields: ["Marker Stroke", "Bubble stroke", "Composite operation", "Label Halo", "Label Offset", "Label Overlap", "Label Placement",
                   "Buckets", "Quantification", "Radius"],
  ...
  _renderField: function(field) {
    ...
    if (!bbgProUI && $.inArray(field.name, this._pro_ui_fields) !== -1) {
      return;
    }
    var self = this;
    ...
    _(field.form).each(function(form, name) {
      if (!bbgProUI && name === "text-face-name" && (field.name === "Label Font" || field.name === "Label Text")) {
        return true;
      }
      ...
      if (Class) {
        ...
        if (form.type === 'select' && !opts.model.has(form.property)) {
          opts.model.set(form.property, form.value);
        }
