...
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.template = cdb.templates.getTemplate('new_dashboard/views/user_notifications');
  ...
  _initBinds: function() {
    // Make it live
    this.user.bind('change', this._resetCollection, this);
  ...
  _generateCollection: function() {
    ...
    d.userType = 'regular';
    d.upgradeUrl = this.userUrls.upgradeUrl(this.user);
    d.contactUrl = this.userUrls.contactUrl(this.user);
    ...
    if (!d.isInsideOrg && d.accountType === 'free' && this.user.get("table_count") > 0) {
      arr.push({
        type:   'try_trial',
        msg:    cdb.templates.getTemplate('new_dashboard/views/notifications_messages/try_trial')(d),
    ...
    if (d.remainingQuota <= 0) {
      arr.push({
        ...
        msg:    cdb.templates.getTemplate('new_dashboard/views/notifications_messages/limits_exceeded')(d),
    ...
    if ((( d.remainingQuota * 100 ) / d.bytesQuota ) < 20) {
      arr.push({
        type:   'close_limits',
        msg:    cdb.templates.getTemplate('new_dashboard/views/notifications_messages/close_limits')(d),
    ...
    if (this.user.get("show_upgraded_message")) {
      arr.push({
        ...
        msg:    cdb.templates.getTemplate('new_dashboard/views/notifications_messages/upgraded_message')(d),
    ...
    if (this.user.get("show_trial_reminder")) {
      arr.push({
        ...
        msg:    cdb.templates.getTemplate('new_dashboard/views/notifications_messages/trial_ends_soon')(d),
    ...
    arr.push({
      type:   'new_dashboard',
      msg:    cdb.templates.getTemplate('new_dashboard/views/notifications_messages/new_dashboard')(d),
  ...
  _openNotifications: function(e) {
    ...
    this._setupDropdown(new NotificationsDropdown({
      target:             this.$el,
      collection:         this.collection,
      horizontal_offset:  5,
      vertical_offset:    -5,
      template_base:      'new_dashboard/views/notifications_dropdown'
    }));
  },

  _setupDropdown: function(view) {
