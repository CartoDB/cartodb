...
describe('dashboard/components/password-confirmation/password-confirmation-view', function () {
  ...
  it('should render properly', function () {
    ...
    expect(view.$('.Form-label')[0].textContent).toContain('components.modals.password-confirmation.form.password-label');
  });
  ...
  it('should display "Old Password" message if the password has been updated', function () {
    view = createViewFn({ updatePassword: true });
    view.render();

    expect(view.$('.Form-label')[0].textContent).toContain('components.modals.password-confirmation.form.old-password-label');
  });
});
