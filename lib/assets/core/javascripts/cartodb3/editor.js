...
var mapCreation = function () {
  ...
  if (onboardingNotification.getKey('onboarding')) {
    var emptylayerDefModel;
    ...
    layerDefinitionsCollection.each(function (layerDefModel) {
      ...
      if (analysisDefinitionNodeModel) {
        var geom = analysisDefinitionNodeModel.queryGeometryModel.get('simple_geom');

        if (_.isEmpty(geom)) {
          var rowsCollection = analysisDefinitionNodeModel.querySchemaModel.rowsCollection;

          if (rowsCollection.length > 0) {
            emptylayerDefModel = layerDefModel;
          }
        }
      }
    }, this);

    if (emptylayerDefModel) {
      AnalysesService.setLayerId(emptylayerDefModel.get('id'));

      GeoreferenceOnboardingLauncher.launch({
        name: emptylayerDefModel.getName(),
        source: emptylayerDefModel.get('source')
      });
    }
  }
