  ...
  setBaseLayer: function(baseLayer) {
    ...
    if (currentBaseLayer) {
      if (currentBaseLayer.get('type') === newBaseLayer.get('type')) {
        this._updateBaseLayer(currentBaseLayer, newBaseLayer);
      } else {
        this._replaceBaseLayer(currentBaseLayer, newBaseLayer);
      }
    } else {
      this._addBaseLayer(newBaseLayer);
    }
    ...
    this.layers.saveLayers({
      ...
      error: function() {
        cdb.log.error("error saving layers");
        self.trigger('savingLayersFinish');
      }
    });
