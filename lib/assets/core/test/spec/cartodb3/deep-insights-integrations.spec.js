...
describe('deep-insights-integrations/dii', function () {
  ...
  describe('autoStyle', function () {
    ...
    beforeEach(function () {
      ...
      spyOn(layerDefinitionModel, 'setCartoCSSFromAutoStyle').and.callThrough();
      spyOn(layerDefinitionModel, 'resetCartoCSSFromAutoStyle').and.callThrough();
    ...
    describe('should preserve custom cartocss when the autostyle is off', function () {
      beforeEach(function () {
      ...
      it('should preserve custom cartocss when the autostyle is off', function () {
        var model = dashBoard.getWidget(category.id);
        model.set({autoStyle: true});
        expect(layerDefinitionModel.get('autoStyle')).toBe(model.id);
        expect(layerDefinitionModel.setCartoCSSFromAutoStyle).toHaveBeenCalled();

        model.set({autoStyle: false});
        expect(layerDefinitionModel.get('autoStyle')).toBe(false);
        expect(layerDefinitionModel.resetCartoCSSFromAutoStyle).toHaveBeenCalled();
        expect(layerDefinitionModel.get('cartocss')).toBe('wadus');
      });
    });
  });
