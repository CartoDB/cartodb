...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  */
  ...
  _fetchWidgets: function(widgets) {
    ...
    _.each(widgets, function(widget) {
      ...
      model.urlRoot = self.widgets.url;

      var options = JSON.parse(model.get("options"));
  ...
  _renderWidget:  function(delayed_animation, data) {
    ...
    text.bind("remove", function(widget) {
      ...
      this.widgets.remove(widget.model);
  ...
  _addWidgets: function() {
    ...
    this.widgets     = new cdb.admin.WidgetCollection([]);
    this.widgets.url = "/api/v1/viz/" + this.vis.get("id") + "/overlays";

    this.widgets.fetch({ reset: true });

    this.widgets.bind("reset", function(widgets) {

      if (widgets.models.length > 0) {
        //self.urls = [];
        //self.urls = self.widgets.at(0).get("overlays");
        self._fetchWidgets(widgets.models);
      }

    });

    this.widgets.bind("remove", function(widget) {
      widget.destroy();
    });

    this.widgets.bind("change", function(widget) {
      widget.save();
    });

    this.widgets.bind("add", function(widget) {
