...
<% } else if (completed && warnings) { %>
    <%= _t('components.background-importer.background-importer-item.warnings', { name: name }) %><% if (service) { %> <%= _t('components.background-importer.background-importer-item.from') %> <%- service %> <% } %>
<% } else { %>
