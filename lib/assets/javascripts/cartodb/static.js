...
(function (window, document) {
  ...
  client.getConfig(function (err, response, data) {
    function getAssetsVersion () {
      ...
      return config.assetVersion || data.user_frontend_version || version;
    }

    function getAssetsBaseUrl () {
      var userAssetsHost = data.asset_host;

      return (config.assetsBaseUrl || userAssetsHost && userAssetsHost + '/assets' || data.config.app_assets_base_url) + '/';
    }
