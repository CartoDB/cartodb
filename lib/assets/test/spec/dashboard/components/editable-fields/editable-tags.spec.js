...
function type ($textarea, word) {
  ...
  for (let i = 0; i < letters.length; i++) {
    const keyCode = letters[i].charCodeAt(0);
    $textarea.trigger(keyPressEvent(keyCode));
  }
}

describe('dashboard/components/editable-fields/editable-tags', function () {
  let model, view, router;
  ...
  it('should display up to 3 tags', function () {
    ...
    expect($($tagItems[0]).attr('href')).toEqual('http://team.carto.com/u/pepe/dashboard/datasets/tag/dolorem');
    expect($($tagItems[1]).attr('href')).toEqual('http://team.carto.com/u/pepe/dashboard/datasets/tag/ipsum');
    expect($($tagItems[2]).attr('href')).toEqual('http://team.carto.com/u/pepe/dashboard/datasets/tag/quia');
