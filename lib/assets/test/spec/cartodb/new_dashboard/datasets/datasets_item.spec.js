...
describe('new_dashboard/datasets/datasets_item', function() {
  ...
  it('should render table size', function() {
    ...
    expect($(this.html).find('.SizeIndicator').text()).toContain('1000.00 bytes');
  });

  it('should render table geometry type', function() {
    this.renderView();
    expect($(this.html).find('.DatasetsList-itemCategory').length).toBe(1);
    expect($(this.html).find('.DatasetsList-itemCategory').hasClass('is--pointDataset')).toBeTruthy();
  });
