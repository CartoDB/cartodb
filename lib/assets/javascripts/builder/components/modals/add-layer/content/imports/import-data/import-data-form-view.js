...
module.exports = CoreView.extend({
  ...
  _initViews: function () {
    if (this.options.fileEnabled) {
      var self = this;
      this.$('.js-fileInput').on('change', function (e) {
  ...
  _initDropzone: function () {
    ...
    $(el).on('dragster:enter', function (e) {
      self._showDropzone();
    });

    $(el).on('dragster:leave', function (e) {
      self._hideDropzone();
    });
  ...
  _destroyDropzone: function () {
    ...
    if (this.dragster) {
      ...
      this.dragster.reset();
      $(el).off('dragster:enter dragster:leave');
    }
