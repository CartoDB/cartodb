...
$(function () {
  ...
  const setupPasswordExpirationInput = function () {
    const expirationInput = $('#organization_password_expiration_in_d');
    const unlimitedExpirationInput = $('#unlimited_password_expiration');

    if (!organization.get('password_expiration_in_d')) {
      unlimitedExpirationInput.prop('checked', true);
      expirationInput.attr('disabled', 'disabled');
    }
