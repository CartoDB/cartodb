...
module.exports = Backbone.Collection.extend({
  ...
  model: function (d, opts) {
    ...
    if (attrs.kind !== 'tiled') {
      o.letter = o.letter || self._nextLetter();
    }
  ...
  _nextLetter: function () {
    ...
    var letter = _.chain(this._letters)
      .sort()
      .reduce(this._getNextAvailableLetter, 'a')
      .value();
    this._letters.push(letter);
    return letter;
  },

  _getNextAvailableLetter: function (memo, letter) {
    if (letter === memo) {
  ...
  _onRemove: function (m) {
    this._letters = _.without(this._letters, m.get('letter'));
  }
