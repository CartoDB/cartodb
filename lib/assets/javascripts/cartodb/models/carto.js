...
function cluster_sql(table, zoom, props, nquartiles) {
  ...
  var sql = "WITH filtered_table AS <%= table %>, ";

  sql += "hgridA AS (SELECT CDB_HexagonGrid(ST_Expand(!bbox!, greatest(!pixel_width!,!pixel_height!) * " + size + "), greatest(!pixel_width!,!pixel_height!) * " + size + ") as cell), " +
  ...
  for (var i = 1; i < nquartiles; i++) {
    ...
    sql += "\n\n" + hgrid + " AS (SELECT CDB_HexagonGrid(ST_Expand(!bbox!, greatest(!pixel_width!,!pixel_height!) * " + size + "), greatest(!pixel_width!,!pixel_height!) * " + size + ") AS cell), " +
    "\n" + bucket + " AS (" +
    "\n\tSELECT * FROM (SELECT ST_Centroid(ST_Collect(i.the_geom_webmercator)) AS the_geom_webmercator, COUNT(i.cartodb_id) AS points_count, 1 AS cartodb_id, array_agg(cartodb_id) AS id_list " +
...
function cluster_generator(table, props, changed, callback) {
  ...
  var cluster_sql_gen = cluster_sql(table, props.zoom, props, nquartiles);
  ...
  for (var i = 0; i < nquartiles; i++) {
    c += "  [src = 'bucket"+grids[nquartiles - i - 1]+"'] { marker-width: " + (i * 20 + 24 )+ "; } \n";
  }
  ...
  simple_polygon_generator(table, props, changed, function(css) {
    callback(c, null, cluster_sql_gen);
  });
}
