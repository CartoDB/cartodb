...
(function() {
  ...
  cdb.admin.CartoDBTableData = cdb.ui.common.TableData.extend({
    ...
    discreteHistogram: function(nbuckets, column, callback, error) {
      var sql = _.template(
      'SELECT distinct(<%= column %>) as bucket, count(*) as value FROM <%= table %> GROUP BY <%= column %> ORDER BY value DESC LIMIT <%= nbuckets %>', {
        column: column,
        nbuckets: nbuckets,
    ...
    histogram: function(nbuckets, column, callback, error) {
      ...
      'with bounds as ( ' + 
       ...
       '(max(<%= column %>) - min(<%= column %>)) as span  ' + 
        'FROM  <%= table %>  ' + 
      ')  ' + 
      'select array_agg(v) val, array_agg(bucket) buckets, bounds.upper, bounds.lower, bounds.span from ' + 
      '( ' + 
      'select  ' + 
        'count(<%= column %>) as v,   ' + 
        'floor(((<%= nbuckets %> - 1)*(<%= column %> - bounds.lower)/bounds.span)) as bucket  ' + 
         'from <%= table %>, bounds  ' + 
         'group by bucket order by bucket ' + 
      ') a, bounds ' + 
       'group by ' + 
      'bounds.upper, ' + 
      'bounds.lower, bounds.span ');
      ...
      function(data) {
        ...
        for(var i = 0; i < nbuckets; ++i) {
          if(hist[i] === undefined) {
            ...
            hist[i] = (hist[i] - min)/span;
          }
        }
        hist.upper = parseFloat(upper);
        hist.lower = parseFloat(lower);
        callback(hist);
      },
      error);
    },
