...
cdb.admin.HeaderOptionsMenu = cdb.admin.DropdownMenu.extend({
  ...
  _appendData: function(e){
    ...
    if (!this.model.isVisualization() && !this.table.isSync()) {
      cdb.god.trigger('mixpanel', "Table data appending");

      var dlg = new cdb.admin.AppendDataDialog({
        table: this.table,
        tables: new cdb.admin.Tables(),
        user: this.user
      });

      dlg.bind('importComplete', function() {
        this.options.globalError.showError('Data appended', '', 3000);
