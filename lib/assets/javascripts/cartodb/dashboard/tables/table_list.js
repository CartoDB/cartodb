...
cdb.admin.dashboard.TableItem = cdb.core.View.extend({
  ...
  render: function() {
    ...
    var template_vars = {
      ...
      current_user_permission: this.model.permission.permissionsForUser(this.options.user) === cdb.admin.Permission.READ_ONLY ? 'READ': null
    };
    ...
    if (name != clean_name) {
      $tableInfo.on("mouseenter", function() {
        ...
        if ($tableInfo.parent().hasClass("locked")) return;
  ...
  _changePrivacy: function(ev) {
    ...
    var privacy = this.privacy = new cdb.admin.PrivacySelector({
      ...
      showCallback: function() {
        $(ev.target).parents("li").addClass("locked");
      },
      hideCallback: function() {
        $(ev.target).parents("li").removeClass("locked")
      },
      direction: 'up',
