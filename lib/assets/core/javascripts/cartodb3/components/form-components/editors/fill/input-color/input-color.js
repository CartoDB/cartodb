...
module.exports = CoreView.extend({
  ...
  _updateImageColor: function () {
    ...
    if (this.$('.js-image').prop('tagName').toUpperCase() === 'IMG') {
      this._loadSVG();
    }
  },
  ...
  _loadSVG: function () {
    ...
    this._requestSVG(src, function (content) {
  ...
  _requestSVG: function (url, callback) {
    var self = this;
    var properUrl = Utils.syncUrl(url, window.location.protocol) + '?req=ajax';

    if (this._lastSVG.url === properUrl) {
      callback && callback(this._lastSVG.content);
    } else {
      $.ajax(properUrl, {
        accepts: { xml: 'image/svg+xml' }
      })
      .done(function (data) {
        self._lastSVG.url = properUrl;
        var content = self._lastSVG.content = data.getElementsByTagName('svg')[0];
        callback && callback(content);
      })
      .fail(function () {
        throw new Error("Couldn't get " + properUrl + ' file.');
      });
    }
  },
