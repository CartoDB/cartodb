...
module Carto
  module NamedMaps
    class Template
      ...
      NAMED_MAP_LAYERS = ['carto', 'tiled', 'background', 'gmapsbase', 'wms'].freeze
      ...
      def auth
        method, valid_tokens = if @visualization.password_protected?
                                 [AUTH_TYPE_SIGNED, [@visualization.make_auth_token]]
      ...
      def layergroup
        ...
        @vizjson[:layers].select { |layer| NAMED_MAP_LAYERS.include?(layer[:kind]) }.each_with_index do |layer, index|
          options = options_for_layer(layer, index)

          layers_data.push(type: options[:layer_name], options: options[:layer_options]) if options
        end

        @vizjson[:layers].select { |layer| !NAMED_MAP_LAYERS.include?(layer[:kind]) }.each_with_index do |layer|
          ...
          if source
            ...
            layer_data[:options][:source] = { id: layer_options.fetch('source') }
          else
            layer_data[:options][:sql] = layer_options.fetch('query')
          end
      ...
      def css_from(options)
        options[:tile_style] ? options[:tile_style] : EMPTY_CSS
      end
