...
describe CartoDB::Log do
  describe '#basic' do
    it 'checks basic operations' do
      ...
      log.to_s.should eq format(Log::ENTRY_FORMAT, timestamp, text1) +
                         format(Log::ENTRY_FORMAT, timestamp, text2) +
                         Log::END_OF_LOG_MARK
    end
  end
  ...
  describe '#circular buffer logic' do
    it 'checks that buffer half works as expected' do
      ...
      log.to_s.should eq format(Log::ENTRY_FORMAT, timestamp, text1) +
                         ...
                         format(Log::ENTRY_FORMAT, timestamp, text3) +
                         format(Log::ENTRY_FORMAT, timestamp, text4) +
                         Log::END_OF_LOG_MARK
      ...
      log.to_s.should eq format(Log::ENTRY_FORMAT, timestamp, text1) +
                         ...
                         Log::HALF_OF_LOG_MARK +
                         format(Log::ENTRY_FORMAT, timestamp, text4) +
                         format(Log::ENTRY_FORMAT, timestamp, text5) +
                         Log::END_OF_LOG_MARK
      ...
      log.to_s.should eq format(Log::ENTRY_FORMAT, timestamp, text1) +
                         ...
                         Log::HALF_OF_LOG_MARK +
                         format(Log::ENTRY_FORMAT, timestamp, text5) +
                         format(Log::ENTRY_FORMAT, timestamp, text6) +
                         Log::END_OF_LOG_MARK
    end
    ...
    it 'checks that loading a log with existing entries works' do
      ...
      log.send(:collect_entries).should eq format(Log::ENTRY_FORMAT, timestamp, text1) +
                                           format(Log::ENTRY_FORMAT, timestamp, text2) +
                                           format(Log::ENTRY_FORMAT, timestamp, text3) +
                                           format(Log::ENTRY_FORMAT, timestamp, text4)
