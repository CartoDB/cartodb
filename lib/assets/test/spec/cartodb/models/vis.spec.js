  ...
  describe("Visualization model", function() {
    ...
    describe('.viewUrl', function() {
      ...
      describe('when given a current user', function() {
        beforeEach(function() {
          this.currentUser = new cdb.admin.User({
            base_url: 'http://team.cartodb.com/u/current-user',
            username: 'current-user'
          });
        });

        it("should return the URL from the perspective of the current user (i.e. it's a shared vis/table)", function() {
          expect(vis.viewUrl(this.currentUser).toString()).toEqual('http://team.cartodb.com/u/current-user/tables/rambo.test_table');
          expect(vis.viewUrl().toString()).toEqual('http://team.cartodb.com/u/rambo/tables/rambo.test_table');
        });
      });
    });
