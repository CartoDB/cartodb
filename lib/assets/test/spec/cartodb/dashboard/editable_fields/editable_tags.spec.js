...
describe('dashboard/editable_fields/editable_tags', function() {
  ...
  it('should display up to 3 tags', function() {
    ...
    var $tagItems = view.$('.DefaultTags-item')
    expect($tagItems.length).toBe(3);
    expect($($tagItems[0]).attr('href')).toEqual('http://paco.cartodb.com/dashboard/datasets/tag/dolorem')
    expect($($tagItems[1]).attr('href')).toEqual('http://paco.cartodb.com/dashboard/datasets/tag/ipsum')
    expect($($tagItems[2]).attr('href')).toEqual('http://paco.cartodb.com/dashboard/datasets/tag/quia')
  ...
  describe('if tags are editable', function() {
    ...
    it('should prevent empty tags from being created', function() {
      ...
      // Type one tag with a trailing comma
