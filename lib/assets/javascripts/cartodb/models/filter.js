...
cdb.admin.models.Filter = cdb.core.Model.extend({
  ...
  getSQLConditionForDate: function() {
    ...
    var upperDate = this._getDateFromTimestamp(upper + 1000);

    var attributes = _.clone(this.attributes);
    var options    = _.extend(attributes, { lower: lowerDate.toUTCString(), upper: upperDate.toUTCString() });
  ...
  getSQLConditionForString: function() {
    ...
    if (items.length > 0) {
      ...
      if (items.length == this.items.length) {
        ...
        return _.template("<%= column %> IN (<%= opts %>) OR <%= column %> IS NULL ")({
          column: this.get('column'),
          opts: items.map(function(i) {
            var bucket = that._sanitize(i.get('bucket'));
            return "'" + bucket + "'";
