  ...
  module InternalGeocoder
    ...
    class Admin0TextPolygons < AbstractQueryGenerator
      ...
      def search_terms_query(page)
        %Q{
          SELECT DISTINCT(quote_nullable(#{@internal_geocoder.column_name})) AS region
      ...
      def copy_results_to_table_query
        %Q{
          ...
          WHERE dest.#{@internal_geocoder.column_name}::text = orig.geocode_string AND dest.cartodb_georef_status IS NULL
        }
      end
