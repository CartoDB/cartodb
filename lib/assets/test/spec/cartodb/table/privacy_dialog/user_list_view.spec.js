...
/*
  ...
  it("should render users alphabetically", function() {
    var u1 = new cdb.admin.User({ username: 'paco', id:'1' });
    var u2 = new cdb.admin.User({ username: 'jar', id:'2' });
    var u3 = new cdb.admin.User({ username: 'arg', id:'3' });
    ...
    expect(view.$('li').length).toEqual(4);
    expect($(items[1]).find('p').html()).toEqual(u3.get('username'));
    expect($(items[2]).find('p').html()).toEqual(u2.get('username'));
    expect($(items[3]).find('p').html()).toEqual(u1.get('username'));
  });
  ...
  it("should not disable read permission for all users when only one user is disabled", function() {
    var u1 = new cdb.admin.User({ username: 'test', id: '1' });
    var u2 = new cdb.admin.User({ username: 'test', id: '2' });
    var u3 = new cdb.admin.User({ username: 'test', id: '3' });
    organization.users.add(u1);
    ...
    expect($org_item.find('a.switch').hasClass('enabled')).toEqual(true);
    expect($user_item.find('a.switch').hasClass('enabled')).toEqual(true);
    expect($user_item2.find('a.switch').hasClass('enabled')).toEqual(true);
    expect($user_item3.find('a.switch').hasClass('enabled')).toEqual(true);

    $user_item.find('a.switch').click();

    expect($org_item.find('a.switch').hasClass('enabled')).toEqual(false);
    expect($user_item.find('a.switch').hasClass('enabled')).toEqual(false);
    expect($user_item2.find('a.switch').hasClass('enabled')).toEqual(true);
    expect($user_item3.find('a.switch').hasClass('enabled')).toEqual(true);
  });
