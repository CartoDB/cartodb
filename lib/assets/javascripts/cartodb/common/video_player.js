...
cdb.admin.VideoModel = cdb.core.Model.extend({
  ...
  _initLocalStorage: function() {
    ...
    var currentVideoStore = this.localStorage.get("currentVideo");

    this.videoData = currentVideoStore;

    if (currentVideoStore === null || !this.localStorage.isReady()) {
      this.videoData = {};
    }

  },

  _setVideoId: function() {

    var video_id = this.get("video_id");

    if (video_id) { 
      this._storeVideoID(video_id);
    } else if (this.videoData && this.videoData.video_id) {
