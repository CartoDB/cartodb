...
function makeColorRamp (props, isTorqueCategory) {
  ...
  // if it contains 'Others', adjust the ramp
  if (props.domain && props.domain.length > InputColorCategories.MAX_VALUES && (props.domain[0] === 'Others' || props.domain[0] === '_')) {
    props.domain = props.domain.slice(1);
    props.range = props.range.slice(1).concat(props.range[0]);
  }
