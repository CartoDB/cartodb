...
describe("cartodb.models.Map", function() {
  ...
  describe('CartoDBLayer', function() {
    ...
    it("should bind to sqlView", function() {
      ...
      sqlView.trigger('reset');
      expect(layer.save).toHaveBeenCalledWith({query: 'testsql',  interactivity : null});
      sqlView.add({ cartodb_id: 1, test: 2})
      sqlView.trigger('reset');
      expect(layer.save).toHaveBeenCalledWith({query: 'testsql',  interactivity :'cartodb_id'});
    })
    ...
    it("should apply a query", function() {
      ...
      expect(layer.sqlView.setSQL).toHaveBeenCalledWith("SELECT * FROM test", {silent:true});
