  ...
  describe('.continue', function() {
    describe('when have selected a column name', function() {
      ...
      describe('when have selected a geometry type', function() {
        beforeEach(function() {
          this.model.set('geometryType', 'polygon');
          this.model.continue();
        });

        it('should set default geocode data', function() {
        ...
        it('should set fallback values for location', function() {
          var d = this.model.get('geocodeData');
          expect(d.kind).toEqual('admin0');
          expect(d.location).toEqual('world');
          expect(d.text).toBe(true);
        });

        describe('when have set a location', function() {
          beforeEach(function() {
            this.model.get('rows').last().set('value', 'loc');
            this.model.continue();
          });

          it('should set custom location for geocode data', function() {
            var d = this.model.get('geocodeData');
