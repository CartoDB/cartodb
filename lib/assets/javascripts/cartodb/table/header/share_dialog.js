  ...
  cdb.admin.ShareDialog = cdb.admin.BaseDialog.extend({
    ...
    _resize: function() {
      ...
      this.$el.find(".block.modal:eq(0)").css({ height: height });

      console.log(height);

      if (height > 635) {
        this.$el.find(".cartodb-map_wrapper").css({ height: height - 172 });
        this.$el.find(".scrollpane").css({ height: height - 201 - 41*2 - 94 - 32 });
        if (this.scrollPaneAPI) this.scrollPaneAPI.reinitialise();
      } else {
        height = 635;
        this.$el.find(".cartodb-map_wrapper").css({ height: height - 175 });
        this.$el.find(".scrollpane").css({ height: height - 201 - 41*2 - 94 - 32 });
        if (this.scrollPaneAPI) this.scrollPaneAPI.reinitialise();
      }
    },
