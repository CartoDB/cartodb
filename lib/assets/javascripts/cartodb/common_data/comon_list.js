  ...
  cdb.admin.CommonTableView = cdb.core.View.extend({
    ...
    events: {
      'click a.copy': '_makeAction',
      'click a.name': '_makeAction'
    },

    initialize: function() {
      _.bindAll(this, '_makeAction');
    ...
    render: function() {
      ...
      data.isInsideOrg = this.options.user.isInsideOrg();
      data.tooltip = 'Copy to your tables';

      // Over quota?
      if (this._isOverQuota()) {
        data.tooltip = !this.options.user.isInsideOrg() ?
          'You don\'t have enough quota. Click to upgrade' :
          'You don\'t have enough quota. Click to contact your admin';
      }
    ...
    _makeAction: function(e) {
      this.killEvent(e);

      // If user belongs to an organization and he/she is over quota > contact admin
      if (this.options.user.isInsideOrg() && this._isOverQuota()) {
        window.location.href = "mailto:" + this.options.user.organization.owner.get('email');
        return false;
      }
