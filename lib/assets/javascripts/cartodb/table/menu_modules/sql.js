...
cdb.admin.mod.SQL = cdb.admin.Module.extend({
    ...
    applyQuery: function() {
      ...
      cdb.god.trigger('mixpanel', "Performing a query", { sql: sql });
    ...
    _showDoc: function(ev) {
      ...
      if (this.user.featureEnabled('new_modals')) {
        dlg = cdb.editor.ViewFactory.createDialogByTemplate('common/dialogs/help/postgres_sql');
      } else {
        dlg = new cdb.admin.PostgresInfo();
      }
