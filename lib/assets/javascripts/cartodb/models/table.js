...
(function() {
  ...
  cdb.admin.CartoDBTableMetadata = cdb.ui.common.TableProperties.extend({
    ...
    initialize: function() {
      ...
      this._data.bind('error', function(e, resp) {
        this.notice('error loading rows', 'error', 0);
      }, this);
    },
    ...
    addColumn: function(columnName, columnType, callback) {
      ...
      this.notice('adding new column', 'info', 0);
      c.save(null, {
          success: function() {
            self.notice('done', 'info');
    ...
    deleteColumn: function(columnName) {
      var self = this;
      var c = this._getColumn(columnName);
      this.notice('deleting column', 'info', 0);
      c.destroy({
          success: function() {
            self.trigger('columnDelete', columnName);
            self.notice('done', 'info');
    ...
    renameColumn: function(columnName, newName) {
      ...
      this.notice('renaming column', 'info', 0);
      c.save(null,  {
          success: function() {
            self.notice('done', 'info');
    ...
    changeColumnType: function(columnName, newType) {
      ...
      var c = this._getColumn(columnName);
      c.set({ type: newType});
      this.notice('changing column type', 'info', 0);
      c.save(null, {
          success: function() {
            self.notice('done', 'info');
            self.fetch();
  ...
  cdb.admin.SQLViewData = cdb.admin.CartoDBTableData.extend({
    ...
    initialize: function(models, options) {
      ...
      this.bind('reset', function() {
        // console.log("sql reset"); // <- why?
      });
      this.bind('add', function() {
        // console.log("sql add");  // <- why?
      });
    },
    ...
    setSQL: function(sql) {
      ...
      this.options.set({ sql : sql ? this._parseSQL(sql): '' });
    },
  ...
  cdb.admin.SQLViewDataAPI = cdb.admin.SQLViewData.extend({
    ...
    url: function() {
      ...
      var u = "http://" + cdb.config.get('sql_api_domain');
