...
describe('MapCard.vue', () => {
  ...
  describe('Methods', () => {
    ...
    it('should toggle mouse hover', () => {
      ...
      mapCard.find('.cell--thumbnail__checkbox').trigger('mouseover');

      expect(mapCard.vm.activeHover).toBe(false);
      expect(mapCard).toMatchSnapshot();

      mapCard.find('.cell--thumbnail__checkbox').trigger('mouseleave');
  ...
  describe('properties', () => {
    ...
    it('should not the map when the user clicks on the card and selectMode property is true', () => {
      const mapCardWrapper = shallowMount(CondensedMapCard, {
        propsData: { selectMode: false, map },
        mocks
      });
