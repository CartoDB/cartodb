  ...
  cdb.admin.LayersPanel = cdb.ui.common.TabPane.extend({
    ...
    _addLayer: function(layer, pos) {
      ...
      if (layer.get('type') == 'CartoDB') {
        ...
        // Check order and set it if it is undefined
        if (!layer.get('order')) {
          layer.save('order', pos);
        }
    ...
    _checkLayers: function() {
      if (this.map.layers.getLayersByType('CartoDB') >= this._MAX_LAYERS) {
      ...
      this.new_layer_dlg = new cdb.admin.NewLayerDialog({
        ok: function(tableName) {
          if(!self.vis.isVisualization()) {
            ...
            self.globalError.showError(self._TEXTS.visualization.loader, 'load', -1);

            self.create_vis_dialog && self.create_vis_dialog.clean();
            self.create_vis_dialog = new cdb.admin.NameVisualization({
              msg: self._TEXTS.visualization.msg,
              onResponse: function(name) {
                self.vis.changeToVisWithLayer(tableName, {
                  success: function() {
                    self.globalError.showError(self._TEXTS.visualization.created, 'info', 3000);
                    self.vis.save({ name: name }, {
                      success: function() {
                        window.location.href = "/viz/" + self.vis.get("id") + "/";
                      }
                    });
                  },
                  error: function(e) {
    ...
    manageLayers: function() {
      ...
      if (this.map.layers.getLayersByType('CartoDB') == 0) return false;
      ...
      var offset = (this.map.layers.getLayersByType('CartoDB') >= this._MAX_LAYERS) ? 15 : - 25;
      $active_layer.css({
    ...
    _remove: function(layer_view) {
      ...
      if (this.map.layers.getLayersByType('CartoDB') <= this._MIN_LAYERS) {
        permitted = false;
      }
