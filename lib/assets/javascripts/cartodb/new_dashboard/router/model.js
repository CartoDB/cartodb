...
module.exports = cdb.core.Model.extend({
  ...
  initialize: function() {
    ...
    var dashboardUrl = this.get('dashboardUrl');
    if (!dashboardUrl) {
      throw new Error('dashboardUrl is required');
    }
    this._baseUrls = {
      '': dashboardUrl, // default path
      datasets: dashboardUrl.datasets(),
      maps: dashboardUrl.maps()
    };
  },
  ...
  url: function(override) {
    ...
    var array = _.extend([ this._baseUrls[this.get('content_type')] ], override);
    return _.compose(
      this._appendPage,
      this._appendSearch,
      this._appendLocked,
