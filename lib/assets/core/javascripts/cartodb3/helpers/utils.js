...
module.exports = {
  ...
  /*
   * Function that sets the protocol of a given input to
   * the desired one.
   * input -> url, protocol
   *
   * @return the modified url with the given protocol
   *
   * example:
   *    ('http://carto.com', 'https') -> 'https://carto.com'
   */
  syncUrl: function (input, protocol) {
    var urlRegex = /^(http:|https:)/i;
    if (input && protocol && urlRegex.test(input) && urlRegex.test(protocol)) {
      return input.replace(urlRegex, protocol);
    }
    return input;
  },
  ...
  formatDate: function (opts) {
    ...
    if (!opts.month) throw new Error();
    if (!opts.year) throw new Error();
