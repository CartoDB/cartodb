...
cdb.admin.CartoDBLayer = cdb.geo.CartoDBLayer.extend({
  ...
  saveStyle: function() {
    var self = this;
    var url = 'http://' + this.get('user_name') + "." + cdb.config.get("tiler_domain") + ':' + cdb.config.get("tiler_port") + '/tiles/' + this.get('table_name') + '/style'
    ...
    $.ajax({
      ...
      type: 'POST',
      crossOrigin: true,
      data:"style="+  encodeURIComponent(this.get('tile_style')),
      success: function() {
        self.trigger('change');
      },
      error: function() {
