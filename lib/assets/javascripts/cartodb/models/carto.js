...
function cluster_sql(table, zoom, props, nquartiles) {
  ...
  var grids = ["A", "B", "C", "D", "E"];
  var sizes = [5, 25, 125, 325, 625];
  var baseSize = 12;

  var size = baseSize * (nquartiles + 1);
  var bucket = "bucket" + grids[0];
  var mainBucket = bucket;

  var sql = "WITH metatile_extent AS ( SELECT ST_SetSRID('BOX3D(-9715452.043159042 4529964.044292687,-9617612.646954017 4627803.440497711)'::box3d, 3857) as ext ), " +
  "  filtered_table AS ( SELECT t.* FROM <%= table %> t, metatile_extent m WHERE t.the_geom_webmercator && m.ext ), " +
  "hgridA AS (SELECT ST_SnapToGrid(ST_Expand(!bbox!, greatest(!pixel_width!,!pixel_height!) * " + size + "), greatest(!pixel_width!,!pixel_height!) * " + size + ") as cell), " +
  bucket + " AS (" +
  "\nSELECT * FROM (SELECT ST_Centroid(ST_Collect(i.the_geom_webmercator)) AS the_geom_webmercator, COUNT(i.cartodb_id) AS points_count, 1 AS cartodb_id, array_agg(cartodb_id) AS id_list " +
  "\nFROM hgridA, filtered_table i " +
  "\nWHERE ST_Intersects(i.the_geom_webmercator, hgridA.cell) " +
  "\n\tGROUP BY hgridA.cell) t " +
  "\n\tWHERE points_count > " + sizes[nquartiles - 1] +
  "), ";

  for (var i = 1; i < nquartiles; i++) {

    var hgrid = "hgrid" + grids[i];
    size = baseSize * (nquartiles - i + 1);
    bucket = "bucket" + grids[i];

    sql += "\n\n" + hgrid + " AS (SELECT ST_SnapToGrid(ST_Expand(!bbox!, greatest(!pixel_width!,!pixel_height!) * " + size + "), greatest(!pixel_width!,!pixel_height!) * " + size + ") AS cell), " +
    "\n" + bucket + " AS (" +
    "\n\tSELECT * FROM (SELECT ST_Centroid(ST_Collect(i.the_geom_webmercator)) AS the_geom_webmercator, COUNT(i.cartodb_id) AS points_count, 1 AS cartodb_id, array_agg(cartodb_id) AS id_list " +
    "\n\tFROM " + hgrid + ", filtered_table i " +
    "\n\tWHERE ST_Intersects(i.the_geom_webmercator, " + hgrid + ".cell) ";

    for (var j = 0; j <= i - 1; j++) {
      var b = "bucket" + grids[j];
      sql += "\n\tAND cartodb_id NOT IN (SELECT unnest(id_list) FROM " + b + ") ";
    }

    sql += "\n\t\tGROUP BY " + hgrid + ".cell) t " +
    "\n\t\t WHERE points_count >  "  + sizes[nquartiles - i - 1] +
    ") ";
  ...
  sql += "\nSELECT the_geom_webmercator, 1 points_count, cartodb_id, ARRAY[cartodb_id] AS id_list, 'origin' AS src, cartodb_id::text cdb_list FROM filtered_table " +

  "\nWHERE cartodb_id NOT IN (select unnest(id_list) FROM " + mainBucket + ") ";

  for (var i = 1; i < nquartiles; i++) {
    bucket     = "bucket" + grids[i];
    sql += "\n\tAND cartodb_id NOT IN (select unnest(id_list) FROM " + bucket + ") ";
  }

  sql +="\nUNION ALL SELECT *, '" + mainBucket + "' AS src, array_to_string(id_list, ',') ";

  for (var i = 0; i < nquartiles; i++) {
    bucket     = "bucket" + grids[i];
    nextBucket = "bucket" + grids[i + 1];

    if (i < nquartiles - 1) {
      sql += "\nFROM " + bucket + " UNION ALL SELECT *, '" + nextBucket + "' AS src, array_to_string(id_list, ',') ";
    } else {
      sql += "\nFROM " + bucket;
    }
...
function cluster_generator(table, props, changed, callback) {
  ...
  for (var i = 0; i < nquartiles; i++) {
    c += "  [src = 'bucket"+grids[nquartiles - i - 1]+"'] { marker-width: " + sizes[i] + "; } \n";
  }

  c += "  [zoom>11]{marker-width: 52;}\n";
  c += "}\n\n";
