    ...
    if (currentType === previousType) {
      ...
      if (this.model.get("type") === 'custom') {
       ...
       if ((currentProperty == previousProperty) && (!difference || (difference && difference.length == 0))) return;
        ...
        if (m.get("marker-fill") == previousWizardProperties["marker-fill"] && m.get("radius_min") == previousWizardProperties["radius_min"] && m.get("radius_max") == previousWizardProperties["radius_max"]) return;
      }
