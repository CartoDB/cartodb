...
module.exports = CoreView.extend({
  ...
  _onConfirm: function () {
    ...
    if (this._layerModel) {
      MetricsTracker.track('Downloaded layer', {
        format: formatName,
  ...
  _fetch: function (options, sql) {
    ...
    // Only remove these columns if the source node has no custom query
    // We have to consider the possibility of a user creating complex queries that produce
    // geometries like the ones from our analyses, and let them export the data as it is.
    if (!this._sourceNodeHasCustomQuery) {
