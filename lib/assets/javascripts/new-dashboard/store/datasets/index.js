...
const datasets = {
  ...
  mutations: {
    ...
    updateDatasetLike (state, {datasetId, liked}) {
      state.list[datasetId].liked = liked;
    },
    updateDatasetNumberLikes (state, {datasetId, likes}) {
  ...
  actions: {
    ...
    likeDataset (context, dataset) {
      const currentLikeStatus = dataset.liked;
      context.commit('updateDatasetLike', { datasetId: dataset.id, liked: true });
      client.like(dataset.id,
        function (err, _, data) {
          if (err) {
            context.commit('updateDatasetLike', { datasetId: dataset.id, liked: currentLikeStatus });
            return;
          }
          context.commit('updateDatasetNumberLikes', { datasetId: dataset.id, likes: data.likes });
        }
      );
    },
    deleteDatasetLike (context, dataset) {
      const currentLikeStatus = dataset.liked;
      context.commit('updateDatasetLike', { datasetId: dataset.id, liked: false });
