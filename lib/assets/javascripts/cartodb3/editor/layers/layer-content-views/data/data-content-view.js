...
module.exports = CoreView.extend({
  ...
  initialize: function (opts) {
    ...
    if (!opts.columnsNumberModel) throw new Error('columnsNumberModel is required');

    this._widgetDefinitionsCollection = opts.widgetDefinitionsCollection;

    this._stackLayoutModel = opts.stackLayoutModel;
    this._infoboxModel = opts.infoboxModel;
    this._columnsNumberModel = opts.columnsNumberModel;
  ...
  _renderColumns: function (stats) {
    _.each(stats, function (stat, index) {
      var isVisible = this._columnsNumberModel.get('columns') > 0 ? index < LIMIT : true;
