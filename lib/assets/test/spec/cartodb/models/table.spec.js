describe("admin table", function() {
  ...
  describe("cdb.admin.Row", function() {

    it("hasGeometry", function() {
    ...
    describe('.toJSON', function() {
      ...
      it("should not include the_geom if is not in geojson format", function() {
        var a = {
          cartodb_id: 100,
          the_geom: JSON.stringify({ type: 'point', coordinates: [2,1] })
        };
      ...
      it("should not include the_geom_webmercator, created_at or updated_at", function() {
        var a = {
          cartodb_id: 100,
          updated_at: 1,
          created_at: 1,
    ...
    describe('.isColumnEmpty', function() {
      it("should return true if column is empty", function() {
        var row = new cdb.admin.Row({
          a: 1,
          b: null,
          c: '',
          d: undefined
        ...
        expect(row.isColumnEmpty('a')).toBeFalsy();
        expect(row.isColumnEmpty('b')).toBeTruthy();
        expect(row.isColumnEmpty('c')).toBeTruthy();
        expect(row.isColumnEmpty('d')).toBeTruthy();
      })
    });
  });
  ...
  describe("cdb.admin.CartoDBTableData", function() {
    var table;
    var tableData;
    ...
    describe("isColumnEmpty", function() {
      ...
      it("should return true if the column is empty for all rows", function() {
        expect(table.data().isColumnEmpty('column1')).toBeTruthy();
        expect(table.data().isColumnEmpty('column2')).toBeTruthy();
      });

      it("should return false if the column is not empty on any of the rows", function() {
        table.data().add({ column1: 'something' });

        expect(table.data().isColumnEmpty('column1')).toBeFalsy();
        expect(table.data().isColumnEmpty('column2')).toBeTruthy();

        table.data().add({ column2: 'something else' });

        expect(table.data().isColumnEmpty('column1')).toBeFalsy();
        expect(table.data().isColumnEmpty('column2')).toBeFalsy();
      })
    })
  });
