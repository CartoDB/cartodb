...
cdb.open.PublicMapTab = cdb.admin.MapTab.extend({
  ...
  _changeBounds: function(ev) {
    this.model.set('bounds', !this.model.get('bounds'));
  },

  _setBoundsCheckbox: function(e) {
    this.trigger('boundsChanged', { map: this.vis.map, bounds: this.model.get('bounds') });
    this.$('.js-bounds .Checkbox-input')[ this.model.get('bounds') ? 'addClass' : 'removeClass' ]('is-checked');
  },
  ...
  _bindBounds: function() {
    ...
    this.vis.mapView.bind('dragend zoomend', function(e) {
      _this.trigger('boundsChanged', { map: _this.vis.map, bounds: _this.model.get('bounds') });
    });
  },
