...
module.exports = Backbone.Collection.extend({
   ...
   */
  createNode: function (nodeAttrs, opts) {
    ...
    if (m.get('type') !== 'source') {
      this._updateOrCreateAnalysis(m, opts);
    }

    return m;
  },

  _updateOrCreateAnalysis: function (nodeModel, opts) {
    opts = opts || {};
    opts.wait = true;
    opts.error = function () {
      nodeModel.destroy();
    ...
    var sourceNodeId = nodeModel.sourceIds()[0];
  ...
  _onAnalysisDefinitionNodeChange: function (m) {
    this._analysis.analyse(m.toJSON());
  }
