...
cdb.admin.MapOverlays = cdb.core.View.extend({
    ...
    /*legend.$el.draggable({
      ...
      start: function(event, ui) {
        ...
        if (!(styles.width && styles.height)) {
          var css = _.extend(styles, {
              width: legend.$el.width(),
              height: legend.$el.height()
          });
          legend.$el.css(css);
        }
      },
  ...
  _addOverlayPropertiesBar: function(model, form_data, hide_actions, forceReset) {
    ...
    // ignore clicks on annotations when export toolbar present
    // (do not display both toolbars simultaneously)
    if ($('.export-options').length) {
      return;
    }
    ...
    if (!this.overlayPropertiesBar) {
      ...
      this.overlayPropertiesBar = new cdb.admin.OverlayPropertiesBar({
        ...
        vis: this.vis,
        form_data: form_data,
        hide_actions: hide_actions || model.attributes.type == 'header'? true : false
      });
  ...
  _positionSearchOverlay: function(overlay_model) {
    ...
    var y = this.header ? this._getHeaderHeight() + 40 : 40;
