...
class AuthenticatedClient extends PublicClient {
  ...
  directDBConnection () {
    return {
      getIPs: callback => {
        const URIParts = ['api/v1/dbdirect/ips'];
      ...
      getCertificates: (callback) => {
        const URIParts = ['api/v1/dbdirect/certificates'];
        return this.get(URIParts, callback);
      },

      createCertificate: (certificate, callback) => {
        const URIParts = ['api/v1/dbdirect/certificates'];
        const opts = {
      ...
      revokeCertificate: (certificateId, callback) => {
        const URIParts = [`api/v1/dbdirect/certificates/${certificateId}`];
