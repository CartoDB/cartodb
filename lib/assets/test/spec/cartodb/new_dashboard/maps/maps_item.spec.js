...
describe('new_dashboard/maps/maps_item', function() {
  ...
  it('should render the URL to the edit map', function() {
    this.renderView();
    expect(this.innerHTML()).toContain('http://team.cartodb.com/u/pepe/viz/8988-8888-8888/map');
  });
  ...
  describe('render owner user info', function() {
    ...
    it("shouldn't render owner info if user is the owner of the map", function() {
      ...
      spyOn(this.vis.permission, 'isOwner').and.returnValue(true);
      spyOn(this.vis.permission, 'getPermission').and.returnValue('w');
    ...
    it('should render owner info if user is not the owner of the map', function() {
      ...
      spyOn(this.vis.permission, 'isOwner').and.returnValue(false);
      spyOn(this.vis.permission, 'getPermission').and.returnValue('r');
  ...
  it('should render an editable field with the description', function() {
    ...
    this.renderView();
    expect(this.view.$('.MapCard-desc.EditableDescription').length).toBe(1);
    expect(this.view.$('.DefaultDescription').length).toBe(1);
    expect(this.view.$('.DefaultDescription')[0].textContent).toEqual('my desc');
  });

  it('should render an editable field with the tags', function() {
    this.vis.set('tags', ['tag1']);
    this.renderView();
    expect(this.view.$('.MapCard-tags.EditableTags').length).toBe(1);
    expect(this.view.$('.DefaultTags-item').length).toBe(1);
    expect(this.view.$('.DefaultTags-item')[0].textContent).toEqual('tag1');
  });
