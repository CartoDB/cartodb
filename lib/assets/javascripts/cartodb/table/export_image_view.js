...
cdb.admin.ExportImageView = cdb.core.View.extend({
  ...
  _updateHelpers: function(x, y, width, height) {
    ...
    if (x >= 0 && y >= 0) {
      ...
      /*if (y < 0) {
        var top = this.$(".HelperWest").position().top;
        this.$(".HelperWest").css({ left: 0, top: 0, width: x + 1, height: height + top });
        this.$(".HelperNorth").css({ top: 0, width: x + width + 1, height: 1});
      }
      if (x < 0) {
        this.$(".HelperWest").css({ left: 0, top: y + 1, width: 1, height: height});
      }*/
    }
  },
