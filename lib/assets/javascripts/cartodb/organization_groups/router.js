    ...
    this.model.createGroupView(this.groups, id, function(group) {
      ...
      if (group.users.length > 0) {
        ...
        contentView = new BatchOrgUsersView({
          ...
          processUsers: function(selectedUsers, errorCallback) {
            var ids = _.map(selectedUsers, function(m) { return m.id });
            group.users.addUsers(ids, {
              success: function() {
                group.users.add(selectedUsers);
                self.routeToShowGroup(group.id); // already on same URL, so just "refresh"
              },
              error: errorCallback
            })
          }
