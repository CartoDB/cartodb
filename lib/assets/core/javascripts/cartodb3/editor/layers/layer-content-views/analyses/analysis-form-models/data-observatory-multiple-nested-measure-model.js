...
var MeasurementModel = require('../../../../../data/data-observatory/measurement-model');
...
module.exports = Backbone.Model.extend({
  ...
  _initBinds: function () {
    ...
    if (measurement) {
      this.listenToOnce(this.measurement, 'change:val', this._checkIfMeasurementExist);
    }
  ...
  _syncMeasurement: function () {
    ...
    if (!this.measurement.getValue()) {
      ...
      // But this doesn't work because nested-form-custom is listening to a different model
      // when the region changes, the form renders again
      // so all nested form are created again, with their models
      // so when this model reaches this point, there is nobody listening
