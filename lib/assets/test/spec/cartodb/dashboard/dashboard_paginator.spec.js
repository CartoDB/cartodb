describe("Dashboard paginator", function() {
  ...
  it("should add 'locked visualizations' link when needed", function() {
    ...
    server.respondWith('/api/v1/viz/?tag_name=&q=&page=1&type=derived&exclude_shared=false&locked=true&per_page=1&tags=', [200, { "Content-Type": "application/json" }, '{"visualizations":[{ "name": "hello"}],"total_entries":1}']);
    server.respond();
    expect(new_paginator.$el.hasClass("empty")).toBeFalsy();
    expect(new_paginator.$('p a').length).toEqual(1);
    expect(new_paginator.$('p a').text()).toBe('View all locked visualizations');
    ...
    expect(new_paginator.$el.hasClass("empty")).toBeFalsy();
    expect(new_paginator.$('p a').length).toEqual(1);
    expect(new_paginator.$('p a').text()).toBe('View non locked visualizations');
