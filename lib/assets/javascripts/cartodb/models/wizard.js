...
cdb.admin.WizardProperties = cdb.core.Model.extend({
  ...
  // save current state
  saveCurrent: function(where) {
    this._savedStates[where] = _.clone(this.attributes);
  },

  getSaved: function(where) {
    return this._savedStates[where] || {};
  },
  ...
  active: function(type, props) {
    ...
    var current = type !== this.get('type') ? {}: this.attributes;
    _.extend(geomForm, this.getSaved(type), current, props);
    geomForm.type = type;
    this.saveCurrent(this.get('type'));
