module CartoDB
  module Stats
    class APICalls
      ...
      REDIS_SOURCES = ['mapviews', 'mapviews_es'].freeze

      # This method will return an array without dates
      def get_api_calls_without_dates(username, options = {})
        calls = get_api_calls(username, options).map { |_day, value| value}
        if options[:old_api_calls]
          raise 'Cannot request old api calls with custom dates' if options[:to] || options[:from]

          # Add old api calls
          old_calls = begin
                        get_old_api_calls(username)
                      rescue StandardError
                        []
                      end
          unless old_calls.blank?
            calls = calls.zip(old_calls).map do |pair|
      ...
      def get_total_api_calls_from_redis(username, visualization_id = nil)
        ...
        REDIS_SOURCES.each do |source|
          ...
          calls += source_calls if !source_calls.nil? && (source_calls != 0)
        end
      ...
      def redis_api_call_key(username, api_call_type, visualization_id = nil)
        ...
        if visualization_id.nil?
          "#{redis_base_key}:global"
        else
          "#{redis_base_key}:stat_tag:#{visualization_id}"
        end
      end
      ...
      def get_api_calls_from_redis_source(username, api_call_type, options = {})
        ...
        calls_in_reverse_order = calls.select { |a| a.is_a? Array }.flatten.each_slice(2).to_a.sort { |a1, a2| a1[0] <=> a2[0]}.reverse
        ...
        if check_available_values(calls_in_reverse_order, first_requested_day, last_requested_day)
          first_requested_index = calls_in_reverse_order.index do |day_and_count|
          ...
          last_requested_index = calls_in_reverse_order.index do |day_and_count|
      ...
      def get_total_api_calls_from_redis_source(username, api_call_type, visualization_id = nil)
        ...
        $users_metadata.ZSCORE(redis_key, 'total').to_i
      end
