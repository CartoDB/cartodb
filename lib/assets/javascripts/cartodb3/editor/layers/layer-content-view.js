    ...
    var tabPaneTabs = [{
      ...
      createContentView: function () {
        ...
        if (_.isString(analysisPayload)) {
          ...
          var analysisFormModel = analysisFormsCollection.addHead(analysisPayload);

          // If form model is valid with defaults persist it right away
          if (analysisFormModel.isValid()) {
            self.userActions.updateOrCreateAnalysis(analysisFormModel);
          }
        } else {
