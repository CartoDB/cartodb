...
describe("Background view", function() {
  ...
  it("should start poll checking when formatter has changed", function() {
    view.render();
  ...
  it("should show an error in the application when geocoder process fails", function() {
    view.render();
    spyOn(view.options.globalError, 'showError');
    ...
    expect(view.options.globalError.showError).toHaveBeenCalled();
    view.model.destroyCheck();
  });
