...
module.exports = CoreView.extend({
  ...
  _onStatusChange: function () {
    ...
    if (showOthers) {
      var otherColorIndex = categoryNames.length;

      domain.unshift(_t('form-components.editors.fill.input-categories.others'));

      if (categoryNames.length >= DEFAULT_COLORS.length) {
        otherColorIndex = categoryNames.length - 1;
      }

      range.unshift(DEFAULT_COLORS[otherColorIndex]);
    }
