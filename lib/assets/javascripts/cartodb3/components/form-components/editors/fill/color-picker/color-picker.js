...
module.exports = CoreView.extend({
  ...
  _onChangeColor: function () {
    ...
    var r = this.model.get('r');
    var g = this.model.get('g');
    var b = this.model.get('b');
    var rgba = _.template('rgba(<%- r %>, <%- g %>, <%- b %>, <%- a %>)');
    var gradient = _.template('linear-gradient(90deg, <%- opaque %>, <%- transparent %>)');
    var bkg = gradient({
      opaque: rgba({ r: r, g: g, b: b, a: 1 }),
      transparent: rgba({ r: r, g: g, b: b, a: 0 }),
    })
    ...
    this.$('.js-alpha').css('background', bkg);
  },
