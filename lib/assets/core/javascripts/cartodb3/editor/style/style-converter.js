...
function renderBlock (block, geometryType, animationType, styleDef) {
  var css = {};
  for (var k in block) {
    var f = StyleGenerationFactory[k];
    if (f) {
      css = _.extend(css, f[geometryType](block[k], animationType, styleDef));
    }
  }
...
function generateCartoCSS (style, geometryType, configModel) {
  ...
  if (geometryType === 'polygon' || StyleUtils.canApplyDotProperties(styleDef.fill)) {
    omittedStyleAttrs.push('stroke');
  }

  // Stroke for dot property
  if (hasStroke && geometryType === 'point' && StyleUtils.canApplyDotProperties(styleDef.fill)) {
    css += '\n#layer::outline {\n';
  ...
  css += '#layer {\n';
  css += renderBlock(_.omit(styleDef, omittedStyleAttrs), geometryType, animationType, styleDef);
  css += '}';
