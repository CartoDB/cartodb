  ...
  var Table = cdb.core.View.extend({
    ...
    */
    ...
    applyQuery: function(sql) {
      ...
      self.sqlView.fetch();
      // close cell editor if it is opened
      if (self.tableTab.tableView._editorsOpened) {
        self.tableTab.tableView.closeEditor();
      }
    ...
    _initModels: function() {
      ...
      this.map.bind('change:dataLayer', _.once(function() {
        ...
        /*self.table.data().bind('reset', function() {
          var table = self.table;
          if(table.isGeoreferenced() && self.menu.activeSection === 'map') {
            self.menu.showTools('map', true);
          } else {
            self.menu.showTools('table', true);
          }
        });
        */
        self.sqlView.bind('reset', function() {
    ...
    _initViews: function() {
      ...
      this.header.bind('clearSQLView', this._writableTableButtons.bind(this));
      ...
      this.map.bind('change:dataLayer', _.once(function() {
        ...
        /* Carto */
        var carto = new cdb.admin.mod.CartoCSS({
          model: self.dataLayer,
          table: self.table,
          map: self.map
        ...
        self.menu.addModule(sql.render(), ['table', 'tableLite', 'map', 'mapLite']);
        self.menu.addModule(carto.render(), ['map', 'mapLite']);
        self.menu.addModule(infowindow.render(), ['map', 'mapLite']);


        var mergeTables = self.menu.addToolButton("merge_tables", 'table');
    ...
    _writableTableButtons: function() {
      if(this.menu.activeSection === 'map' || this.menu.activeSection === 'mapLite') {
        this.menu.showTools('map', true);
      } else {
        this.menu.showTools('table', true);
      }
    },

    _readOnlyTableButtons: function() {
      if(this.menu.activeSection === 'map' || this.menu.activeSection === 'mapLite') {
        this.menu.showTools('mapLite', true);
      } else {
        this.menu.showTools('tableLite', true);
      }
    },
