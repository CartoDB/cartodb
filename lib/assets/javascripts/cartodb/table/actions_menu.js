  ...
  cdb.admin.LayersPanel = cdb.ui.common.TabPane.extend({
      ...
      this.new_layer_dlg = new cdb.admin.NewLayerDialog({
        ...
        ok: function(tableName, private_vis) {
          ...
          if(!self.vis.isVisualization()) {
            ...
            // Unbind reset of the map
            self.map.layers.unbind('reset',  self._resetLayers, self);

            // Add new layer
            self.map.addCartodbLayerFromTable(tableName, userName, {
              success: function() {
                // Bind again reset layer bind
    ...
    manageLayers: function() {
      ...
      // TODO: not use jquery
      this.$el.find('.layer_panel').not('.layer_panel.active').css({ height: 66 });
    ...
    _switchTo: function(l, notOpen) {
      ...
      if (this.activePane == l && !notOpen && this.isOpen) {
        this.hide();
        return false;
      }
      // If not, activate it
