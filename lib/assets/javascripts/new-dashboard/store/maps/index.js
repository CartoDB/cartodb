...
const maps = {
  ...
  mutations: {
    ...
    setOrderDirection (state, direction = 'desc') {
      state.orderDirection = direction;
    },

    setPerPage (state, perPage) {
      console.info('mutation: setPerPage', perPage);
  ...
  actions: {
    fetchMaps (context) {
      ...
      console.info('action: fetchMaps', context.state.perPage);
    ...
    orderMaps (context, orderOpts) {
      context.commit('setPagination', 1);
      context.commit('setOrder', orderOpts.order);
      context.commit('setOrderDirection', orderOpts.direction);
      context.dispatch('fetchMaps');
    },
    ...
    setPerPage (context, perPage) {
      console.info('action: setPerPage');
