...
F.prototype._onLayerDefinitionChanged = function (m) {
  if (!m.isNew()) {
    ...
    if (!_.isEqual(this._lastTSChange, animatedChanged)) {
      ...
      if (widgetModel) {
        if (m.get('type') === 'torque') {
          this._removeTimeseries();
          this._createTimeseries(newLayer, animatedChanged);
        } else {
          this._removeTimeseries();
        }
      } else if (newLayer.get('type') === 'torque') {
        this._createTimeseries(newLayer, animatedChanged);
      }
...
F.prototype._createTimeseries = function (newLayer, animatedChanged) {
  this._removeTimeseries();
