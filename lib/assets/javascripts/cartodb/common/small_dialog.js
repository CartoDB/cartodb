...
cdb.admin.EditMarkdownDialog = cdb.admin.SmallDialog.extend({
  ...
  initialize: function() {
    _.defaults(this.options, {
      template_name: 'common/views/dialog_markdown_edit',
      ok_title: 'Save',
  ...
  render_content: function() {
    ...
    input += ' type="text"/><div class="hint"><strong>**bold**</strong> <em>*italics*</em> [link](url)</div></div>';
    return input;
  },
  ...
  _focusInput: function() {
    ...
    setTimeout(function(){
      var width = self.$el.outerWidth() - self.$el.find("a.button").outerWidth() - 35;
      self.$el.find(".input_field").width(width).focus();
    },0);
  },
