...
describe('editor/layers/layer-content-view/analyses/analysis-forms-collection', function () {
  ...
  describe('for a form model', function () {
    ...
    describe('.save', function () {
      ...
      describe('when form model represents a now non-existing node', function () {
        beforeEach(function () {
          spyOn(this.nodeDefModel, 'set');
          this.model.set('id', 'x9');
          this.model.save(this.analysisDefinitionNodesCollection);
        });

        it('should not try to update or create', function () {
          expect(this.nodeDefModel.set).not.toHaveBeenCalled();
          expect(this.layerDefModel.createNewAnalysisNode).not.toHaveBeenCalled();
        });
      });
      ...
      describe('when form model represents a new node', function () {
        ...
        it('should create a new node when the form model represents a new node', function () {
          expect(this.layerDefModel.createNewAnalysisNode).toHaveBeenCalled();
