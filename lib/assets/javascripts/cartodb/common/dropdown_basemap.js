...
cdb.admin.DropdownBasemap = cdb.ui.common.Dropdown.extend({
  ...
  */
  ...
  _append: function($element) {
    ...
    var $ul = this.$el.find('ul:last-child');

    if ($ul.length <= 0 || ($ul.length > 0 && $ul.find("li").length >= 5)) {

      var ul_count = this.$el.find("ul").length;
      this.$el.append("<ul class='" + ul_count +"'></ul>");
      $ul = this.$el.find('ul:last-child');
    }
