...
(function() {
    ...
    cdb.admin.TableView = cdb.ui.common.Table.extend({
      ...
      _onSQLView: function() {
        ...
        this.options.sqlView.bind('loading', function() {
          self.cleanEmptyTableInfo();
          //self._renderBodyTemplate('table/views/sql_loading');
          self._renderBodyTemplate('table/views/empty_sql'); // TEMP FIX... RESTORE PREVIOUS LINE BEFORE MERGIN
        }, this)
        ...
        this.options.sqlView.bind('reset', function() {
          ...
          if(this.model.isInSQLView()) {
            var empty = this.isEmptyTable()
              , total = this.model.data().total_rows;
      ...
      _scrollMagic: function(ev) {
        ...
        if (this.scroll_position.x != actual_scroll_position.x) {
          ...
          if (this.scroll_position.last == "vertical") {
            this.scroll_position.x = actual_scroll_position.x;
            this.$el.find("thead > tr > th > div:not(.dropdown)").css({"marginLeft": "0px"})
            this.$el.find("thead div > div:not(.dropdown)").css("top", actual_scroll_position.y + "px");
          ...
          if (this.scroll_position.last == "horizontal") {
            ...
            this.$el.find("thead > tr > th > div > div:not(.dropdown)").removeAttr('style')
      ...
      _moveQueryInfo: function(type) {
        if (this.model.isInSQLView()) {
          //TODO: if this object exists, move to the left
          //var $queryLoader = this.$el.find("div.loaderSQLContainer");
          var $table_query_text = this.$el.find("div.sqlview p");

          if (type == "show") {
            $table_query_text.addClass('displaced');
          } else if (type == "hide") {
            $table_query_text.removeClass('displaced');
          } else {
      ...
      */
      ...
      _renderBodyTemplate: function(tmpl) {
        ...
        this.$('tfoot').remove();
        this.$el.hide();
        var content = cdb.templates.getTemplate(tmpl)();
        var $footer = $('<tfoot><tr><td colspan="100">' + content + '</td></tr></tfoot>');
        this.$el.append($footer);
        this.$el.fadeIn();
      },
