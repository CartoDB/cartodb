...
describe('editor/layers/layer-content-view/legend/legends-view', function () {
  ...
  it('should disable size tab when it\'s not possible to add one', function () {
    var disabledTabLabel = '.CDB-NavSubmenu-item.is-disabled';
    var isSizeCompatible = spyOn(this.view, '_isStyleCompatibleWithLegendSize');

    isSizeCompatible.and.returnValue(false);
    this.view.render();
    expect(this.view.$el.find(disabledTabLabel).length).toBe(1);

    isSizeCompatible.and.returnValue(true);
    this.view.render();
    expect(this.view.$el.find(disabledTabLabel).length).toBe(0);
  });
});
