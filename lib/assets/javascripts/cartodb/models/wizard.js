...
cdb.admin.FormSchema = cdb.core.Model.extend({
  ...
  validators: {
    ...
    torque_cat: columnExistsValidator
  },
...
cdb.admin.WizardProperties = cdb.core.Model.extend({
  ...
  linkLayer: function(layer) {
    ...
    */
    ...
    var changeLayerStyle = function(st, sql, layerType) {
      ...
      if (layer.isNew()) {
        layer.set(attrs, { no_override: true });
      } else {
  ...
  getEnabledWizards: function() {
    var _enableMap = {
      'point': ['polygon', 'cluster', 'choropleth', 'bubble', 'density', 'category', 'intensity', 'torque', 'torque_cat'],
  ...
  _EVENTS: {
    'marker-fill':          'New color chosen for marker-fill',
    'polygon-fill':         'New color chosen for polygon-fill',
    'line-color':           'New color chosen for line-color',
