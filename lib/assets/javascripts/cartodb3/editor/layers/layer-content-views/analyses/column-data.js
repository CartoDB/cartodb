...
var QUERY = 'SELECT min(<%= column %>) as min, max(<%= column %>) as max FROM (<%= sql %>) _table_sql';
...
module.exports = Backbone.Model.extend({
  ...
  _onChangeStatus: function () {
    ...
    if (this._querySchemaModel.rowsSampleCollection.size() > 0) {
      var results = this._querySchemaModel.rowsSampleCollection.models[0];
