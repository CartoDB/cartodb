...
describe CartoDB::Importer2::Excel2Csv do
  before(:each) do
    allow(CartoDB).to receive(:python_path).and_return('')
    allow(CartoDB).to receive(:python_bin_path).and_return(`which python`.strip)
  end
  ...
  describe '#excel2csv' do
    ...
    describe '#run' do
      it "parse xls file to csv" do
        ...
        allow(@excel2csv).to receive(:converted_filepath).and_return("/tmp/converted_filepath_excel2csv")
        @excel2csv.run
      end
