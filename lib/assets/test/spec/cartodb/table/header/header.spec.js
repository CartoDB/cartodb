describe("Table/Visualization Header tests", function() {
  ...
  it("should open a warning dialog when user changes the title", function() {
    $(this.header.el).find('.title').click();
    waits(25);

    runs(function() {
      this.header.title_dialog.$('input').val('example');
      this.header.title_dialog.$('.ok').click();
    })

    waits(25);

    runs(function() {
      expect(this.header.change_confirmation).toBeTruthy();
      this.header.title_dialog.clean();
      this.header.change_confirmation.clean();
    })

  })

  it("should not open the warning dialog when user changes the title", function() {
    ...
    runs(function() {
      expect(this.header.change_confirmation).toBeFalsy();
      this.header.title_dialog.clean();
    })
  })
  ...
  it("should not let the user change the visualization/table name when the table is syncable", function() {
