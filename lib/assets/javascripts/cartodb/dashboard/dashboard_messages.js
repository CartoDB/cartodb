...
cdb.admin.dashboard.MessageItem = cdb.core.View.extend({
  ...
  initialize: function() {
    this.template = _.template('<div class="inner"><p><%= message%></p><% if (!sticky) {%><a href="#/close" class="smaller close">x</a><% } %><% if (trial) {%><a class="button small green upgrade" href="<%= upgrade_url %>?utm_source=Dashboard_Limits_Reached&utm_medium=referral&utm_campaign=Upgrade_from_Dashboard&utm_content=Upgrade%20now%20%28button%29">Test now</a><% } %><% if (upgrade) {%><a class="button small green upgrade" href="<%= upgrade_url %>?utm_source=Dashboard_Limits_Reached&utm_medium=referral&utm_campaign=Upgrade_from_Dashboard&utm_content=Upgrade%20now%20%28button%29">Upgrade now</a><% } %></div>');
  },
...
cdb.admin.dashboard.Messages = cdb.core.View.extend({
  ...
  _MESSAGES: {
    ...
    try_trial:                  { template: 'Try the trial, bro', store: true, sticky: false, trial: true },
