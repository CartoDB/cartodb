...
describe('cartodb3/data/analysis-definitions-collection', function () {
  beforeEach(function () {
    ...
    this.layerDefinitionsCollection = new Backbone.Collection([{ source: 'a1' }]);
  ...
  it('should save analysis for a layer when a node changes its geometry', function () {
    spyOn(this.collection, 'saveAnalysisForLayer');
    var nodeDefModel = this.analysisDefinitionNodesCollection.findWhere({ id: 'a1' });
    nodeDefModel.set('simple_geom', 'line');
    var involvedLayer = this.layerDefinitionsCollection.first();
    expect(this.collection.saveAnalysisForLayer).toHaveBeenCalledWith(involvedLayer);
  });
