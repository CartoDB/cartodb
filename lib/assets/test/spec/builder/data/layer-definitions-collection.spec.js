...
describe('data/layer-definitions-collection', function () {
  ...
  describe('.setBaseLayer', function () {
    ...
    it('should update high resolution url template if present when base layer is updated', function () {
      spyOn(collection, '_isHdpi').and.returnValue(true);
      ...
      collection.reset([
        {
          ...
          options: {
            ...
            category: 'CartoDB',
            baseType: 'BASE_TYPE',
            urlTemplate: 'URL_TEMPLATE_OLD',
            urlTemplate2x: 'URL_TEMPLATE_OLD_2X'
          }
        }
      ]);
      ...
      collection.setBaseLayer({
        ...
        baseType: 'BASE_TYPE',
        urlTemplate: 'URL_TEMPLATE',
        urlTemplate2x: 'URL_TEMPLATE_2X'
      });

      var newLayerAttributes = LayerDefinitionModel.prototype.save.calls.mostRecent().args[0];

      expect(newLayerAttributes).toEqual({
        type: 'Tiled',
        category: 'CartoDB',
        baseType: 'BASE_TYPE',
        urlTemplate: 'URL_TEMPLATE_2X',
        urlTemplate2x: 'URL_TEMPLATE_2X'
      });
    });

    it('should update high resolution url template in legend layer', function () {
      spyOn(collection, '_isHdpi').and.returnValue(true);
