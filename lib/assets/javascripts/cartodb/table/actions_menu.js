  ...
  cdb.admin.LayersPanel = cdb.ui.common.TabPane.extend({
    ...
    _checkLayers: function() {
      ...
      var canUpgrade = cdb.config.get('upgrade_url') && !cdb.config.get('custom_com_hosted') && ( !this.options.user.isInsideOrg() || this.options.user.isOrgAdmin() );
      if (cartodbLayers >= this._MAX_LAYERS && !canUpgrade) {
    ...
    _addLayerDialog: function(e) {
      ...
      if (this._MAX_LAYERS === cartodbLayers) {
    ...
    _manageLayers: function() {
      ...
      if (this._MAX_LAYERS === cartodbLayers && !canUpgrade) {
        offset = offset + 43;
      }
    ...
    _openDeleteLayerDialog: function(layer_view) {
      ...
      this.remove_dlg = new cdb.admin.BaseDialog({
        ...
        descriptionSafeHtml: self._TEXTS[permitted ? 'remove' : 'cant_remove'].desc,
    ...
    clean: function() {
      ...
      if (this.resize) clearTimeout(this.resize);
      var self = this;
      _.each(this.layer_panels, function(l) {
        l.unbind(null, null, self);
      });
      this.activePane.unbind(null, null, self);
      this.removeTabs();
      this._unbindSort();
      this.unbind('tabEnabled');
