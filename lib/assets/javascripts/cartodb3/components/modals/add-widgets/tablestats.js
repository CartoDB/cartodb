  ...
  _getPgStats: function (table, callback) {
    ...
    if (this.tables[table]) {
      ...
      var querySchemaModel = new QuerySchemaModel({
        query: 'with a as (select reltuples from pg_class where relname = \'' + table + '\'), b as (select * from pg_stats where tablename = \'' + table + '\' and schemaname = \'' + schema + '\') select * from a,b',
        rows_per_page: 40
      }, {
        configModel: this.configModel
      });

      querySchemaModel.on('change:status', function (model, status) {
        if (status === 'fetched') {
          var rows = querySchemaModel.rowsCollection.map(function (m) { return m.attributes; }).map(function (r) {
