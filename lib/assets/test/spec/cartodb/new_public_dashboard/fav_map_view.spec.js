...
describe('new_public_dashboard/fav_map_view', function() {
  ...
  describe('given there is no data to create map from an user visualization', function() {
    beforeEach(function() {
      ...
      this.attrs.basemapTemplateUrl = 'http://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png';
      this.createFavMapView();
    });

    it('should create a fallback map', function() {
      expect(this.innerHTML()).toContain('leaflet-map');
    });

    it('should create the fallback map with a given basemap template URL', function() {
      expect(L.tileLayer).toHaveBeenCalledWith(this.attrs.basemapTemplateUrl);
    });
  });
