...
class AuthenticatedClient extends PublicClient {
  ...
  updateNotification (userId, apiKey, notification) {
    return new Promise((resolve, reject) => {
      const URL = [`/api/v3/users/${userId}/notifications/${notification.id}?api_key=${apiKey}`];
      const opts = {
        data: JSON.stringify({
          notification
        }),
      ...
      return this.put(URL, opts, callback);
    });
  }
}
