shared_examples_for 'user table models' do
  describe '#validation' do
    ...
    it 'validates that name cannot be taken' do
      create(:carto_user_table, user: Carto::User.find(user.id), name: 'ut_spec_wadus')
  ...
  describe 'dependent visualizations' do
    ...
    before do
      @map, @other_table, @table_visualization, @visualization = create_full_visualization(user)
      user_table.reload
    end

    let(:first_layer) { @visualization.data_layers.first }
    let(:second_layer) do
      layer = create(:carto_layer, kind: 'torque', maps: [@map])
      layer.update_attribute(:kind, 'torque')
      layer
    end
