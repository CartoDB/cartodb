...
describe('editor/components/modals/add-basemap/wms/wms-layer-model', function () {
  ...
  describe('.createProxiedLayerOrCustomBaselayerModel', function () {
    ...
    describe('when is a WMTS GetTile request with the default OGC standard variables', function () {
      beforeEach(function () {
        this.model.set({
          type: 'wmts',
          url_template: 'https://www.ign.es/wmts/ign-base?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=%(layer)s&TILEMATRIXSET=%(tile_matrix_set)s&TILEMATRIX=10&TILEROW=384&TILECOL=500&FORMAT=%(format)s&STYLE=%(style)s',
          matrix_sets: ['25830', '4326'],
          name: 'Callejero',
          formats: [],
          styles: []
      ...
      it('should create the tile layer with a proper XYZ URL', function () {
        expect(this.model._byCustomURL).toHaveBeenCalled();
        expect(this.model._byCustomURL.calls.argsFor(0)[0]).toEqual('https://www.ign.es/wmts/ign-base?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=Callejero&TILEMATRIXSET=25830&TILEMATRIX=10&TILEROW=384&TILECOL=500&FORMAT=image/png&STYLE=default');
      });
    ...
    describe('when is a WMTS GetTile request with the OGC standard variables', function () {
      beforeEach(function () {
        this.model.set({
          type: 'wmts',
          url_template: 'https://www.ign.es/wmts/ign-base?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=%(layer)s&TILEMATRIXSET=%(tile_matrix_set)s&TILEMATRIX=10&TILEROW=384&TILECOL=500&FORMAT=%(format)s&STYLE=%(style)s',
          ...
          styles: [
            {
              'default': false,
              'id': 'style1',
              'title': 'Estilo 1 de representación del Mapa base'
            }, {
              'default': true,
              'id': 'style2',
              'title': 'Estilo 2 de representación del Mapa base'
            }, {
              'id': 'style3',
              'title': 'Estilo 3 de representación del Mapa base'
            }
          ]
        });
        spyOn(this.model, '_byCustomURL').and.callThrough();
      ...
      it('should create the tile layer with a proper XYZ URL', function () {
        expect(this.model._byCustomURL).toHaveBeenCalled();
        expect(this.model._byCustomURL.calls.argsFor(0)[0]).toEqual('https://www.ign.es/wmts/ign-base?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=Callejero&TILEMATRIXSET=25830&TILEMATRIX=10&TILEROW=384&TILECOL=500&FORMAT=image/jpeg&STYLE=style2');
      });
