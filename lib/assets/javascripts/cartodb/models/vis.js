...
cdb.admin.Visualization = cdb.core.Model.extend({
  ...
  changeTo: function(new_vis, callbacks) {
    ...
    var func_success = function() {
      this.map.layers.unbind('reset', func_success);
      this.map.layers.unbind('error', func_error);
    ...
    this.map.layers.bind('reset', func_success, this);
    this.map.layers.bind('error', func_error, this)
