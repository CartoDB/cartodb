...
describe('editor/layers/layer-content-views/analyses-form-types/area-of-influence-form-model', function () {
  ...
  it('should set defaults if they are not defined', function () {
    ...
    expect(this.model.get('radio')).toBe(100);
    expect(this.analysisDefinitionNodeModel.get('radio')).toBe(100);
  });
  ...
  it('should adjust radio (taking into account distance) if analysis is a buffer', function () {
    spyOn(AreaOfInfluenceFormModel.prototype, '_adjustRadioDistance').and.callThrough();
    ...
    expect(model._adjustRadioDistance).toHaveBeenCalled();
    expect(model.get('radio')).toBe(1);
    expect(this.analysisDefinitionNodeModel.get('radio')).toBe(1000);
  });
  ...
  describe('on update', function () {
    ...
    describe('on params change', function () {
      it('should just update analysis definition node model', function () {
        this.model.set('radio', 101);
        expect(this.analysisDefinitionNodeModel.set).toHaveBeenCalled();
        expect(this.analysisDefinitionNodeModel.get('radio')).toBe(101);
      });
    });
    ...
    describe('on type change', function () {
      ...
      it('should remove previous params', function () {
        expect(this.model.get('radio')).toBeUndefined();
        expect(this.analysisDefinitionNodeModel.get('radio')).toBeUndefined();
