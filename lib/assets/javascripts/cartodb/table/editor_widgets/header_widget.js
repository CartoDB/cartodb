cdb.admin.widgets.Title = cdb.admin.widgets.Text.extend({
  ...
  template_name: 'table/views/widgets/title',
  ...
  _onChangeExtra: function() {
    ...
    if (extra["show_title"]) this.$title.fadeIn(150, callback);
    else this.$title.fadeOut(150, callback);

    if (extra["show_description"]) {
      this.$description.fadeIn(150, callback);
    }
    else this.$description.fadeOut(150, callback);
