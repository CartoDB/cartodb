...
module.exports = BaseDialog.extend({
  ...
  _onCardClick: function(column) {
    ...
    // Get the proper wizard name if the name of visualisation used is different 
    var wizardName = this._getWizardName(column.get("visualizationType"));

    this.layer.wizard_properties.active(wizardName, properties);

    this._panes.active('applying');

    cdb.log.info("%cApplying wizard: " + column.get("visualizationType"), "font-weight: bold; color:brown;");
  },

  _getWizardName: function(name){
    var mappings = {"heatmap": "torque_heat"};
    return mappings[name] || column.get("visualizationType");
  },
  ...
  _onLoadWizard: function() {
    ...
    if (wizard === "category") {
      properties = this._getCategoriesProperties(properties);
    } else if (wizard === 'choropleth') {
      properties = this._getChoroplethProperties(properties);
    } else if(wizard === "heatmap") {
      properties = this._getHeatmapProperties(properties);
    }
  ...
  _getChoroplethProperties: function(properties) {
    ...
    properties.color_ramp = cdb.CartoCSS.getMethodProperties(stats).name;

    cdb.log.info("%cApplying: " + properties.color_ramp +  " and " +  properties.qfunction + " to " + property + " (" + dist + ")", 'color: blue; ' );
    return properties;
  },
