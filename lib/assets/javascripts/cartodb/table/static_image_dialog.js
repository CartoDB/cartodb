...
cdb.admin.StaticImageDialog = cdb.admin.BaseDialog.extend({
  ...
  _ok: function(e) {
    ...
    if (this.options.privacy === "PRIVATE" || this.options.privacy === "PASSWORD") {
      this._showError("Your visualization is private. Please, change the privacy settings and try again.");
      return;
    }

    this.$el.addClass("is-loading");

    cdb.Image(this.options.vizjson, { https: true }).size(width, height).getUrl(this._onImageCallback);
