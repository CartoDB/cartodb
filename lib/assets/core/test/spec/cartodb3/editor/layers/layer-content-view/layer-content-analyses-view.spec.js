...
describe('editor/layers/layer-content-view/layer-content-analyses-view', function () {
  beforeEach(function () {
    ...
    spyOn(this.view, '_renderControlsView');
    spyOn(this.layerDefinitionModel, 'canBeGeoreferenced').and.returnValue(false);
    this.view.render();
  });

  describe('when layer has no analysis', function () {
    it('should have js-georeference button if layer can be georeferenced', function () {
      this.layerDefinitionModel.canBeGeoreferenced.and.returnValue(true);
      this.view.render();
      expect(this.view.$el.html()).toContain('js-georeference');
    });

    it('should show the old placeholder for georeferenced layers', function () {
