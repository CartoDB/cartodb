  ...
  cdb.open.TablePublic = cdb.core.View.extend({
    ...
    _initViews: function() {
      ...
      this.mapTab.bind('boundsChanged', function(options) {
        var sql = options.bounds ? (self.query + " WHERE the_geom && ST_MakeEnvelope("+options.map.get('view_bounds_ne')[1]+", "+options.map.get('view_bounds_ne')[0]+", "+options.map.get('view_bounds_sw')[1]+", "+options.map.get('view_bounds_sw')[0]+", 4326)") : self.query;
