...
describe('new_dashboard/background_importer/imports_model', function() {
  ...
  describe('.shouldRedirectAfterComplete', function() {
    beforeEach(function() {
    ...
    it('should return true if was completed, only one table was created, the service name is not twitter, and flag is not set to false', function() {
      expect(this.model.shouldRedirectAfterComplete()). toBeTruthy();
    });

    it('should return false if not complete yet', function() {
      this.model.set('state', 'process or whatever');
      expect(this.model.shouldRedirectAfterComplete()).toBeFalsy();
    });

    it('should return false if more than one table is created', function() {
      this.model.imp.set('tables_created_count', 2);
      expect(this.model.shouldRedirectAfterComplete()).toBeFalsy();
    });

    it('should return false if service name is twitter', function() {
      this.model.imp.set('service_name', 'twitter_search');
      expect(this.model.shouldRedirectAfterComplete()).toBeFalsy();
    });

    it('should return false if flag to skip redirection is set to false', function() {
      this.model.set('shouldRedirectAfterComplete', false);
      expect(this.model.shouldRedirectAfterComplete()).toBeFalsy();
    });
  });
