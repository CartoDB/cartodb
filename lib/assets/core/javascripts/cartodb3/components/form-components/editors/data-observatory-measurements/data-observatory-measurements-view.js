...
Backbone.Form.editors.DataObservatoryDropdown = Backbone.Form.editors.Base.extend({
  ...
  render: function () {
    ...
    var placeholder = this._getPlaceholder(isDisabled);
    var label = isNull ? placeholder : (item ? item.getName() : name || false);
    var title = item ? item.getName() : '';
  ...
  _getPlaceholder: function (isDisabled) {
    ...
    if (isDisabled) {
      placeholder = this.options.placeholderDisabled || _t('components.backbone-forms.select.disabled-placeholder', { keyAttr: keyAttr });
    } else {
