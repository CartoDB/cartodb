...
cdb.admin.DropdownBasemap = cdb.ui.common.Dropdown.extend({
  ...
  add: function(lyr) {
    ...
    if (baseType) {
      // TODO: get origin of tiles checking the model and not the url template
      var here   = baseType.indexOf("nokia") !== -1;
      var stamen = baseType.indexOf("stamen") !== -1;
      var rainbow = baseType.indexOf("rainbow") !== -1;;
      var available_basemaps = this.options.available_basemaps;

      if (baseType.indexOf("cartodb") !== -1 || baseType === "flat_blue") {
        var $ul = this.$el.find('.custom ul.cartodb');
        $ul.append($element);
      } else if (available_basemaps.stamen && stamen) {
        var $ul = this.$el.find('.custom ul.stamen');
        $ul.append($element);
      } else if (available_basemaps.here && here) {
        var $ul = this.$el.find('.custom ul.here');
        $ul.append($element);
      } else if (available_basemaps.rainbow && rainbow) {
        var $ul = this.$el.find('.custom ul.rainbow');
        $ul.append($element);
      } else if (!stamen && !here && !rainbow){
