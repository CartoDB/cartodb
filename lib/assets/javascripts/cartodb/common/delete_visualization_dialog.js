  ...
  cdb.admin.DeleteVisualizationDialog = cdb.admin.BaseDialog.extend({
    ...
    render_content: function() {
      var $div = $('<div>');
      var template = this.getTemplate('common/views/delete_visualization_dialog')();
      $div.append(template);
    ...
    _generateUsersList: function() {
      ...
      console.log(this.model.permission.acl);

      return this.model.permission.get('acl').map(function(p) {
        var entity = p.entity;
        return {
          id:             entity.id,
          type:           p.type,
          username:       entity.username || entity.name, // organization is name! :S
