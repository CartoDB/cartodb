...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _showPecan: function() {
    ...
    var hasNewModals = true; // this.user.featureEnabled('new_modals');

    var hasData = this.options.table && this.options.table.data() && this.options.table.data().length > 0 ? true : false;

    if (hasPecan && hasNewModals && hasData) {

      var skipPencanDialog = 'pecan_' + this.options.user.get("username") + "_" + this.options.vis.tableMetadata().id;

      if (!localStorage[skipPencanDialog]) {

        this.pecanView = new cdb.editor.PecanView({
          table: this.options.table,
          row_count: this.options.vis.get("table").row_count,
