...
cdb.admin.Permission = cdb.core.Model.extend({
  ...
  hasReadAccess: function(model) {
    ...
    return !!this._findRepresentableAclItem(model);
  },

  hasWriteAccess: function(model) {
    var access = this._maybeAccessForAclItem(this._findRepresentableAclItem(model));
  ...
  // @deprecated use .hasReadAccess
  canRead: function(model) {
    return this.hasReadAccess(model);
  },

  // @deprecated use .hasWriteAccess
  canWrite: function(model) {
    return this.hasWriteAccess(model);
  },

  // @deprecated use .access instead
  getPermission: function(model) {
    if (this.hasWriteAccess(model)) {
      return this.constructor.READ_WRITE;
    } else if (this.hasReadAccess(model)) {
      return this.constructor.READ_ONLY;
    } else {
      return null;
    }
  },
  ...
  _findRepresentableAclItem: function(model) {
