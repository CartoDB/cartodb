...
/*
  TODO:
  - Manage ok button.
  - Abort button should appear when uploads
  - Review code.
  - Check GDrive.
  - Set state of the dialog in every function.
  - When close dialog, you cannot drag mamufas again.
  - Link options.

  - Answer Serengeti
  - Answer Support.
  - Tests for new table dialog.
  - Tests for upload file.
  - Tests for dropbox module.
...
cdb.admin.CreateTableDialog = cdb.ui.common.Dialog.extend({
  ...
  events: function(){
    return _.extend({},cdb.ui.common.Dialog.prototype.events,{
      'click .option.dialog-tab': '_optionClicked'
    });
  },
  ...
  initialize: function() {
    ...
    // Create a model to know the state of the dialog
  ...
  _checkFiles: function() {
    ...
    if (this.options.files) {
      ...
      setTimeout(function(){
        ...
        self._uploadFromUrl(self.options.url);
      },150);
    }
  },
  ...
  _optionClicked: function(e) {
    ...
    // // Specific functions for dialog options
    // switch (name) {
    //   case 'scratch':
    //     this.$("a.ok")
    ...
    //     this.$("a.ok")
    //       .addClass("disabled")
    //       .text(this._TEXTS.ok.normal);
    //     break;
    //   default: break;
    // }

    // // Move pane blocks
    // this.$content.find('.dialog-tabs li a.' + name)
    //   .parent()
    //   .append(this.dialog_panes.$el);
  },
  ...
  _keydown: function(e) {
    if (e.keyCode === 27 && this._isEnabled()) {
      this._cancel();
    }
  },
  ...
  ok: function(ev) {
    ...
    switch (this.dialog) {
      ...
      case 'scratch':
        ...
        break;
      default:
        cdb.log.info("Not listed option in create window");
    }
    //   case 1:
    //     // Web file
    //     this._onSubmitForm();
