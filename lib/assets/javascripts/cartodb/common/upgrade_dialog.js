  ...
  cdb.admin.UpgradeDialog = cdb.admin.BaseDialog.extend({
    ...
    render_content: function() {
      ...
      var xhr = $.ajax({
        url: window.location.protocol + '//' + this.options.config.account_host + '/account/' + this.model.get("username") + '.json',
        dataType: 'jsonp',
        success: function(r) {
          ajaxDone = true;
    ...
    _renderPlans: function($content, r) {
      ...
      // Add ghost form
      this.$content.prepend('<form class="hide" action="' + window.location.protocol + '//' + this.options.config.account_host + '/account/' + username + '/upgrade/" method="get"><input type="text" name="recurly_plan_code" value="" /></form>');
