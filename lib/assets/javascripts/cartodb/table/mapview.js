    ...
    _updateLayerDefinition: function(layer) {
      ...
      if(this.groupLayer) {
        ...
        if(!(_.size(layer.changed) == 1 && layer.changed.extra_params)) {
          this.groupLayer.setLayerDefinition(this.map.layers.getLayerDef());
          this._setInteraction();
        }
      }
    },
...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    //this.map.layers.bind('add',        this._addLegend,  this);
  ...
  */
  enableMap: function() {
    ...
    if (!this.map_enabled) {
      ...
      // 1
      //this.map.layers.bind('add',   this._addLegend,  this);
      //this.map.layers.bind('reset', this._addLegends, this);
  ...
  */
  ...
  _addLegends: function() {
    ...
    this.map.layers.each(function(layer) {
      var legend = self._addLegend(layer);
    });
  ...
  _renderLegends: function() {
    ...
    //if (this.legends.length > 1) this._renderStackedLengeds();
    //else this._renderLegend();
  ...
  _renderStackedLengeds: function() {
    ...
    console.log('rendering stacked legend');
    ...
    //this.stackedLegend._checkVisibility();
  ...
  _addInfowindow: function() {
    ...
    if(this.table && this.mapView) {
      ...
      this.infowindow.bind('openInfowindowPanel', function() {
        this.activeLayerView.showModule('infowindow');
      }, this);
