...
describe('editor/widgets/widgets-form/widgets-form-column-options-factory', function () {
  ...
  describe('.isColumnOfType', function () {
    beforeEach(function () {
      this.querySchemaModel.columnsCollection.reset([
        {name: 'cartodb_id', type: 'number'},
        {name: 'title', type: 'string'},
    ...
    it('returns falsy if there is no column with that name', function () {
      expect(this.factory.isColumnOfType('this_column_does_not_exist', 'string')).toBeFalsy();
    });

    it('returns false if the column doesnt match the type', function () {
      expect(this.factory.isColumnOfType('cartodb_id', 'string')).toBe(false);
    });

    it('returns true if the column matches the type', function () {
      expect(this.factory.isColumnOfType('cartodb_id', 'number')).toBe(true);
    });
  });
});
