...
function generateCartoCSS (style, geometryType) {
  ...
  if (styleDef.animated.enabled.toString() === 'true') {
    styleDef = adaptAttributeForTorque(styleDef);
  }
...
function adaptAttributeForTorque (definition) {
  if (definition.fill.color.attribute) {
    definition.fill.color.attribute = 'value';
  }
  if (definition.fill.size.attribute) {
    definition.fill.size.attribute = 'value';
  }
  return definition;
}
