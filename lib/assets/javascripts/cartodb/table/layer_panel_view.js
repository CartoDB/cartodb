  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    _onChangeStatus: function() {
      ...
      if (this.view_model.get('state') == "idle") {
        ...
        $el.find('input')
          .val(this._getTitle(this.dataLayer))
          .show()
          .focus();
      }
    },

    _getTitle: function(layer) {
        var title = "";
        layer.get('table_name_alias')? title = layer.get('table_name_alias'): title = layer.get('table_name');
        return title;
    },
    ...
    setLayerName: function(layer) {
      // grab table name
      var table_title = this._getTitle(layer);

      if (this.vis.isVisualization()) {
        // titlecase table name and sync icon if table synced
        if (this.table.isSync()) {
          table_title = Sugar.String.titleize(table_title);
          this.$('.layer-info .info .name')
            .text(table_title)
            .append($('<i>').addClass('synced'));
        } else {
          table_title = table_title.replace(/_/g,' ');
          this.$('.layer-info .info .name')
            .text(table_title);
        ...
        this.$('.layer-info .info .name_info')
          .html('view of ')
          .append($('<a>').attr('href', layerUrl).text(table_title));
        ...
        this.$('.layer-info .info .name').text(table_title);
      }
    },
