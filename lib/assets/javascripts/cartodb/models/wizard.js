...
cdb.admin.FormSchema = cdb.core.Model.extend({
      ...
      for (var k in field.form) {
        ...
        if (f.columns) {
          ...
          for(var i in types) {
            ...
            if (f.exclude_empty_columns) {
              columns = _.select(columns, function(column) {
                return !self.table.data().isColumnEmpty(column);
              });
            }
  ...
  changedDynamicProperty: function() {
    var changed = [];
  ...
  dynamicProperty: function(c) {
    return _.keys(this.get(c.name).form)[0];
  },

  dynamicValues: function(c) {
    var v = this.get(c.name);
    var k = this.dynamicProperty(c);
    ...
    if (t) {
      ...
      f.bind('change', function() {
        if (!f.isValid(this.get('type'))) {
          ...
          var self = this;
          var c = f.changedDynamicProperty();
          var propertiesChanged = [];
          if(c.length) {
            _.each(c, function(form_p) {
              var k = f.dynamicProperty(form_p);
              if (self.has(k) && !_.contains(f.dynamicValues(form_p), self.get(k))) {
